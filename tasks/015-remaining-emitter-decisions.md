...
inventory the current emitter only far enough to select a substantial coherent production ownership
...
After committing, if emitter policy remains, leave task 015 unchecked and record the next substantial
coherent area in STATUS.md. Mark task 015 complete only when `ColumnarIlEmitter.cs` is deleted
