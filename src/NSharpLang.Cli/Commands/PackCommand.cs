...
public static class PackCommand
{
    ...
    /// Replicates the EnsureProjectFiles logic from Program.cs for use in this standalone command.
    /// </summary>
    static string EnsurePackProjectFiles(string projectRoot, ProjectConfig config, string projectName)
    {
        ...
        var outputType = config.OutputType?.ToLowerInvariant() == "library" ? "Library" : "Exe";
