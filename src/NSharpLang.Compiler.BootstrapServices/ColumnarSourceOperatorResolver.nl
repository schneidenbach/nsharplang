    ...
    constructor(
        ...
        if declaringType == null || parameterTypes == null || returnType == null {
            throw new InvalidOperationException(
                "Source operator selection facts cannot be null.")
        }
        if status == ColumnarSourceOperatorStatus.Selected {
            if sourceDefinition == null || operatorDefinition == null || method == null {
                throw new InvalidOperationException(
                    "A selected source operator requires exact declaration facts.")
            }
        } else if sourceDefinition != null || operatorDefinition != null || method != null
            || parameterTypes.Length != 0 {
            throw new InvalidOperationException(
                "An unselected source operator cannot carry executable facts.")
        }
...
class ColumnarSourceOperatorResolver {
    static func ResolveUnary(
        symbol: string,
        operandType: Type,
        sourceDefinitions: IEnumerable<ColumnarStructDef>): ColumnarSourceOperatorSelection {
    ...
    static func ResolveBinary(
        ...
        if rightOwner != null
            && !ColumnarConstructionPlanner.SameObject(rightOwner, leftOwner) {
    ...
    static func Select(
        candidates: List<ColumnarSourceOperatorCandidate>,
        operandTypes: Type[]): ColumnarSourceOperatorSelection {
        ...
        return new ColumnarSourceOperatorSelection(
            ColumnarSourceOperatorStatus.Selected,
            candidate.Owner,
            definition,
            method,
            declaringType,
    ...
    static func AppendExactCandidates(
        owner: ColumnarStructDef,
        methodName: string,
        operandTypes: Type[],
        candidates: List<ColumnarSourceOperatorCandidate>) {
        ...
        if overloads == null {
            throw new InvalidOperationException(
                "Source operator overload facts cannot be null.")
        }
        ...
        for candidate in overloads {
            ...
            while index < candidates.Count {
                if ColumnarConstructionPlanner.SameObject(
                    candidates[index].Definition, candidate) {
                    duplicate = true
                    break
                }
                index += 1
    ...
    static func IsExactCallableOperator(
        definition: ColumnarStaticMethodDef,
        operandTypes: Type[]): bool {
        method: MethodInfo = definition.Builder
        if !method.get_IsPublic() { return false }
        if !method.get_IsStatic() { return false }
        if !method.get_IsSpecialName() { return false }
        if method.get_IsAbstract() { return false }
        if method.get_IsGenericMethod() { return false }
        if IsVarArgs(method) { return false }
        if definition.ParamTypes.Length != operandTypes.Length { return false }
        if definition.ParamModifierKinds.Length != operandTypes.Length { return false }
        if definition.ReturnType.get_IsByRef() { return false }
        if definition.ReturnType.FullName == "System.Void" { return false }

        index := 0
        while index < operandTypes.Length {
            if definition.ParamModifierKinds[index] != 0 { return false }
            if definition.ParamTypes[index].get_IsByRef() { return false }
            if !ColumnarSourceDirectCallResolver.ExactTypeShapeMatches(
                definition.ParamTypes[index], operandTypes[index]) { return false }
            index += 1
        }
        return true
    }

    static func ValidateOperatorFact(
        owner: ColumnarStructDef,
        methodName: string,
        definition: ColumnarStaticMethodDef) {
        if owner == null || owner.Builder == null || owner.StaticMethods == null
            || definition == null || definition.Builder == null
            || definition.ParamTypes == null
            || definition.ParamModifierKinds == null
            || definition.ReturnType == null {
            throw new InvalidOperationException(
                "Source operator declaration facts cannot be null.")
        }
        ...
        while parameterIndex < definition.ParamTypes.Length {
            if definition.ParamTypes[parameterIndex] == null {
                throw new InvalidOperationException(
                    "Source operator parameter facts cannot contain null values.")
            }
            parameterIndex += 1
        }
        modifierIndex := 0
        while modifierIndex < definition.ParamModifierKinds.Length {
            ...
            if modifier < 0 || modifier > 4 {
                throw new InvalidOperationException(
                    "Source operator modifier facts are invalid.")
            }
            modifierIndex += 1
        }
        ...
        if !method.get_IsStatic() || method.get_Name() != methodName
            || !ColumnarConstructionPlanner.SameObject(
                method.get_DeclaringType(), ownerType)
            || !ColumnarSourceDirectCallResolver.ExactTypeShapeMatches(
                method.get_ReturnType(), definition.ReturnType) {
            throw new InvalidOperationException(
                "Source operator facts do not identify an exact static declaration.")
        }
    }

    static func TryFindExactOwner(
        operandType: Type,
        sourceDefinitions: IEnumerable<ColumnarStructDef>,
        out owner: ColumnarStructDef?): bool {
        owner = null
        ...
        for candidate in sourceDefinitions {
            if candidate == null || candidate.Builder == null {
                throw new InvalidOperationException(
                    "Source operator type definitions cannot be null.")
            }
            if !ColumnarConstructionPlanner.SameObject(
                candidate.Builder, operandType) {
                continue
            }
            if owner != null
                && !ColumnarConstructionPlanner.SameObject(owner, candidate) {
                throw new InvalidOperationException(
                    "One exact operator operand type cannot map to two definitions.")
            }
            owner = candidate
        }
        return owner != null
    ...
    static func UnaryMethodName(symbol: string): string? {
        if symbol == "+" { return "op_UnaryPlus" }
        if symbol == "-" { return "op_UnaryNegation" }
        if symbol == "!" { return "op_LogicalNot" }
        if symbol == "~" { return "op_OnesComplement" }
        return null
    }

    static func BinaryMethodName(symbol: string): string? {
        if symbol == "+" { return "op_Addition" }
        if symbol == "-" { return "op_Subtraction" }
        if symbol == "*" { return "op_Multiply" }
        if symbol == "/" { return "op_Division" }
        if symbol == "%" { return "op_Modulus" }
        if symbol == "==" { return "op_Equality" }
        if symbol == "!=" { return "op_Inequality" }
        if symbol == "<" { return "op_LessThan" }
        if symbol == "<=" { return "op_LessThanOrEqual" }
        if symbol == ">" { return "op_GreaterThan" }
        if symbol == ">=" { return "op_GreaterThanOrEqual" }
        if symbol == "&" { return "op_BitwiseAnd" }
        if symbol == "|" { return "op_BitwiseOr" }
        if symbol == "^" { return "op_ExclusiveOr" }
        if symbol == "<<" { return "op_LeftShift" }
        if symbol == ">>" { return "op_RightShift" }
        return null
    }
    ...
    static func ValidateInputs(
        ...
        if symbol == null || leftType == null || rightType == null
            ...
            throw new InvalidOperationException(
                "Source operator resolution inputs cannot be null.")
        }
    }

    static func Unselected(
        status: ColumnarSourceOperatorStatus): ColumnarSourceOperatorSelection {
        if status == ColumnarSourceOperatorStatus.Selected {
            throw new InvalidOperationException(
                "A selected source operator requires declaration facts.")
        }
        return new ColumnarSourceOperatorSelection(
            status,
            null,
