...
class ColumnarConstructionPlanner {
    ...
    static func IsAdmittedValueSyntax(
        ...
        if kind == ColumnarExpressionNodeKind.ObjectInitializerExpression() {
            ...
            while index < childCount {
                ...
                if nameNode < 0 || nameNode >= nodes.Kinds.Length
                    || nodes.Kind(nameNode)
                        != ColumnarExpressionNodeKind.IdentifierExpression()
                    || !ColumnarDirectCallPlanner.IsAdmittedValueSyntax(
                        nodes, valueNode, depth + 1) {
                    return false
                }
                index += 2
            }
    ...
    public static func IsAdmittedConstructionValueSyntax(
        ...
        if kind == ColumnarExpressionNodeKind.ObjectInitializerExpression() {
            ...
            typeKind := nodes.Kind(typeNode)
            if typeKind == ColumnarExpressionNodeKind.NewExpression() {
                if !IsAdmittedConstructionValueSyntax(
                        nodes, source, typeNode, bindings, handles,
                        depth + 1) {
                    return false
                }
            } else if typeKind != 0 && typeKind != 1 {
            ...
            while index < childCount {
                ...
                if nameNode < 0 || nameNode >= nodes.Kinds.Length
                    || nodes.Kind(nameNode)
                        != ColumnarExpressionNodeKind.IdentifierExpression()
                    || !ValueSyntaxIsAdmitted(
                        nodes, source, valueNode, bindings, handles,
                        depth + 1) {
                    return false
                }
                index += 2
        ...
        while index < nodes.ChildCount(candidate) {
            if !ValueSyntaxIsAdmitted(
                    nodes, source, nodes.Child(candidate, index),
                    bindings, handles, depth + 1) {
                return false
            }
            index += 1
        }
    ...
    static func TryAppend(
        ...
        if TryResolveExplicitUnionCase(
                nodes, canonical, bindings,
                out unionClaimed,
                out unionDefinition, out caseDefinition,
                out unionType, out caseType, out typeArguments)
            && unionDefinition != null && caseDefinition != null {
            if !TryAppendUnionCasePositionalConstruction(
                    nodes, source, candidate, bindings, handles, plan,
                    fragment, depth, caseDefinition, unionType, caseType,
                    typeArguments,
        ...
        if targetType.get_IsGenericType()
            ...
            if TryAppendClosedGenericConstruction(
                    nodes, source, candidate, bindings, handles, plan,
    ...
    static func TryAppendSizedArray(
        ...
        if !ValueSyntaxIsAdmitted(
                nodes, source, lengthNode, bindings, handles,
                depth + 1) {
            ownership = ColumnarDirectCallOwnership.NotOwned
            legacyWholeSubtreePlanning = true
            return false
        ...
        if !ColumnarDirectCallPlanner.TryGetPlannableValueType(
                nodes, source, lengthNode, bindings, handles, depth + 1, true,
    ...
    static func TryAppendInferredArray(
        ...
        while index < elementCount {
            ...
            if !ColumnarDirectCallPlanner.TryGetPlannableValueType(
                    nodes, source, elementNode, bindings, handles,
                    depth + 1, true,
                    out currentType, out nestedOwnership) {
                if nestedOwnership == ColumnarDirectCallOwnership.OwnedRejected {
                    ownership = nestedOwnership
                }
                return false
    ...
    static func TryAppendObjectInitializer(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        bindings: ColumnarFragmentBindings,
        ...
        if rootKind == ColumnarExpressionNodeKind.NewExpression() {
            ...
            nestedFragment := plan.BeginFragment(
                fragment, rootKind, typeRoot)
            if !TryAppend(
                    nodes, source, typeRoot, bindings, handles, plan,
                    nestedFragment, depth + 1,
                    out nestedOwnership, out nestedLegacy,
                    out constructedType) {
            ...
            if !TryFindSourceDefinition(
                    constructedType, bindings, out constructedDefinition) {
                TryFindClosedSourceDefinition(
                    constructedType, bindings, out constructedDefinition)
            }
            if constructedDefinition == null
                && (constructedType is TypeBuilder
                    || ContainsBuilderBoundType(constructedType)) {
                ownership = ColumnarDirectCallOwnership.NotOwned
                legacyWholeSubtreePlanning = true
                return false
            }
            if constructedDefinition != null
                && !constructedDefinition.IsReference {
                return false
            }
            if !TryAppendObjectMembers(
                    nodes, source, node, bindings, handles, plan,
                    fragment, depth, constructedType,
        ...
        canonical := ""
        if !TryBuildTypeCanonical(
                nodes, source, typeRoot, 0, out canonical) {
            return false
        }
        ...
        if TryResolveExplicitUnionCase(
                ...
                out unionType, out caseType, out typeArguments)
            && unionDefinition != null && caseDefinition != null {
            if !TryAppendUnionCaseObjectInitializer(
                    nodes, source, node, bindings, handles, plan,
                    fragment, depth, caseDefinition, unionType, caseType,
                    typeArguments,
                    out ownership, out legacyWholeSubtreePlanning) {
        ...
        if definition != null {
            if !TryAppendSourceObjectInitializerConstruction(
                    nodes, source, node, bindings, handles, plan,
                    fragment, depth, definition, targetType,
                    out ownership, out legacyWholeSubtreePlanning) {
                return false
            }
            resultType = targetType
        ...
        if !IsApprovedRuntimeObjectInitializerType(targetType)
            || !TryAppendRuntimeObjectInitializerConstruction(
                nodes, source, node, bindings, handles, plan,
                fragment, depth, targetType,
                out ownership, out legacyWholeSubtreePlanning) {
            return false
    ...
    static func TryAppendSourceObjectInitializerConstruction(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        bindings: ColumnarFragmentBindings,
        ...
        ownership = ColumnarDirectCallOwnership.OwnedRejected
        legacyWholeSubtreePlanning = false
        closed := targetType.get_IsGenericType()
            && !targetType.get_IsGenericTypeDefinition()
        ...
        if definition.IsReference {
            ...
            if closed {
                constructor = TypeBuilder.GetConstructor(
                    targetType, definition.DefaultCtor)
            }
            constructorIndex := plan.AddConstructorWithSignature(
                constructor, targetType, new Type[](0))
            plan.AppendConstructorInstruction(
                ColumnarCodePlanContract.Newobj(), constructorIndex)
            return TryAppendObjectMembers(
                nodes, source, node, bindings, handles, plan,
                fragment, depth, targetType, definition,
                out ownership, out legacyWholeSubtreePlanning)
        }

        localIndex := BeginDefaultValueConstruction(plan, targetType)
        if !TryAppendValueTypeObjectFields(
                nodes, source, node, bindings, handles, plan,
                fragment, depth, targetType, definition, localIndex,
                out ownership, out legacyWholeSubtreePlanning) {
            return false
        }
    ...
    static func BeginDefaultValueConstruction(
        plan: ColumnarCodePlan,
        targetType: Type): int {
        typeIndex := plan.AddType(targetType)
        localIndex := plan.DeclarePlanLocal(typeIndex)
        plan.AppendPlanLocalInstruction(
            ColumnarCodePlanContract.Ldloca(), localIndex)
        plan.AppendTypeInstruction(
            ColumnarCodePlanContract.Initobj(), typeIndex)
        return localIndex
    }

    static func CompleteDefaultValueConstruction(
        plan: ColumnarCodePlan,
        localIndex: int) {
        plan.AppendPlanLocalInstruction(
            ColumnarCodePlanContract.Ldloc(), localIndex)
    }

    static func AppendDefaultValueConstruction(
        plan: ColumnarCodePlan,
        targetType: Type) {
        localIndex := BeginDefaultValueConstruction(plan, targetType)
        CompleteDefaultValueConstruction(plan, localIndex)
    ...
    static func TryAppendRuntimeObjectInitializerConstruction(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        bindings: ColumnarFragmentBindings,
        ...
        return TryAppendObjectMembers(
            nodes, source, node, bindings, handles, plan,
            fragment, depth, targetType, null,
            out ownership, out legacyWholeSubtreePlanning)
    }

    static func TryAppendObjectMembers(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        bindings: ColumnarFragmentBindings,
        ...
        while index < nodes.ChildCount(node) {
            ...
            if definition != null {
                ...
                if TryFindObjectInitializerProperty(
                        definition, memberName,
                        out propertyOwner, out property)
                    && propertyOwner != null && property != null {
                    if property.Setter == null
                        || property.SetterParameterCount != 1 {
                        return false
                    }
                    propertyOwnerType := typeof(object)
                    propertyOwnerArguments := new Type[](0)
                    if !TryResolveExactObjectMemberOwnerType(
                            definition, targetType, propertyOwner,
                            out propertyOwnerType,
                            out propertyOwnerArguments) {
                        return false
                    }
                    propertyType := propertyOwnerArguments.Length > 0
                        ? SubstituteTypeArgument(
                            property.PropertyType, propertyOwnerArguments)
                    ...
                    if !SameObject(propertyOwnerType, propertyOwner.Builder) {
                        setter = TypeBuilder.GetMethod(
                            propertyOwnerType, property.Setter)
                    }
                    plan.AppendInstructionWithoutOperand(
                        ColumnarCodePlanContract.Dup())
                    if !TryAppendObjectInitializerValue(
                            nodes, source, valueNode, bindings, handles,
                            plan, fragment, depth + 1, propertyType,
                            out ownership,
                            out legacyWholeSubtreePlanning) {
                        return false
                    }
                    ...
                    methodIndex := plan.AddMethodWithSignature(
                        ...
                        RequiredVoidType(),
                        false,
                        setter.get_IsAbstract())
                    plan.AppendMethodInstruction(
                        ColumnarCodePlanContract.Callvirt(), methodIndex)
                    index += 2
                    continue
                }
                ...
                if !TryFindObjectInitializerField(
                        definition, memberName, out fieldOwner, out field)
                    || fieldOwner == null || field == null {
                    return false
                }
                ...
                if !TryResolveExactObjectMemberOwnerType(
                        definition, targetType, fieldOwner,
                        out fieldOwnerType, out fieldOwnerArguments) {
                    return false
                }
                fieldType: Type = typeof(object)
                fieldType = selectedField.get_FieldType()
                if fieldOwnerArguments.Length > 0 {
                    fieldType = SubstituteTypeArgument(
                        fieldType, fieldOwnerArguments)
                }
                fieldHandle: FieldInfo = selectedField
                declaringType: Type = fieldOwnerType
                if !SameObject(fieldOwnerType, fieldOwner.Builder) {
                    fieldHandle = TypeBuilder.GetField(
                        fieldOwnerType, selectedField)
                }
                plan.AppendInstructionWithoutOperand(
                    ColumnarCodePlanContract.Dup())
                if !TryAppendObjectInitializerValue(
                        nodes, source, valueNode, bindings, handles,
                        plan, fragment, depth + 1, fieldType,
                        out ownership, out legacyWholeSubtreePlanning) {
                    return false
                }
                fieldIndex := plan.AddFieldWithSignature(
                    fieldHandle, declaringType, fieldType, false)
            ...
            if setterCandidate == null || setterCandidate.get_IsStatic()
                || setterCandidate.GetParameters().Length != 1 {
                return false
            }
    ...
    static func TryAppendValueTypeObjectFields(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        bindings: ColumnarFragmentBindings,
    ...
    static func TryAppendObjectInitializerValue(
        nodes: ColumnarNodeTable,
        source: string,
        valueNode: int,
        bindings: ColumnarFragmentBindings,
        ...
        if nodes.Kind(candidate)
                == ColumnarExpressionNodeKind.NullLiteralExpression() {
            return ColumnarNullableArgumentLowering.TryAppendNullArgument(
                plan, fragment, nodes.Kind(candidate), candidate, expectedType)
        }
        if TryAppendTargetTypedInitializerInteger(
                nodes, source, candidate, expectedType, plan, fragment) {
            return true
        }
        if !ValueSyntaxIsAdmitted(
                nodes, source, valueNode, bindings, handles, depth) {
            ownership = ColumnarDirectCallOwnership.NotOwned
            legacyWholeSubtreePlanning = true
            return false
        }
        ...
        nestedOwnership := ColumnarDirectCallOwnership.NotOwned
        if !ColumnarRangeIndexPlanner.TryAppendConstructionValue(
                nodes, source, valueNode, bindings, handles, plan,
                fragment, depth, out actualType, out nestedOwnership) {
            if nestedOwnership == ColumnarDirectCallOwnership.OwnedRejected {
                ownership = nestedOwnership
            }
            return false
        ...
        return ColumnarDirectCallPlanner.AppendArgumentConversion(
            plan, actualType, expectedType,
            bindings.SourceTypeDefinitions)
    }

    static func TryAppendTargetTypedInitializerInteger(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        expectedType: Type,
        ...
        if ColumnarNullableArgumentLowering.TryGetSupportedNullableElement(
                expectedType, out nullableElement) {
            literalType = nullableElement
            liftNullable = true
        }
        if !ColumnarSourceDirectCallResolver.CanAdoptIntegerLiteral(
                literalType, value, negative) {
            return false
        }
        valueFragment := plan.BeginFragment(
            fragment, nodes.Kind(node), node)
        if literalType == typeof(long) || literalType == typeof(ulong) {
            valueIndex := plan.AddInt64(value)
            plan.AppendInt64Instruction(
                ColumnarCodePlanContract.LdcI8(), valueIndex)
        } else {
        ...
        return !liftNullable
            || ColumnarNullableArgumentLowering.TryAppendValueLift(
                plan, literalType, expectedType)
    }

    static func TryFindObjectInitializerProperty(
        definition: ColumnarStructDef,
        memberName: string,
        out owner: ColumnarStructDef?,
        out property: ColumnarPropertyDef?): bool {
        owner = null
        property = null
    ...
    public static func TryResolveExactObjectMemberOwnerType(
        definition: ColumnarStructDef,
        targetType: Type,
        owner: ColumnarStructDef,
        out ownerType: Type,
        out ownerArguments: Type[]): bool {
        ...
        while currentDefinition != null {
            ...
            if SameObject(candidate, owner) {
                ...
                if currentType.get_IsGenericType()
                    && !currentType.get_IsGenericTypeDefinition() {
            ...
            if currentType.get_IsGenericType()
                && !currentType.get_IsGenericTypeDefinition() {
                currentType = SubstituteTypeArgument(
                    openBaseType, currentType.GetGenericArguments())
            } else {
    ...
    static func TryFindObjectInitializerField(
        definition: ColumnarStructDef,
        memberName: string,
        out owner: ColumnarStructDef?,
        out field: FieldBuilder?): bool {
        owner = null
        field = null
        current: ColumnarStructDef? = definition
        while current != null {
    ...
    public static func IsApprovedRuntimeObjectInitializerType(
        targetType: Type): bool {
        return targetType == typeof(JsonSerializerOptions)
    ...
    static func TryResolveExplicitUnionCase(
        nodes: ColumnarNodeTable,
        canonical: string,
        bindings: ColumnarFragmentBindings,
        out claimed: bool,
        out unionDefinition: ColumnarUnionDef?,
        out caseDefinition: ColumnarUnionCaseDef?,
        out unionType: Type,
        out caseType: Type,
        ...
        ownerClaimed := false
        if !TryResolveExactType(
                nodes, ownerCanonical, bindings,
                out resolvedUnionType, out ownerClaimed) {
            ...
            if genericSuffix.Length > 0
                && TryResolveExactType(
                    nodes, ownerHead, bindings,
                    out openOwnerType, out openOwnerClaimed)
                && TryFindSourceUnionDefinition(
                    openOwnerType, bindings, out openOwnerDefinition) {
                claimed = true
            }
            return false
        }
        ...
        if resolvedUnionType.get_IsGenericType()
            && !resolvedUnionType.get_IsGenericTypeDefinition() {
        ...
        if !TryFindSourceUnionDefinition(
                openUnionType, bindings, out selectedUnion)
            || selectedUnion == null {
            return false
        }
        claimed = true
        ...
        for pair in selectedUnion.Cases {
            ...
            if pair.Key.EndsWith(suffix, StringComparison.Ordinal) {
                if selectedCase != null
                    && !SameObject(selectedCase, pair.Value) {
                    return false
                }
        ...
        if caseArguments.Length > 0 {
            ...
            if !caseDefinitionType.get_IsGenericTypeDefinition()
                || caseDefinitionType.GetGenericArguments().Length
                    != caseArguments.Length {
                return false
            }
            try {
                resolvedCaseType = caseDefinitionType.MakeGenericType(
                    caseArguments)
            } catch {
                return false
            }
    ...
    static func TryFindSourceUnionDefinition(
        targetType: Type,
        bindings: ColumnarFragmentBindings,
        out selected: ColumnarUnionDef?): bool {
        selected = null
        ...
        for candidate in bindings.SourceUnionDefinitions {
            if candidate == null || candidate.Base == null {
                throw new InvalidOperationException(
                    "Construction union facts cannot contain null values.")
            }
            candidateType: Type = candidate.Base
            if ColumnarSourceDirectCallResolver.ExactTypeShapeMatches(
                    candidateType, openTarget) {
                if selected != null && !SameObject(selected, candidate) {
                    throw new InvalidOperationException(
                        "One construction union target cannot map to two definitions.")
                }
                selected = candidate
            }
        }
    ...
    static func TryAppendUnionCaseObjectInitializer(
        ...
        fragment: int,
        depth: int,
        definition: ColumnarUnionCaseDef,
        unionType: Type,
        caseType: Type,
        typeArguments: Type[],
        out ownership: ColumnarDirectCallOwnership,
        ...
        while index < nodes.ChildCount(node) {
            ...
            if !assigned.Add(fieldName)
                || !TryResolveUnionCaseField(
                    definition, caseType, typeArguments, fieldName,
                    out fieldHandle, out fieldType)
                || fieldHandle == null {
                return false
            }
            index += 2
        ...
        referenceCase := false
        if !TryAppendUnionCaseCreation(
                plan, definition, unionType, caseType, typeArguments,
                pairCount, out referenceCase) {
            return false
        }
        if !referenceCase {
            return true
        ...
        while index < nodes.ChildCount(node) {
            ...
            fieldHandle: FieldInfo? = null
            fieldType := typeof(object)
            if !TryResolveUnionCaseField(
                    definition, caseType, typeArguments, fieldName,
                    out fieldHandle, out fieldType)
                || fieldHandle == null
                || !TryAppendUnionCaseFieldValue(
                    nodes, source, nodes.Child(node, index + 1),
                    bindings, handles, plan, fragment, depth,
                    caseType, fieldHandle, fieldType,
                    out ownership, out legacyWholeSubtreePlanning) {
                return false
            }
            index += 2
    ...
    static func TryAppendUnionCasePositionalConstruction(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        bindings: ColumnarFragmentBindings,
        ...
        if definition.FieldOrder == null
            || definition.FieldOrder.Length != argumentCount {
            return false
        }
        ...
        if !TryAppendUnionCaseCreation(
                plan, definition, unionType, caseType, typeArguments,
                argumentCount, out referenceCase) {
            return false
        }
        if !referenceCase {
            return true
    ...
    static func TryAppendUnionCaseCreation(
        plan: ColumnarCodePlan,
        definition: ColumnarUnionCaseDef,
        unionType: Type,
        caseType: Type,
        typeArguments: Type[],
        assignmentCount: int,
        out referenceCase: bool): bool {
        referenceCase = false
        if definition.IsValueStruct {
            if assignmentCount != 0 || typeArguments.Length != 0
                || definition.ValueStructFactory == null
                || !SameObject(unionType, definition.UnionBase) {
                return false
            }
            methodIndex := plan.AddMethodWithSignature(
                definition.ValueStructFactory,
                unionType,
                new Type[](0),
                unionType,
                true,
        ...
        if !ConstructorBuilderMatches(
                definition.Ctor, definition.CaseType) {
            return false
        }
        ...
        if typeArguments.Length > 0 {
            try {
                constructor = TypeBuilder.GetConstructor(
                    caseType, definition.Ctor)
            } catch {
                return false
            }
        }
        constructorIndex := plan.AddConstructorWithSignature(
            constructor, caseType, new Type[](0))
        plan.AppendConstructorInstruction(
            ColumnarCodePlanContract.Newobj(), constructorIndex)
        referenceCase = true
        return true
    ...
    static func TryResolveUnionCaseField(
        definition: ColumnarUnionCaseDef,
        caseType: Type,
        typeArguments: Type[],
        fieldName: string,
        out fieldHandle: FieldInfo?,
        out fieldType: Type): bool {
        fieldHandle = null
        fieldType = typeof(object)
        openField: FieldBuilder? = null
        if definition.Fields == null
            || !definition.Fields.TryGetValue(fieldName, out openField)
            || openField == null || openField.get_IsStatic()
            || openField.get_IsInitOnly()
            || !SameObject(
                openField.get_DeclaringType(), definition.CaseType) {
            return false
        }
    ...
    static func TryAppendUnionCaseFieldValue(
        nodes: ColumnarNodeTable,
        source: string,
        valueNode: int,
        bindings: ColumnarFragmentBindings,
        ...
        fieldIndex := plan.AddFieldWithSignature(
            fieldHandle, caseType, fieldType, false)
        plan.AppendFieldInstruction(
            ColumnarCodePlanContract.Stfld(), fieldIndex)
        return true
    }
    ...
    static func TryAppendSourceConstruction(
        ...
        if argumentCount == 0 && !definition.IsReference
            && definition.Constructors.Count == 0 {
            AppendDefaultValueConstruction(plan, targetType)
            return true
        }
        ...
        if !ColumnarDirectCallPlanner.AppendArguments(
                ...
                depth + 1, true,
                argumentTypes, suppliedParameters, argumentFacts) {
            return false
        }
    ...
    static func TryAppendRuntimeConstruction(
        ...
        if !ColumnarDirectCallPlanner.AppendArguments(
                nodes, source, node, bindings, handles, plan, fragment,
                depth + 1, true,
                argumentTypes, parameters, argumentFacts) {
            return false
        }
    ...
    static func TryAppendClosedGenericConstruction(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        bindings: ColumnarFragmentBindings,
        ...
        ownership = ColumnarDirectCallOwnership.OwnedRejected
        legacyWholeSubtreePlanning = false
        if targetType == null || !targetType.get_IsGenericType()
            || targetType.get_IsGenericTypeDefinition() {
            return false
        }

        definition: ColumnarStructDef? = null
        if TryFindClosedSourceDefinition(
                targetType, bindings, out definition)
            && definition != null {
            return TryAppendClosedSourceConstruction(
                nodes, source, node, bindings, handles, plan,
                fragment, depth, definition, targetType,
                out ownership, out legacyWholeSubtreePlanning)
        }

        return TryAppendClosedRuntimeConstruction(
            nodes, source, node, bindings, handles, plan,
            fragment, depth, targetType,
            out ownership, out legacyWholeSubtreePlanning)
    }

    static func TryAppendClosedSourceConstruction(
        nodes: ColumnarNodeTable,
        source: string,
        node: int,
        bindings: ColumnarFragmentBindings,
        ...
        if argumentCount == 0 && definition.IsReference
            && definition.DefaultCtor != null {
            if !ConstructorBuilderMatches(
                    definition.DefaultCtor, definition.Builder) {
                throw new InvalidOperationException(
                    "Closed source default-constructor facts do not identify their open owner.")
            }
            defaultConstructor := TypeBuilder.GetConstructor(
                targetType, definition.DefaultCtor)
            defaultConstructorIndex := plan.AddConstructorWithSignature(
                defaultConstructor, targetType, new Type[](0))
            plan.AppendConstructorInstruction(
                ColumnarCodePlanContract.Newobj(), defaultConstructorIndex)
            return true
        }
        ...
        for candidate in definition.Constructors {
            ...
            parameters := SubstituteTypeArguments(
                candidate.ParamTypes, closedArguments)
            defaultsSupported := true
            defaultIndex := argumentCount
            while defaultIndex < parameters.Length {
                if !CanUseConstructorDefault(
                        nodes,
                        parameters[defaultIndex],
        ...
        selectedIndex := BestSourceConstructorIndex(
            candidateParameters, argumentTypes, argumentFacts)
        if selectedIndex < 0 {
            return false
        }
        ...
        suppliedParameters := PrefixTypes(
            selectedParameters, argumentCount)
        if !ColumnarDirectCallPlanner.AppendArguments(
                nodes, source, node, bindings, handles, plan, fragment,
                depth + 1, true,
                argumentTypes, suppliedParameters, argumentFacts) {
        ...
        while defaultIndex < selectedParameters.Length {
            if !TryAppendConstructorDefault(
                    nodes,
                    plan,
                    selectedParameters[defaultIndex],
                    selected.DefaultKinds[defaultIndex],
                    selected.DefaultTexts[defaultIndex],
                    bindings) {
                return false
        ...
        constructor := TypeBuilder.GetConstructor(
            targetType, selected.Builder)
        if constructor == null {
            return false
        }
        constructorIndex := plan.AddConstructorWithSignature(
            constructor, targetType, selectedParameters)
        plan.AppendConstructorInstruction(
            ColumnarCodePlanContract.Newobj(), constructorIndex)
        return true
    }
    ...
    static func TryAppendClosedRuntimeConstruction(
        ...
        if IsSupportedValueTupleType(targetType) {
            ...
            openConstructor = openType.GetConstructor(
            ...
            if argumentCount == 0 {
                ...
                if ColumnarSourceDirectCallResolver.ArgumentsScoreWithFacts(
                        capacityParameters, argumentTypes, argumentFacts) >= 0 {
                    ...
                    comparerType := SubstituteTypeArgument(
                        openParameters[0].get_ParameterType(),
                        targetType.GetGenericArguments())
                    comparerParameters := Types1(comparerType)
                    if ColumnarSourceDirectCallResolver.ArgumentsScoreWithFacts(
                            comparerParameters, argumentTypes, argumentFacts) < 0 {
                        return false
                    }
        ...
        if !ColumnarDirectCallPlanner.AppendArguments(
                nodes, source, node, bindings, handles, plan, fragment,
                depth + 1, true,
                argumentTypes, parameterTypes, argumentFacts) {
            return false
        }

        constructor := ResolveClosedRuntimeConstructor(
            targetType, openConstructor)
        if constructor == null {
            return false
        }
        constructorIndex := plan.AddConstructorWithSignature(
            constructor, targetType, parameterTypes)
        plan.AppendConstructorInstruction(
            ColumnarCodePlanContract.Newobj(), constructorIndex)
        return true
    }

    static func TryFindClosedSourceDefinition(
        targetType: Type,
        bindings: ColumnarFragmentBindings,
        ...
        for candidate in bindings.SourceTypeDefinitions {
            ...
            if !ColumnarSourceDirectCallResolver.ExactTypeShapeMatches(
                    candidateType, openType) {
                continue
            }
            if selected != null && !SameObject(selected, candidate) {
                throw new InvalidOperationException(
                    "One closed construction target cannot map to two source definitions.")
            }
            selected = candidate
        }
        return selected != null
    ...
    static func SubstituteTypeArguments(
        parameterTypes: Type[],
        arguments: Type[]): Type[] {
        result := new Type[](parameterTypes.Length)
        index := 0
        while index < parameterTypes.Length {
            result[index] = SubstituteTypeArgument(
                parameterTypes[index], arguments)
            index += 1
        }
        return result
    }

    static func SubstituteTypeArgument(
        signatureType: Type,
        arguments: Type[]): Type {
        if signatureType.get_IsGenericParameter()
            && signatureType.get_DeclaringMethod() == null {
            position := signatureType.get_GenericParameterPosition()
            if position < 0 || position >= arguments.Length {
                throw new InvalidOperationException(
                    "Construction generic parameter position is invalid.")
            }
        ...
        if signatureType.get_IsByRef() {
            ...
            if element == null {
                throw new InvalidOperationException(
                    "Construction by-reference signature has no element type.")
            }
        ...
        if signatureType.get_IsSZArray() {
            ...
            if element == null {
                throw new InvalidOperationException(
                    "Construction array signature has no element type.")
            }
        ...
        if signatureType.get_IsGenericType()
            ...
            while index < rawArguments.Length {
                closedArguments[index] = SubstituteTypeArgument(
                    rawArguments[index], arguments)
                index += 1
            }
    ...
    static func IsConstructibleCollectionDefinition(
    ...
    static func IsComparerCollectionDefinition(definition: Type): bool {
        name := definition.FullName
        return name == "System.Collections.Generic.Dictionary`2"
            || name == "System.Collections.Generic.HashSet`1"
    }

    static func IsSupportedValueTupleType(valueType: Type): bool {
        if valueType == null || !valueType.get_IsGenericType()
        ...
        name := definition.FullName
        return name == "System.ValueTuple`2"
    ...
    static func FindOpenComparerConstructor(
        definition: Type): ConstructorInfo? {
        ...
        while index < constructors.Length {
            ...
            if parameters.Length == 1 {
                ...
                if parameterType.get_IsGenericType()
                    && !parameterType.get_IsGenericTypeDefinition() {
                    parameterDefinition := parameterType.GetGenericTypeDefinition()
                    if parameterDefinition.FullName
    ...
    static func ResolveClosedRuntimeConstructor(
        targetType: Type,
        openConstructor: ConstructorInfo): ConstructorInfo? {
        ...
        index := 0
        while index < parameterTypes.Length {
            expected[index] = SubstituteTypeArgument(
                parameterTypes[index].get_ParameterType(), closedArguments)
            index += 1
        }
    ...
    static func TryGetConstructorArguments(
        ...
        while index < nodes.ChildCount(node) {
            if !ValueSyntaxIsAdmitted(
                    nodes, source, nodes.Child(node, index),
                    bindings, handles, depth + 1) {
                ownership = ColumnarDirectCallOwnership.NotOwned
                legacyWholeSubtreePlanning = true
                return false
        ...
        if !ColumnarDirectCallPlanner.TryGetArgumentTypes(
                nodes, source, node, bindings, handles, depth, true,
    ...
    static func BestSourceConstructorIndex(
        candidateParameters: List<Type[]>,
        argumentTypes: Type[],
        argumentFacts: ColumnarDirectCallArgumentFacts): int {
        bestIndex := -1
        bestScore := -1
        bestCount := 0
        candidateIndex := 0
        while candidateIndex < candidateParameters.Count {
            expected := PrefixTypes(
                candidateParameters[candidateIndex], argumentTypes.Length)
            score := ColumnarSourceDirectCallResolver.ArgumentsScoreWithFacts(
                expected, argumentTypes, argumentFacts)
            if score > bestScore {
    ...
    static func TryResolveStringEnumDefault(
        ...
        exactClaimed := false
        if TryResolveDeclaredStringEnumMember(
                expectedType, ownerName, memberName, bindings,
                out value, out exactClaimed) {
            return true
        }
        if exactClaimed {
            return false
    ...
    static func TryResolveNumericEnumDefault(
        ...
        exactClaimed := false
        if TryResolveDeclaredNumericEnumMember(
                expectedType, ownerName, memberName, bindings,
                out value, out exactClaimed) {
            return true
        }
    ...
    static func TryResolveDeclaredStringEnumMember(
        expectedType: Type,
        ownerName: string,
        memberName: string,
        bindings: ColumnarFragmentBindings,
    ...
    static func TryResolveDeclaredNumericEnumMember(
        expectedType: Type,
        ownerName: string,
        memberName: string,
        bindings: ColumnarFragmentBindings,
    ...
    static func ExactScopeSelectsEnumDefinition(
        ...
        return !scope.TryResolveExactExplicitType(
                ownerName,
                emptyBindings,
                out typeWithoutCandidate,
                out claimedWithoutCandidate)
            || !ColumnarSourceDirectCallResolver.ExactTypeShapeMatches(
                typeWithoutCandidate, candidateType)
    }
    ...
    static func TryResolveRuntimeEnumMember(
        ...
        if enumType is TypeBuilder
            ...
            || !enumType.get_IsEnum()
            || Enum.GetUnderlyingType(enumType).FullName != "System.Int32"
    ...
    static func TryResolveExactType(
        ...
        return TryResolveExactType(
            nodes, canonical, bindings, out resultType, out claimed)
    }
    ...
    static func TryResolveExactType(
        ...
        scope := nodes.BindingScope
        return scope != null
            && scope.TryResolveExactExplicitTypeInContext(
    ...
    static func VisibleTypeParameterHandleIsMissing(
        ...
        if genericOpen > 0
            ...
            if VisibleTypeParameterRootIsInvalid(
                    nodes,
                    baseName.Substring(0, genericOpen),
                    bindings,
                    false) {
                return true
            }
            arguments := ColumnarTypeCanonicalizer.SplitTopLevelCommas(
                baseName.Substring(
                    genericOpen + 1,
                    baseName.Length - genericOpen - 2))
            for argument in arguments {
                if VisibleTypeParameterHandleIsMissing(
                        nodes, argument, bindings) {
                    return true
                }
            }
            return false
        }
        return VisibleTypeParameterRootIsInvalid(
            nodes, baseName, bindings, true)
    }

    static func VisibleTypeParameterRootIsInvalid(
        nodes: ColumnarNodeTable,
        canonical: string,
        bindings: ColumnarFragmentBindings,
        allowExactParameter: bool): bool {
        index := 0
    ...
    static func ValueSyntaxIsAdmitted(
        ...
        if ColumnarPrimitiveBinaryPlanner.IsAdmittedSyntax(
                nodes, source, node, depth) {
            syntaxAdmitted = true
        } else if MayPlanRoot(nodes, node) {
            syntaxAdmitted = IsAdmittedConstructionValueSyntax(
                nodes, source, node, bindings, handles, depth)
        } else {
            syntaxAdmitted = ColumnarDirectCallPlanner.IsAdmittedValueSyntax(
                nodes, node, depth)
        }
        if !syntaxAdmitted {
            return false
        ...
        if !ColumnarRangeIndexPlanner.TryAppendConstructionValue(
                ...
                -1, depth, out resultType)
            || resultType.FullName == "System.Void" {
            return false
        }
