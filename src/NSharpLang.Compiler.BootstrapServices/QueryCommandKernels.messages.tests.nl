...
test "a description gains an alias suffix only when there are aliases to name" {
    assert QueryCommandKernels.GetDescriptionWithAliases("List symbols", "ls, names")
...
test "the routing refusals name the subcommand and the way out of them" {
    assert QueryCommandKernels.GetUnknownSubcommandMessage("nope")
        == "Unknown query subcommand: nope. Run 'nlc query help' for usage."
    assert QueryCommandKernels.GetNoCompilationUnitForFileMessage("Missing.nl")
        == "No compilation unit found for --file Missing.nl"
    assert QueryCommandKernels.GetNoCompilationUnitsMessage() == "No compilation units in project."
    assert QueryCommandKernels.GetProjectDirectoryNotFoundMessage("/tmp/missing")
        == "Project directory not found: /tmp/missing"
...
test "the position sentences all carry file, line and column in the same shape" {
    assert QueryCommandKernels.GetPositionUsageMessage("hover")
        == "Usage: nlc query hover --file <path> --pos <line>:<col>"
    assert QueryCommandKernels.GetInvalidPositionMessage("bad")
        == "Invalid position format: bad. Expected <line>:<col> (e.g. 5:12)"
    assert QueryCommandKernels.GetNoSymbolAtPositionMessage("Program.nl", 5, 12)
        == "No symbol found at Program.nl:5:12"
    assert QueryCommandKernels.GetNoTypeInformationAtPositionMessage("Program.nl", 5, 12)
        == "No type information found at Program.nl:5:12"
    assert QueryCommandKernels.GetNoDefinitionAtPositionMessage("Program.nl", 5, 12)
        == "No definition found at Program.nl:5:12"
    assert QueryCommandKernels.GetNoInterfaceAtPositionMessage("Program.nl", 5, 12)
        == "No interface found at Program.nl:5:12"
}

test "the json-only routes say so in their own words" {
    assert QueryCommandKernels.GetPerformanceJsonOnlyMessage()
        == "Performance facts are only available as JSON output."
    assert QueryCommandKernels.GetTrustedJsonOnlyMessage()
    ...
    assert QueryCommandKernels.GetInspectCompactTextUnsupportedMessage()
...
test "the semantic-references refusal promises, in words, that no fallback was used" {
    ...
    assert QueryCommandKernels.GetSemanticReferencesUnavailableMessage()
...
test "the documentation miss quotes the query it could not answer" {
    assert QueryCommandKernels.GetNoDocumentationMessage("Missing.Type", null)
        == "No documentation found for 'Missing.Type'."
}
