...
public class OperatorFacts {
    ...
    public static func GetBinaryClrName(op: BinaryOperator): string? {
        if op == BinaryOperator.Add { return "op_Addition" }
        if op == BinaryOperator.Subtract { return "op_Subtraction" }
        if op == BinaryOperator.Multiply { return "op_Multiply" }
        if op == BinaryOperator.Divide { return "op_Division" }
        if op == BinaryOperator.Modulo { return "op_Modulus" }
        if op == BinaryOperator.Equal { return "op_Equality" }
        if op == BinaryOperator.NotEqual { return "op_Inequality" }
        if op == BinaryOperator.BitwiseAnd { return "op_BitwiseAnd" }
        if op == BinaryOperator.BitwiseOr { return "op_BitwiseOr" }
        if op == BinaryOperator.BitwiseXor { return "op_ExclusiveOr" }
        if op == BinaryOperator.LeftShift { return "op_LeftShift" }
        if op == BinaryOperator.RightShift { return "op_RightShift" }
        if op == BinaryOperator.Less { return "op_LessThan" }
        if op == BinaryOperator.Greater { return "op_GreaterThan" }
        if op == BinaryOperator.LessOrEqual { return "op_LessThanOrEqual" }
        if op == BinaryOperator.GreaterOrEqual { return "op_GreaterThanOrEqual" }

        return null
    }

    public static func GetBinarySymbol(op: BinaryOperator): string? {
        if op == BinaryOperator.Add { return "+" }
        if op == BinaryOperator.Subtract { return "-" }
        if op == BinaryOperator.Multiply { return "*" }
        if op == BinaryOperator.Divide { return "/" }
    ...
    public static func GetUnaryClrName(op: UnaryOperator): string? {
        if op == UnaryOperator.Negate { return "op_UnaryNegation" }
        if op == UnaryOperator.Not { return "op_LogicalNot" }
        if op == UnaryOperator.BitwiseNot { return "op_OnesComplement" }
        if op == UnaryOperator.PreIncrement { return "op_Increment" }
        if op == UnaryOperator.PostIncrement { return "op_Increment" }
        if op == UnaryOperator.PreDecrement { return "op_Decrement" }
        if op == UnaryOperator.PostDecrement { return "op_Decrement" }

        return null
    }

    public static func GetUnarySymbol(op: UnaryOperator): string? {
        if op == UnaryOperator.Negate { return "-" }
        if op == UnaryOperator.Not { return "!" }
        if op == UnaryOperator.BitwiseNot { return "~" }
        if op == UnaryOperator.PreIncrement { return "++" }
    ...
    public static func IsSupportedExpressionTreeBinaryOperator(op: BinaryOperator): bool {
        if op == BinaryOperator.Add { return true }
        if op == BinaryOperator.Subtract { return true }
        if op == BinaryOperator.Multiply { return true }
        if op == BinaryOperator.Divide { return true }
        if op == BinaryOperator.Modulo { return true }
        if op == BinaryOperator.Equal { return true }
        if op == BinaryOperator.NotEqual { return true }
        if op == BinaryOperator.Less { return true }
        if op == BinaryOperator.LessOrEqual { return true }
        if op == BinaryOperator.Greater { return true }
        if op == BinaryOperator.GreaterOrEqual { return true }
        if op == BinaryOperator.And { return true }
        if op == BinaryOperator.Or { return true }
        if op == BinaryOperator.BitwiseAnd { return true }
        if op == BinaryOperator.BitwiseOr { return true }
        if op == BinaryOperator.BitwiseXor { return true }
        if op == BinaryOperator.LeftShift { return true }
        if op == BinaryOperator.RightShift { return true }

        return false
    }

    public static func IsSupportedExpressionTreeUnaryOperator(op: UnaryOperator): bool {
