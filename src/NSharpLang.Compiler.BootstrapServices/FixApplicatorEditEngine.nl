...
public class FixApplicatorEditEngine {
    ...
    public static func ApplyOrderedTextEdits(
        ...
        if !EditTableShapeIsValid(ref edits) {
            return -1
        }

        capacity := LineCapacity(source, ref edits)
        lines := new FixApplicatorLineTable { Lines: new string[](capacity), Count: 0 }
        if SplitLogicalLinesInto(source, ref lines) < 0 {
            return -1
        }

        i := 0
        while i < count {
            if ApplySingleEdit(ref lines, ref edits, i) < 0 {
                return -1
            }
    ...
    static func ValidateOrderedTextEditsCore(
        ...
        BuildLineLengthsInto(source, lineLengths)
        eofLine := lineCount + 1
    ...
    static func EditTableShapeIsValid(edits: &FixApplicatorEditTable): bool {
        if edits.Count < 0 {
            return false
        }
    ...
    static func LineCapacity(source: string, edits: &FixApplicatorEditTable): int {
        capacity := CountLogicalLines(source) + edits.Count + 1
        i := 0
        while i < edits.Count {
            if edits.NewTexts[i].Length > 0 {
                capacity = capacity + CountLogicalLines(edits.NewTexts[i]) + 1
            }
    ...
    static func CountLogicalLines(source: string): int {
        count := 1
        position := 0
    ...
    static func BuildLineLengthsInto(source: string, lineLengths: int[]): int {
        position := 0
        lineStart := 0
        lineIndex := 0
        sourceLength := source.Length
    ...
    static func SplitLogicalLinesInto(source: string, lines: &FixApplicatorLineTable): int {
        position := 0
        lineStart := 0
        lineIndex := 0
        sourceLength := source.Length
    ...
    static func ApplySingleEdit(lines: &FixApplicatorLineTable, edits: &FixApplicatorEditTable, index: int): int {
        ...
        if startLine >= lines.Count {
            if newText.Length > 0 {
                newLines := new FixApplicatorLineTable {
                    Lines: new string[](CountLogicalLines(newText)),
                    Count: 0
                }
                if SplitLogicalLinesInto(newText, ref newLines) < 0 {
                    return -1
                }
        ...
        if startLine == endLine && startColumn == endColumn {
            if startLine < lines.Count {
                ...
                column := MinInt(startColumn, line.Length)
            ...
            if newText.IndexOf('\n') >= 0 {
                ...
                if SplitLogicalLinesInto(combined, ref splitLines) < 0 {
                    return -1
                }
        ...
        if SplitLogicalLinesInto(replacement, ref replacementLines) < 0 {
            return -1
        }
    ...
    static func InsertLines(lines: &FixApplicatorLineTable, startIndex: int, insertedLines: &FixApplicatorLineTable): int {
    ...
    static func JoinLines(lines: &FixApplicatorLineTable): string {
