...
public static class CompilationStubEmitter
{
    ...
    private sealed class Writer(ProjectConfig config, IReadOnlyList<CompilationUnit> compilationUnits)
    {
        ...
        private enum TypeMemberOwnerKind
        {
            Other,
            Class,
            Struct,
        ...
        public string Write()
        {
            ...
            _output.AppendLine("#pragma warning disable CS0649, CS8618");
            _output.AppendLine();
        ...
        private void EmitNamespaceMembers(IReadOnlyList<Declaration> declarations)
        {
            ...
            foreach (var declaration in declarations)
            {
                ...
                EmitDeclaration(declaration, isInterfaceMember: false, isNestedType: false, containingTypeName: null, ownerKind: TypeMemberOwnerKind.Other);
                firstType = false;
            }
        }
            ...
            TypeMemberOwnerKind ownerKind)
        {
            switch (declaration)
            {
                ...
                case FieldDeclaration fieldDeclaration:
                    EmitField(fieldDeclaration, isInterfaceMember, ownerKind);
                    break;
        ...
        private void EmitClass(ClassDeclaration declaration, bool isNestedType)
        {
            ...
            WriteLine("{");
            _indentLevel++;
            EmitTypeMembers(declaration.Members, isInterface: false, containingTypeName: declaration.Name, ownerKind: TypeMemberOwnerKind.Class);
            _indentLevel--;
            WriteLine("}");
        }
        ...
        private void EmitStruct(StructDeclaration declaration, bool isNestedType)
        {
            ...
            WriteLine("{");
            _indentLevel++;
            EmitTypeMembers(declaration.Members, isInterface: false, containingTypeName: declaration.Name, ownerKind: TypeMemberOwnerKind.Struct);
            _indentLevel--;
            WriteLine("}");
        }
        ...
        private void EmitRecord(RecordDeclaration declaration, bool isNestedType)
        {
            ...
            EmitTypeMembers(declaration.Members, isInterface: false, containingTypeName: declaration.Name, ownerKind: TypeMemberOwnerKind.Record);
            _indentLevel--;
            WriteLine("}");
        }
        ...
        private void EmitInterface(InterfaceDeclaration declaration, bool isNestedType)
        {
            ...
            WriteLine("{");
            _indentLevel++;
            EmitTypeMembers(declaration.Members, isInterface: true, containingTypeName: declaration.Name, ownerKind: TypeMemberOwnerKind.Other);
            _indentLevel--;
            WriteLine("}");
        }
            ...
            TypeMemberOwnerKind ownerKind)
        {
            ...
            {
                EmitDeclaration(members[i], isInterface, isNestedType: true, containingTypeName, ownerKind);
        ...
        private void EmitField(FieldDeclaration declaration, bool isInterfaceMember, TypeMemberOwnerKind ownerKind)
        {
            ...
            var shouldEmitAsProperty = (ownerKind is TypeMemberOwnerKind.Class or TypeMemberOwnerKind.Record
                ...
                || declaration.PropertyModifier.HasFlag(PropertyModifier.Init);

            if (shouldEmitAsProperty)
            {
                var modifiers = GetMemberModifierString(declaration.Modifiers, declaration.Name);
                var isRequired = declaration.PropertyModifier.HasFlag(PropertyModifier.Required)
                    || (ownerKind == TypeMemberOwnerKind.Record
                        && VisibilityConventions.IsExportedIdentifier(declaration.Name, declaration.Modifiers)
                ...
                var useInit = declaration.PropertyModifier.HasFlag(PropertyModifier.Init)
                    || ownerKind == TypeMemberOwnerKind.Record;
                var accessors = useInit
                    ? "{ get; init; }"
                    : "{ get; set; }";
        ...
        private static string TranspileUnionTypeReference(UnionTypeReference union)
        {
            var arms = FlattenUnionTypeReference(union).ToList();
            if (arms.Count != 2)
                return "object";

            return $"NSharpLang.Runtime.Union<{TranspileTypeReference(arms[0])}, {TranspileTypeReference(arms[1])}>";
        }
