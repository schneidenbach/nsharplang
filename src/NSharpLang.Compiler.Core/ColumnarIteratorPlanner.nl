...
class ColumnarIteratorBodyPlanner {
    ...
    static func AppendLambda(emit: ColumnarMoveNextEmit, node: int, delegateType: Type): bool {
        ...
        if ColumnarLambdaNodeFacts.IsAsyncLambda(nodes.Kind(node)) {
            ...
            // gives it. Declining by shape beats emitting a body whose value is the task's
        ...
        ownerType: Type = emit.Context.StateMachineType
        if display != null {
            ownerBuilder = display.Definition
            ownerType = display.MethodOwnerType
        }
        ...
        if lambdaScope == null || !AppendLambdaBody(emit, nodes.Child(node, childCount - 1), lambdaMethod, lambdaScope, ownerType, returnType) {
            return false
        }
    ...
    static func AppendLambdaBody(emit: ColumnarMoveNextEmit, bodyNode: int, lambdaMethod: MethodBuilder, scope: ColumnarIteratorBodyScope, ownerType: Type, returnType: Type): bool {
        context := emit.Context
        plan := new ColumnarCodePlan()
        plan.PrepareMethodBody()
        thisArgument := plan.AddArgument(0, plan.AddType(context.StructuralTypeReferences.SelectRuntimeType(ownerType), context.StructuralTypeReferences))
