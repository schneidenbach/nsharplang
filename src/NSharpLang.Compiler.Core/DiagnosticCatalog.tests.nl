...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    codes := DctCodes()
    // 96 = 86, plus NL111 (an expression nested past what the toolchain reads), NL413 (a member that
    ...
    // (a `yield` inside a protected region that cannot resume), NL933 (an attribute outside its
    // AttributeUsage targets), NL934 (an attribute repeated without AllowMultiple) and NL935 (an
    // attribute at a position N# has none — a target prefix, or an enum member).
    assert codes.Count == 96
    ...
    // linter, the rest are the compiler's. 86 + 10 = 96, so nothing is uncounted or double-counted.
    //
