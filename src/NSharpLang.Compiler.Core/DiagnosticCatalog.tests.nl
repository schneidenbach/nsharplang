...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    codes := DctCodes()
    // 94 = 85, plus NL111 (an expression nested past what the toolchain reads), NL413 (a member that
    ...
    // `ref`, `out` or `in` parameter), NL933 (an attribute outside its AttributeUsage targets),
    ...
    // linter, the rest are the compiler's. 84 + 10 = 94, so nothing is uncounted or double-counted.
    //
