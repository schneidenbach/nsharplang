...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    ...
    // NL333 (a `using` resource with no way to be released), NL933 (an attribute outside its
    // AttributeUsage targets), NL934 (an attribute repeated without
    // AllowMultiple) and NL935 (an attribute at a position N# has none — a target prefix, or an enum member).
