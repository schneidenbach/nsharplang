...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    codes := DctCodes()
    // 93 = 85, plus NL111 (an expression nested past what the toolchain reads), NL413 (a member that
    ...
    // a type its elements cannot become), NL933 (an attribute outside its AttributeUsage targets),
    // NL934 (an attribute repeated without AllowMultiple) and NL935 (an attribute at a position N# has
    // none — a target prefix, or an enum member).
    assert codes.Count == 93
    ...
    // linter, the rest are the compiler's. 83 + 10 = 93, so nothing is uncounted or double-counted.
    //
