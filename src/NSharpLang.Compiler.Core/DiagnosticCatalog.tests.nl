...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    codes := DctCodes()
    // 92 = 85, plus NL111 (an expression nested past what the toolchain reads), NL413 (a member that
    ...
    // a type its elements cannot become), NL933 (an attribute outside its AttributeUsage targets) and
    // NL934 (an attribute repeated without AllowMultiple).
    assert codes.Count == 92
    ...
    // linter, the rest are the compiler's. 82 + 10 = 92, so nothing is uncounted or double-counted.
    //
