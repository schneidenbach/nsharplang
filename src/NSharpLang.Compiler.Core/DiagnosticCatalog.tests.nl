...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    ...
    // a type its elements cannot become), NL332 (a `yield` inside a protected region that cannot
    // resume), NL933 (an attribute outside its AttributeUsage targets) and NL934 (an attribute repeated
