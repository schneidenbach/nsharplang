...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    ...
    // delegate), NL339 (one type name declared in two files of one namespace), NL342 (a write to an interface's value member, which is a read slot), NL933 (an attribute
