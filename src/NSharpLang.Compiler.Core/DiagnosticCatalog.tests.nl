...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    codes := DctCodes()
    // 102 = 92, plus NL111 (an expression nested past what the toolchain reads), NL413 (a member that
    ...
    // delegate), NL933 (an attribute outside its AttributeUsage targets), NL934 (an attribute repeated without AllowMultiple) and
    // NL935 (an attribute at a position N# has none — a target prefix, or an enum member).
    assert codes.Count == 102
    ...
    // linter, the rest are the compiler's. 92 + 10 = 102, so nothing is uncounted or double-counted.
    //
