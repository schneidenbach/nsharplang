...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    codes := DctCodes()
    // 103 = 93, plus NL111 (an expression nested past what the toolchain reads), NL413 (a member that
    ...
    // delegate), NL339 (one type name declared in two files of one namespace), NL933 (an attribute
    ...
    // linter, the rest are the compiler's. 93 + 10 = 103, so nothing is uncounted or double-counted.
    //
