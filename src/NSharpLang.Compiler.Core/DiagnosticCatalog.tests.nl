...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    codes := DctCodes()
    // 91 = 85, plus NL111 (an expression nested past what the toolchain reads), NL413 (a member that
    // must be called or passed as a delegate), NL328 (a field initializer reaching the instance), NL329
    // (a struct field initializer with no constructor to run in), NL933 (an attribute outside its
    // AttributeUsage targets) and NL934 (an attribute repeated without AllowMultiple).
    assert codes.Count == 91
    ...
    // linter, the rest are the compiler's. 81 + 10 = 91, so nothing is uncounted or double-counted.
    //
