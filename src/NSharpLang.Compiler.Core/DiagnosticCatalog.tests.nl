...
test "EVERY code is distinct, and the catalog is exactly its two builders" {
    codes := DctCodes()
    // 106 = 96, plus NL111 (an expression nested past what the toolchain reads), NL413 (a member that
    ...
    // linter, the rest are the compiler's. 96 + 10 = 106, so nothing is uncounted or double-counted.
    //
