...
class BuildCommandKernels {
    ...
    static func BuildArgumentKind(arg: string): int {
        ...
        // A colour flag is a valueless option, not the source file to build. `DiagnosticColorPolicy`
        // reads it off the command line itself, so nothing here needs its VALUE — only its shape.
        if DiagnosticColorPolicy.IsColorOption(arg) {
            return 5
        }
