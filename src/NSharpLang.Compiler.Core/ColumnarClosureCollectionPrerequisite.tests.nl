...
test "exact Dictionary Keys result types cover every closure value shape and no namesake" {
    ...
    // `SortedDictionary.Keys` is an ORDINARY readable property on an ordinary external reference
    // receiver, so it resolves through the general arm and names its own declaring type. It is NOT a
    // dictionary key collection the closure lowering knows, which is the point of the last assertion:
    // being selectable and being one of the modelled key collections are separate questions.
