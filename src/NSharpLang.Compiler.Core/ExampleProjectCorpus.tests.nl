...
test "11-advanced-features, THE ONE EXAMPLE THE PRODUCT GATE'S OWN FILTER SKIPS" {
    // Nine sources in nine sub-directories, no top-level `.nl` and no `project.yml`, so the gate's
    // `nlc check` sweep drops it and the deleted `[Theory]` row was the only thing that looked at
    // it. The recursive walk finds all nine.
    assert EpcProjectReport("11-advanced-features") == "files=9 parseErrors=0 lint="
}
...
test "the whole corpus is 78 discovered files, zero parse errors and zero lint diagnostics" {
