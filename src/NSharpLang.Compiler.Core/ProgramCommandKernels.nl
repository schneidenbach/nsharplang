...
class ProgramCommandKernels {
    static func GetCommandKind(args: string[]): int {
        ...
        if CommandEquals(command, "help") || CommandEquals(command, "--help") || CommandEquals(command, "-h") {
            return 29
        }
    ...
    static func GetVersionText(version: string): string {
        return "nlc " + version
    }

    static func GetHelpText(version: string): string {
    ...
    // THE LOWERCASING IS THE KERNEL'S, AND IT IS INVARIANT. `Program.cs` used to fold the argument
    // itself with `args[0].ToLower()`, which is the CURRENT culture: under a Turkish culture
    // `nlc FROBNICATE` answered `Unknown command: frobnıcate`, so the CLI's error text depended
    // on the machine's locale. The host now hands the argument over untouched.
    static func GetUnknownCommandMessage(command: string): string {
        return "Unknown command: " + command.ToLowerInvariant() + ". Run 'nlc help' to see available commands."
    }

    static func GetErrorLine(message: string): string {
        return "Error: " + message
    }

    static func FormatElapsedMilliseconds(elapsedMilliseconds: long): string {
