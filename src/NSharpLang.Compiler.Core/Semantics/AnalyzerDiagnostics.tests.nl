...
func SuggestionDidYouMean(candidate: string, name: string): string {
    return "Did you mean '" + candidate + "'? Otherwise add the 'import' or package reference that provides '" + name + "'."
}
...
test "the comparison is case-insensitive but the suggestion keeps the candidate's own spelling" {
    ...
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "widget",
        SuggestionCandidates(["Widget"])
    ) == SuggestionDidYouMean("Widget", "widget")
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "WIDGET",
        SuggestionCandidates(["Widget"])
    ) == SuggestionDidYouMean("Widget", "WIDGET")
...
test "candidates shorter than three characters, and the name itself, are skipped" {
    ...
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "Ab",
        SuggestionCandidates(["Ax", "Cd", "Zz"])
    ) == SuggestionFallback("Ab")

    // Exactly three characters IS eligible — the cut is below three, not at it.
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "Abc",
        SuggestionCandidates(["Abd"])
    ) == SuggestionDidYouMean("Abd", "Abc")
    ...
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "Widget",
        SuggestionCandidates(["Widget", "Widgets"])
...
test "the nearest candidate wins, and ties keep the caller's FIRST one" {
    ...
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "Widgit",
        SuggestionCandidates(["Widgets", "Widget"])
    ) == SuggestionDidYouMean("Widget", "Widgit")
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "Widgit",
        SuggestionCandidates(["Widget", "Widgets"])
    ...
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "Widgit",
        SuggestionCandidates(["Widgot", "Widget"])
    ) == SuggestionDidYouMean("Widgot", "Widgit")
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "Widgit",
        SuggestionCandidates(["Widget", "Widgot"])
    ) == SuggestionDidYouMean("Widget", "Widgit")
    ...
    assert AnalyzerDiagnostics.UnresolvedTypeSuggestion(
        "Widgit",
        SuggestionCandidates(["Zzzzzzzz", "Ab", "Widget", "Qq"])
    ) == SuggestionDidYouMean("Widget", "Widgit")
}
