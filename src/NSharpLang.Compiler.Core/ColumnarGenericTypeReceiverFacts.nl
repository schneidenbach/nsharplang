...
// generic (`Box<int>.Create`) cannot emit yet: the constructed type is a `TypeBuilder` instantiation
// whose static storage is per-constructed-type work the emitter does not do, and the analyzer's
// NL323 refuses such a declaration outright. `TryResolveReceiverType` therefore answers `false` with
// `claimedBySource = true` so the emitter can decline at a site that says which of the two it was,
// instead of reporting an unhandled node kind.
class ColumnarGenericTypeReceiverFacts {
    ...
    static func TryResolveReceiverType(nodes: ColumnarNodeTable, source: string, node: int, bindings: ColumnarFragmentBindings, out resolved: Type, out claimedBySource: bool): bool {
        ...
        // A source-claimed head means a user-declared generic type, whose per-constructed-type static
        // storage the emitter does not own. Declining here rather than emitting is what keeps the
        // decline a decline: the analyzer has already refused the DECLARATION with NL323, so this arm
        // is only reachable while that guard is being changed.
        if claimed {
