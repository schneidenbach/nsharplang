...
// THE `using` STATEMENT AS THE KERNEL LANDS IT. Kind 77 is the synchronous statement and kind 79 the
...
class ColumnarFunctionBodyUsingProbe {
    ...
    constructor(source: string) {
        ...
        if Status >= 0 {
            ...
            n := 0
            while n < bodyNodeCount {
                if nodeKinds[n] == 77 || nodeKinds[n] == 79 {
                    if UsingNodeCount == 0 {
...
// ---- the `using` statement (kinds 77 / 79) ----
...
test "`await using` lands as kind 79, the same shape released asynchronously" {
    probe := new ColumnarFunctionBodyUsingProbe(
