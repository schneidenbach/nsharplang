...
// `src/NSharpLang.Compiler.Syntax` below it, `src/NSharpLang.Compiler.Tooling` and
// `src/NSharpLang.Compiler.Driver` above it), and the top slice, Driver, reaches every other one --
// Tooling and Core included -- through the `project:` dependencies of its project.yml, transitively.
