...
ALWAYS: For fast iteration, use `./scripts/dev.sh` as the INNER LOOP instead of the full gate. It builds the N# CLI and runs a FOCUSED slice of the test estate, deliberately skipping the benchmark, VS Code, examples, ilverify, and interop steps (seconds, not the ~5-minute gate). There is no C# unit suite: the two bodies are the compiler-service ESTATE (`src/NSharpLang.Compiler.Core/<Slice>/**/*.tests.nl`, one directory per compiler slice) and the NATIVE PROJECTS (`tests/native/<dir>`), and `./scripts/dev.sh --list` names every slice. Pass a name pattern (`./scripts/dev.sh Columnar`, which selects native projects by directory name and estate rows by name), add `--estate [filter]` for the estate alone, or use `--since` to auto-select the slices for files you changed via `git diff` (`./scripts/dev.sh --since`, or `--since <ref>`); `--since` is fail-safe — a central or unmapped change runs EVERYTHING and tells you why, so it never silently skips coverage. A pattern that matches nothing exits non-zero, because zero matched tests is not evidence. Run this constantly while building, and commit small compiler slices once the relevant focused evidence is green.
...
ALWAYS: The test-all.sh script:
  - Runs the compiler-service estate and every native N# test project (gate Step 3a)
...
ALWAYS: Testing strategy is layered, not one-size-fits-all:
  ...
  - Scope selection: prefer the narrowest semantically relevant `dev.sh` slice first, then broaden when touching shared compiler, SDK/runtime, build config, fixtures, or anything unmapped. `dev.sh --since` is fail-safe and will run the estate plus every native project for central/unmapped changes.
