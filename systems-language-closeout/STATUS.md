...
verified unchanged baseline sources. [Boundary and evidence](decodes/2026-09-06-parameter-default-ownership.md).
...
| slice | commit | what moved | durable finding | headline numbers |
|---|---|---|---|---|
| Consecutive fresh gate bootstrap reseeding | this commit after r25 at `78cf8f4a` | Refresh SDK/runtime bootstrap cache trees from the caller before each isolated run, retaining gate-specific sibling versions | Step 4 clears the package-ID cache and later restores only an IL-stamped version; directory existence cannot prove the next run has its bootstrap version. Before/after controls cover missing versions, repeated eviction and stale overlapping bytes | Shell 567/502 → 566/501; ownership 18/18 after observed two-key `head-v1:873549ff87d77915` repin; no SDK/feed republish; [record](decodes/2026-09-04-takeover-verification.md) |
| Gate unit failure evidence | this commit after r24 at `242e401d` | Explicit minimal console logger preserves assertion details alongside quiet build output | A quiet MSBuild test invocation otherwise prints only a failing test name. Invalid setup-environment control proves the assertion and child stderr survive; original installer failure remains unexplained and is not claimed fixed | Same compiled unit suite 593/593; focused installer 9/9; ownership 18/18, shell 916/815 unchanged, both observed head keys `head-v1:94e6b5916c964abc`; [record](decodes/2026-09-04-takeover-verification.md) |
