...
`738996c29` (**compression PR B**, six commits, rebased) → `c44d6206e` (**the throughput gate's
same-run control**, rebased, landed FIRST on purpose) → `1f8ba2f3b` … → `5a869e9db` (**compression
PR C**, five commits, rebased). **The gates at `5a869e9db` are RUNNING in an isolated copy as this
cursor is written, and NOTHING is claimed for them.**
...
twenty red runs after the last one. Run **`35843843835`** at **`9ef0f5b32`** is green too, and so is
...
including `35829299886` — already fired under the old `main`-only filter. What `dc2c7cdd1` actually
added is coverage for **pushes to `systems-language`** and for **PRs whose base is
`systems-language`** (the split lanes, e.g. #191). The first run under the genuinely new trigger is
therefore the **push** run **`35843838914`**, not the PR run.

**The release job's per-PR pre-release is PRE-EXISTING and DELIBERATELY KEPT.** A same-repo PR still
publishes `Unofficial pr-190 (<sha>)`; that is the behaviour the campaign has relied on since
2026-09-10. What `345e7899a` changed is the PUSH arm only: `channel` falls back to `main` for any
push, so once `systems-language` became a push trigger a push to it would have created an
`unofficial-main-*` release named for a branch it did not come from. The ref check therefore guards
the push arm alone — **push runs publish only from `main`** — and leaves the PR arm untouched.
...
| NU5026 | **ROOT-CAUSED AND FIXED**, both halves | `e85a4fc2d` (SDK), `f8deff31a` (fixture) |
| Last C# assertion project | **DELETED** — 576 lines of `.cs` → an N# native project | `f8deff31a` |
...
| Seventh reseed (packed source `eed29c71c`) | **SUCCESS** | `evidence/reseed-eed29c71c/reseed.log` |
| Gates on seed commit `03f02b764` | **both PASS** (non-VS 22m44s, VS 22m04s); pushed | `evidence/seed-03f02b764/` |
| Actions `35829299886` at `03f02b764` | **GREEN — first green since 2026-09-11** | `gh run view 35829299886` |
| Gate at `345e7899a`, non-VS | **FAILED — throughput ONLY**, at load 5.09; folded into the next gate | `evidence/combined-345e7899a/product-non-vscode.FAILED-throughput-under-load.log` |
...
| **Throughput gate — same-run control** | **LANDED**, rebased as `c44d6206e`, ahead of PR C on purpose. Step 2c is load-immune | `evidence/throughput-c44d6206e/` (9 logs) |
| **Compression PR C (audit PRs 8–10)** | **LANDED** at `5a869e9db` — five commits, rebased | `git log c44d6206e..5a869e9db --stat` |
| **Gates at `5a869e9db`** | **PENDING — running as this was written; NOTHING claimed** | none yet |
| `Compiler.Core` split S0–S7 | **DESIGNED AND MEASURED, NOT STARTED**; R1–R3 in flight on `census/sdk-incr`; D-A and D-B open | `FABLE-CORE-SPLIT-DESIGN.md` (untracked) |
...
`gh pr view 191` exactly. Three things, each measured: **net product N# −906 lines** from the
`CommandOutputKernels` (−580), `ColumnarPlannerSupport` (−275) and
`RuntimeTypeShapeFacts`/`WellKnownTypeCatalog` (−165) extractions, with CLI output byte-identical
across 116 captured runs; **estate selection FIXED** — the rows were namespaced, so
...
**THE THROUGHPUT GATE NOW MEASURES ITS OWN CONTROL — `c44d6206e`, and it landed BEFORE PR C on
purpose** (integration rule: a lane that changes the gate itself lands first, so the lanes gated after
it run under the new rules). Step 2c's verdict used to be a median measured here divided by a number
taken once, on 2026-09-01, on an idle Apple M4 (`8cf40128a`). What replaced it:

- **`ControlKernels.nl`**, a **frozen N# transcription of the six kernels those medians came from**. It
  is a transcription, not a reference, because **the C# ancestor was deleted at `a50cb4000`** ("Remove
  C# export command and transpiler") — there is nothing left to point at.
- **A `--paired` protocol.** Both sides settle the JIT together, alternating until **each** has had
  **≥ 500 ms and ≥ 40 invocations**; then **31 repetitions** per cell, one trial of each side per
  repetition, **live-first on even repetitions, control-first on odd**. The verdict is the **median of
  the per-repetition `live / control` ratios** at the **unchanged 1.20x** — deliberately *not* the
  quotient of the two medians, which under load drifts by up to 24% (a measured window is ~10 ms and a
  contended machine costs whole windows). `compare` keeps the ports' 15 / 21 trials; only the gate takes 31.
- **The stored 2026-09-01 numbers are demoted to an informational drift table**, printed after the
  verdict and **never part of the exit status** — a machine or toolchain moving under *both* sides at
  once is the one thing a same-run ratio cannot see.
- **A new `--il-shape` check on both sides.** The gate reads back **out of the emitted IL** which
  `SimdReductions` helper each of the twelve kernels calls and fails an answer `ExpectedSimdHelper` does
  not name — the vectorizer-fallback regression a shared-compiler ratio is blind to, checked directly
  instead of statistically. The paired mode also **refuses to report any timing** if the two sides ever
  compute different answers.
- **Contracts:** `tests/native/gate-script-contracts/SystemsThroughputGate.tests.nl`, new, 182 lines,
  pinning the six columns, the paired verdict, the drift table's never-gating status, the IL-shape check
  and the control's six kernels, reading the runner as text.
- **A latent `RunProcess` pipe deadlock, fixed on the way.** stdout was drained to the end before stderr
  was touched, which deadlocks against a full 64 KiB pipe; the kernel project's systems-audit warnings
  are **68 KiB** and the gate hung until killed.

| run (`evidence/throughput-c44d6206e/`) | verdict | cell range | informational drift |
|---|---|---|---|
| `quiet-1/2/3` | **PASS** 12/12 | **0.98x–1.02x** | median 0.95x–0.96x |
| `loaded-1/2/3`, six bounded `yes` spinners | **PASS** 12/12 | **0.84x–1.08x** | median **1.33x–1.38x**, worst cell 1.57x |
| `regression`, a deliberately slowed `parse-eight-digits` | **FAIL** 2/12 | **6.52x** and **6.42x**; other ten 0.98x–1.01x | median 0.96x |
| `product-non-vscode.PASSED.log`, the **full non-VS gate** | **PASS**, **25m10s** (Step 2c 2m11s) | all twelve **0.95x–1.01x** at one-minute load **11.17** on 10 cores | median **1.98x**, range 0.87x–2.43x |

**Two figures written down before this evidence landed do not match it.** `tasks/TOOLCHAIN-NATIVE.md`'s
*Lanes in flight* said the loaded runs were "every cell 0.95x-1.06x" with the drift row "2x-4x". The
logs say **0.84x–1.08x** and drift medians **1.33x–1.38x**; the only run whose drift approaches 2x is the
full gate at load 11.17 (median 1.98x, worst 2.43x), and **no log reaches 4x**. Corrected in both docs.

**THE CONSEQUENCE, which is why the change was made: gates no longer require pausing lanes.** The full
non-VS gate above passed at a one-minute load of **11.17 on a 10-core box**, and the gate log prints its
own note that the load exceeds the core count and the verdict is unaffected. **The one remaining
load-sensitive judgement is the compile-time bench's TIMING verdict**, which still needs load < 2 to be
judged and is informational when it is not.

**COMPRESSION PR C (audit PRs 8–10), `c44d6206e..5a869e9db`.** Five commits, rebased onto the throughput
control. Verdicts and headline numbers; the proofs are in the commit messages.

| commit | what moved | verdict |
|---|---|---|
| `1f8ba2f3b` | `get_X()` → property: **3,658** external-member reads measured, **3,544 converted**. The tell was the tree reading the same members both ways (`FullName` 125 accessor vs 23 property; `ReturnType` 130 vs 3) | **`Compiler.Core.dll` is BYTE-IDENTICAL** to the base build outside the PE timestamp, checksum and MVID — **24 masked bytes in 4,473,856**. Pure substitution, re-derived line by line over all 2,674 changed line pairs. **114 kept**, each family a FILED compiler gap: **CORE-C-1** (blocker) struct enumerator over a source element type declines at `emit.local.initializer` (87); **CORE-C-2** (not a blocker) value-type receiver takes a defensive copy (27) — **keeping the accessor there is what makes the byte-identity true rather than nearly true**. Core **298,189, unchanged** — same line either way. Core's own 1,390 estate accessor calls untouched: the estate is the oracle |
| `61959279a` | **`GrowArray<T>` 16 → 1** and **`AddAll<T>` 6 → 1** — **the FIRST generic functions in this compiler's own source** (2 generic classes and 0 generic functions in 298k lines), so the shape was **probed against the pinned seed BEFORE the sweep**. Plus eight copies of one 25-line O(n²) span scan → one `ParserDeclarationNameSpansDistinct`, and three byte-identical 20-line functions → one | **−434.** And the audit's W7 **−3,500 to −5,300 does not survive reading the bodies**: its 67 functions are dominated by `ParseStructDeclarationCore` at 564 lines, **which is not a copy of anything** — the family is a NAME shape, not a BODY shape. Name- and type-erased body hashing finds 1,689 redundant lines total, most of it not generic-shaped either |
| `46aa8e7c8` | `ColumnarParserKernels.nl` **16,902 lines → twelve files by concern** (1,548 … 1,013). Its first 2,061 lines were fifteen files **mechanically concatenated** behind `// ---- X.nl ----` markers, tables hoisted, functions in a 14,800-line tail — **a bootstrap artifact, not a design** | **A PURE MOVE, CHECKED NOT CLAIMED.** Against the old 16,896 body lines the twelve bodies are the **same multiset**: zero lines invented; only the fifteen markers and five formatter-collapsed blanks are not carried. Boundaries are **anchor strings, not line numbers**, so it survived a tree PRs 8–9 had edited. **`nlc query ast --json` over all 150 projects is byte-identical: 51,237,009 bytes.** **+69 lines** — twelve headers for one. `ColumnarParserKernels.tests.nl` deliberately keeps its name |
| `7b327f479` + `5a869e9db` | front door **1,317 → 1,314**, then the ownership repin | Zero additions, **three removals — two of them FALSE POSITIVES THE OLD IDIOM MANUFACTURED**: the NL905s sat under a guard the source already had, and `property.get_SetMethod() == null` **narrowed nothing**, because flow narrowing tracks a member read and cannot track a call. NL905 423 → 421, NL002 239 → 238; NL202/NL010/NL012/NL011/NL304 unmoved; 278 files carry a diagnostic, down from 279. `test-all-core.sh` is a reviewed DELIVERY row, so its row (**1,262 / 1,147 / `text-v1:e55605418fbdf21b`**) and the head moved in **both** keys → **`head-v2:e160ad289069605b`**; audit **25/25**. Both this edit and `c44d6206e`'s are **disjoint regions that survive the rebase**, so the row was measured at THIS tree |

**Measured at `5a869e9db`.** `Compiler.Core` **298,189 → 297,824**, net **−365** (−0 / −434 / +69). Front
door **1,314** — **strictly quieter across three PRs without adding a diagnostic or hiding one**. Estate
**9,621 / 0** and **129 native projects** as each commit reports them; `format --check` over Core clean.
**Nothing whole-gate (135 projects, IL verification, VS smoke) has been re-measured at this tip — the
gates are PENDING.**

**THE CAMPAIGN TOTAL, AND WHAT IT SAYS ABOUT THE AUDIT.** **PR A −906 · PR B −2,506 · PR C −365 =
`−3,777`** for audit PRs 1–10. **The audit estimated `≈20,400–24,800` lines overall and `~−18,000` for
PRs 1–10 alone.** Record it plainly: **the audit's line estimates were UPPER BOUNDS**, sized from name
shapes and census counts rather than from bodies. **W5, W6 and W7 were each re-measured and refused with
numbers** — W7 above; W1's `match` rewrite already declined in PR B (no `} else if` cascade in
`EmitExpressionCore`, and no statement-block `match` arm exists in 300k lines of N#); W6 (~−2,300) and W5
(~−600) are unstarted and should be read the same way. **What the idiom work bought is real and is not a
line count**: properties with a byte-identical assembly proving the equivalence and two gaps filed, the
first generic functions in the compiler, named node kinds, `for … in` over 842 loops, and a parser front
end that is twelve files instead of one concatenation. **THE SPEED LEVER IS THE SPLIT, NOT THE
COMPRESSION** — PR C is where that stopped being an opinion and became arithmetic.

**Also corrected here, and one conflict left OPEN.** (1) Both docs have said PR C converts `get_X()` at
**3,708** sites; **3,708 is the audit's W3 census** — measured 3,658, converted 3,544. (2) The PR B
record above says flatly *"no 140-assembly corpus"* and that the `140` in gate logs is a test-row count.
**PR C's commits name 140** ("the 140 assemblies of the examples and `tests/native` corpus are emitted
byte for byte as before") alongside the 150-project parse corpus. **Both cannot be right and this was not
resolved by rebuilding** (the tree is under gate): treat 120-vs-140 as **OPEN**, and have the next lane
print the count rather than inherit either number.

**THE `Compiler.Core` SPLIT — DESIGNED AND MEASURED, NOT STARTED.**
`census-briefs/FABLE-CORE-SPLIT-DESIGN.md` (untracked) extracted the graph from the tree rather than
assuming it.

- **Eight slices S0–S7 with sizes:** S0 `.Model` 88 files / 23,492 lines · S1 `.Syntax` 15 / 31,221 ·
  S2 `.Semantics` 144 / 79,062 · S3 `.Backend.Plan` 112 / 75,962 · S4 `.Backend.Emit` 11 / 33,135 ·
  S5 `.CodeIntel` 76 / 20,631 · S6 `.Tooling` 32 / 12,115 · S7 `.Driver` 87 / 23,136. **S2 and S3 must
  sub-split.**
- **The audit's cycle claim is wrong as stated.** It says the only genuine cycle is emitter ↔ planners.
  Under its own bucketing **fourteen of the fifteen subsystems are ONE SCC today** — only `formatter` is
  outside. At file level, 9 non-trivial SCCs, the largest **155 files / 129,989 lines (43.5% of Core)**.
- **But a file cycle only blocks the split when it crosses a slice boundary.** Buckets mapped 1:1 leave
  **601 upward refs (2.04%) in 110 pairs**; **51 file re-bucketings** take that to **69 refs (0.23%) in
  33 pairs**, each listed with a fix. A brute-force search over all 8! orderings confirms S0 → … → S7.
- **`internalsVisibleTo:` costs ZERO** — no slice needs it for another.
- **Predicted cycles: ≈13–14 s for the four leaves (S4–S7), ≈36–38 s for S2/S3, against ~2m 35s today**
  (`STREAM-DEV-EVIDENCE.md:13`, a filter matching nothing — build only). **`ColumnarIlEmitter.nl`, the
  heaviest file in the compiler at 30,528 lines, is in S4 — a leaf.**
- **The L7 prerequisite was verified from the targets and the emit task and it FAILS — twice.** (1) A
  reference assembly *is* produced but **is not one**: `SynchronizeReferenceAssembly` Cecil-rescopes the
  *implementation* assembly, stripping **no** method bodies and removing **no** non-visible members (the
  96 KB delta is the rewritten reference table, 2.2% of the file), degenerating to `File.Copy` when there
  are no reference owners. (2) Even a perfect one would not help: **`Sdk.targets:172`'s `Inputs` uses
  `@(ReferencePath)`**, whose identity for a project reference is the **implementation** assembly in
  `bin/`; `@(ReferencePathWithRefAssemblies)` is what C#'s `CoreCompile` uses and the N# task does not. A
  third, supporting finding: the **MVID is non-deterministic** (`deterministicIdProvider: null`), so
  `CopyRefAssembly` has nothing stable to compare.
- **The user therefore sanctioned R1–R3 FIRST** — reference-assembly-aware `Inputs`, deterministic MVID,
  a real reference assembly — **in flight on `census/sdk-incr`**. Also sanctioned: **compress first, then
  split** (why PRs 1–10 ran ahead of any carving); **a slower COLD build is accepted** (serial full build
  **53.5 s → 81.8 s**, eight MSBuild floors replacing one, parallel wall time ~60 s and roughly
  unchanged — **the split buys nothing on full builds and everything on the inner loop**); and the
  throughput control, which is done.
- **TWO OPEN USER DECISIONS, and nothing should be carved before they are answered.** **D-A** — DocQuery
  and Linter below or above CodeIntel (13 of the 33 residual references are this one question). **D-B** —
  what the compile-time bench measures after the split: keep a façade as the subject, or repin the corpus
  onto a named slice.
...
- **L1** (`5438bde28`) — the compile-time bench ran three `nlc build`s of Compiler.Core (**382,887 ms**
  in the last recorded in-gate run) for a median the block then **refused to read**, because Step 3a's
  own load had already declined the judgement. `BenchGateRunCount` turns that one load reading into
  the run count: three when the timing half will be judged, **one** when it will not.
- **L2** (`c74158fa3`) — `src/NSharpLang.Compiler` and `src/NSharpLang.Playground` carry the ceiling
  `-1` (BLOCKED) and were each being **fully front-end compiled — all 551 Core files, about 4m14s of
  an 11m50s step — to produce a number the step does not count**, because `check` builds Core from
  source first and that build cannot succeed while Core reports 1,318. The block is now read off
  Core's own count in the same run and **no doomed build is started**. The day Core reaches 0 the
...
- **L4** (`1428e94b5`) — product-only and tests-included builds shared one `obj`, so each one's emit
  ...
  the SEED SDK, so this bought nothing until the seventh reseed shipped it.**
- **L3** (`6e40ac705`, hardened by `94c7f4953`) — Step 3a's ~129 whole `nlc test` processes ran
  strictly one at a time. They now run under capped workers with a **pinned serial group that runs
  first**, each member for a named reason, replayed by the parent in **discovery order** so two runs
  still diff cleanly. Measured on the 10-core M4 in `9c0a12082`: **three parallel sweeps at 339 s,
  377 s and 486 s against a ~14m18s serial native half**, all three with identical counts.
  `94c7f4953` is the flake this exposed and fixed — `sdk-project-reference-boundary` failed 27/28
  because its per-row `dotnet pack` writes the SHARED `src/*/obj` and `src/*/bin`, so packs from
  three different projects overlapped; the feed is now packed **once per run, in the parent, before
  any worker exists**.

**Measured gate totals, and they are honest about the load.** At `eed29c71c`: non-VS **21m48s**, VS
**24m45s**. At `9ef0f5b32`: non-VS **19m11s**, VS **23m19s**. Against the **~33m / ~34m** pairs this
campaign was running at `0188fb5ef`, and the **42m40s / 37m58s** at `59c965af0`. Step 2d fell from
about 10m to **7m45s** and Step 3a to **8m30s**.

**Two throughput trips, recorded because they happened.** Step 2c compares against a baseline
measured **on an idle M4** and this box was building concurrently:

| gate | load at Step 2c | cells failed | worst |
|---|---|---|---|
...
failing, not the tree.** *(Superseded: this cursor previously recorded the same-run control as an OPEN
USER DECISION, "proposed and NOT done". It was sanctioned and **landed as `c44d6206e`**; the table above
is history, not a standing risk. See* The throughput gate now measures its own control.*)*
...
**1.87** one-minute; the gate's Step 2c, a few minutes later, printed **3.08**. Both are written down
because the one-minute number before the gate is not the number the gate judged on.

**NU5026 had TWO causes and both are fixed.** (1) **The SDK's own default.** `EmitNSharpIlAssembly`
has no symbol writer, so an N#-SDK project's output holds a `.dll` and **no `.pdb`** — but
`Microsoft.NET.Sdk.props` defaults `DebugType` to `portable`, the common targets put the `.pdb` into
`@(DebugSymbolsProjectOutputGroupOutput)`, and NuGet hands it to `PackTask`, which fails for a file
the compiler never claimed to write. `dotnet pack` of any N#-SDK project therefore worked **only when
the caller repaired the project's declaration from the command line**. `e85a4fc2d` (equivalent to
`census/ci2`'s `46f8644e9`) makes the project declare the truth: `DebugType` is read during
EVALUATION, so it is set in `Sdk.props` **before** the base SDK import, with a `debugType:` key in
`project.yml` for the day the emitter gains a symbol writer. (2) **The C# fixture.**
`scripts/lib/packages.sh` passes `-p:DebugSymbols=false -p:DebugType=None` for **both** compiler
packages; `tests/NSharpLang.IntegrationTests` passed them for `Compiler.Core` and **not** for
`Compiler`, and that one plain `dotnet pack` is where CI stopped.

**The last C# assertion project is gone (`f8deff31a`).** `tests/NSharpLang.IntegrationTests` —
**576 lines across 3 `.cs` files** (`ToolchainTests.cs` 337, `ToolchainFixture.cs` 170,
`DockerFactAttribute.cs` 69) — is deleted, its sln entry with it, and
`tests/native/installed-toolchain-integration` replaces it. **`tests/` holds no C# at all.**

**The new project's row split, MEASURED, because the number in circulation was the OLD project's.**
The gate prints `Passed: 14, Failed: 0, Skipped: 13, Total: 27`. That is **27 rows = 13 Docker-gated
+ 14 ungated**, not "12 + 15": the C# `ToolchainTests.cs` carried **12** `[DockerFact]` rows and
`InstalledToolchain.tests.nl` carries **13**, while `ToolchainCommandContracts.tests.nl`'s **14**
ungated rows hold everything about the Docker half that is not the daemon — the argv of every
`docker` invocation, the pack/publish command lines and the Dockerfile's own staging.

**Six language slices landed, all under the standing rule "no compatibility shims".**

| decision | spelling | commit |
|---|---|---|
| D5 exception filters | `catch e: T when cond`, a **real CLR filter** — the guard runs before the stack unwinds | `c308ef4a4` |
| D3 `in` parameters | large struct passed by read-only reference instead of copied | `f661a127a` |
| `file` REMOVED | gone from the lexer in both directions; `Modifiers.File` (32768) **deleted**, not left set by nothing | `129430505` |
| D11 camelCase types package-private | a top-level camelCase type is now emitted `assembly`, the way nested types and members already were | `129430505` |
| `type` demoted to CONTEXTUAL | a keyword at exactly one production (`type Name = Underlying`), an ordinary identifier everywhere else | `e86630495` |
| D1 explicit interface implementation | `func IEnumerable.GetEnumerator(): IEnumerator`; **NL345–NL349**; catalog 108 → 113 codes | `9ef0f5b32` |

**Language gaps that remain, each REFUSED with a named decline rather than mis-emitted.** A **generic
interface METHOD** (a generic slot's MethodImpl bits are decided from name and arity before the
signature exists), an **explicit EVENT** (the grammar has no `add`/`remove` accessors), and a
**`static` member** (no virtual slot to name). Separately, **EXT-SOURCERECV**: an extension-method
call whose receiver is a SOURCE type — `bag.Count()` — does not bind, while the static spelling
`Enumerable.Count<string>(bag)` does. `FindClosedImplementation` reads the receiver's interfaces by
reflection and a `TypeBuilder` answers nothing before `CreateType`. Same family as the wave-12
`OfType`/`Cast` finding; filed in `census-briefs/CLI2-COMPILER-BLOCKERS.md` with the measured
declines and a minimal repro, and it needs its own pass.
...
| `bea64ac92` (eighth) | `9ef0f5b32` | `d8b6e1c2e9bf4e3f313be645462fb50cd9f72b8a990c8649f6d5bafe111d5cc4` | `32fa51c71279e0fe24e55e56f4791ee33c8f956a56d43ea545562bc331fb69de` | `head-v2:48628c3d8a182bcb` (25/25) |

**Measured at the `9ef0f5b32` gate.** Estate **9,621 rows, 0 failed**. Front door **1,318 diagnostics
(at the ceiling)**, Build.Tasks **0**, both BLOCKED projects reported without the build. Step 3a
**135 projects** (11 serial, then 124 under up to 6 workers) / **4,867 rows** — 4,853 passed, 0
failed, 14 skipped, of which 13 are the Docker rows. IL verification **83 N# assemblies**, no new
errors against the baseline. *(The `9ef0f5b32` commit message's "129 projects / 4,821 rows" and
"ilverify 107 assemblies" were measured on that lane's own base, not on this combined tree; the
combined gate logs are the numbers above, and the `ilverify` line reads 83 here and 82 at
`eed29c71c`.)*

**Remaining production C#, re-measured in this worktree at `bea64ac92`** with
...
| `src/NSharpLang.TestHost` · `.LanguageServer` | **0** | **0** |
| `tests/` (all) | **0** | **0** |
| **total (production)** | **957** | **7** |

**Unchanged at 957.** Runtime: SimdReductions 472 · Result 187 · Union 149 ·
NSharpEventSubscription 53. Wasm: PlaygroundExports 62 · Program 9. Cli: `Program.cs` alone.
...
`census/throughput` is **DONE** — the same-run control, landed first as `c44d6206e`.
`census/sdk-incr` carries **R1–R3** from the split design's §3.4 — the prerequisites the user
sanctioned before any slice is carved. **Building as this was written; nothing is claimed for it.**
*(Superseded note, kept because the numbers travelled: this paragraph used to say `census/core-c` was
building and converted `get_X()` at 3,708 sites. 3,708 is the audit's census; measured 3,658,
converted 3,544.)*

**Still OWED at `5a869e9db`.** (1) **Gates at this tip** — **PENDING**, running in an isolated copy as
this was written; until they are in, the throughput control and PR C are *landed on the branch* and
*not gate-verified together*. (2) **Rendered visual IDE verification** — unchanged, five waves old;
the only successful extension reload remains the PRE-FLIP one at `2b9271828`, which built a C#
server. (3) **The rest of the `Compiler.Core` compression** — PRs 1–10 have **all** landed (A, B, C)
for a measured **−3,777**; **W6 (`Try/out`, ~−2,300) and W5 (bidirectional tables, ~−600) are
unstarted**, L5–L10 with them, and the `match` rewrite is **declined with a reason**, not pending.
**Every remaining audit line estimate is an upper bound** — that is what A, B and C measured.
(4) **The `Compiler.Core` split** — designed and measured, **not started**; **R1–R3 must land first**
(in flight) and **D-A / D-B are open user decisions**. (5) **Runtime / Playground.Wasm / the Cli
floor** — still open user decisions: **Cli 25, Runtime 861, Playground.Wasm 71** (957, unchanged),
plus **`editors/visualstudio/` 359** across 4 files, never inside the 957 and the **lowest priority of
anything here**. (6) **The 120-vs-140 corpus-assembly count** — OPEN; print it, do not inherit it.
(7) ~~**The throughput gate's baseline**~~ — **DONE**, landed as `c44d6206e`. *(The previous list's
...
language slices the audit asked for have landed, **three compression PRs have taken a measured
`−3,777` lines out of `Compiler.Core` and in doing so established that the audit's 20–25k estimate was
an upper bound and the split is the real speed lever**, and **the throughput gate measures its own
control, so gates no longer require pausing lanes** — only the compile-time bench's timing verdict
still needs load < 2 to be judged. **The toolchain floor has not moved, the split has not started, the
gates at this tip are still running, and the visual IDE proof is still owed.**
...
`git reflog show origin/systems-language`). **Twenty-four commits** since the wave-14 docs tip
`07c0a0af4`; whole-range diff **125 files, +10,287 / −3,207**. The integration checkout is clean.
**A new bootstrap seed WAS published** — `407b43f9b` republishes the two-pass seed packed at source
`c79137bc4`, superseding `977d336cf`.

| Required step | State | Receipt |
|---|---|---|
| CLI owner conversion | **AT ITS FLOOR** — 1 file, 25 lines (`Main` + `GetVersion`) | `c1edbad92`, `dc3085766` |
| TestHost ROOT DECISION | **RECORDED AND IMPLEMENTED** — N#-SDK assembly, CLI-only reference | `c1edbad92`, `b00b45a87` |
| LanguageServer conversion, lanes 4–5 | **PARTIAL** — C# **5,738 → 4,356** (−1,382, −24%); 31 files throughout | `4ddea0530`, `377d8e529`, `7e650b212`, `a4e5900ee` |
| Compiler blockers, rounds 4–7 | **10 LANDED** + 2 measurement repairs | `867f8d8a5` … `003c5b935` |
| Playground / Runtime / Wasm-hosting | **NOT STARTED**; assessed, scope is an open user decision | `census-briefs/FABLE-REMAINING-OWNERS-ASSESSMENT.md` (untracked) |
| Gates at `f0da9f94f` | **both PASS**; pushed | `evidence/combined-f0da9f94f/` |
| Gates at `47189f761` | **both PASS**; pushed | `evidence/combined-47189f761/` |
| Third real reseed (packed source `c79137bc4`) | **SUCCESS, first try** | `evidence/reseed-c79137bc4/reseed.log` |
| First gate on seed commit `407b43f9b` | **FAILED** — `daemon-command` NL402 ×4, an honest consequence of the seed | `evidence/seed-407b43f9b/product-non-vscode.FAILED-daemon-harness-at-407b43f9b.log` |
| Gates at `f59f486a8` | **both PASS** | `evidence/seed-407b43f9b/at-f59f486a8/` |
| Push to `origin/systems-language` | DONE 2026-09-21 | `f59f486a8` |
...
**Not a completion claim.** The managed-toolchain production migration is NOT complete. Measured in
the worktree at `f59f486a8` with
`find <project> -name '*.cs' -not -path '*/obj/*' -not -path '*/bin/*' | xargs wc -l`:
...
lines / zero C#**: ReflectionTestRunner 321 · XunitTestRunner 247 · TestCommandHost 230 ·
WatchCommandHost 216 · CliPipeline 132 · NativeTestLoadContext 38. Runtime: SimdReductions 472 ·
Result 187 · Union 149 · NSharpEventSubscription 53. Wasm: PlaygroundExports 62 · Program 9.

**Compiler blockers, rounds 4–7 — ten landed.** `867f8d8a5` **reference-type nullability written
into emitted metadata** (new `ColumnarNullableMetadata.nl` 270 + `ColumnarSignatureMetadataEmitter`
+139; new `census-nullable-metadata`) — the widest CLI2 defect, **closed**; `5d71175ab`
`Exception.Data` / `AggregateException.InnerExceptions`, by gating an admitted property on the
backend's own type question (new `census-exception-members`); `30b7188d3` a value stored into a
**`Nullable<T>` member of a referenced assembly**, all three write doors through
`TryEmitAssignableValue` (new `census-external-nullable-members`, 8); `d5faf8e37` a **type forwarder
lands when the back end resolves a name**, which is what put **`System.Uri`** in reach —
`ColumnarCatalogTypeAdmission` flips Missing → Found with identity `System.Private.Uri`, typeof
catalog `Uri|System|1` → `Uri|System|0` (new `census-external-type-reach`), with `f0da9f94f` saying
what an unlistable forward-target directory means; `d8076b5f7` a **sibling static inside a lambda**,
all four lambda-body emitters reading `_enclosingType` from one lexical-owner helper (new
`census-closures/LexicalStatics.nl`, 4); `ffc4558c1` **shared params expansion** — a params tail
packed at the call site, a nullable default filled (new `ColumnarParamsExpansion.nl` 133; new
`census-params-expansion`); `d62f68fd5` the **kind-38 generic-callee receiver shape**, a *parser node
shape change* (callee expression becomes child 0, type arguments from child 1) which is what makes
**chained generic calls** work, deleting `ColumnarGenericExtensionReceiverChain` outright, plus the
**ordinary** static/instance door wired to params expansion (`IsIntrinsicExcludedShape` had refused
every params parameter, so `string.Join(sep,a,b,c)` was "not modeled") and **external interface value
members + NL103 detail** — 22 files, **+1,231/−315**, the largest compiler commit in the range;
`c79137bc4` orders the expanded params tier **by conversion, not by position** (C# conversion-first)
and says what an interface left empty, naming type + slot instead of a bare `false` and walking the
inherited interface closure; `003c5b935` **`unbox.any` for a boxed external value type**, closing
census blocker 28 (new `census-conversions/BoxedValueTypes.nl`).

**Durable finding — an unsummarised test project is not a green one.** `84006a77e` and `baf0b4eed`
repaired `sdk-project-reference-boundary` rows, and `84006a77e`'s message states why nobody noticed:
that project **produces no summary at all until `src/NSharpLang.Build.Tasks` has been built**, so
**every ad-hoc sweep since reference-type nullability reached emitted metadata silently skipped it**.
It is 27/27 with Build.Tasks, LanguageServer and Playground built. `baf0b4eed` was found the same
way — a scratch republish surfaced 2 failures out of 9,429 under a tip-built seed. **Check a sweep's
row count against expectation before reading its verdict.**

**LS lanes 4–5 — 5,738 → 4,356, no file deleted, no C# file grew.** Totals are summed `currentLines`
for `src/NSharpLang.LanguageServer/**/*.cs` in `non-nsharp-growth-ratchet.v2.json` at each commit:
`4ddea0530` symbol tables + completion menu (`EditorSymbolTableFacts` +925, `EditorCompletionMenuFacts`
+558; DocumentManager 1,449→923, CompletionHandler 620→376) → **4,968**; `377d8e529` project map,
paint tables, workspace search (`EditorWorkspaceFacts` +353, `EditorSemanticTokenFacts` +161,
`EditorWorkspaceSymbolFacts` +128; DocumentManager 923→702, SemanticTokensHandler 260→141,
WorkspaceSymbolHandler 179→102) → **4,551**; `7e650b212` squiggle, rename guard, code-fix diagnostic,
highlight (four new fact owners; LspDiagnosticConverter 100→60, PrepareRenameHandler 171→135,
DocumentHighlightHandler 142→100, CodeActionHandler 217→203) → **4,416**; `a4e5900ee` both
call-hierarchy groupings + the bound model's completions (`EditorCallHierarchyFacts` +155;
CallHierarchyHandler 399→379, CompletionHandler 376→336) → **4,356**. `47189f761` then dropped the
rename guard owner's unused import. `4ddea0530` is verified by a **45-request-shape LSP stdio probe
diffing byte-identical**. New estate rows 25 + 30 + 25 + 10 = **90**; estate 9,443 → **9,520**.

**Recording gap, stated not papered over: there is NO prose owners list for the LS lanes.** The only
Markdown owners table written in the range is the CLI/TestHost one (`b00b45a87`,
`memory/components/cli-toolchain.md`). LS ownership moves are recorded **machine-readably only** —
two lines each in `tests/native/ownership-audit/OwnershipAudit.nl` plus the ratchet repin. The list
above is assembled from commit stats, not quoted. **Writing a real one is a followup.**

**TestHost ROOT DECISION — recorded.** The `nlc test` runner went into a **new N#-SDK assembly
`src/NSharpLang.TestHost`** (`TestHost.csproj` is exactly `<Project Sdk="NSharpLang.Sdk" />`),
referenced by **the CLI and nothing else** — explicitly **not** `Compiler.dll`, which the MSBuild
task host loads inside every `dotnet build` of every N# project, which is published with the
LanguageServer, and which sits in the browser-wasm trim graph; `xunit.runner.utility` moved out of
`Cli.csproj` into `TestHost.csproj`. `c1edbad92` deleted **`Program.Testing.cs` (614)** and took
`Program.cs` 163 → 74; `dc3085766` deleted **`Commands/WatchCommand.cs` (155)** and took `Program.cs`
74 → **25** (**the 163 → 25 figure is two commits, not one**). What remains is `Main` + `GetVersion`,
**irreducible** while `nlc --version` must report `Cli.dll`'s own `AssemblyInformationalVersion` —
the gate greps it for build provenance — and `typeof(Program).Assembly` is the only spelling naming
`Cli.dll` from inside it; the version is now a **parameter** carried into a watched re-entry.
**Tool package gains exactly four files and loses nothing.** `cli-command-contracts` **205 → 213**
(eight new rows, all on the reflection route, which had none) and holds at 213; byte-compare matrix
**43 → 87 cases**, byte-identical stdout/stderr/exit against a CLI built from `f0da9f94f`; native
sweep 113 projects / 4,651 / 0 / 1; estate **9,455/0**; ownership **25/25**; TestHost.dll passes IL
verification; `CS0436` **3 → 2**.

**On "stress 440/440" — quote it correctly.** The lane's recorded result is **220 repeated `nlc test`
processes on each of the two routes under CPU contention — 440 in all — 0 failures, 0 hangs**, plus
60 reflection-route runs in one host process showing 8 MB RSS growth after warm-up vs the baseline's
12 MB (so nothing stopped unloading). **No log under `evidence/` prints a "440/440" line and no
`tests/native` stress project exists.** It is the lane's own verification restated as a total, not a
gate stage.

**The three `// SEED:` workarounds collapsed at `f59f486a8`.** An N#-SDK project is compiled by the
COMMITTED seed, not HEAD, so the TestHost lane routed around three shapes until the seed moved. Each
maps 1:1 onto a round 4–7 fix now ancestral to the seed: a referenced assembly's `Nullable<T>`
property → `30b7188d3` (filed then as `4d4f8b8a6`); `AggregateException.InnerExceptions` →
`5d71175ab` (filed then as `910218d2d`); an `object` holding a `ValueTask`, blocker 28 →
`003c5b935`. `f59f486a8` is **3 files, +20/−51 — a net deletion** — and `nlc test` output over a
passing, a failing and a skipped case is **byte-identical** across the collapse in text and JSON,
plain and `--verbose`. At the collapse: `daemon-command` 32, `cli-command-contracts` 213,
`test-assembly-load-contexts` 5, `query-integration` 87, `cli-parity-audit` 53.

**Gate history — two clean pairs, a seed, a real failure, then a clean pair. Honestly, in order.**

1. **`f0da9f94f` — both PASS, pushed 03:50:58.** `evidence/combined-f0da9f94f/`,
   `load averages: 1.97 2.92 3.36` at 02:44. non-VS `EXIT=0` **32m40s cache `58d319bcbdeea52a`**
   (1961s); VS `EXIT=0` **33m19s cache `61cd99afe9b28862`** (1999s).
2. **`47189f761` — both PASS, pushed 09:01:21.** `evidence/combined-47189f761/`,
   `load averages: 1.95 3.74 4.43` at 07:54. non-VS `EXIT=0` **33m01s cache `db3d583c62efcede`**
   (1982s); VS `EXIT=0` **33m46s cache `a7810e902312041f`** (2027s).
3. **`407b43f9b` non-VS FAILED (`EXIT=1`)** on `tests/native/daemon-command` — **four NL402
   function-call errors** in `DaemonCommand.tests.nl`. **An honest consequence of the seed, not a
   regression.** `DaemonHarness.nl` spelled `QueryParameters` and `FilePositionParameters` as
   `Dictionary<string, object>`, a workaround that only compiled because the *old* seed emitted no
   reference-type nullability; the new seed carries `867f8d8a5`, so four call sites correctly stopped
   matching `Dictionary<string, object?>`. **`2a4a418be` restored the honest `object?` value slot**
   (1 file, +4/−8); `daemon-command` 32 passed, 0 failed. **A seed that starts emitting real metadata
   breaks the workarounds that existed because it did not — budget for that on every reseed.**
4. **`f59f486a8` — both PASS.** `evidence/seed-407b43f9b/at-f59f486a8/`, both logs verify against
   `SHA256SUMS`; `load averages: 1.90 2.62 2.87` at 12:36.

**Third real reseed — SUCCESS on the first try** (`evidence/reseed-c79137bc4/reseed.log`, `EXIT=0`),
at packed source `c79137bc4`. Stage 1 and stage 2 clean self-rebuilds of `Compiler.Core` after
`rm -rf obj bin` + `--force-evaluate` restore: **0 Error(s)** both. **Exact byte equality both
stages** — `reseed.sh` SHA-256-compares the restored NuGet-cache package against the verified
bootstrap package and aborts with `Error: restored NuGet cache package differs from verified
bootstrap`; it did not abort, and `verify-bootstrap.py` printed `Pinned bootstrap SDK and runtime
verified` at both stages. Estate against the new seed **9,520 / 0 / 0** in 17s. Published hashes: Sdk
`80c7b9ef99e05bab507272052a6280d83cf2b47a9ff2ba36f9a15dfe898343bf`, Runtime
`b65260863effaa092d41b0a8dbb70a5bca2538075d420f73f26bd09861f2fbaa` (previous seed: `59627b7d…` /
`b6fb63a5…`). Seed commit **`407b43f9b`**, ownership head **`head-v2:f5da8a1a06f01fed`**, **audit
25/25**; whole diff five files, six lines (Sdk nupkg 1,803,735 → 1,882,439 bytes).

**Gate pair at `f59f486a8` — both PASS.** Both runs were cache **misses** that then stored a result
(`Fresh isolated test run required: pre-commit verification`). non-VS **`EXIT=0`, 33m34s, cache
`296c64f4ba7693ec`** (2015s); VS-enabled **`EXIT=0`, 34m35s, cache `5192549b44040569`** (2075s);
DepKey `889b02b8a0bcd2a1` in both. VS Code smoke **36 passing, 0 failing, 0 pending/skipped** in 42s
(stage 0m52s); non-VS prints `Skipping VS Code tests (VSCODE_TESTS=skip)`. Stages, exactly as the
logs print them — **the two runs agree on every figure except the two noted**:

- **Estate:** `Failed: 0, Passed: 9520, Skipped: 0, Total: 9520` — **9,520 / 0**. 17s non-VS, 15s VS.
- **Native:** **122 stage rows** = 1 estate row + **121 per-project rows**. Per-project **4,717
  passed / 0 failed / 1 skipped / 4,718 total**; grand total including the estate row **14,237 / 0 /
  1 / 14,238**. Identical in both logs. The one skip is the same intentional `census-testrefs`
  **7/0/1/8**.
- **Contracts — there is no single "contracts" stage.** Four contract-named units: compiler-service
  contracts **9,520**, `cli-command-contracts` **213**, `gate-script-contracts` **38**,
  `error-docs-contract` **13**.
- **Ownership audit:** `tests/native/ownership-audit` **25 / 0 / 0 / 25**.
- **Throughput:** `PASS: 12 cells, 0 failed, tolerance 1.20x`, baseline unchanged (2026-09-01, idle
  Apple M4, `8cf40128a`). **Worst ratio 1.06x in BOTH runs but on a different cell**: non-VS
  `rolling-hash`/**64** (42.249 → 44.955 ns) at **load { 4.51 3.32 3.11 }**; VS
  `rolling-hash`/**4096** (4765.592 → 5038.510 ns) at **load { 5.15 4.17 3.72 }**. 10 cores both.
- **Self-host front door:** `Compiler.Core: 1340 diagnostics (at the ceiling)`; `Build.Tasks: 0
  diagnostics (at the ceiling)`; **`Compiler` and `Playground` BLOCKED behind Compiler.Core's own
  front door, "not counted yet"**. 11m51s non-VS / 11m55s VS.
- **IL:** `All 80 N# assemblies pass IL verification (no new errors vs baseline)`, 19s / 20s.
- **Examples:** Step 8 **26** example projects, Step 9 **38** single-file examples, Step 10 `nlc
  check` over **26** example directories — all PASSED. **Format contract gate** PASSED.

**TWO CORRECTIONS to how earlier waves quoted these logs — binding.**
(1) **The native row count was under-reported.** Earlier waves quoted "115 rows / 4,651", the
per-project figure for a different row set. These logs print **122 stage rows**, per-project
**4,717 / 0 / 1 / 4,718**, grand total **14,237 / 0 / 1 / 14,238**. Say which of the three you mean.
(2) **There is no "functional / judged / unjudged" split in these logs.** `grep -i -E
"judg|unjudg|functional"` returns **zero matches in both**. The compile-time stage is an ordinary
native test project reporting **74 / 74**. Earlier waves' "74 functional pass, timing STILL
UNJUDGED" describes a real state of affairs — no timing verdict, no load and no ms is printed for
that row — but **is not a quotation of anything these logs emit**. Do not attribute it to them.

**Extension reload SUCCEEDED at this tip; rendered visual IDE proof STILL OWED.**
`evidence/seed-407b43f9b/reload-extension.log`: `nsharp-0.6.0.vsix` packaged (**300 files, 4.92 MB**),
`Extension 'nsharp-0.6.0.vsix' was successfully installed.`, sample project opened,
**`LanguageServer.dll` pid 46453 (child of plugin host 46448) started Mon Sep 21 13:45:00 2026** —
twelve seconds after the `f59f486a8` push, so the installed extension is built from this tip.
**That closes the reload debt and nothing else.** No rendered observation has ever been made in this
campaign, and the debt now also covers lanes 4–5's editor work (symbol tables, completion menu,
project map, paint tables, workspace search, squiggle, rename guard, code-fix diagnostic, document
highlight, both call-hierarchy groupings) on top of wave 14's nine handlers and wave 13's unseen
signature help and hover accessibility marker.

**Converter census at the new tip** (`evidence/seed-407b43f9b/converter/RECEIPT.md`; converter
unchanged at `b9a49e0`, `convert-all.sh` exit 0, census exit 0, `census-merge` clean before and
after): **41 files / 58 diagnostics / 1 stub** against **43 / 62 / 2** at `977d336cf` — runtime 0 (0),
languageserver **8** (8), cli **0** (4), playground-wasm 22 (22), tests 28 (28); mapped constructs
**17,534 → 11,432**. **The −4 is entirely DELETION** — NL907 ×3 and NL301 ×1, all attached to CLI C#
that no longer exists; **NL907 is now 0**, from 31 two censuses ago. **The LanguageServer row is the
cleanest control reading in the series: 1,382 lines of C# left — 24% — and the census did not move
at all** (8 → 8, same codes, same kinds, one first-site line shifted 73:34 → 79:34).
`runtime`/`playground-wasm`/`tests` are identical row-for-row, code-for-code, construct-for-construct
and first-site-for-first-site. **The CLI row is at its floor** (25 lines / 1 file / 31 constructs / 0
diagnostics / 0 stubs) — stop quoting CLI deltas as progress. **The one converter defect this series
found is still open and unmoved**: NL402 ×4 + NL010 ×4 against the LS glue handlers, three censuses,
identical each time. Reference-root noise unchanged at **45 rows (78%)**, none of it language. The
three rebuilt projects: Runtime 0/0, **LanguageServer 0 warnings / 0 errors for the first time in
this campaign** (the 5 CS86xx nullable warnings went with the code lanes 4–5 moved), Cli 0 errors /
**2** CS0436.

**Remaining-owners assessment — recorded, NOT decided.**
`census-briefs/FABLE-REMAINING-OWNERS-ASSESSMENT.md` is untracked working material **measured at
`977d336cf`, not at this tip**. For **`src/NSharpLang.Runtime`** it recommends **"KEEP C# for this
closeout; convert as its own staged campaign afterwards"**, because (a) the ratchet already marks all
four files `campaignScope: "separate-campaign"`, so converting inside the closeout contradicts the
repo's own ratchet; (b) Runtime is the one assembly every N# program references, so emitting its
surface without `NullableAttribute`/`NullableContextAttribute` would be a **public API contract
regression on the most-referenced package in the product**; (c) lost named tuple element names would
hit `MinMaxInt32`/`CountTransitionsInt32` and the PDB gap would ship nlc's runtime dependency without
symbols; (d) **the throughput risk is unmeasured** — `SimdReductions` is the compiler's own
vectorizer lowering target and the N# port turns Roslyn's unrolled `for` headers into `while` loops,
same semantics and different IL shape, and **nothing in the repository measures this today**; and (e)
**the language is explicitly NOT the blocker** — ~828 lines of N# owner plus ~3,150 lines of
assertion already exist for all 861 lines, so conversion is mostly a *move*. It proposes a staged
S0–S5 plan and is explicit that (b)–(d) argue for converting *later*, not never. For
**`src/NSharpLang.Playground.Wasm`** it recommends **"KEEP C#, and declare it the documented
mechanical host boundary"**: 71 lines of which 9 are an empty `Main` and ~50 are one-line delegations
to `PlaygroundCompiler`, which is already an N#-SDK project; **`[JSExport]` is a source-generator
contract, not an attribute** — the .NET JS-interop **Roslyn generator** writes the marshalling stubs
and runs inside `CoreCompile`, which the N# SDK skips entirely, so this is a dependency on a Roslyn
generator, not a translation gap a compiler slice closes; `browser-wasm` is RID-specific, trimmed and
AOT-adjacent while the N# `PersistedAssemblyBuilder` path emits a plain managed assembly with no PDB.
Its concrete ask: record `PlaygroundExports.cs`, `Program.cs` and `main.js` in the ratchet as a
**permanent mechanical host boundary** rather than `existing-debt`, with the generator dependency as
the stated reason **and a size ceiling so the shim cannot grow logic**. **The NETSDK1150 error
reported earlier has its exact text recorded NOWHERE** in this repo or in `census-briefs/` — it must
be re-measured, not carried forward as if its meaning were known.

**TWO OF THE ASSESSMENT'S ARGUMENTS ARE STALE.** Measured a wave ago, it still treats "no nullability
metadata" as open, citing a line range that points at an older pass's "did not reach" list.
**Reference-type nullability is written into emitted metadata as of `867f8d8a5`, in this wave**, and
its tuple-element-names argument is stale for the same reason. Re-measure both premises before acting
on the Runtime recommendations.

**OPEN DECISIONS — the user's. Seven, not two.** A recommendation is not an answer.
(1) **Runtime scope** — does `src/NSharpLang.Runtime` convert inside this closeout at all, given the
ratchet marks it `separate-campaign`? *Rec: no, defer to its own campaign.*
(2) **Does the nullability-metadata question gate Runtime**, or is a shipped-package annotation
regression acceptable? *Rec: gate* — **but the premise is stale, see above.**
(3) **Wasm permanence** — is `Playground.Wasm` a PERMANENT mechanical host boundary, or does the
`[JSExport]` generator dependency get solved someday? *Rec: permanent; move it out of
`existing-debt` into a boundary state with a size ceiling.*
(4) **The `TestHost` NAME and the precedent it sets** — does the earlier CLI-boundary correction
still stand, was `Cli.Core` rejected on its merits or only on that lane's implementation, and should
TestHost be spelled `NSharpLang.Cli.Core` as that boundary's first tenant? *The assessment declines
to recommend and calls it a root call.* Nothing behavioural turns on it; the name and the precedent
do.
(5) **Portable PDB scheduling** — is shipping `nlc` (and eventually Runtime) without PDBs an
acceptable interim, or does the emit path get fixed first? *Rec: fix the emit path* — two shipped
packages already pack `DebugSymbols=false` and a third makes it policy by accident. This plus the
`packAsTool`/`toolCommandName`/`copyLocalLockFileAssemblies` project.yml keys is all that stands
between `src/NSharpLang.Cli` and csproj-free.
(6) **`file`/`type` as hard keywords** — still deferred by an earlier root decision; the last
recorded open *language* decision. *Rec carried forward: demote `file` to contextual.*
(7) **Explicit-interface-implementation syntax — round 7 DID propose one.**
`IReadOnlyCollection<T>` and anything inheriting `IEnumerable<T>` need it; the two `GetEnumerator`
slots differ **only in return type, and N# has no spelling for the second**. Filed as a root decision
and called a **language-surface slice, not a backend one**. It lives in
`census-briefs/CLI2-COMPILER-BLOCKERS.md`, **not** in the assessment, which predates it.
Also live: **may `GetVersion` switch to `Assembly.GetEntryAssembly()`?** The gate's provenance grep
depends on it. *Rec: yes, after one measurement under both `dotnet Cli.dll` and the apphost* — it
would differ only under a host that loads `Cli.dll` as a library, and nothing does that today. Until
that measurement exists, `typeof(Program).Assembly` stays and the CLI stays at 25 lines.

**Still open (compiler) — READ THE PROVENANCE FIRST.** The blocker files
(`census-briefs/{CLI2,LS2}-COMPILER-BLOCKERS.md`, untracked) have their newest recorded pass
**measured at `6bee3fb89`, a commit NOT in this published range and not an ancestor of `f59f486a8`**;
neither file mentions `f59f486a8`. The list is "open as of `6bee3fb89`."
**Closed this wave:** nullability metadata emission (`867f8d8a5`, **the widest CLI2 defect**),
`System.Uri` reach (`d5faf8e37`), `Exception.Data`/`InnerExceptions` (`5d71175ab`), cross-assembly
`Nullable<T>` writes (`30b7188d3`), sibling statics in lambdas (`d8076b5f7`), shared params expansion
incl. the ordinary door (`ffc4558c1`, `d62f68fd5`, `c79137bc4`), chained generic calls via the kind-38
receiver shape (`d62f68fd5`), external interface value members + NL103 detail, blocker 28's
`unbox.any` (`003c5b935`); on the LS side **all five original `BLOCK-n` and all five `ls4` entries**.
**Still open:** a **generic interface METHOD cannot be implemented implicitly** (`ILogger.Log<TState>`
— slot and impl `TState` are different `GenericTypeParameterBuilder`s compared by identity; the
remaining half of external-interface implementation, the other half landed at `57ee38341`);
**explicit interface implementation has no N# spelling** (decision 7); **a kernel call nested in
another call's argument list declines** (`emit.call.static-member-unmodeled`/`emit.return.expression`)
— recorded **not reducible**, hit five times in the TestHost lane alone; **the narrowed receiver an
UNNAMED `is T` leaves** (`if x is ValueTask { x.AsTask() }` looks up `Object.AsTask`; only the
named-binding form emits — **reported, not fixed**, and why the TestHost's `ValueTask` arm is spelled
with a binding); `new Thread(<lambda>)`/`new Thread(<method group>)`; no `volatile` modifier; **no
exception filter** (`catch (T) when (cond)`) — which is exactly the converter census's one remaining
`// CONVERT:` stub; an iterator over a directory stack (`emit.iterator.unsupported-shape`);
**`LoggerFactory.Create(configure)`** (a static call whose argument is a delegate-typed local),
named in "what the LS flip still needs"; 1-arg `JsonSerializer.Serialize(x)` and 0-arg
`JsonElement.Deserialize<T>()`; `ldelema`/`ref arr[i]` and `in` arguments; `nlc format` not
round-tripping `{{` plus the **12 out-of-gate-scope `format --check` failures** (11 under
`tests/native` + `src/NSharpLang.Playground/PlaygroundCompiler.nl`; `tests/native` is not in the
gate's format step, which is why the gate is green — carried unbroken since blockers4, **not
re-measured at this tip**); `file`/`type` as hard keywords (decision 6); **seven shapes the COMMITTED
SEED declines** when compiling Compiler.Core (`String.TrimEnd(char,char)`,
`HashCode.Add(value,comparer)`, a coalescing arg to `HashCode.Add`, `Math.Max` with a nested
struct-property arg, a free function with a nullable reference parameter, a member access on a call
result inside `assert`, a typed local from a static property returning a derived type) plus
`for file in …` reporting `parse.struct` at the class's line tens of lines above — **none blocked any
lane**, all have workarounds, all seed-path only, reported not fixed; **portable PDB / NU5026 — N#
emits NO PDB of any kind**, recorded **assessed, not implemented**; **`CS0436` duplicate-`Program`
warnings now 2**, both in the 25-line `Program.cs`, irreducible until decision 7's measurement exists
— do not silence with `NoWarn`; the **IVT metadata-grant test needs a new subject when LS flips**;
and IVT2's three original leftovers (non-friend MEMBER refusals still emit-time `NL103`; `query def`
on a metadata member → `noSymbol`; free functions of a referenced N# assembly unreachable).
**One entry is ambiguous — re-read it, do not assume it closed:** "the self-host front door is
compiled by the seeded bootstrap SDK, not HEAD." blockers5 grouped it as open; blockers6 and 7
silently narrowed the grouping so it dropped out. **Nothing records it as closed.**

**Census-briefs bookkeeping.** Round 5's verification tables cited **pre-rebase** SHAs. All **11
occurrences** across `CLI2-COMPILER-BLOCKERS.md` and `LS2-COMPILER-BLOCKERS.md` (and no other file in
`census-briefs/`) were substituted: `94c2ce813` → `84006a77e` (2), `4d4f8b8a6` → `30b7188d3` (4),
`b242332a2` → `d5faf8e37` (1), `efd5c9f41` → `f0da9f94f` (3, one a 40-character form). Every site was
a verification table, a verification heading or a commit-provenance line — **none a historical
narrative where the pre-rebase SHA is the correct record**. Corroboration: `f0da9f94f` was already
recorded in both files as round 6's and the TestHost lane's *base* while `efd5c9f41` was round 5's
*tip*, so the rename makes base and tip line up, which they did not before. Those files are untracked;
nothing there is committed.

**Lessons (also filed in §2).** An **unsummarised test project is not a green one** — check a sweep's
row count against expectation before reading its verdict. **A seed that starts emitting real metadata
breaks the workarounds that existed because it did not** — budget a failure per reseed. **A `// SEED:`
marker is a debt with a known payoff date**, and its collapse should be a net deletion verified by
byte-identical output, not by re-reasoning. **Quote a log; do not paraphrase one into a number** —
two figures this campaign kept repeating (the native row count, the compile-time "functional/judged"
split) do not survive contact with the logs. **Check whether the artifact you cite is newer or older
than your tip** — the blocker files are measured at a commit outside this range and the
remaining-owners assessment argues from a defect since fixed. **Say when a record does not exist** —
there is no prose owners list for the LS lanes and no "440/440" line in any log.

**NO COMPLETION CLAIM.**

### Census wave 14 — superseded cursor (2026-09-20)

*Superseded by the wave-15 cursor above. Retained as written.*

**Tip.** Wave 14 is integrated, **pushed, gated and reseeded** through `977d336cf` on `census/merge`,
which **equals `origin/systems-language`** (pushed 2026-09-20). Nineteen commits since the wave-13
docs tip `ec3bb3050`. The integration checkout is clean. **A new bootstrap seed WAS published** —
`977d336cf` republishes the two-pass seed packed at source `ebaf2eb1d`, superseding `6a50c373e`.

| Required step | State | Receipt |
|---|---|---|
| CLI owner conversion, lane 4 | **PARTIAL** — 4 owners moved; blocked on a **user decision** | `3195323f8`, `c8484368a`, `c61df0392` |
| LanguageServer owner conversion, lane 3 | **PARTIAL** — nine handlers → protocol glue | `f250f9773`, `fec639eb5`, `23c6c650e` |
| Compiler blockers, rounds 2–3 | **9 LANDED**; LS2 BLOCK-1's mechanism CORRECTED | `7336b2d48` … `77215f1fe` |
| Async-iterator completion race | **FIXED**, pre-existing | `ebaf2eb1d` |
...
| Gates at `ebaf2eb1d` | **both PASS**; pushed | `evidence/combined-ebaf2eb1d/` |
| Second real reseed (packed source `ebaf2eb1d`) | **SUCCESS, first try** | `evidence/reseed-ebaf2eb1d/reseed.log` |
| Gates on seed commit `977d336cf` | **both PASS** | `evidence/seed-977d336cf/` |
...
`977d336cf` with `find <project> -name '*.cs' -not -path '*/obj/*' -not -path '*/bin/*' | xargs wc -l`:
...
| `.Compiler`, `.Compiler.Core`, `.Build.Tasks`, `.Sdk` | 0 | 0 | 0 |

Remaining CLI C#: `Program.Testing.cs` 614 · `Program.cs` 163 · `Commands/WatchCommand.cs` 155.
Largest LanguageServer survivors: `DocumentManager.cs` 1,449 · `CompletionHandler.cs` 620 ·
`CallHierarchyHandler.cs` 399 · `SemanticTokensHandler.cs` 260 · `TypeHierarchyHandler.cs` 231 ·
`CodeActionHandler.cs` 217.

**What moved — CLI lane 4.** `3195323f8` daemon server + `nlc daemon` → `DaemonServer.nl` (760) and
`DaemonCommand.nl` (91), `Daemon/DaemonServer.cs` (632) and `Commands/DaemonCommand.cs` (96) both
**deleted**; `c8484368a` `nlc query` → `QueryCommand.nl` (1,401), `Commands/QueryCommand.cs` (1,081)
**deleted**; `c61df0392` `build`/`run`/`publish`/`new`/`format` → `ProgramCommands.nl` (592),
`Program.cs` **757 → 163**; `b5429fe99` reads a perf fact's path from `filePath` so an N# type can
carry it. All four owners land in `src/NSharpLang.Compiler`, not in the Cli project.

**What moved — LS lane 3.** `f250f9773` moved seven handlers' decisions into N# `Editor*Facts`
owners (on-type formatting 221→85, document link 186→77, go-to-implementation 272→104, document
symbol 262→99, inlay hint 373→81, type hierarchy 402→231, selection range 552→79); `fec639eb5` two
more (call hierarchy 734→399 `EditorCallHierarchyFacts`, semantic tokens 842→260
`EditorSemanticTokenFacts`); `23c6c650e` added five glue-level rows estate rows structurally cannot
reach (`System.Uri` canonicalisation, a link on the third line of a block comment, every document
symbol's range containing its own selection range at every depth, selection-range parents ending at
the whole file, the closing-brace edit replacing its whole line) — `language-server-handlers`
139 → 144 — and bound seven inline nullable dereferences to asserted locals, taking Core's front
door **back to 1,340 exactly**. Findings recorded rather than papered over: the inlay hint's type
text **really is nullable** (the C# declared `string` while returning it), so the owner says
`string?`; the selection-range walk's threaded target COLUMN was never supplied by any caller and is
gone, with a row saying so; the call-hierarchy outgoing walk's reach is **partial and always was**
(`for` header, `try`, `switch`, `using`, `lock` unsearched) and now says so.

**Compiler blockers, rounds 2–3 — nine landed.** `7336b2d48` a `catch` clause's type resolved in the
compilation's own type universe (new `census-catch-types` 10); `ff9c7351c` `Process`'s whole
instance surface + a struct's defaulted optional (new `census-process-members` 8); `a249df6c2` an
inherited interface member through a source-typed generic receiver (new
`census-source-typed-generic-receiver` 6); `22844df87` `System.IO.Pipelines` in the one
common-assembly table (new `census-pipelines` 3); `fb7e5e473` enumerator-protocol import repair;
`179023f47` a **compiler-owned `AssemblyLoadContext`** for package references; `c44be120b` a
by-reference argument may name a static field (`Ldsflda`; new `census-static-field-references` 8);
`d556db18c` a quoted region inside an interpolation hole is opaque; `77215f1fe` declared variance
read when a constructed generic upcasts.

**LS2 BLOCK-1's reported mechanism was WRONG — durable finding.** `nlc restore` was not the defect:
`Sdk.targets` runs `LoadProjectReferences` before `_GenerateRestoreProjectSpec`, so a `project.yml`
`nuget:` dependency already becomes a real `PackageReference`, restores, and arrives in
`@(ReferencePath)`. Writing the same items into the generated props as well would only produce
**NU1504 duplicates — measured.** The real defect was assembly **identity**: `Assembly.LoadFrom`
binds into the DEFAULT context, which holds one assembly per simple name, so inside MSBuild the
SDK's own `Microsoft.Extensions.Logging.Abstractions` answered instead of the project's 9.0.0
package; `TryLoadExactRuntimeAssembly` rightly refused the mismatched identity and the reference
stayed metadata-only, declining every signature naming one of its types at
`emit.declaration.field-type`. `ExternalAssemblyScan` now owns one load context and consults it only
after the default context fails to answer with the exact identity. **Re-measure a reported mechanism
before building on it.**

**Async-iterator completion race — `ebaf2eb1d`, pre-existing, not a regression.** An `async func*`
drive completed its pending `MoveNextAsync` from inside the body's protected regions and only then
ran the `leave`. `SetResult` releases the consumer, and the promise is built
`RunContinuationsAsynchronously` so resumption runs **beside** the frame; the `leave` and the
state-guarded handlers it walks all read `<>__state`, which the re-drive's dispatch writes first —
lose the race and a handler runs the `finally` twice. **Measured pre-existing: 35/5000 at the parent
and 37/5000 at the published base `ec3bb3050`, where `EmitAsyncComplete` is byte-identical.** After
the fix **0/5000**: relay stress 5000/5000 idle and 5000/5000 under 8-way contention (load ~11), the
named test 200/200 idle and 150/150 contended. The completion is now **recorded rather than
delivered** — stored in `<>__result`, the one `SetResult` standing past the last `EndExceptionBlock`
as the last thing `MoveNextCore` does — and the regression reads that ordering off the realized plan
rows, so it is a **deterministic fact, not a race that has to be provoked**.

**Gate history — two failures, then a clean pair, then the seed. Honestly, in order.**

1. **`23c6c650e` non-VS FAILED (`EXIT=1`, 32m41s)** on `census-internals-visible-to`. A
   **cross-lane fixture break, not a product defect**: the friend-grant fixture stood on
   `SemanticTokenLocation`, an internal record struct the LS ownership lane moved into
   `EditorSemanticTokenFacts` and deleted; 27 rows went red. `load averages: 1.90 2.50 2.86`.
2. **`11197708d`** re-points the fixture at internals the lanes **produce**: the positive arm on
   `NSharpLang.LanguageServer.Program` (named in full, because N# puts this file's free functions in
   `Tests.Program`; constructed with `newobj` on a non-public type, passed across two call
   signatures and read back through `typeof`, so the CLR re-checks the grant at run time), the
   refusal arm on `CallHierarchyProtocol`. Its **non-VS gate PASSED** (`EXIT=0`, 32m43s, cache
   `5e47abbf6cfc310c`); its **VS-enabled gate FAILED** (`EXIT=1`, 33m34s) on `census-iterators` —
   the async race above, surfacing under the VS run's higher load.
3. **`ebaf2eb1d` — both PASS** (`evidence/combined-ebaf2eb1d/`, `load averages: 1.90 2.76 3.37`):
   non-VS `EXIT=0` **32m38s cache `a53a7509754f0d53`** (1958s); VS-enabled `EXIT=0` **33m40s cache
   `7f6b99bc8493c148`** (2020s). Pushed, and the packed source for the reseed.

**Second real reseed — SUCCESS on the first try** (`evidence/reseed-ebaf2eb1d/reseed.log`, `EXIT=0`;
byte-identical to `evidence/seed-977d336cf/reseed.log`). Contrast 2026-09-19, which needed four
attempts. Stage 1 and stage 2 clean self-rebuilds of `Compiler.Core` after `rm -rf obj bin` +
`--force-evaluate` restore: **0 Warning(s), 0 Error(s)** both, 1m07.89s and 1m08.32s. **Exact byte
equality both stages** — `reseed.sh` SHA-256-compares the restored NuGet-cache package against the
verified bootstrap package and aborts with `Error: restored NuGet cache package differs from
verified bootstrap` if they differ; it did not abort, and `verify-bootstrap.py` printed
`Pinned bootstrap SDK and runtime verified` at both stages. Estate against the new seed
**9,418 / 0 / 0** in 17s. Published hashes: Sdk
`59627b7d892ed9e9c19d0a94d3f900c7a7998c97faa8fc15219a8d8f6252b9f6`, Runtime
`b6fb63a56874a534fb3546618745c2732495819f17da98d5a886efc68fca5098` (previous seed: `bf4a1f9c…` /
`35f1a227…`). Seed commit **`977d336cf`**, packed source `ebaf2eb1d`, ownership head
**`head-v2:1b2165aa23326dff`**, **audit 25/25**; the commit's whole diff is five files, six lines.

**Gate pair on the seed commit `977d336cf` — both PASS** (`evidence/seed-977d336cf/`, pinned in
`SHA256SUMS`; `load-before-gate.txt` records `load averages: 1.94 2.82 2.91` at 20:37): non-VS
**PASS `EXIT=0`, 32m37s, cache `89e9ee3b0f336d6d`** (1957s); VS-enabled **PASS `EXIT=0`, 33m40s,
cache `c8bb9fd0492ef697`** (2020s) with **36 VS Code smoke passing, 0 pending/skipped** in 42s.
Stages exactly as the non-VS log prints them:

- **Format contract gate** PASSED — `All files are properly formatted.` ×4.
- **Throughput:** `PASS: 12 cells, 0 failed, tolerance 1.20x`; **worst 1.01x**
  (`count-transitions`/4096, 583.086 → 587.840 ns); **load average { 2.98 2.86 2.91 }**, 10 cores;
  baseline unchanged (2026-09-01, idle Apple M4, `8cf40128a`). 0m32s. VS run: **worst 1.04x**
  (`rolling-hash`, both sizes, 42.249 → 44.103 and 4765.592 → 4962.264) at **load { 4.37 3.99
  3.65 }**.
- **Self-host front door:** `Compiler.Core: 1340 diagnostics (at the ceiling)`; `Build.Tasks: 0
  diagnostics (at the ceiling)`; **`Compiler` and `Playground` BLOCKED behind Compiler.Core, NOT
  COUNTED**. 11m31s.
- **Estate:** `Failed: 0, Passed: 9418, Skipped: 0, Total: 9418`, 14s (VS run 16s).
- **Native:** **115 rows, 4,651 passed, 0 failed, 1 skipped, 4,652 total**, 17m05s. Within it:
  **gate-script contracts 38/38 MEASURED**; **ownership audit 25/25**; **compile-time bench 74
  functional pass, timing STILL UNJUDGED** — no verdict, no load and no ms printed for that row.
  Do not claim it judged.
- **`nlc check`:** 26 example directories, all PASSED. **IL:** `All 80 N# assemblies pass IL
  verification (no new errors vs baseline)`, 19s.
- **Row delta fully accounted:** 110 → **115** rows (+5, all new projects); 4,593 → **4,651**
  (**+58**) = five new blocker regressions **+35** (`census-catch-types` 10,
  `census-process-members` 8, `census-static-field-references` 8,
  `census-source-typed-generic-receiver` 6, `census-pipelines` 3) plus six existing projects **+23**
  (`language-server-handlers` 139→144, `census-emit-shapes` 100→104, `cli-command-contracts`
  201→205, `daemon-command` 28→32, `readonly-dictionary-widening` 10→14,
  `sdk-project-reference-boundary` 25→27). The one skip is the same intentional
  `census-testrefs` 7/0/1/8. Estate **9,326 → 9,418 (+92)**.

**Extension reload SUCCEEDED; rendered visual IDE proof STILL OWED.**
`evidence/seed-977d336cf/reload-extension.log`: `nsharp-0.6.0.vsix` packaged (300 files, 4.87 MB),
`Extension 'nsharp-0.6.0.vsix' was successfully installed.`, sample project opened, **`LanguageServer.dll`
pid 19822 (child of plugin host 19809) started Sun Sep 20 21:45:30 2026**. The two prior waves'
"VS Code would not quit" blocker is **gone**, and the installed extension now matches `977d336cf`.
**That closes the reload debt and nothing else.** No rendered observation has ever been made, and
the debt now covers this wave's nine migrated handlers' user-visible behaviour on top of wave 13's
still-unseen signature help and hover accessibility marker.

**Converter census at the new seed** (`evidence/seed-977d336cf/converter/RECEIPT.md`; converter
unchanged at `b9a49e0`, `convert-all.sh` exit 0, `census-merge` clean before and after):
**43 files / 62 diagnostics / 2 stubs** against **51 / 86 / 11** at `d932566aa` — runtime 0 (0),
languageserver **8** (10), cli **4** (26), playground-wasm 22 (22), tests 28 (28); mapped constructs
32,591 → 17,534. **Read the delta as DELETION, not fixing:** the CLI corpus went 11 converted files
→ 3, and 22 of the CLI's 24 lost diagnostics were attached to files that no longer exist.
`runtime`/`playground-wasm`/`tests` are the control group — C# unchanged, identical row-for-row,
code-for-code and first-site-for-first-site. **Two rows moved the WRONG way: NL402 +4 and NL010 +2**,
new converter debt created by this round's own N# lanes (calls into the new `Editor*Facts` owners
and the imports around them are not yet taught).

**OPEN DECISIONS — the user's, and the CLI lane is blocked on the first.**
(1) **Test-runner host: `Compiler.dll` or a new assembly?** `Program.Testing.cs` (614 lines) holds
the xunit and reflection runners; `Compiler.csproj` does not and must not reference xunit, and
`NativeTestLoadContext` derives from `AssemblyLoadContext`. `Execute` cannot move while `nlc test`
is C#; `WatchCommand.cs` re-enters `Program.Execute`; `GetVersion` must keep reading `Cli.dll`'s
assembly or `nlc --version` starts lying. **Architectural, not a compiler defect.**
(2) **Ship the CLI as an N#-SDK project symbol-less until a portable PDB exists?**

**Still open (compiler).** Nullability metadata across the assembly boundary (the widest CLI2
defect); `System.Uri` unmodeled — which is exactly why document-link canonicalisation stayed in C#;
`JsonSerializer` 1-arg `Serialize` and `Deserialize<T>`; `OfType` on a **call receiver**
(`MakeList().OfType<T>()`; `b73d55d41` answered only the non-generic extension SLOT);
`ldelema` / `in` arguments; `nlc format` does not round-trip `{{`; **12 out-of-gate-scope
`format --check` failures** (11 under `tests/native` + `src/NSharpLang.Playground/PlaygroundCompiler.nl`
— `tests/native` is not in the gate's format step, which is why the gate is green while these stand;
carried from the dispatch brief, **not re-measured at this tip**); **3 `CS0436` duplicate-`Program`
warnings** in the Cli build, now at `Commands/WatchCommand.cs(129,20)`, `Program.cs(156,23)`,
`Program.cs(159,23)` (line numbers moved with `c61df0392`) — do not silence with `NoWarn`; and the
**IVT metadata-grant test will need a new subject when LS/Cli flip** — `11197708d` says so itself:
when `LanguageServer.Program` or `CallHierarchyProtocol` goes, `LanguageServer.dll` and its
`InternalsVisibleTo("Tests")` grant go with it and the fixture is rewritten whole. IVT2's three
original leftovers are unchanged (non-friend MEMBER refusals still emit-time `NL103`; `query def` on
a metadata member → `noSymbol`; free functions of a referenced N# assembly unreachable).

**Lessons (also filed in §2):** a conversion lane deleting a type can break another lane's fixture —
pick subjects the lanes **produce**, not ones they remove; a load-sensitive race hides in the
cheaper gate (non-VS passed and VS failed at the same commit) — never read one green gate as
covering the pair; measure "pre-existing" before calling it pre-existing (harness at the parent AND
at the published base, plus a byte-identical-emitter check); convert an ordering race into a
plan-row assertion so the regression is deterministic; a reported blocker's mechanism can be wrong
(LS2 BLOCK-1 named `nlc restore`; the defect was assembly identity in the default load context).

**NO COMPLETION CLAIM.**

### Census wave 13 — superseded cursor (2026-09-20)

*Superseded by the wave-14 cursor above. Retained as written.*
...
`origin/systems-language`**. The integration checkout is clean. Two pushes landed on 2026-09-19:
`9b4c46174` (the docs closeout at `c6ab6f2e7`) and then `d932566aa`. **19 census worktrees and
branches plus `codex/toolchain-integration` have been retired**; three stale lanes and
`census/lsconvert-source-wip` were archived by tag. The post-seed converter census has run and
measured **zero movement**.

| Required step | State | Receipt |
|---|---|---|
| Pre-seed gates (non-VS, VS Code) | DONE | `c6ab6f2e7` sections below |
| Pre-seed extension reload | DONE | `astra-c6ab6f2e/` |
| Rendered visual IDE proof | **OWED** | never produced |
| Docs, review, push | DONE | `9b4c46174` → `d932566aa` on `origin/systems-language` |
| Retirement (19 + 1) | DONE | below |
| Two-pass reseed | DONE after 3 failures | `evidence/reseed-*`, `seed-6a50c373e/reseed.log` |
| Repin + seed commit | DONE | `6a50c373e` from packed source `5d9e2de4b` |
| Post-seed gates | DONE, both PASS at `d932566aa` | `evidence/seed-6a50c373e/` |
| Post-seed converter rerun | DONE, zero delta | `seed-6a50c373e/converter/RECEIPT.md` |
| Post-seed extension reload | **FAILED, owed** | `reload-extension.log` |
| IVT2 · SIGHELP · owner conversion | **OPEN** | not started |

**Ownership rule (supersedes every earlier assignment in this file).** Root (Fable) plans, reviews,
integrates, runs shared builds and the product gates, performs extension reload and visual IDE
verification, owns the bootstrap seed, package publication, the shared caches and the worktree/branch
lifecycle, and makes every commit and push. **Implementation is delegated to Opus** in bounded
worktrees from the accepted tip; workers run no shared builds, product gates, cache operations or
commits. The Luna Max, Sol, Terra and GLM-CLI implementation assignments recorded throughout this file
and in `tasks/TOOLCHAIN-NATIVE.md` are **historical** and no longer in force.

**Not a completion claim.** The managed-toolchain production migration is NOT complete. CLI,
LanguageServer, Playground, Runtime and Wasm-hosting owners and their assertions are still
outstanding. The seed block is gone — the seed is published and gated — but **nothing was converted
against it**, which is exactly why the post-seed census moved by zero. No Runtime C# source changed
in this wave.

#### The push, the retirement, the reseed and the seed (2026-09-19)

**Push.** `9b4c46174` then `d932566aa`, both to `origin/systems-language`. The seven commits between
them are the reseed repairs and their consequences: `b73d55d41`, `f369e5d22`, `ca8381cdb`,
`23ed2f0bc`, `5d9e2de4b`, `6a50c373e`, `d932566aa`. Six of the seven exist because the reseed or the
post-seed gate found a defect.

**Retirement.** Run after the push, with every row re-checked against the **actual pushed ref**
(`git -C <worktree> status --porcelain=v1`, `git merge-base --is-ancestor <tip> <pushed ref>`, and a
cwd-in-use check). **19 census worktrees and branches removed, plus `codex/toolchain-integration`**
(`5ebd18b72`, fully absorbed). Three stale lanes — `codex/check-remaining-assertions`,
`codex/cli-native-owner`, `codex/runtime-owner` — had their **worktrees removed but their branches
kept and tagged `archive/<branch>`**. **`census/lsconvert-source-wip` was kept and tagged
`archive/census/lsconvert-source-wip`**: it is the **sole copy of the non-compiling full
LanguageServer conversion** and must not be deleted before the post-seed LS reconversion gives it a
documented disposition. `census/merge` is preserved.

**Reseed — three failures, then success.**

| attempt | outcome | defect → fix |
|---|---|---|
| `reseed-9b4c46174` | **FAILED** stage-1 self-rebuild | `NL103` on `OfType`: no receiver relation for a **non-generic** extension slot → **`b73d55d41`** |
| `reseed-b73d55d41` | **FAILED** estate, **2 of 9,295** | duplicate MSBuild attribute written twice → **`f369e5d22`** |
| scratch validation | **FAILED** the CLI build | empty public `Program` holders for types-only namespaces colliding as **`CS0433`** → **`ca8381cdb`** |
| — | — | bench corpus repin **`23ed2f0bc`** (134 → its real **135**); runbook note **`5d9e2de4b`** |
| `reseed-5d9e2de4b` | **SUCCESS**, EXIT 0 | both stages **0 warnings / 0 errors**; tests-enabled estate **9,295 passed / 0 failed / 0 skipped**; **exact hash equality bootstrap == stage-2 == restored cache** |

Pinned seed: `NSharpLang.Sdk.0.1.0.nupkg`
**`bf4a1f9c663aa1343306137adb761c440b767ba7aa02115e9e4f6fc24a0154c4`**,
`NSharpLang.Runtime.0.1.0.nupkg`
**`35f1a2271f2498fbaf79bd2cfc088ed025bd4836c28d592c661cbc6bdd3ad9f6`**. **Seed commit `6a50c373e`**
commits both `.nupkg` files and `SHA256SUMS` together; the **packed source is `5d9e2de4b`**.
Ownership head **`head-v2:5ce298ee5fe53bf8`**, ownership audit **25/25**.

#### Post-seed product gates at `d932566aa` — both PASS

**The first non-VS gate on the seed itself FAILED.** At `6a50c373e`: **EXIT 1, FAILURES: 1**, 31m49s,
cache not updated. The one failing row was `tests/native/census-duplicate-declarations`, which had
**pinned the old empty `Catalog.Program` holder** that `ca8381cdb` stopped emitting — the test
asserted the defect. Corrected by **`d932566aa`**; nothing was relaxed.

| Stage | non-VS at `d932566aa` | VS Code-enabled at `d932566aa` |
|---|---|---|
| Result | **EXIT 0**, **31m39s**, cache **`e0e1da00c8cb7ec0`** (1899s) | **EXIT 0**, **32m30s**, cache **`d6778e34d329aa6c`** (1950s) |
| Systems throughput | **12 cells, 0 failed**, tolerance 1.20x, worst **1.04x** (`rolling-hash`), load `{ 3.61 3.87 3.82 }` / 10 cores, baseline unchanged | identical 12/12 |
| Self-host front door | `Compiler.Core` **1342** *(ceiling)*, `Build.Tasks` **0**; `Compiler`/`Playground` BLOCKED, uncounted — 11m14s | identical — 11m11s |
| Compiler-service estate | **Failed 0 / Passed 9295 / Skipped 0 / Total 9295** (15s) | identical |
| Native sweep | **110 rows, 4,552 passed, 0 failed, 1 skipped**; the skip is `census-testrefs` (7/1/8) | identical |
| — ownership audit | **25 passed, 0 failed, 0 skipped** | identical |
| — compile-time bench | **74 passed, 0 failed, 0 skipped** — functional only | identical |
| VS Code smoke | skipped (`VSCODE_TESTS=skip`) | **36 passing (42s)**, 0 pending/skipped |
| `nlc check` examples · IL | 26 dirs pass · **80 assemblies**, no new errors | 26 dirs pass · 80 assemblies, no new errors |

**109 → 110 rows, 4,536 → 4,552 passed (+16), fully accounted:** the one new project is
`tests/native/source-typed-explicit-generic-extension` (**9 passed**, from `b73d55d41`);
`sdk-project-reference-boundary` 20 → 25 (`f369e5d22` added its regression inside the existing
project, registering no new one); `census-free-function-identity` 18 → 20 (`ca8381cdb`). No other
row moved.

**Two things these logs do NOT say.** (1) **No gate-script-contracts row is printed** by either log,
so 38/38 is carried from `c6ab6f2e7` and is **unmeasured at this tip**. (2) **Compile-time timing was
not judged** — the benchmark appears only as the 74-pass native row, with no timing verdict, no load
reading and no millisecond figure anywhere in either log. The single `load average` line belongs to
the throughput gate. Record 74 functional pass, timing **not judged**.

#### Post-seed converter census at `d932566aa` — zero delta

`ROOT=…/census-merge ./convert-all.sh` (exit 0) plus `run.sh census` (exit 0), 2026-09-19, converter
`b9a49e0` with `nlc 0.1.0+d932566aa`. The worktree `Cli.dll` was stale at `+6a50c373e` and was
rebuilt first (Cli and Runtime 0W/0E; LanguageServer 0 errors, 7 pre-existing CS86xx warnings).
Receipt: `evidence/seed-6a50c373e/converter/RECEIPT.md`.

**51 files, 86 diagnostics, 11 `// CONVERT:` stubs** — runtime **0**, languageserver **10**, cli
**26**, playground-wasm **22**, tests **28**. **The delta against the pre-seed census is zero on
every row**, and the per-code histogram matches code-for-code and count-for-count (NL907 31, NL412
26, NL201 18, NL202 3, NL301 2, NL010 2, NL002 1, NL303 1, NL402 1, NL905 1) down to the first-site
line and column; mapped constructs 32,591 in both.

**Zero is expected and is not progress.** No C# file under the five converted projects changed
between `c6ab6f2e7` and `d932566aa`, so byte-identical output is what correctness demands — and the
converter repo's `git status` and `git diff --stat` are byte-identical before and after the run. It
establishes that the reseed perturbed neither conversion nor diagnosis. It establishes **no**
migration: the two censuses measure the same unconverted corpus twice. The converter repo stays
dirty at the same 22 paths it started with (prior regenerated output, never committed); nothing was
committed or pushed there.

#### Still OWED after the seed

**The post-seed extension reload FAILED** — *"VS Code is still running after 30s"*
(`seed-6a50c373e/reload-extension.log`). **The installed extension is still the 13:01 build from
`c6ab6f2e7`**, so it carries a pre-seed language server and no IDE observation made now describes
the reseeded tip. **The rendered visual IDE proof remains owed** and has never been produced. The
user directed on 2026-09-19 to proceed with other work rather than block on flaky VS Code;
proceeding is sanctioned and both debts stay open.

#### Lessons (2026-09-19) — binding for the next reseed

- **Gates cannot see seed-hidden defects.** Estate probes are compiled *by the seed*, so a defect
  baked into the seed is invisible to the estate the seed compiles. All three reseed failures were
  found by the reseed, never by a passing gate. A green gate says nothing about whether that tip can
  reseed.
- **A test can pin the defect.** `census-duplicate-declarations` asserted the empty `Catalog.Program`
  holder and failed the moment it was fixed. Re-read the covering tests before calling a post-fix
  gate failure a regression.
- **Reseed scratch mode requires temporarily repointing the root `NuGet.config` feed** — undocumented,
  found by debugging. Filed as a runbook/script fix (§1 pending, `census-briefs/FOLLOWUPS.md`).
- **Two declines re-confirmed on the emit-only columnar path:** `this` as a value declines at
  `parse.struct`; a call-expression receiver `MakeList().OfType<T>()` still declines, since
  `b73d55d41` fixed only the non-generic extension-slot receiver relation. Both in
  `census-briefs/FOLLOWUPS.md`.

#### Pre-seed history: the `c6ab6f2e7` cursor

The four subsections that follow — the two `c6ab6f2e7` gate receipts, the pre-seed extension-reload
note and the pre-seed converter census — plus the retirement precheck describe the **pre-seed** tip
and were superseded on 2026-09-19 by everything above. Retained as the record of how the pre-seed
tip was validated; do not quote their numbers as current (their sweep is 109 rows / 4,536 passed and
their estate 9,291, against 110 / 4,552 / 9,295 now).

#### First valid fresh non-VS product gate — PASSED at `c6ab6f2e7`

`NSHARP_TEST_KEEP_RUN=1 VSCODE_TESTS=skip ./scripts/test-all.sh --commit` finished **EXIT 0 in 36m36s
(2,196s)**; a full-log audit found zero `FAILED:` markers and zero JSON `"failed": [1-9]` rows. Durable
evidence copy: `/Users/spencer/repos/nsharp-worktrees/evidence/astra-c6ab6f2e/`
(`astra-c6ab6f2e-product-non-vscode.log`, with `SHA256SUMS`).

| Stage | Result |
|---|---|
| Build, format | pass |
| Systems throughput | **12/12**, max ratio 1.06, baseline and ×1.20 tolerance unchanged |
...
**Do not call the compile-time stage a timing pass.** Its timing was not judged: load 3.43 (threshold
2), runs 144,486 / 147,030 / 145,668 ms, median 145,668 ms — numerically under the 185,827 ms budget,
but skipped-by-load is not green.

**The judged timing evidence is separate and earlier** (standalone, not a gate, at `edcd6294`): the
root-only idle five-build baseline measured 125,410 / 123,533 / 123,885 / 123,124 / 126,452 ms — median
**123,885 ms**, median RSS 1,521,696,768 bytes, 526 non-test files / 288,658 lines, idle load 1.91
(`artifacts/astra-build-baseline-measurement`). The full 74-test benchmark then passed 74/74 with 0
failed and 0 skipped and its verdict **was judged ok** at load 1.89: runs 126,462 / 141,056 /
148,862 ms, median 141,056 ms against baseline 123,885 ms and the ×1.5 limit 185,827 ms. Keep the two
apart in every future record.

**Seven prior full-gate attempts failed or were aborted** before this one — the analyzer-clean-source
EF fixture, repeated Systems throughput cells, a stale delivery fingerprint, and an intermittent
`setup-local.sh` dry-run row traced to a Bash 3.2 `EINTR` silent-helper failure. None is a receipt.
All were fixed in `8f4fd091`, `fc2ae92a`, `d8553f18`, `2602fa07` and `c6ab6f2e` with no baseline,
tolerance, ceiling or allowlist relaxed.

#### Fresh VS Code-enabled product gate — PASSED at `c6ab6f2e7`, 2026-09-19

`./scripts/test-all.sh --commit` with the VS Code stage **enabled** (no `VSCODE_TESTS=skip`), run
fresh at `c6ab6f2e7` on 2026-09-19, finished **EXIT 0**. Forced-fresh isolated run; it stored the
validated isolated cache result **`37b5ea361a7805a7` (1858s)**, and the gate's own timing summary
totals **30m57s**. Full-log audit: **zero `FAILED:` markers**, zero JSON `"failed": [1-9]` rows.
Evidence: `evidence/astra-c6ab6f2e/product-vscode-20260919.log` with its `SHA256SUMS` entry.

| Stage | Result (VS Code enabled) |
|---|---|
| Clean, build, format | pass (incremental clean; build 1m08s) |
| Systems throughput | **12 cells, 0 failed**, tolerance **1.20x**, worst ratio **1.03x** (`rolling-hash` 64), load average `{ 2.11 2.21 1.79 }` / 10 cores, baseline unchanged (2026-09-01, `8cf40128a`) |
| Self-host front door | pass, 11m14s — `Compiler.Core` **1342** *(at the ceiling)*, `Build.Tasks` **0**; `Compiler` / `Playground` BLOCKED behind `Compiler.Core` and not counted |
| Compiler-service estate | **Passed 9291 / Failed 0 / Skipped 0 / Total 9291** (17s) |
| Native sweep | **109 rows, 4,536 passed, 0 failed, 1 skipped, 4,537 total**; the one skip is `tests/native/census-testrefs` (7/1/8) |
| Gate-script contracts | **38 passed, 0 failed, 0 skipped** |
| Ownership audit | **25 passed, 0 failed, 0 skipped** |
| Compile-time benchmark | `tests/native/compile-time-bench` **74 passed, 0 failed, 0 skipped** — functional only, see below |
| VS Code integration smoke | **36 passing (42s)**, 0 pending/skipped (extension, diagnostics, hover, completion) |
| Pack, templates, template project, examples, `nlc check` (26 dirs) | pass |
| IL verification | **80 assemblies pass, no new errors** vs baseline |

**This log judges nothing about compile-time timing.** The benchmark appears only as the native row
above: 74 passed, 0 failed, 0 skipped. There is **no timing verdict, no load reading and no
millisecond figure** for it anywhere in the log — so record 74 functional pass, timing **not judged**,
never a timing pass. The single `load average` line in the log belongs to the Systems throughput
gate, which passed its own tolerance check.

#### Extension reload — DONE; visual IDE proof — STILL OWED

The extension was rebuilt and reinstalled on 2026-09-19 via `./scripts/reload-vscode-extension.sh`
(`nsharp-0.6.0.vsix` from this worktree at `c6ab6f2e7`, installed to
`~/.vscode/extensions/nsharp.nsharp-0.6.0`, VS Code 1.137.0, language-server process confirmed
running).

**No screenshots were taken and no rendered UI was observed** — computer-use access to VS Code was
unavailable. The visual IDE proof **remains owed**. Only protocol-level evidence exists
(`evidence/astra-c6ab6f2e/visual-ide-20260919/RECEIPT.md`): `NL202` published on a deliberate error
with correct range and severity; **0** diagnostics on the clean hello-world; hover **4/4** correct;
completion **39** `string` members after `greeting.`; output channel and server log free of
exceptions, crashes and `[ERR]` entries.

Signature help works for same-document user functions but returns **null** for external/BCL members
and for cross-file N# types. This is **pre-existing, not a regression** — identical on `systems-language`
`9faf75a1c` — and is tracked as **SIGHELP** in `census-briefs/FOLLOWUPS.md`: the C#
`SignatureHelpHandler` reads only the current document's `SymbolsInfo`. Fix it in the LanguageServer
N# conversion as a project-snapshot-driven overload-signature owner; do not grow the C# handler.

The user explicitly directed on 2026-09-19 to proceed with other work rather than block on flaky VS
Code computer-use. Proceeding is sanctioned; the visual proof stays an open debt.

**History, superseded:** an earlier VS Code-enabled attempt at `c6ab6f2e7` was interrupted by the user
at the throughput stage and terminated with exit 143 (`astra-c6ab6f2e-product-vscode.log`). An
interrupted gate is neither a pass nor a failure receipt; it is kept here as history only and no
longer describes the current state.

#### Converter census at `c6ab6f2e7` — a pre-seed census, not proof of migration

`ROOT=/Users/spencer/repos/nsharp-worktrees/census-merge ./convert-all.sh` (exit 0) plus `run.sh census`,
2026-09-19, converter `b9a49e0` with `nlc 0.1.0+c6ab6f2e7` (the stale `88cf7534c` CLI binary was
rebuilt first). Receipt: `evidence/astra-c6ab6f2e/converter-20260919/RECEIPT.md`.

**51 files, 86 diagnostics, 11 `// CONVERT:` stubs** — runtime **0**, languageserver **10**, cli **26**,
playground-wasm **22**, tests **28** — against **109** on 2026-09-14. It measures converter output
against the pre-seed compiler and **closes nothing**: 45 of the 86 rows are missing-reference-root
artifacts rather than language gaps, and the `tests` row is not comparable to 2026-09-14 because the
C# unit suite was deleted (19 files → 3). `census-merge` was clean before and after; nothing was
committed. The post-push and post-seed converter reruns remain outstanding.

#### Retirement precheck — read-only; nothing retired

`census-briefs/FABLE-RETIREMENT-AND-STALE-LANES-REVIEW.md` (2026-09-19) checked all 19 manifest
candidates against `c6ab6f2e7`: **19/19 clean**, **19/19 ancestors**, 0 unique commits each, no
process holding a cwd in any of them. Four stale codex lanes are recommended for retirement —
`codex/toolchain-integration` (`5ebd18b72`), `codex/check-remaining-assertions` (`537e248ac`, tag
first), `codex/cli-native-owner` (`89ed8508f`), `codex/runtime-owner` (`0e61b67fe`). `census-merge`
and the unmerged `census/lsconvert-source-wip` (`f81e37df2`) are must-preserve. **Nothing has been
retired**; every row is re-checked against the actual pushed ref before removal.

#### Wave-12 stream classification

**Rows updated 2026-09-19 for `d932566aa`.** These five supersede their namesakes in the table that
follows; every other row there stands and is re-validated by the post-seed gates.

| Stream | State at `d932566aa` | Receipt |
|---|---|---|
| RESEED | **DONE** after three failures | `b73d55d41` (NL103 `OfType`), `f369e5d22` (2/9295 duplicate MSBuild attribute), `ca8381cdb` (CS0433 empty `Program` holders, found in scratch validation), `23ed2f0bc` (corpus 134 → 135), `5d9e2de4b` (runbook). `reseed-5d9e2de4b` SUCCESS: both stages 0W/0E, estate 9295/0/0, exact bootstrap == stage2 == restored-cache hash equality. Seed `6a50c373e`, packed source `5d9e2de4b`. |
| SEED-HIDDEN DEFECTS | **New durable finding** | Three defects passed a green gate and were caught only by the reseed, because estate probes are seed-compiled. One of them was *pinned* by `census-duplicate-declarations`, which then failed when it was fixed → `d932566aa`. |
| RETIREMENT | **DONE** | 19 census worktrees/branches + `codex/toolchain-integration` removed against the pushed ref. Three stale lanes: worktrees removed, branches kept as `archive/<branch>`. `census/lsconvert-source-wip` kept and tagged — sole copy of the non-compiling full LS conversion. |
| PUSH / POST-SEED GATES | **DONE** | `9b4c46174` → `d932566aa` on `origin/systems-language`. Gate at the seed `6a50c373e` FAILED 1; both gates PASS at `d932566aa` (exit 0, 31m39s `e0e1da00c8cb7ec0` and 32m30s `d6778e34d329aa6c`, 36 VS Code smoke). |
| CONVERTER CENSUS | **Post-seed run DONE; reconversion OPEN** | 51 / 86 / 11 at `nlc +d932566aa`, **zero delta**. No C# input changed; a reproducibility and no-seed-regression result, not migration. |
| EXTENSION RELOAD / VISUAL IDE | **OWED** | Post-seed reload FAILED; installed extension still the 13:01 `c6ab6f2e7` build. No rendered UI ever observed. |

The historical wave-12 table follows, retained as written at `c6ab6f2e7`.

| Stream | Classification | Evidence / next required action |
|---|---|---|
| CLICONVERT | Accepted checkpoint | N# `DocCommand` owns project loading, symbol ordering, HTML/JSON/text results and browser launch. Rerun CLI and LanguageServer conversion from the published seed. |
| ENUMATTR | Accepted historical implementation | Preserve the enum declaration/member attribute evidence; revalidate through the gates and conversion results. |
| SELFHOST | Accepted repair; front door still a backlog | `88cf7534` set the ceiling to 1,342 after a 946-file report (1,329 errors, 13 warnings), zero stable-identity additions and 32 removals versus the fixed 1,374 baseline; the gate re-proved 1,342 at `c6ab6f2e7`. The ceiling is a backlog to drive to zero and must never be raised. |
| TESTSCONVERT / DEV-EVIDENCE | Accepted migration / checkpoint | Native N# tests own the migrated C# unit coverage; the estate wrapper rejects zero-executed, unmatched, skipped-only, split and mixed-failure output. Contracts 38/38 at this tip. |
| GENERICSIG | Accepted checkpoint (`580c4347`) | Retain the generic-signature, typed-lambda and lexical-owner evidence. |
| COMPLETION / EDITOR PROJECTIONS | Accepted checkpoints with read-only visual evidence | Inherited `Name: string` / CLR `Count: int`, `required init User: string` and lambda `value: int` with zero Problems. Repeat the affected-editor proof with the IDE gate. |
| INITREQ | Accepted checkpoint (`2db3c5f0`) | Metadata and method-owned generic initializer repair; ancestor of `c6ab6f2e7`. |
| STATICRECV | Accepted checkpoint (`96c51cef`) | Qualified static receivers, `must` rollback, real OmniSharp delegate initialization. |
| NAMEDARGS | Accepted checkpoint (`b21fd876`) | Final placement and source-owner repair; qualified `second: int` / `first: U` signature help proven read-only. |
| CAPTURE3 | Accepted implementation checkpoint | Generic-local `e75764af`, async `2a28e38e`, sibling `1550db30`, loop `f0c0aa55`, ledger `fd08ab819` — all ancestors of `c6ab6f2e7`. `ColumnarModifiedMemberReferenceLedger` is the one assembly-local ledger, carried through `BodyFacts`, forwarding ordinary async DirectCall, expanding no public host signature. Postfix mutation of a lambda's own parameter or local stays a documented refusal in sync and async. |
| BENCHMARK STAGE / BASELINE / WARMUP / ORDER | Accepted checkpoints | Schema-2 phase guard `602da978` (73/73, BUILD canary 0.183s, schema-1 baseline intentionally refused before timing); measured baseline `edcd6294`; warmup coverage `fc2ae92a`; stage reordering that runs throughput after build+format and before the long self-host/native stages `d8553f18`; reviewed delivery and head repin `2602fa07`. |
| INSTALLER ITERATION | Accepted checkpoint (`c6ab6f2e7`) | `packages.sh` and `setup-local.sh` package loops moved to synchronous array iteration after a proven Bash 3.2 `EINTR` silent-helper failure. Contracts 38/38, ownership 25/25; packages delivery `text-v1:ef8f2afcc9069810` (138/115), setup delivery `text-v1:4b95c568a243574b4` (401/352). |
...
| IVT2 | OPEN — only after the actual reseed | No IVT2 validation is recorded; COMPLETION's historical IVT 14 evidence does not substitute for it. |
| CLI / LanguageServer / Playground / Runtime / Wasm-hosting conversion | OPEN | Owner and assertion conversion unfinished. Preserve `census/lsconvert-source-wip` (`f81e37df2`, no registered worktree, **not** an ancestor of `c6ab6f2e7`) until post-seed reconversion documents its disposition. |
| SIGHELP | OPEN — pre-existing, fix in the LS conversion | `textDocument/signatureHelp` returns null for external/BCL members and cross-file N# types; the C# handler reads only the current document's `SymbolsInfo`. Identical on `9faf75a1c`, so not a regression. Fix as an N# overload-signature owner over the project snapshot with native regressions; do not grow the C# handler. |
| CONVERTER CENSUS | Pre-seed census taken; reruns OPEN | 2026-09-19 at `c6ab6f2e7`, converter `b9a49e0`: 51 files / 86 diagnostics / 11 stubs versus 109 on 2026-09-14. A census, not a migration proof. |
| RETIREMENT PRECHECK | Precheck done; retirement OPEN | 19/19 clean ancestors, 0 unique commits each; four stale codex lanes recommended for retirement; `census/lsconvert-source-wip` preserved. Nothing removed. |
| FINAL GATES / PUSH / RETIREMENT | Gates PASSED; visual proof, push and retirement OPEN | Both required fresh gates have passed at `c6ab6f2e7` — the non-VS gate and the VS Code-enabled gate of 2026-09-19. The visual IDE proof is still owed; nothing has been pushed, reseeded, reconverted or retired. Follow the sequence below exactly. |

#### Accounting — the repin is now unblocked and still owed

Checkpoint accounting carried from `88cf7534`: catalog 108 (98 compiler plus 10 linter), 103 native
projects, corpus pin 134, and a net 10,919 C# lines removed versus `systems-language`; no Runtime source
changed. **Two of those numbers have since moved.** The `c6ab6f2e7` gate reported **109 native rows**
and the post-seed gates report **110** — the added project is
`tests/native/source-typed-explicit-generic-extension` (`b73d55d41`). The compile-time bench corpus
pin was corrected by `23ed2f0bc` from **134** to its real **135**, because that same new project had
been added without moving the pin, so the row whose job is to notice a corpus change was the one
thing the change did not reach. The reseed has now happened, so re-measure and repin catalog,
native-project, corpus, line-delta and ownership facts. Do not infer a Runtime conversion or a
completed production migration from these numbers.

#### Required closeout sequence — steps 1 and 3–8 have receipts; 2, 9, 10, 11 do not

**Updated 2026-09-19.** Steps 3–8 have executed. What remains is step 2's visual proof, the failed
post-seed reload, and steps 9–11, none of which has started.

1. ~~**Fresh VS Code-enabled gate** at the accepted tip.~~ **DONE 2026-09-19** — EXIT 0, isolated cache
   result `37b5ea361a7805a7` (1858s); receipt above. Its compile-time timing was **not judged** and must
   not be restated as a timing pass. Superseded by the post-seed gates at `d932566aa`.
2. **Extension reload and visual IDE verification** — ~~pre-seed rebuild and reinstall (DONE
   2026-09-19, `nsharp-0.6.0.vsix` via `./scripts/reload-vscode-extension.sh`)~~; **the post-seed
   reload FAILED** (VS Code would not exit within 30s) so the installed extension is still the 13:01
   `c6ab6f2e7` build and must be reloaded again. **Visual verification still OWED**: observe the
   affected completion, hover, signature-help and diagnostic behavior in the real rendered editor and
   capture screenshots — after a successful reload, so that what is observed is the reseeded build.
   Protocol-level probes do not discharge this.
3. ~~**Tracked closeout documents**, reviewed root commit, push the first finalized integration ref.~~
   **DONE 2026-09-19** — `9b4c46174` then `d932566aa` on `origin/systems-language`.
4. ~~**Converter rerun after each push**
   (`ROOT=/Users/spencer/repos/nsharp-worktrees/census-merge ./convert-all.sh`).~~ **DONE 2026-09-19
   at `d932566aa`** — 51 / 86 / 11, zero delta. The formerly dirty `out/languageserver/project.yml`
   was absorbed by the pre-seed run and no longer appears. **This did not close LS/CLI
   reconversion**: nothing was converted, so nothing moved.
5. ~~**Worktree retirement — 19 candidates**, re-checked per row against the actual pushed ref.~~
   **DONE 2026-09-19** — 19 + `codex/toolchain-integration` removed; three stale lanes and
   `census/lsconvert-source-wip` kept as `archive/` tags; `census/merge` preserved.
6. ~~**Actual two-pass reseed** (`./scripts/reseed.sh`), root alone.~~ **DONE 2026-09-19 at
   `5d9e2de4b`, after three failed attempts** — both stages 0W/0E, estate 9,295/0/0, exact hash
   equality across bootstrap, stage-2 and restored cache. **Next time:** scratch mode requires
   temporarily repointing the root `NuGet.config` feed — undocumented; filed as a runbook/script fix.
7. ~~**Repin** bootstrap fingerprints and the ownership head; commit packed source, `.nupkg` files and
   `SHA256SUMS` together.~~ **DONE** — seed `6a50c373e` from packed source `5d9e2de4b`; Sdk
   `bf4a1f9c…0154c4`, Runtime `35f1a227…3ad9f6`; ownership head `head-v2:5ce298ee5fe53bf8`, audit
   25/25.
8. ~~**Post-seed gates** on the bootstrap commit, then push and rerun the converter.~~ **DONE
   2026-09-19** — the first non-VS gate on the seed FAILED 1 and produced `d932566aa`; both gates
   then passed at `d932566aa`, the ref was pushed and the converter was rerun. **The reload and
   visual IDE proof this step also required did NOT happen** — see step 2.
9. **IVT2** in an isolated worktree from the accepted tip: resolve actual accessibility by reflection,
   preserve the semantic non-friend refusal, cover source grants, the metadata attribute and
   editor/query behavior, keep one `InternalsVisibleToGrants` owner.
10. **Remaining CLI, LanguageServer, Playground, Runtime and Wasm-hosting owner and assertion
    conversion** from the exact tip with explicit `ROOT` and serialized converter ownership. Compiler
    defects either worker finds go to root for exclusive ownership; the workers must not edit shared
    compiler sources concurrently. No legacy fallback, no C# or allowlist growth, no boundary-only slice
    that leaves the old owner required. Dispose of `census/lsconvert-source-wip` only after its
    documented post-seed disposition.
11. **SIGHELP**, inside step 10's LanguageServer conversion: an N# overload-signature owner returning
    per-overload parameter rows from the project snapshot, the C# `SymbolsInfo` path deleted, and native
    regressions in `tests/native/language-server-handlers` for external instance and static members,
    overloads, active parameter and cross-file types. Fix `website/docs/getting-started.md:165` with it.

#### Evidence — filled receipts and the placeholders still open

- **FILLED — fresh VS Code-enabled gate:** `c6ab6f2e7`, 2026-09-19, EXIT 0, `37b5ea361a7805a7` (1858s),
  30m57s; throughput 12/12 at worst 1.03x; self-host 1342/0; estate 9291/9291; native 109 rows /
  4,536 / 0 / 1 skip; contracts 38/38; ownership 25/25; VS Code smoke 36 passing; IL 80 assemblies;
  compile-time 74 functional pass with **no timing verdict emitted**. `product-vscode-20260919.log` +
  `SHA256SUMS`.
- **PARTIALLY FILLED — reload done, visual IDE proof OWED:** `nsharp-0.6.0.vsix` rebuilt and reinstalled
  2026-09-19; protocol-level evidence only in `visual-ide-20260919/RECEIPT.md`. **No rendered UI was
  observed and no screenshots exist.**
- **FILLED — pre-seed converter census:** converter `b9a49e0` + `nlc +c6ab6f2e7`, 51 files / 86
  diagnostics / 11 stubs (0 / 10 / 26 / 22 / 28) versus 109 on 2026-09-14 —
  `converter-20260919/RECEIPT.md`. A census, **not** proof of migration.
- **FILLED — retirement precheck only:** 19/19 clean ancestors, four stale codex lanes recommended for
  retirement, `census/lsconvert-source-wip` preserved. **Nothing retired.**
- **FILLED 2026-09-19 — push:** `9b4c46174` then `d932566aa` on `origin/systems-language`.
- **FILLED 2026-09-19 — retirement against the pushed ref:** 19 census worktrees/branches plus
  `codex/toolchain-integration` removed after per-row clean, ancestry and in-use checks; three stale
  lanes' branches and `census/lsconvert-source-wip` kept as `archive/` tags; `census/merge` preserved.
- **FILLED 2026-09-19 — reseed:** three failures (`NL103` `OfType` → `b73d55d41`; estate 2/9295
  duplicate MSBuild attribute → `f369e5d22`; scratch `CS0433` empty `Program` holders → `ca8381cdb`),
  then `reseed-5d9e2de4b` SUCCESS — both stages 0W/0E, tests-enabled estate **9,295 / 0 / 0**, exact
  hash equality bootstrap == stage-2 == restored cache.
- **FILLED 2026-09-19 — bootstrap fingerprint, ownership-head repin and packed source:** packed
  source **`5d9e2de4b`**, seed commit **`6a50c373e`**; Sdk
  `bf4a1f9c663aa1343306137adb761c440b767ba7aa02115e9e4f6fc24a0154c4`, Runtime
  `35f1a2271f2498fbaf79bd2cfc088ed025bd4836c28d592c661cbc6bdd3ad9f6`, committed with `SHA256SUMS`;
  ownership head `head-v2:5ce298ee5fe53bf8`, audit **25/25**.
- **FILLED 2026-09-19 — post-seed gates and push:** seed gate at `6a50c373e` FAILED 1
  (`census-duplicate-declarations` pinned the old empty `Catalog.Program` holder) → `d932566aa`;
  then non-VS **PASS exit 0, 31m39s, `e0e1da00c8cb7ec0`** and VS-enabled **PASS exit 0, 32m30s,
  `d6778e34d329aa6c`**, 36 VS Code smoke passing; estate 9,295/0/0; native 110 rows / 4,552 / 0 / 1;
  ownership 25/25; IL 80; compile-time 74 functional, **timing not judged**; **no contracts row
  printed**. **Excludes the visual IDE proof this step also required.**
- **FILLED 2026-09-19 — post-seed converter rerun:** converter `b9a49e0` with `nlc +d932566aa`
  (stale `+6a50c373e` binary rebuilt first), 51 / 86 / 11 — **zero delta** on every row and code.
  Reproducibility and no-seed-regression, **not** migration.
- **OWED — post-seed extension reload FAILED:** VS Code would not exit within 30s; the installed
  extension is still the 13:01 `c6ab6f2e7` build.
- [PLACEHOLDER: rendered-editor visual IDE proof with screenshots, against a successfully reloaded
  post-seed extension.]
- [PLACEHOLDER: IVT2 result.]
- [PLACEHOLDER: SIGHELP — the N# overload-signature owner, the deleted C# `SymbolsInfo` path, and its
  native regressions.]
...
#### Remaining pending (2026-09-19) — nothing here is claimed complete

| Pending | Why it is open |
|---|---|
| **Rendered visual IDE proof** | Never produced; no screenshot has ever been taken in this closeout. |
| **Post-seed extension reload** | FAILED; the installed extension is still the 13:01 `c6ab6f2e7` build, so the visual proof cannot be meaningful until the reload succeeds. |
| **IVT2** | Not started; unblocked now that the reseed has happened. |
| **SIGHELP — N# overload-signature owner** | Not started; belongs inside the LS conversion. |
| **CLI / LanguageServer / Playground / Runtime / Wasm-hosting owner and assertion conversion** | Not started. The post-seed census measured zero movement precisely because none of it has happened. |
| **Runbook/script fix: reseed scratch mode and the root `NuGet.config` feed** | Scratch mode requires temporarily repointing the root `NuGet.config` feed. Undocumented, found by debugging; write it into the runbook or automate it in `scripts/reseed.sh`. |
| **`this` as a value declines at `parse.struct`** | Emit-only columnar path; filed in `census-briefs/FOLLOWUPS.md`. |
| **Call-expression receiver `MakeList().OfType<T>()` declines** | `b73d55d41` fixed only the non-generic extension-slot receiver relation; filed in `census-briefs/FOLLOWUPS.md`. |
| **Gate-script contracts unmeasured at `d932566aa`** | Neither post-seed log prints a contracts row; 38/38 is carried from `c6ab6f2e7`. Re-measure at the next gate. |
| **Accounting repin** | Catalog, native-project count, corpus, line delta and ownership facts still carry `88cf7534` numbers. |

### Historical cursor records

The census wave-11 and earlier entries below are historical proof. Their text, commit references, gate
timings and owner names are preserved exactly as written and are no longer the operative state; the
wave-12 cursor above supersedes them. The standing subsections that follow them — queue state, the
verification bar, baselines, harness state, toolset republish and the growth-ratchet repin — remain
operative procedure.

**Active follow-on, 2026-09-09 (historical):** [managed toolchain conversion](../tasks/TOOLCHAIN-NATIVE.md)
was authorized. Astra integrated and Luna Max implemented complete owners and N# assertions.
Facade and MSBuild task lanes ran from `06186dc6d`; CLI/LSP/Playground/Runtime/Wasm followed.
Visual Studio was deferred; the compiler completion and rename evidence below remains accepted.
...
ENUM2, ITER, TUPLE2 — each verified with its native project(s) and the estate at every merge (final estate 8,679/8,679,
...
Final completion and evidence (`2026-09-09-compiler-only-ownership-complete.md`).
...
and evidence snapshots are preserved. Rename acceptance (`2026-09-09-compiler-core-name.md`).
...
Boundary and evidence (`2026-09-06-interface-realization-ownership.md`), evidence
...
matches the reviewed source. Boundary and acceptance (`2026-09-07-complete-systems-analyzer-ownership.md`),
...
Boundary and acceptance (`2026-09-07-complete-columnar-input-builder-ownership.md`), evidence
...
Boundary and acceptance (`2026-09-08-complete-columnar-emitter-ownership.md`).
...
Boundary and acceptance (`2026-09-08-complete-multifile-compiler-ownership.md`).
...
Complete resolver acceptance (`2026-09-08-complete-reference-resolver-ownership.md`).
...
Acceptance (`2026-09-09-complete-sdk-emit-task-ownership.md`). The earlier failed gate and
...
Boundary and acceptance (`2026-09-06-complete-analyzer-ownership.md`), evidence
...
Boundary and evidence (`2026-09-06-constructor-declaration-ownership.md`), evidence
...
Boundary and evidence (`2026-09-06-closure-analysis-ownership.md`), evidence
...
Boundary and evidence (`2026-09-06-reference-coercion-ownership.md`), evidence
...
Boundary and evidence (`2026-09-06-compiler-reference-ownership.md`).
...
shared constant are removed. Metadata ordering/partial mutation, exact primitive/null/string behavior,
source-enum precedence and runtime reflection/read order remain intact. Seven new canonical controls
plus three existing selected controls pass 10/10; native declarations 121/121 and constructors 7/7 match
verified unchanged baseline sources. Boundary and evidence (`2026-09-06-parameter-default-ownership.md`).
Emitter 18,213→18,001 lines and 17,308→17,113 nonblank; all 380 other rows and epoch ceilings unchanged.
Ratchet head `head-v1:52dd4405924e3517`; audit 18/18. Fresh final backend gate at `565bd331`
passed in 446s: 590 unit / 7,868 canonical / 52 native projects / 12 throughput / 68 IL; SDK/templates/examples green.
The single proved styled-Int32 prerequisite seed `f95d8ad14` separately passed its 449s gate and normal
packaged probe 2/2; all 12 live SDK/cache payloads remain unchanged. Compiler-wide ownership remains
open. Next coherent area to assess: compiler reference and loaded-assembly type resolution, including
test-framework attribute lookup and the ASP.NET reference/context resolution callers.
...
declaration controls, SDK/templates/examples green. Boundary and evidence (`2026-09-06-static-initializer-ownership.md`).
...
Boundary and evidence (`2026-09-06-generic-binding-ownership.md`). Integrated owner `3949eb515` removes all seven methods with fifteen direct N# routes;
...
seed gate passed at `dd4d945e` in 452s (593 unit / 7,835 canonical / 52 native projects / 68 IL).
Setup and packaged verification passed 4/4, with all 12 SDK/cache payloads matching the Release
build; receipt `/private/tmp/nsharp-sibling-constraint-ownership-20260906/seed/acceptance.json`.
The full seven-method owner is integrated at `244c0998a`: seven C# decision methods disappear,
six production routes call N# directly, and no C# behavior/test/helper is added. Canonical controls
...
payloads still match. Acceptance (`2026-09-06-sibling-constraint-ownership.md`).
...
green. Acceptance (`2026-09-06-generic-constraint-ownership.md`). The live seed remains verified.
The selected sibling validation area above supersedes this prior next-area cursor. After its
checkpoint, assess the connected generic-call inference and return-substitution group with its
production callers and dependencies. Task 015 remains open.
...
Verification record (`2026-09-04-takeover-verification.md`). The earlier r23 tip was `1cd8ec19c`
...
|---|---|---|
| writer / `stream/023-s2-writer` | S2.1(f), `dc3131202`, plus shared empty row `08c67e468`; merge `dd4853044` | Pre/pre, pre/historical, pre/cache and historical/cache each compare 94 assemblies with zero IL/output-set/outcome differences; 73/75 projects and 2,117 native tests per arm. The two identical NL402 template declines remain. One-byte mutation changes the predicted assembly and stdout. Proof (`2026-09-04-s21f-parity-proof.md`) |
...
| internal errors / `stream/invariant-diagnostics` | N# boundary `e885d7926`, merge `16784b0e1`; portable newline contracts `35ee5d396` | Escaping CLI exception renders NL924 on stderr and exits 2; synthetic exact output plus production Main invocation tested. `nlc run` still forwards child exit 2 without inventing NL924. Full Execute migration remains blocked by the measured dependency/mixed-language boundary; C# dispatcher ownership is not claimed retired. Decode (`invariant-diagnostics-boundary.md`) |
| diagnostic honesty / `stream/diag-honesty` | NL002 import-hygiene wording, merge `f83bf86e2` | Child exit status and complete error output replace the inherited weak failed-build comparison; unsupported “and run” claims removed. Admission reconciliation `a810444c9`, merge `27f06a531`, verifies all fifteen parameter-and-boxing pairs build both ways |
| editor acceptance / `codex/ide-auto-import` | `c370be48f`, merge `1527e823`: completion now supplies its required import | N# placement shared with NL002 quick fix; C# scope policy removed, host 638 → 620 lines. Native wire 11/11 over 15 applied/check fixtures. Rebuilt/reinstalled editor: package import accepted, interior suffix replaced, multiline header/docs preserved, NL002 quick fix applied; final workspace No Problems. Fresh server 1684 exited on window close |
...
**Superseded map-only brief (expanded to the active area above):** S2.2(m): constrained-call map production (`2026-09-06-s22m-constraint-map-next-cut.md`).
...
Proof (`2026-09-06-s22l-parity-proof.md`); gate, documentation and exact-push receipts at
...
ties and runtime/MLC defaults. Read-only decode (`2026-09-04-static-call-next-cut.md`).
...
The self-hosting decode (`2026-09-04-self-hosting-next-cuts.md`) revalidates 52 lint findings plus
...
Both commands still exit 1 on strict compiler sources. The verification decode (`2026-09-04-takeover-verification.md`) records the scope and raw logs; no baseline was changed. Current corpus census is **75 tracked projects**,
...
### Baselines (dated gate evidence and current ownership; re-measure at your tip)

| measure | value |
|---|---|
| unit suite (`tests/Tests.csproj`) | **593/593** in the fresh S2.1(g) gate at `54faf94a`; current checkpoint source/verdict is recorded in the S2.1(h) gate directory above |
| BootstrapServices estate (`.tests.nl` blocks) | **7,666/7,666** in the complete focused run at `0cde66a0`; prior fresh gate `54faf94a` had 7,658. Current checkpoint gate source/verdict is recorded above |
...
| live-tree strict `nlc check` on BootstrapServices | `0cde66a0`: 425 files / 259 results, exit 1; both compilers/current source arrays match exactly. All baseline findings remain; four ColumnarInputs rows move +9 lines with two matching explanation-line changes. `1527e823` was 425 / 260 and takeover `6ea697316` was 423 / 261. Raw comparisons `/private/tmp/nsharp-s21h-check-*.json`, prior `/private/tmp/nsharp-s21g-check-*.json` and `/private/tmp/nsharp-takeover-live-check/`; self-hosting remains open |
| `ColumnarIlEmitter.cs` | 20714 lines / 19703 non-blank after S2.1(h), `0cde66a0`; `Analyzer.cs` 2357 (2,798 before 3b; 3b removed 441 lines) |
| compiler C# files (`src/NSharpLang.Compiler`, excl. obj/bin) | 10 |
| growth-ratchet head (BOTH keys: manifest header AND `OwnershipAudit.nl`) | `head-v1:1a04757c9e9e81ad` (observed on integrated S2.1(h), `0cde66a0`; when two streams repin, keep per row the side that SHRANK the file — ceilings never rise — then `OWN008` prints the observed head; derive the repin LAST, at the commit's own tree, never carry it through a rebase) |
...
| packaged SDK 0.1.0 in both feeds | packed from `27a6d24f6` on 2026-09-03 ~13:00 (the fourth republish; BootstrapServices.dll md5 `bc9b6e0a…` staging == installed; estate 7,514 through the packaged SDK; probes: construction 42/True/True/outer, override admission `class LengthComparer: StringComparer` prints 2). Earlier packs: `94ff758b5` 2026-09-02 21:15, `71bec2dcb`, `385b7e8d1`. A slice that makes the ESTATE use a shape this package has not admitted is a two-stage boundary — stop before the estate-side step and report. |
...
its report on a decline). Wave-3 candidates, remaining: **every printed "Read more" link 404s** — measured 2026-09-02 by the error-docs decode: the
catalog publishes 101 codes (pinned) and `website/docs/errors/` holds FIVE pages (NL105, NL319, NL320, NL324, NL325; NL924's
page lands with 2b-ii-b), the 19 NSYS codes have none, and the host itself — `docs.n-sharp.dev` — is NXDOMAIN (the site
deploys to `https://schneidenbach.github.io/nsharplang/`); the golden suite pins `errors/<category>/<code>` while production
builds `errors/<code>`; three of the five pages are not in the sidebar; 26 of 91 `ErrorCode` members have no production
reporting site (9 dead perf/AOT rows NL950–954/NL960–963 documented in cli-reference as "emitted by the optimizer", 3
lint duplicates NL901/902/904, 14 undecided incl. soundness-shaped ones such as `new AbstractShape()` NL803 and a sealed
base NL802). DONE on `stream/error-docs-pages` (merged `08ec08a82`, row §4.11): one URL constant at the published base (a domain
is still the owner's call), 23 dead rows retired (catalog 101 → 78 = 68 compiler + 10 linter; NL805 by probe, NL302 by call-site
sweep), autogenerated sidebar, contract A (a code without a page cannot be added) and contract B (every page's example is
EXECUTED through the shipped `nlc check`; it caught three shipped-broken examples and five new ones, and grew multi-file,
fence-`title=` and multi-code marker forms because pages could not otherwise be honest), 90 pages, 78/78, the new native
project `tests/native/error-docs-contract` (corpus pin 73). Product defects the page-writing surfaced, QUEUED: the
`<error>` placeholder cascade · NL701's inverted hint · NL321/NL702 suggesting fixes that do not compile · NL001/NL012
counting a write as a use; and a corrected fact: `out` parameters are NOT covered by definite assignment (the old claim in
`memory/components/error-reporting.md` is gone); the analyzer's twelve `InvalidOperationException` internal-invariant throw sites become NL924-class diagnostics; **language gaps found by the page writers (measured by probe):** `default` is unusable in either spelling (`v := default` is NL203; `v: int = default`, the form the message recommends, is an NL103 parse decline) — the message recommends a form the parser refuses; a DOTTED unresolved type (`Sales.Buyer`) is not checked by NL201 and surfaces as an NL103 field-type decline; NL206 and NL302 have no production reporter (an unresolved expression name reports NL301) and join the retirements; `out`-parameter definite assignment is NOT enforced (a `func TryGet(found: bool, out value: int)` assigning on one branch checks clean — NL304 covers fields and locals only; the soundness stream); NL312 and the linter's NL006 double-report the plainest unreachable statement (the O3 class); `new Box()` as a bare statement declines as NL103 although NL313's own hint lists construction as a legal statement; same-name top-level functions of different arity decline as NL103 at the declaration scan (no top-level overloads — either a rule or a diagnostic is owed); an obvious NL304 fix (`let label: string` then assigned on both `if`/`else` branches) declines at emit; **linter defects measured by the lint-page writer:** NL001/NL012 count a WRITE as a use (`total := 0; total = 5` with no read is accepted although the message says "never read"); NL010 is blind to `catch` clause types; NL011's suggestion is misleading (a comment-only catch still reports); NL003 matches literals only; NL020 is mostly shadowed by NL316; NL002's import suggestion is a 25-name whitelist that says nothing for an unlisted name; NL006 never fires alone (`LinterWalk.IsFlowTerminator` accepts a subset of `AnalyzerStatementTermination.AlwaysReturns`, so every NL006 co-fires with NL312 at the same line — one owner should decide); **a door-coverage decode for the 015 B-arc** (measured by the constraints stream at twenty build cycles): the SAME call emits from `AnalyzerConstruction` and declines from `AnalyzerTypeResolver` — the calling class is the variable; a `Dictionary<string, TypeInfo>` declines as a local-initializer VALUE while a constructed one emits; a nine-argument call carrying `AnalyzerAssignability` + `AnalyzerDiagnosticSink` declines same-file; a bool-returning same-file static with `(TypeInfo, List<TypeInfo>, Dictionary<string, TypeInfo>)` arguments declines as a local initializer; a cross-class static returning a user class declines as a local initializer; a `TypeInfo` read from a `List<TypeInfo>` INDEX cannot be passed as a `TypeInfo` argument while one from an `out` parameter can — so the substitution map and the by-index read, the only two ways to reach a bound type, are both shut, and BOTH constraint families stay at the `new` site (2b) until the door admits these shapes; the full record-and-drain attempt, NL924 and the `BeginAnalysis` invariant are preserved at the coordinator scratchpad `2bii-*-attempt.nl.txt` and need only a door that admits them (NL924 is NOT in the tree); **a soundness-rules stream** — seven codes exist in the catalog for rules nothing enforces, measured by probe: `v as string` on an `int` (NL204), a wildcard arm before a live arm (NL502), inheriting a sealed class (NL802), `new AbstractShape()` (NL803), circular inheritance / multiple inheritance / a wrong-arity constructor (NL307/NL801/NL806 reach emit and fail as an NL103 decline); NL010 falsely reports `import System` unused in a file with 105 `Type` references (`ColumnarConstructionPlanner.tests.nl`); `nlc check` never looks at `.tests.nl` while `nlc test` lints them build-blocking (NL002/NL010) and the SDK path lints them not at all — two bars; **206 of 296 `.tests.nl` carry a dead `import System`** (`Type`, `InvalidOperationException` and `typeof` resolve without it), a backlog no gate sees — an estate import-hygiene slice (`nlc fix` across the estate, then a gate step that lints the estate at the `nlc test` bar); the external-call ARGUMENT position for constant conversions (`TryMatchReflectionParameter`, eight call sites) stays unthreaded — a slice of its own; the `compile-time-bench` timing gate must be load-aware — **DONE, §4.10: it reads the one-minute load before its runs and refuses to judge the median at or above 0.2 × logical cores, recording SKIPPED-BY-LOAD with every number; the correctness half still gates**; `nlc query symbols` calls a bare `Name: Type` member a property while the AST, the factories and the metadata call it a field; the documented interface-property form `Id: Guid { get; }` does not parse; `IsSupportedRuntimeTypeName`'s 32-row closed list and `ColumnarTypeOfPlanner.IsSupportedType`'s closed list retired for "a catalog-resolvable type is supported" (2f); a fully-qualified static receiver does not bind without the import; `DateTime.TryParse(s, out d)` is static-member-unmodeled; a field write on a generic struct local and a union case's payload read decline at `emit.local.initializer`; D4 NL002 anchor (DONE); `nlc query type` renders methods as a
...
- **Pinned stage-0 fixture types (023/S2.1i):** typeof(void), IDisposable, IEnumerator, IEnumerable and IList<int> decline; use ExecutorVoidType or the existing runtime-Type helpers (generic definition plus MakeGenericType). typeof(Type[]) works. These operand declines did not prove the suspected custom-return limitation: direct typed Complete calls pass. Executed matrix (`2026-09-04-s21i-parity-proof.md`).
...
- `type` (017/1, the family's first; again 017/9, 20B, 45, 58; 020/41), `newtype` (017/4, 12B, 64),
  `record` (017/5, 18, 53; 019/10; 020/37), `partial` (017/7, 17, 25; 020/11), `record` as an IDENTIFIER (a local named `record` declines the
  whole declaration — chips), `file` (017/9; 018/4;
...
- `typeof` of a type outside the core library does not emit: `Uri`, `ConsoleColor`, `Guid`, `Regex`,
  ...
  `TextWriter`, `StreamWriter`, `StringBuilder` unless fully qualified, `StringComparison` and every enum type,
...
- `typeof(X)` as a static-call ARGUMENT is narrower still: arrays, nullables and external enum/attribute
  ...
  refused `typeof` reads `emit.call.static-user-argument` inline and `emit.local.initializer` once bound to
  a local, so "it declines as an argument" is a wrong diagnosis to draw from the site alone — bind it to a
  local and re-read before concluding, or route the type through `Type.GetType` (022/3a).
...
- ~~Classes do not take `where` (016 slice 6)~~ **OVERTURNED (chips 2a/2d/2d-ii)**: `where` parses,
  formats, reaches `nlc query ast` and reaches CLR metadata on `class`, `struct`, `record`,
  `interface` and `union`. A union is TWO owners — base and every case — and the CLR refuses a case
  less constrained than its base. A BCL-interface constraint still declines at
  `emit.type.generic-constraint` (`ColumnarTypeOfPlanner.IsSupportedType` is a closed list), and the
  FUNCTION path declines it identically. Nested-generic `List<List<T>>` trips the `>>` tokenizer —
...
- Omitting a DEFAULTED parameter declines — on free functions, on instance methods and on STATIC methods
  (re-learned 2026-09-02 as a build failure at `ColumnarParserKernels.nl:8517`, `emit.local.initializer`); pass every
  defaulted argument explicitly there. SCOPE CORRECTED 2026-09-02 (measured at `7af97ac86`, three call positions): a
  CONSTRUCTOR with a defaulted trailing parameter DOES accept an N# caller that omits it, so a defaulted trailing
  constructor parameter is the right way to widen an AST node without moving its hand-built contracts. The decline was recorded by 017/5, 55, 57, 66; 019/1, 10, 20;
...
- **`this` as a VALUE declines at `parse.struct`** on the emit-only columnar path (re-confirmed
  2026-09-19 during the reseed). Filed in `census-briefs/FOLLOWUPS.md`.
- **A CALL EXPRESSION as the receiver of an extension call still declines** —
  `MakeList().OfType<T>()` (re-confirmed 2026-09-19). `b73d55d41` fixed the receiver relation for a
  non-generic extension SLOT, which is a different thing; do not read it as covering this. Filed in
  `census-briefs/FOLLOWUPS.md`.

### 2.2 `build` — estate, SDK, feed, restore and gate mechanics

- **A GREEN GATE SAYS NOTHING ABOUT WHETHER THAT TIP CAN RESEED** (2026-09-19, the hardest lesson of
  the seed). Every estate probe is compiled *by the seed*, so a defect baked into the seed is
  invisible to the estate the seed compiles. Three real defects — `NL103` on a non-generic extension
  slot's `OfType`, a duplicate MSBuild attribute, and empty public `Program` holders colliding as
  `CS0433` — all passed the full gate at `c6ab6f2e7` and were found only by the reseed itself. Budget
  for reseed failures; they are the first honest measurement of the tip.
- **A test can PIN the defect the reseed then fixes.** `census-duplicate-declarations` asserted the
  empty `Catalog.Program` holder, so it failed the moment `ca8381cdb` stopped emitting it, and the
  first post-seed gate came back FAILED 1. Before treating a post-fix gate failure as a regression,
  re-read the tests that cover the fixed behavior.
- **Reseed scratch mode requires temporarily repointing the root `NuGet.config` feed.** Undocumented;
  cost a debugging cycle on 2026-09-19. Until `scripts/reseed.sh` or the runbook handles it, expect
  scratch validation to fail confusingly without it.
...
- **A timing gate must not judge a machine it cannot compare to its baseline.** The compile-time gate cost
  five red sweeps in two days at one-minute load averages of 4.4–8.1 on this 10-core box (medians
  11,941–17,010 ms against an 11,802 ms limit; 6–7 s quiet), all of them lies about the code. It now reads
  the load before its runs (`sysctl -n vm.loadavg`, else `/usr/bin/uptime`; the throughput gate's source)
  and SKIPS the timing judgement at or above `0.2 × logical cores` — 2.0 here — while still enforcing the
...
- **A STORED BASELINE MEASURES THE MACHINE IT WAS TAKEN ON; A SAME-RUN CONTROL MEASURES THE CODE.** Step 2c
  divided a median measured here by a number taken once on an idle M4 (2026-09-01, `8cf40128a`) and tripped
  on **three** gates under concurrent builds with nothing regressed. `c44d6206e` replaced the number with a
  **program** — a frozen `ControlKernels.nl` measured beside the live kernels in one process, paired and
  order-alternated, verdict = median of per-repetition ratios at the unchanged 1.20x — and the stored
  numbers became an informational, never-gating drift row. **Consequence: gates no longer require pausing
  lanes.** The full non-VS gate passes with all twelve cells 0.95x–1.01x at one-minute load **11.17** on 10
  cores; **the only judgement still needing load < 2 is the compile-time bench's TIMING verdict.** What a
  same-run ratio cannot see is checked directly instead: **drift** (both sides moving together) by the
  informational table, and **lost vectorization** (both sides sharing a compiler) by reading the
  `SimdReductions` helper back out of the emitted IL (`--il-shape`).
- **AN UNSUMMARISED TEST PROJECT IS NOT A GREEN TEST PROJECT.** `tests/native/sdk-project-reference-boundary`
  produces **no summary at all** until `src/NSharpLang.Build.Tasks` has been built, so every ad-hoc sweep
  since reference-type nullability reached emitted metadata silently skipped it — and it was not passing
  (27/27 only once Build.Tasks, LanguageServer and Playground are built). A sibling case surfaced 2
  failures out of 9,429 the same way. **Check a sweep's row count against expectation before reading its
  verdict** (`84006a77e`, `baf0b4eed`, wave 15).
- **Quote a log; do not paraphrase one into a number.** Two figures this campaign repeated for several
  waves do not survive `grep`: the gate's native row count was quoted as "115 rows / 4,651" when the logs
  print 122 stage rows, per-project 4,717/0/1/4,718 and a grand total of 14,237/0/1/14,238 — say which you
  mean; and the compile-time "functional / judged / unjudged" vocabulary **appears nowhere in the gate
  logs** (`grep -i -E "judg|unjudg|functional"` → zero matches), which report an ordinary 74/74 native
  project. The underlying state (no timing verdict, no load, no ms for that row) is real; the quotation is
  not. **Say when a record does not exist** — there is likewise no prose owners list for the LS lanes and
  no "440/440" stress line in any log (wave 15).
...
- **Final counters do not prove event order.** S2.2(h)'s final controls record actual Dispose and
  ...
  controls throw the same exception types the inner catch handles, from guards outside that catch.
  Focused filter counts can overlap: getter 8 includes lookup 3, so the new canonical total is 18.
- **`Type.IsAssignableFrom` ACROSS TYPE UNIVERSES ANSWERS `False` IN BOTH DIRECTIONS AND NEVER THROWS** — measured against a real
  `MetadataLoadContext` over the shared framework. Any predicate comparing an MLC type with a live `typeof` degrades SILENTLY and no
  contract can see it; `FullName` is byte-identical across universes and is the only safe comparison. Chips A/B/C (§4.10).
...
- **A SEED THAT STARTS EMITTING REAL METADATA BREAKS THE WORKAROUNDS THAT EXISTED BECAUSE IT DID NOT.**
  The first gate on seed commit `407b43f9b` FAILED on four NL402s in `tests/native/daemon-command`, whose
  harness spelled `Dictionary<string, object>` only because the *old* seed emitted no reference-type
  nullability. That is the fix working, arriving as a failure; `2a4a418be` restored the honest `object?`.
  **Budget one such failure per reseed.** Corollary: a `// SEED: simplify after next reseed (<sha>)` marker
  is a debt with a known payoff date — its collapse should be a **net deletion** verified by byte-identical
  output (`f59f486a8`: 3 files, +20/−51), not by re-reasoning (wave 15).
- **Check whether the artifact you cite is NEWER or OLDER than your tip.** The untracked blocker files'
  newest recorded pass is measured at a commit that is not in the published range and not an ancestor of
  the tip, and the remaining-owners assessment was measured a wave earlier and argues from a defect since
  fixed. Date and anchor every working document you quote, and re-measure a premise before acting on it
  (wave 15).
...
- **Three cross-assembly emit walls and a reserved word (error-docs chip, 2026-09-02):** a cross-assembly walk of
...
| slice | commit | what moved | durable finding | headline numbers |
|---|---|---|---|---|
| soundness-rules | `0291d7692`, `6526ab9ad`, `39f535c1c` (merged `563524668`, gate r21) + `1af767a05`, `a9085ef7a`, `3afa07bb9` (branch `stream/soundness-rules` off `2cdc8be44`) | the error-docs arc's SEVEN exemptions become seven enforced rules and the exemption list reaches ZERO: NL803 `new` on a type with no instances (abstract class / interface / static class, at the `new` site), NL802 an unextendable base, NL801 more than one base class, NL806 a constructor arity, NL307 a base chain that closes on itself, NL502 an arm below a catch-all, NL204 a cast or `as` with no conversion — plus NL304 widened to `out` parameters. Contract A is now unconditional | **THE ESTATE, NOT REVIEW, FOUND EVERY FALSE POSITIVE — three of them, each in a pre-existing contract or in the compiler's own source.** `AnalyzerTargetTypedOperands.tests.nl`'s lenient-resolution contract caught NL204 accusing `(NoSuchTypeAnywhere)v` (a cast resolves its target with `ResolveType`, so an unresolved name arrives as a bare `ExternalTypeInfo`, and `NL201` did NOT fire — the reader was told the one thing that was not true). The 103-project differential caught the `out` rule accusing `NumericLiteralFacts.TryParseUnsignedIntegerMagnitude`, which returns from its `try` and all three `catch` clauses: `AnalyzeTry` had returned a flat `false` for "always exits" since it was written. And ten `match` fixtures went red on NL502 — nine because the rule was RIGHT about a genuinely unreachable arm the fixture had spelled incidentally, one because an `IdentifierPattern` named "one" is a BINDING, not the literal it stood for | catalog 78, exemptions **7 -> 0**, pages 90 -> 98 (NL801/802/803/806/307/502/204 new, NL304 extended); estate 7,524 -> **7,616**, Failed 0; 5 differentials over 102-103 projects, every one BYTE-IDENTICAL at the end of its slice; audit 18/18; unit 593/593; **1 C# line changed** (`Analyzer.cs:1944`, `Begin(expr)` -> `Begin(expr, _patternReachability)`, 1 insertion / 1 deletion, `currentLines` 2357 unchanged) with a two-key repin `head-v1:7f26d22e0b4b070a` -> `head-v1:30dd07bb9e9a5cb8` |
| error-docs | `35ae2555d` (arc `27a6d24f6`..) | every printed "Read more" link went from 404 to a real page: one URL constant, 23 dead codes retired, an autogenerated sidebar, an 8-code index, 90 pages, and two contracts | a docs page is only as true as the compiler run behind it — contract B executes all 90 examples through the shipped `nlc check` and was RED on its first run against pages that had shipped months earlier | catalog 101 -> 78 (68 compiler + 10 linter); pages 5 -> 90; contract 12/12; estate 7,511; corpus pin 71 -> 73 |
...
**Soundness-rules arc findings (2026-09-03, `3afa07bb9`).**

- **A rule with no reporter is not a rule, and seven of them shipped.** Each was reproduced through the
  shipped CLI before a line moved, and four of the seven were WORSE than silent — `new Stream()`,
  `new Console()`, `class Both : Left, Right`, `new Point(1, 2)` and `class A : B` / `class B : A` all
  reached the emitter and died as an `NL103` naming the columnar backend rather than the mistake. Enforcing
  the rule in the analyzer replaces a backend sentence with a language one.
- **THE ANALYZER'S CONSERVATISM IS PER-QUESTION, AND REUSING IT ACROSS QUESTIONS IS THE BUG.**
  `IsPatternPossible` answers "can this value BE that type" — right for `is`, wrong for a cast, because
  `(int)3.5` is a legal conversion between two types no value belongs to both of. `IsCastPossible` wraps it
  with the numeric domain, enums and user conversion operators. The mirror of the same lesson: two unrelated
  unsealed classes are still admitted, because narrowing that would move `is` and every match arm with it.
- **A DEFAULTED TRAILING PARAMETER IS FREE ON A CONSTRUCTOR AND COSTS 53 CALL SITES ON A METHOD.**
  `ClassTypeInfo` gained `isAbstract` with zero estate churn; `CheckLocals` and
  `AnalyzerTargetTypedOperands.Begin` each had to be written out at every call site, because an N# caller
  cannot omit a defaulted parameter on an instance method.
- **`AnalyzerDefiniteAssignment` IS AT THE PER-CLASS MEMBER-FUNCTION CEILING** — adding ANY member
  function declines the whole class at `parse.struct`, measured with a one-function probe — so the `out`
  rule's sentence lives on a sibling class taking the sink as an argument. And `where` is a RESERVED WORD:
  naming a local `where` declined the class at its header with nothing said about the identifier.
- **`virtual func` + `override func` in ONE compilation unit declines** at
  `emit.declaration.override-target: no overridable base`, over an abstract base and over a plain one. The
  NL803 page's fix examples use a subclass that ADDS a member because that is what compiles today.
- **A bare `Name: Type` member does not parse inside an `interface`** (`parse.interface`), so an interface
  used in a docs example must declare methods.
- **The caller's half of `out` was already true and the notes said otherwise.** An `out` ARGUMENT has
  always marked its target assigned; a plain argument does not. `memory/components/error-reporting.md` now
  says which half landed when.

**Error-docs arc findings (2026-09-03, `35ae2555d`).**

- **Every "Read more:" link the compiler had ever printed was dead.** `docs.n-sharp.dev` is NXDOMAIN and always
  was; there were THIRTY spellings of it plus a second, incompatible `errors/<category>/<code>` shape pinned by the
  golden suite. `DiagnosticDocs.Base` is now the only place the product names a documentation host.
- **Twenty-three of the catalog's 101 codes had NO production reporter** — measured by sweeping every `ErrorCode`
  member against `*.nl` and `*.cs`. Retired, each naming what enforces the rule instead: NL950-954/NL960-963 (the
  systems analyzer's NSYS010/020/030/040/060), NL901/NL902 (NL001/NL006), NL904, NL205, NL403, NL404, NL406, NL408,
  NL603, NL604, NL804 (NL325), NL805 (NL202, at the argument), NL206 (NL201), NL302 (NL201/NL301). `NL805` and
  `NL302` were found by probe and by call-site sweep respectively, not by reading names.
- **A contract that reads nothing passes everything.** Contract B silently passed every example until it was
  noticed that `nlc check --text` writes DIAGNOSTICS TO STDERR and only its summary to stdout. Any harness that
  scrapes `nlc` output must read both streams.
- **Three emit walls on the contract path:** `DotnetRunner.RunProcess` (a cross-assembly static returning a user
  class) declines, though `Process`/`ProcessStartInfo` themselves cross; `psi.StandardOutputEncoding = Encoding.UTF8`
  declines; `RunProcess`'s `Nullable<TimeSpan>` will not take a bare `null`.
- **The growth ratchet governs `website/`.** `sidebars.js` is a tracked non-N# file: a new `_category_.json` beside
  the pages was refused outright as OWN003, and a 20-line explanatory comment as OWN004 "never raise a ceiling". The
  sidebar edit had to SHRINK the file. `website/docs/**` carries no rows, so pages are free — but a Docusaurus build
  leaves `website/.docusaurus/` and `website/build/`, which the audit reads as OWN009. Delete both before a gate.
- **Seven codes are exempt because nothing enforces their rule**, each with a `code|defect|sentence` row and a probe
  program the contract runs every time: NL204 (`v as string` on an int, silent), NL502 (a wildcard arm before a live
  arm, silent), NL802 (a sealed base, accepted), NL803 (`new AbstractShape()`, accepted), NL307/NL801/NL806 (reach
  emit and die as an NL103 decline). The day one starts reporting, the contract fails and demands its page.
- **Two codes CANNOT be reported alone, and the marker syntax had to admit it.** The linter's terminator test is a
  strict subset of the analyzer's, so every NL006 co-fires with NL312 at the same line; NL923's report is gated on an
  unresolved-name error, so it only ever appears beside one. `// ERROR NL006, NL312` claims both.
- **Product defects found by writing the pages** (queued as chips, not fixed here): `out` parameters are NOT covered
  by definite assignment though the notes claimed they were; a parser `<error>` placeholder cascades into NL903 and
  NL012; `new Box()` as a bare statement declines at NL103 against NL313's own hint; two top-level functions of the
  same name and different arity decline as NL103 rather than NL306; NL701's hint says paths are project-root-relative
  and they are file-relative; NL321's and NL702's suggested fixes do not themselves compile; NL010 is blind to `catch`
...
| slice | commit | what moved | durable finding | headline numbers |
|---|---|---|---|---|
| Consecutive fresh gate bootstrap reseeding | this commit after r25 at `78cf8f4a` | Refresh SDK/runtime bootstrap cache trees from the caller before each isolated run, retaining gate-specific sibling versions | Step 4 clears the package-ID cache and later restores only an IL-stamped version; directory existence cannot prove the next run has its bootstrap version. Before/after controls cover missing versions, repeated eviction and stale overlapping bytes | Shell 567/502 → 566/501; ownership 18/18 after observed two-key `head-v1:873549ff87d77915` repin; no SDK/feed republish; record (`2026-09-04-takeover-verification.md`) |
| Gate unit failure evidence | this commit after r24 at `242e401d` | Explicit minimal console logger preserves assertion details alongside quiet build output | A quiet MSBuild test invocation otherwise prints only a failing test name. Invalid setup-environment control proves the assertion and child stderr survive; original installer failure remains unexplained and is not claimed fixed | Same compiled unit suite 593/593; focused installer 9/9; ownership 18/18, shell 916/815 unchanged, both observed head keys `head-v1:94e6b5916c964abc`; record (`2026-09-04-takeover-verification.md`) |
| Diagnostic honesty integration — admitted external types | this slice, `codex/diag-admission`, based on `dd4853044` | Replaced the stale Regex/Queue emission-failure contract with fifteen with/without-import parameter-and-boxing pairs; updated NL002 documentation and adjacent comments, without changing its message or rule | 022/2f-b made all fifteen documented shapes build. Every pair must exit 0, print a success banner, omit NL002 and have identical complete stderr; member/constructor admission is outside these probes. The old test correctly caught the integration change at 31/32 | Snapshot of the integrated Debug CLI; diagnostic-honesty 32/32; error-docs-contract 13/13; focused NL002 estate 24/24; root format green. No C# changes or ownership repin; coordinator owns final product gate |
| External completion auto-import — accepted names receive their required import | this commit on `codex/ide-auto-import` | One N# `ImportEditPlanner` owns quick-fix and completion placement; C# only maps whole-word primary and disjoint additional edits. Retired C# namespace-scope helpers and the reflective quick-fix placement. Existing parser header phase extracted unchanged; linter scope seeded from the current namespace/package | Directive start lines are not header extents: consume multiline names and preserve spanning comments before insertion. Merge touching imports into the primary edit in source order; replace identifier suffixes too. Keep explicit-import accounting separate so NL010 remains effective. Decode (`2026-09-04-completion-auto-import.md`) | C# 638/559 → 620/546 total/nonblank lines; full estate 7,639/7,639; focused LSP 20/20; native LSP lifetime/completion 11/11 (15 accepted-source fixtures, all checked); diagnostic-honesty 32/32; ownership 18/18 and root format green. Installed-editor verification and fresh VS Code-enabled product gate remain coordinator work |
| Diagnostic honesty 8 — NL002 states missing import hygiene | `02f8f87a6`, `ad24f97cf`, and this review commit on `stream/diag-honesty` | The N# message says the name is used without its providing import; suggestion and span stay stable. Estate, diagnostic golden, playground span and NL002/NL201 page pins follow the sentence | A failed build alone does not prove the import leaves the failure unchanged. Review now asserts both exit codes and the complete stderr stream, plus the expected emission-failure sentence; successful library probes require exit 0. Removed the unsupported “and run” claim from build-only contracts and their explanation | Fresh Debug CLI build; compiler-service estate 7,631/7,631; diagnostic-honesty 32/32; error-docs-contract 12/12; root format green. No C# changes or ownership repin. Integration and installed-editor verification remain coordinator work |
| invariant diagnostics (c): NL924 process boundary | this slice, `stream/invariant-diagnostics` | `InternalErrorBoundary.nl` owns the catch, exact diagnostic, docs link and exit 2; Main delegates in one existing expression, with zero C# growth. NL924 catalog/page, 3 estate and 5 native contracts; the missing-page exemption list stays zero, with one narrow source-repro exemption pointing at the exact output contract | Full Execute migration cannot fit this boundary: cross-assembly Program is inaccessible; Microsoft.NET.Sdk ignores `.nl`, NSharpLang.Sdk omits `.cs`, and a reverse BootstrapServices reference would cycle. Existing command-local catches still own their exit-1 behavior. `nlc run` can forward child exit 2, so NL924 identifies the internal failure. Decode: `decodes/invariant-diagnostics-boundary.md` | Program.cs 786 → 786; catalog 78 → 79, pages 97 → 98; dev CLI 124/124, catalog+boundary estate 13/13, native CLI 103/103, error docs 13/13; fingerprint-only two-key repin |
| COMPILE-TIME GATE LOAD-AWARENESS — the timing gate stops judging machines it cannot compare | this commit; branch `stream/compile-time-gate-load` off `af07060ac`; **ZERO C# ADDED** (three `.nl` files only — no shell, no JSON, no ratchet repin; corpus pin untouched at 73) | **THE DEFECT.** `tests/native/compile-time-bench`'s gate block compared a median of three `nlc build` runs with an idle-machine baseline and failed on any loaded box — five red product gates in two days, each one a lie about the code. **THE FIX.** The block now reads the one-minute load average BEFORE its runs (`sysctl -n vm.loadavg`, else `/usr/bin/uptime` — one parser reads the sysctl brace shape, the macOS `load averages:` shape and the Linux comma shape, in thousandths via the existing `BenchParseFixed3`, because this estate has no float parse) and REFUSES to judge the median at or above `0.2 × logical cores`, recording SKIPPED-BY-LOAD with every number instead of failing. The verdict split into a correctness half (exit codes, failure banner) that NEVER skips and a timing half that does; `BenchGateVerdict` keeps its signature and its seven contracts. 15 new contracts (parser shapes incl. the `1 user` trap, threshold both sides, live-reader guard, skip path, fail path, correctness-never-skips, record line, commit reason) | **An unreadable load must skip and a live contract must guard the reader** — otherwise one typo switches a timing gate off in silence. **`cliCommit=unknown` was never the harness's fault:** `tests/scripts/test-all.sh` rsyncs the isolated gate tree with `--exclude='.git/'` and NOTHING stamps the commit into the build, so `git rev-parse HEAD` cannot answer there; the message now names the reason (`unavailable (isolated gate copy — …)`) and a later slice may stamp the commit at build time. **The threshold cannot be generous:** measured inflation here is ≈ `1 + 0.13 × load`, so load alone spends the whole ×1.5 tolerance by load 3.9. **Consequence:** inside a full product gate the box is loaded by Step 3a itself, so the timing judgement is usually skipped there — the compile-time number is a quiet-box measurement, as the re-baseline recipe already required; a container's `uptime` reports the HOST's load | Six measured red medians at loads 4.4–8.1: 11,941 / 12,319 / 13,241 / 15,623 / 17,010 / 12,401 ms against the 11,802 ms limit (1.52×–2.16× the 7,868 ms baseline), 6–7 s quiet. Induced-load proof at load 3.69/10 cores: `runs=[14857, 15375, 15536] median=15375ms` — 1.95× baseline, a certain red gate before — now **66 passed, 0 failed** with the numbers in `artifacts/compile-time/last-gate-run.txt`. Live `cliCommit` is the real 40-char sha here |
| IDE-LIFETIME — the stdio server outlives its client, and the reload script's proof line | `1ad0d1f02` (server + contract + corpus pin) · this commit (scripts + two-key repin); branch `stream/lsp-lifetime-2` off `90723258a`; **ZERO C# ADDED — `Program.cs` SHRANK 157/141 → 155/140 and the reload script sits AT its ceiling 47/37** | **DEFECT A.** `src/NSharpLang.LanguageServer/Program.cs` handed `Console.OpenStandardInput()` to OmniSharp and awaited `server.WaitForExit` with NO stdin-EOF handling and NO parent watch, so no window's death took its server with it — the fourth visual round counted fourteen orphans with ppid 1, started July 7 to September 2. A headless probe speaking real LSP framing found both doors shut: stdin EOF before or after `initialize` ≥ 20 s, the declared `processId` killed with stdin open ≥ 25 s, and only an orderly `exit` ending it. Two event-driven ends now answer, neither a poll loop: stdin is PUMPED into a `System.IO.Pipelines.Pipe` whose copy completing at EOF exits the process (`WithInput(clientInput.Reader)`), and `OnInitialize` watches `InitializeParams.ProcessId` through `Process.GetProcessById(...).WaitForExitAsync()`, a client already dead at handshake time exiting sooner rather than faulting the handshake. Both continuations name `TaskScheduler.Default`; both tasks are discarded. The six lines were paid INSIDE the file: the five-line `logPath` `Path.Combine` folded to one, the two-statement `LogInformation` pair in `OnInitialize` to one sentence, and the eleven-line pair of accumulating diagnostic `foreach` loops to a `Select`/`Concat`. NEW contract `tests/native/lsp-lifetime/` (auto-discovered by Step 3a, which runs after the unit step's project reference has built `LanguageServer.dll`), three blocks on the proven `ProcessStartInfo`/`Start`/`WaitForExit(ms)`/`Kill`/`Dispose` surface: `exec dotnet <dll> --stdio < /dev/null` (the `exec` makes the waited pid BE the server); a `mkfifo`-held stdin plus a framed `exit` that lands WHILE THE PIPE IS STILL OPEN; and a `sleep` spawned as the declared `processId`, stdin healthy through the same fifo, killed. Unix-only and it says so — a fifo is the only way to hold a child's stdin open without `RedirectStandardInput`. **DEFECT B.** The reload script's proof line named the wrong process two ways: `pgrep -f 'LanguageServer.dll' | head -n 1` takes the LOWEST pid — a stale orphan, since every orphan has ppid 1 — and `pgrep -n` alone names the .NET Roslyn server, whose OWN command line carries the ERE in a `--extension .../Microsoft.VisualStudio.Copilot.Roslyn.LanguageServer.dll` argument. It now takes the newest `Code Helper (Plugin)` and asks for THAT process's newest `server/LanguageServer\.dll --stdio` child, printing the pid, the helper pid and `ps -o lstart=`; with no plugin host it prints one honest line and continues instead of dying under `set -euo pipefail`. `nsharp_kill_vscode`'s bounded kill wait was already right and was NOT touched — `lib/vscode-extension.sh` has zero non-blank headroom | **THE LIFETIME POLICY CANNOT BE N#-OWNED YET — three door gaps, filed in these words: `Process.GetCurrentProcess()`, `Process.GetProcessById()` and `ProcessStartInfo.RedirectStandardInput` all DECLINE at columnar emit.** Those three ARE the parent watch and the stdin door, so the CONTRACT is N#-owned and the six lines it guards are not. **THE RECORDED DESIGN CARRIED ONE WRONG NUMBER, overturned by re-measurement: a bare `exit` does NOT end the server in 0.21 s.** OmniSharp QUEUES notifications until `initialize` completes, so an `exit` sent before the handshake is never acted on (≥ 20 s, at both 0.8 s and 4 s of startup delay); `exit` AFTER the handshake, with or without `shutdown`, ends it in 0.03 s. A second edge sits under it: an `exit` written in the SAME BURST as the handshake is dropped as well (measured, 25 s), so the contract WAITS FOR THE INITIALIZE RESPONSE to appear on the server's stdout before sending anything more — a readiness fact, not a sleep long enough to usually work. **A LIFETIME CONTRACT MUST NOT LEAK THE THING IT FORBIDS:** every block runs under `/bin/sh` with its own streams redirected (Step 3a parses that stdout as JSON, and a job-control `Terminated: 15` line broke it once), and an overrunning block is killed with its whole process TREE — the negative control left zero new orphans. **THE ORPHAN CENSUS IS A MOVING NUMBER:** fourteen at the record, ONE here (pid 1007, ppid 1, `nsharp-0.6.0/server/LanguageServer.dll --stdio`) because the machine rebooted between the two; it was NOT killed — the user decides. The `-P` selection is verified structurally (the ERE matches the N# server and not Roslyn; `-n -P <pid> -f <pattern>` composes on macOS; an empty `-P` is a usage error `2>/dev/null || true` swallows) and both print branches were smoked with a stand-in parent; the proof against a REAL plugin host belongs to the fifth visual round | probe, before → after: stdin EOF before `initialize` ≥ 20 s → 0.03 s; after `initialize` ≥ 20 s → 0.02 s; declared `processId` killed ≥ 25 s → 0.13 s; `shutdown`+`exit` 0.03 s → 0.03 s. Contract **3/3 in 5.0 s**; NEGATIVE CONTROL against the copy-reverted parent **1 passed / 2 failed** — (a) and (c) fail and (b) PASSES, which is exactly right: the orderly path was never broken and (b) is the control proving the pump did not replace it. `Program.cs` 157/141 → **155/140**, build warnings 11 → 7. `reload-vscode-extension.sh` 46/36 → **47/37** (epoch 47/37); `lib/vscode-extension.sh` untouched at 44/37. Corpus pin **71 → 72** (`tests/native/lsp-lifetime` is the 72nd `project.yml`). Ratchet repinned two-key from audit-observed values — rows `text-v1:98831d21be20e41a` and `text-v1:3d289f37a5003958`, head `ad178a5b1f1491c3` → **`c84392a028a8c371`**; ownership audit **18/18** and the live `AuditLiveRepository` prints `passed`. LSP unit tests **272/272**; root `nlc format --check --project .` clean (it caught the new `.tests.nl` on the first pass) |
| CHIPS: the literal `\x1b`, and `where` on TYPE declarations | `8fb40b489` `3d8e3ac3c` `32289c5f7` (chip 1) · `e4c6786d2` `c821c2577` `19f2911b6` `ae8f4bdb1` `0e382403c` (chip 2); **ZERO C# added — both C# files SHRANK** | **CHIP 1.** Coloured `nlc` diagnostics wrote the ten literal characters of `\x1b[1;31m`. The colour kernel was not the defect: `StringLiteralDecoder` owned an ELEVEN-escape table with no `\x`, `\u`, `\U` or `\e`, and its tolerant entry point passed an unrecognised escape through as text — so the literal was valid, idempotent, silent, WRONG code, and N# could not spell a control character at all. Both entry points now read ONE table; `\x` takes 1-4 hex digits greedily, `\u` exactly 4, `\U` exactly 8 (refusing lone surrogates and anything past the last plane). A colour POLICY was then forced: with a real ESC an unconditional `true` would put SGR into every CI log, so `DiagnosticColorPolicy` decides from the `--color` flag, `NO_COLOR`, `FORCE_COLOR` and the stream, in that precedence. NL105 gained the unrecognised-escape arm at the two literal call sites. **CHIP 2.** `where` on a type declaration did not parse on any of the five keywords while `types.md` documented it in two sections with five examples. `ParseGenericConstraints` had existed for functions all along; the type arms never called it. Constraints now parse, format, reach `nlc query ast` and reach CLR METADATA on class/struct/record/interface/union, through one C# helper over N# rules (`ColumnarGenericConstraintPlanner`). A false NL208 that rejected EVERY argument of `where T : IComparable<T>` was root-caused to `IsSubtypeOf` having no arm for a built-in at all. NL208 is now reported at `new` sites, which it never was | **THE BOOTSTRAP STAGE IS A TWO-STAGE BOUNDARY AND IT DECIDED CHIP 1's SHAPE.** `BootstrapServices.csproj` is `Sdk="NSharpLang.Sdk"`, resolved from the NuGet package cache, so `CompilerError.nl`'s literals decode with the PACKAGED compiler's table, not this tree's — measured: after the decoder fix user code returned 27 for `"\x1b"[0]` while `nlc build` still emitted ZERO real ESC. **The colour of the compiler's own diagnostics must not depend on which compiler compiled the compiler**, so the nine literals became `Csi(...)` over `((char)27)`. The decoder fix reaches the estate at the next toolset republish. **THE CONSTRAINTS DO NOT COME FROM THE AST**, which set chip 2's whole shape: a function's come from the COLUMNAR parser's result table, so the type cores had to PARSE the clause, not skip it — and the `!= 129` brace gates that demanded `{` are why a constrained declaration declined wholesale. **A UNION IS TWO GENERIC-PARAMETER OWNERS**: base and every nested case, and the CLR refuses to load a case less constrained than its base; the base's builders were discarded at declaration and are read back off `GetGenericArguments()`. **THE FALSE NL208 WAS NEVER ABOUT SELF-REFERENCE** — a constraint that never mentions `T` failed identically; it was the BOUND TYPE, and `string`/`int` arrive as `SimpleTypeInfo`, which `IsSubtypeOf` had no arm for. **2b-ii IS BLOCKED BY THE EMITTER DOOR, NOT BY DESIGN** — six shapes refused, filed below | **Chip 1:** coloured line `5c78 3162 5b32 6d` → `1b5b 326d`; stderr 0 real ESC / 8 literal → **8 real ESC / 0 literal**; on a real pty 8, and 0 under `NO_COLOR`. `"\x1b[1;31m".Length` 10 → **7**. Estate 7,335 → 7,340 → 7,353 → 7,383. `cli-command-contracts` 89 → **94**. **Chip 2:** all five keywords carry constraints to metadata — `ReferenceTypeConstraint`, `NotNullableValueTypeConstraint+DefaultConstructorConstraint`, `DefaultConstructorConstraint`, `constraints=[IThing]` — and `Maybe+Present`/`Maybe+Absent` carry the SAME word as their base; the assembly loads and runs. `columnar-emit-facts` 41 → **44** (the type-load contract). NL208 at `new` for struct/class/interface, each paired with the satisfying argument silent; `cli-command-contracts` 94 → **98**; NL208 census over the gate paths **0**. Estate 7,413 → 7,428 → 7,435 → 7,441 → **7,449**, Failed 0 throughout. **`ColumnarIlEmitter.cs` 20784 → 20784 (non-blank 19768 → 19766) and `ColumnarProgramInputBuilder.cs` 1051 → 1044 (981 → 975)** — every addition paid by deletion. `analyzer-clean-source` 991/991 at every step; audit 18/18 after each repin |
| IDE OBSERVATIONS O1–O3 + THE RELOAD HARNESS — the four the 2026-09-02b visual re-verification filed (LANDED) | `481dfe358` O2 · `bff2f027b` O1 · `531be90b0` O3 · `60d3ad2b7` harness · `d3b76ae5c` repin (branch `stream/ide-chips-o1-o3` off `9c3a41ca9`) | **O2**: member completion offered `summaries`, a camelCase field on a class in another package — new N# owner `CompletionVisibilityFacts` holds the analyzer's own predicate (`IsOfferableAcrossPackages` = exported OR same package), the declaring namespace found by simple name and tied-broken by DECLARATION POSITION; `GetTypeMemberItems` gains the filtered form, `CompletionReceiverFacts` takes the project's parsed files as a seventh argument. **O1**: 107 rows under 39 names collapse to one row per NAME with the count in the detail; `CompletionEngineKernels` gains `CompletionKindSortRank`/`CollapseCompletionOverloads`/`OrderCompletionItems`/`FlattenCompletionGroups` and `PlaygroundCompiler.DeduplicateCompletions` is DELETED; the count is taken in `BuildReflectionMemberItems`. **O3**: `ValidateTrustedFunction`'s two arms fold into one finding whose sentence names what is absent. **Harness**: `nsharp_kill_vscode` waits until `pgrep -x Code` is empty (bounded 30 s) and the reload script prints the new server's pid and start time. ZERO new C#; six non-N# rows shrank. | **THE O1 RULE WAS ALREADY IN THE REPOSITORY, SPELLED ONCE, IN THE PLAYGROUND'S C#.** `PlaygroundCompiler.DeduplicateCompletions` had been grouping by label and ordering by kind rank then label since 020/s38 — so the playground's lists were clean while the CLI's and the editor's showed reflection order. A shared decision with three spellings and the two a developer looks at were the wrong ones. **O2 IS NOT A COMPILER HOLE AND THE RULE IS NOT `camelCase == file-private`.** `nlc check` already answers NL308 on the leak; the analyzer compares the declaring file's NAMESPACE against the current file's, so a casing-only filter would have hidden members that compile and run — measured, not assumed. **O3's TWO ARMS WERE TWO DISTINCT FACTS, WHICH IS WHY ONE CODE AT ONE SPAN WAS THE WRONG SHAPE**; the header comment that called the doubling deliberate is rewritten to say why. | Estate 7,481 → 7,499 → 7,501 Failed 0 (+8 O2, +10 O1, +2 O3; count-diff exact at each step). `string` receiver 107 rows / 27 duplicated names → **39 rows, zero duplicates**, 27 carrying `overloads` (`Split` 11, `IndexOf` 10). Cross-namespace `service.` drops `summaries` and `helperCount`; the same receiver from a file in `WeatherDemo.Services` keeps both; the cross-namespace PascalCase record still lists all four. NSYS100 twice at `5:12+7` → once. Native: `playground-tooling-surfaces` 33/33 (101 rows re-pinned from a print-only copy built outside the repo; label censuses and the 171 item count BYTE-IDENTICAL), `systems-analysis-census` 66/66, `query-integration` 75/75, `cli-command-contracts` 98/98, `completion-engine` 13/13, `query-completions` 1/1, `doc-query` 11/11, `systems-gauntlet-facts` 13/13, `systems-proof-corpus` 44/44, `systems-vectorization-facts` 92/92. LSP **272/272** including the CLI↔LSP comparator. `tsc --noEmit` clean; root `nlc format --check --project .` exits 0. Ratchet 17/18 → **18/18**, both keys `head-v1:5b72f3c669441a59` → `head-v1:3feee84058e6552b`. |
| LINTER/ANALYZER CHIPS A–E — five silent holes, three of them shipping wrong code (LANDED) | `901f9132c` A · `4ddcc37ff` B · `f38c83ea7` C · `ad6588f8e` D · `d726f040e` E (branch `stream/chips-linter-analyzer` off `b3ca5afe6`); **ZERO C#** in all five | **A** — `LinterWalk.VisitStatement` had arms for 21 of 29 `Statement` kinds and RETURNED for the rest, so it never entered an `unsafe`/`alloc`/`allow` body: the whole linter was off inside them. Three arms, plus a tail that THROWS for any kind not named in the new `IsBodylessStatement`. **B** — a `TypeReference` is not an `Expression`, so no walk reaches one without an explicit arm; `typeof` had one and the other six slots did not. `TypePattern`, `IsExpression`, `CastExpression`, `SizeOfExpression`, `StackAllocExpression` and `CallExpression.TypeArguments` now route through `TrackTypeReference`, and the recovery parser stamps the type pattern's `SimpleTypeReference` (it was built at 0,0). `VisitPattern` gains the same throwing tail. **C** — `ValidateOverrideTargets` `continue`d past every non-function member; NEW property classifiers on both sides (source by `DeclaredMemberKind.Property`, metadata by `GetProperties` + accessor `IsVirtual && !IsFinal`) with the two-arg statics untouched, plus `NL311` for `virtual`/`abstract`/`override` on a FIELD. **D** — new `NL324`: a concrete class inheriting an unimplemented `abstract` member. **E** — new `NL325`: a class, struct or record declaring an interface it does not implement; `DeclaredMemberInfo` gains `HasBody` (defaulted, no call site moved) | **A MISSING ARM DOES NOT WEAKEN ONE RULE, IT SWITCHES THE WHOLE LINTER OFF FOR THAT SUBTREE** — inside a wrapper body a local read only there was NL001 (an ERROR, on correct source), a dead local was never reported, an import used only there was NL010 and an empty catch was never NL011. The statement arm was fail-OPEN while the expression arm was fail-SAFE, and that asymmetry had already cost `off`; both walks now name their silent shapes and throw for anything else, which is what stops the NEXT node kind re-opening it. **TRACKING A PATTERN FIXED NL010 AND LEFT NL002 SILENT, AND THE SECOND HALF WAS IN THE PARSER**: `ReportMissingImportForReference` drops any finding without a position, and the type pattern's reference had none — six golden rows in two files had pinned that as a fact, with rationales written against them. **THE PARSER SPLITS A CLASS'S `: A, B, C` BY POSITION**, so `class R : IDisposable` carries an interface in the base-CLASS slot; D shipped answering it "does not implement inherited abstract member 'Dispose'" and E corrects the sentence and the owner. **A DEFAULT IMPLEMENTATION IS INVISIBLE IN EVERY OTHER FIELD OF THE MEMBER MODEL** — E's first census produced exactly one finding and it was E's own bug, accusing correct N# (`RecordsAndInterfaces.nl`) of not implementing the member its interface had written out. **THE PARSER CHOOSES FIELD-VS-PROPERTY FROM WHAT FOLLOWS THE TYPE, NEVER FROM THE MODIFIERS** (measured, and the emitted metadata agrees), which is what made C's third rule the right shape. New wall for §2.4: `git stash` is REPO-WIDE — a bare `pop` in one worktree takes a sibling session's stash | Census over 98 projects, both sides re-measured at each rebased base, every finding accounted for: **A 951 -> 955 -> 951** (4 rows = 3 real `marker` locals, fixed with the `_` prefix, plus 2 more in `systems-analysis-census`'s inline fixtures); **B, C, D byte-identical** (951/951, 951/951, 953/953); **E 953 -> 954 on the first cut** (the rule's own bug) **then 956 -> 956**, proven non-vacuous by two mutations on real estate source (deleting `GetArea` from the example's `Circle` and `Dispose` from the `ManagedResource` fixture each produce NL325). B's zero crossed 2,059 ` as <Type>` and 370 ` is <Type>` production sites; C's and D's are predicted by 68 `override func` and ZERO override properties, fields with inheritance modifiers, or unimplemented abstracts. Estate **7,413 -> 7,473 Failed 0** (+7 A, +8 B, +5 C, +5 D, +7 E, each baseline re-measured at its own rebased tip). Catalog census repinned 99 -> 101 codes / 80 -> 82 compiler rows; `NL324.md` and `NL325.md` are full error pages. All 42 native projects green (`compile-time-bench` is a wall-clock gate that fails IDENTICALLY at the pristine tip — `b3ca5afe6` median 15,623 ms vs this branch's 12,497 ms against the same 11,802 ms limit — so the control says loaded box, not branch). Unit 595/595, LSP untouched, ownership audit 18/18, no repin owed, `nlc format --check` clean |
| CODE-INTELLIGENCE CHIPS A/B/C — the three D2 filed (LANDED) | `0dc628aab` A · `376abfb80` B · `bb39e0aef` C (branch `stream/chips-code-intelligence` off `114c4cda3`) | **ZERO non-`.nl` files in all three.** (A) `CodeIntelligenceNavigation.TypeAtPosition` renders a metadata METHOD's `resolvedType` through hover's own `GetReflectedMemberSignatureText`; `ReflectedMemberAtPosition`'s body is extracted as `ReflectedMemberAtExpression` so `query type` hands over the walk it already did and both commands answer about the SAME node. (B) new `CodeIntelligenceMemberDocs.nl` bridges a `ReflectedMemberHandle` to an ECMA-334 doc id and a new `DocQuery.SummaryForDocId` + `DocQueryTypeIndex.SeedReferencePackDirectories`; `DocQueryKernels.GetDocSummarySentence` cuts to one sentence; the index is memoized by a new `ProjectSnapshot.Documentation()`. (C) new public `AstNodeFinderCore.FindCallExpressionAtPosition`; `ReflectedMemberOfTypeForArity` becomes `ReflectedMemberOfTypeForCall` and `ChooseReflectedOverload` scores arity-then-type-identity; `VisibleOverloadCount` drops `(+N overloads)` only where a call actually narrowed. | **THE PLACEHOLDER HAD TWO PRODUCERS AND FIXING EITHER ONE ALONE WOULD HAVE HALF-LANDED A.** `TypeInfoFromExpression` asks the SEMANTIC MODEL first, so at a position the analyzer reached the `ToArray(...)` text comes from `AnalyzerMemberResolution`/`AnalyzerDeclarationContext`/`AnalyzerExtensionMethodResolution`, and elsewhere from the code-intelligence-local arm. Neither is touched — they are the analyzer's DIAGNOSTIC text and its messages are pinned on it — and the single rendering seam is fixed instead. **SCHEMA: A VALUE MOVED, NOT A KEY**, so `schemaVersion` holds at 1 on D2's own precedent, and the identity `hover.signature == kind + " " + name + ": " + resolvedType` is now a COMPARATOR rather than two goldens. **THE DECLARING ASSEMBLY CANNOT KEY A DOC LOOKUP, MEASURED BY EXECUTION.** The analyzer's MLC searches the SHARED FRAMEWORK, which ships ZERO `.xml`; its facades type-forward, so an MLC probe reports `System.String`'s assembly as `System.Private.CoreLib`, for which no reference pack has ever carried an XML. Nor is there a namespace rule: `DateTime.AddDays` is in `System.Runtime.xml` and ``List`1.ToArray`` in `System.Collections.xml`. The DOC ID is the only universe-independent key, which is also what makes CLI and LSP agree by construction. **N# HAS NO PROCESS-WIDE SLOT AND BOTH CANDIDATES WERE PROBED SHUT AT THIS TIP** — a mutable static field still declines the whole assembly at emit, and `AppDomain` is `emit.local.unsupported-type` — so the toolchain-global index is memoized on `ProjectSnapshot`, the only long-lived object the LSP's `DocumentManager` and the daemon both already hold. **`nlc query hover` IS NOT A DAEMON METHOD** (`DaemonMethodKind` has nine query kinds and hover is not one), so the CLI cannot amortize across invocations — filed, not fixed. **CROSS-UNIVERSE `Type.IsAssignableFrom` ANSWERS FALSE IN BOTH DIRECTIONS AND NEVER THROWS** (probed against a real MLC over the shared framework), so a predicate built on it would have degraded to arity silently and no contract could have seen it; `FullName` is byte-identical across universes and is what C compares. **C WAS WORSE THAN D2 RECORDED**: not "the first overload with a count" but a signature the call CANNOT bind — one argument shown for a two-argument call and vice versa — while `preferredArgumentCount` sat in the code with every production caller passing `-1`. **B's SEED FOLLOWS 022/2d**: `RuntimeEnvironment.GetRuntimeDirectory()` first, `Assembly.get_Location()` second, because under a single-file/AOT host the latter is the EMPTY STRING. | Estate **7,413 → 7,418 Failed 0** (+1 sentence kernel, +4 chip C blocks); native `query-integration` **69 → 75 all passed** (2 per chip; the count diff proves they execute), `cli-command-contracts` 94/94, `playground-tooling-surfaces` 33/33, `doc-query` 11/11, `completion-engine` 13/13, `query-completions` 1/1, `analyzer-semantic-model` 51/51; LSP `~LanguageServer` **272/272 Failed 0**. Before/after on the report's own positions: `AddDays(...)` → `DateTime AddDays(double value)`, `ToArray(...)` → `WeatherForecast[] ToArray()`; five BCL hovers gained a summary sentence; `IndexOf(char value) (+9 overloads)` → `IndexOf(string value, StringComparison comparisonType)` and `Append(char value, int repeatCount) (+25 overloads)` → `Append(string? value)`. **The sharpest control: ONE method, TWO call sites, TWO answers** — `Next(-20, 55)` → `int Next(int minValue, int maxValue)` and `Next(summaries.Length)` → `int Next(int maxValue)`, each carrying its OWN summary. B's cost: source-symbol hover 0.42/0.42/0.47 s (baseline 0.45 s, unmoved — the index is never built for it), first metadata hover 1.33/1.39/1.49 s, peak RSS 87 → 282 MB, over 354 XML files / 73 MB / 144,416 members; amortization proven on the native suite (six added metadata hovers across two snapshots cost +1.1 s where six unmemoized builds would cost ~5.4 s). D2's deliberately LOOSE `Next` pin is now exact. No golden moved, no schema version changed, ownership audit untouched. |
| IDE D4 — NL002 anchored on `new`, and the import rule that inspected one position (LANDED) | `6f92309d3` anchor · `b63f23a4a` coverage · `85a00c0ce` span (branch `stream/ide-d4-nl002-anchor` off `7af97ac86`) | `LinterTypeReferenceName.BaseNameSpan` beside `Base` (arm for arm; the union arm spans the arm `Base` picked); `CheckMissingImportForType` reports at the type name's `NameSpan`; `TrackTypeReference` — the one site NL010 already visits at all fourteen written-type positions — now asks NL002 too; the bare-identifier span passes the identifier's own length so `DiagnosticSpanResolver`'s dot-crossing is never asked. ZERO non-`.nl` files. | **NL002 is import hygiene, not a resolution claim** — `func takes(sb: StringBuilder)` with no `import System.Text` checks clean, builds and runs (the compiler resolves BCL simple names without imports; `NotARealTypeAtAll` → NL201 on the name), so widening the rule to every type reference is consistent, not a new finding class. **`CodeActionHandler.cs` does no anchoring** (it matches the client's cursor-intersected diagnostics by line+column; the edit derives from suggestion text + last import line) — the LSP defect was the linter's span, nothing else. **The shipped golden already had the right anchor** (`top25.golden.txt` carets under `List`): the linter came to the golden. **A contract had pinned the wrong span by name** ("AND THE SPAN IS THE `new` KEYWORD … pinned rather than corrected") — retitled and re-pinned as the product decision it said it awaited. Wall for §2.1: `partial` is reserved as a LOCAL name and surfaces only through `nlc format --check` ("Unexpected token 'partial' in expression"). | `new List<int>()` 2:11+3 → 2:15+4; census over 93 projects / 509 production `.nl`: BEFORE 1 → AFTER 3 → FINAL 0 (two REAL missing imports in the compiler's own source fixed: `Preprocessor.nl` `import System.Text`, `ColumnarRuntimeTypeFacts.nl` `import System.IO`; zero false positives across examples/templates/fixtures/samples; caveat: `nlc check` excludes `.tests.nl`, 27 of 93 projects contribute no files); pins moved: 3 estate contracts + playground-diagnostic-spans 114/2 → 116/116 (field-type `List<>` with no import, the fix landing); estate 7,335 → 7,352; LSP 272/272; unit 595/595; ratchet 18/18 no repin. |
| FORMAT-FOLLOWUPS — the Playground evaluated raw strings wrongly, and format discovery walked other sessions' checkouts | `118a37a57` (chip A), `aa0795571` (chip B); branch `stream/format-followups` off `7af97ac86`; **ZERO new C#** (two arguments changed in `PlaygroundRunner.cs`, which did not grow) | **CHIP A.** The latent decoder defect resolved to a DIFFERENT owner on inspection: all thirteen `StringLiteralDecoder.Decode` call sites pass a SOURCE SLICE, so its classifiers work and the emit path was never wrong. The live defect was `PlaygroundRunFacts.DecodeStringLiteralText`, a SECOND implementation handed `StringLiteralExpression.Value` — which for a raw literal is the BARE BODY — whose `"""` guard therefore never fired and whose escape chain did: the Playground **evaluated `"""slash\n"""` to `slash` + a real newline**. Fixed by TELLING it (`IsRaw`, already stamped by the parser) rather than sniffing, with `Decode` taking the same parameter so the three string forms are one contracted family; `PlaygroundRunFacts.tests.nl:134` RETIRED (it fed the owner `"""a\nb"""` with delimiters — the one input the two call sites cannot supply); `SystemsAttributePolicy.Unquote`'s false comment corrected and its both-ends guard contracted; `DecodeInterpolatedText`'s `$"""`-only arm contracted, not changed (unreachable — both entry points gate on `$` first). **CHIP B.** The exclusion list already contained `.worktrees` and the layout is `.claude/worktrees`, so neither segment matched and the root walk entered three other sessions' checkouts. Replaced by gofmt's dot half — skip any segment beginning with `.` — with the six dot-named entries DELETED and `bin`/`obj`/`node_modules` kept because they are not hidden | **A NAME LIST CANNOT KEEP A WALK OUT OF OTHER TOOLS' DIRECTORIES, AND `.worktrees` IS THE PROOF — IT WAS ADDED FOR EXACTLY THIS PURPOSE AND STILL MISSED.** A list has to predict a directory name in advance; a rule about the FIRST CHARACTER does not. The underscore half of gofmt's rule is REFUSED on measured evidence (`editors/vscode/test/fixtures/simple/_rename_var.nl` and two siblings are real N# sources; gofmt skips `_` only because the Go toolchain does), and `testdata` for the same reason. **And the THIRD contract in two batches that pinned a shape its production caller cannot produce** — after `[A(n = 1)]` and `!CanStartExpression(TripleQuoteStringLiteral)`, now `DecodeStringLiteralText("""a\nb""")`. All three exercised a branch nothing reached while the branch that ran was wrong. **The rule this establishes: a contract that constructs its own input is only as good as its guess about the caller — state the caller's shape, or reach the owner through the caller.** Corollary from chip A: `StringLiteralExpression.Value` means two different things by form, so ANY consumer holding a `Value` must be told which it has; sniffing cannot work, because a raw body is arbitrary text | **CHIP A: the measured defect is now an executed assertion** — `DecodeStringLiteralText("a\\nb", false)` is asserted to be `"a\nb"` of Length 3, beside the fixed `(…, true) == "a\\nb"`. `nlc run` UNAFFECTED and checked (`RunCommand` routes through the real compiler; `PlaygroundRunner` is Playground-only). `PlaygroundRunner.cs` 938 lines unchanged; ratchet repin fingerprint-only, both keys audit-observed (`text-v1:dc333791ebd991ee`, `head-v1:9a9904c46e3c0d73`). **CHIP B, measured with the same instrument in the LIVE checkout with all three nested worktrees present: BEFORE exit 1 with 840 files listed, 840 of 840 under `.claude`; AFTER exit 0, "All files are properly formatted", zero declines**, main checkout untouched (`--check` writes nothing). 2,712 `.nl` files under `.claude` no longer walked. Explicit hidden root proven end to end: `--project .claude/worktrees/nsharp-vscode-ide-verify-742da3` still reports its 280 files. Estate **7,335 → 7,342 Failed 0** (+3 chip A, +4 chip B); full unit suite **595/595** at both commits; ownership audit 18/18; this worktree's own root `--check` exit 0, so the previous batch's fixed point holds |
| FORMAT-FIDELITY — six defects that rewrote or deleted what the author wrote, and the estate reformat that closes them | `7e10ab857`, `cbf950717`, `11f8e3671`, `e5750b66e`, `ae95c6bc1`, `ebd81f403`, `116dedbe0` (branch `stream/format-rawstring-reparse` off `40f4a6eb5`); **ZERO C#** | Six fixes in the N# owners, each a case of the formatter writing back something the author did not write. (0) **ATTRIBUTES** — `FormatAttributeInline` rebuilt each one from `Name` + `Arguments`, which holds neither the argument's spelling nor the attribute's line structure: `[aotSafe(mono-wasm)]` re-rendered as a SUBTRACTION and a five-line `[trusted(...)]` was joined onto one line with `reason = ` for `reason: `, which took `systems-proof-corpus` to 42/44. `AttributeNode.SourceText` carries the `[`-to-`]` span and the walk writes it verbatim. (1) **`CanStartExpression` was missing `TripleQuoteStringLiteral`** — a LANGUAGE defect: `ParseReturnStatement` asks it whether the `return` has a value, so `return """abc"""` parsed as a bare `return` plus a stray statement and `nlc check` answered NL305 + NL312 + NL006 on correct source; `"a" + """b"""` the same. Plus `TokenEndLine`: `EndLine` was `Previous().Line`, which under-reports for the two raw literals (one token, many lines), so the formatter wrote a blank line above whatever followed. (2) **RAW STRINGS** — `Lexer.ReadTripleQuoteString` appends neither delimiter, so `StringLiteralExpression.Value` is the bare content and the formatter emitted an IDENTIFIER where the author wrote a string; `IsRaw` + verbatim re-emission. **`let`** was unspellable (`HasLetKeyword` preserves it, `TypedDeclarationNeedsLet` supplies it where the bare form cannot re-parse). (3) **DIGIT SEPARATORS** — `ReadNumber` drops `_` so `Parse` accepts the value; `Token.Spelling` carries the source text ON THE SEPARATOR BRANCH ONLY (null everywhere else, nothing allocated on the hot path). (4) **`alloc`/`stackalloc` had no arms at all** (the only two unhandled `Expression` subclasses), and `GetCompletionKind` tested `failed` before the verify arm so ONE decline silenced the whole `--check` report. (5) **GENERIC CONSTRAINTS were deleted** — no arm read `FunctionDeclaration.Constraints` | **THE DECLINE WAS THE SAFETY NET, NOT THE DEFECT, AND FIVE OF THE SIX WERE INVISIBLE BECAUSE THE FILE IN FRONT OF THEM DECLINED FOR AN UNRELATED REASON.** A raw literal re-emitted bare is valid, idempotent, wrong code wherever the content lexes as one expression: `v := """abc"""` → `v := abc` and `P("""abc""")` → `P(abc)` were WRITTEN TO DISK with both gates green, and `DocumentFormattingHandler` calls the same `FormatSafe`, so format-on-save did it in the editor. Only where the bare content failed to re-read did anything object — so the eight decliners were the LUCKY files. The same shape produced the other four: `ScalarCodePlan.tests.nl` holds the estate's only bare separated numerals and had always declined on its raw string; `38-unmanaged-sort-comparer/Program.nl` holds the estate's only real `where` clause and had always declined on `AllocExpression`. **Fixing one defect is what exposes the next**, so the census must be re-run after every fix, and "count the files `FormatSafe` refused" is the wrong instrument — "format twice and diff" is the right one. Two contracts asserted defects AS FACTS with rationales written against them (`[A(n = 1)]` "that is the grammar, not an oversight"; "the two expression-start tables differ by exactly two tokens… folding them would break raw string literals"); both are overturned, and both would have been caught by a contract that named the three string forms, or the two argument spellings, as ONE FAMILY | **`nlc format --check --project .` at the repo root exits 0, "All files are properly formatted", DECLINES 23 → 0** (8 raw-string/`let` + 15 `AllocExpression`), and the same for `tests/fixtures/issue-tracker` as its own root. **FIXED POINT: a second pass over both roots reports "Formatted 0 file(s)".** Estate **7,302 → 7,320 Failed 0** (+18 contracts); full unit suite **596/596** at every commit (`dev.sh --since` selected it — central changes). `nlc check --json` **BYTE-IDENTICAL** over all ten reformatted projects (md5 per project, ten for ten). Contracts on the reformatted files all green: `systems-proof-corpus` **42/44 → 44/44** (and still 44/44 after the reformat), columnar-emit-facts 41/41, external-base-interface 18/18, readonly-init 18/18, record-with 12/12, lambda-placement 9/9, extension-calls 4/4, generic-scope-invalid 3/3, scalar-code-plan 2/2. **ZERO C# in 29 files (+1,175/−124)**; ownership audit 18/18, ratchet untouched. Reformat = 10 files, the only ten that were unformattable at the start |
...
**Durable findings (IDE O1–O3 + the reload harness — LANDED).**

- **A DEFECT REPORT'S OWN VOCABULARY IS NOT THE PRODUCT'S RULE, AND CHECKING COSTS ONE COMMAND.** O2 was
  filed as "camelCase is file-private, so the list offers something the analyzer will reject". Half true:
  `nlc check` does answer NL308, but the rule is PACKAGE-scoped — the same read from a file that shares the
  namespace checks clean, 0 errors. `AGENTS.md` and the memory note carry the "file-private" shorthand;
  `website/docs`, the diagnostic sentence and `IsCrossPackageFile` all say package. A filter written to the
  report's wording would have hidden members that compile and run.
- **A RULE WITH THREE SPELLINGS HAS TWO WRONG ONES, AND THE SURFACE NOBODY WATCHES KEEPS THE RIGHT ONE.**
  The completion dedupe-and-order rule existed only in `PlaygroundCompiler.DeduplicateCompletions`, so the
  playground's lists have been clean since 020/s38 while the CLI and the editor — the two a developer
  actually looks at — showed reflection order. Same shape as the playground union-shorthand chip: look for
  the third spelling before writing a first.
- **AN OVERLOAD COUNT CANNOT BE RECOVERED DOWNSTREAM OF THE PRODUCER THAT DROPPED THE SIGNATURE.**
  `BuildMemberItemsFromRows` writes `null` parameters for reflected members, so eleven `Split` rows are
  identical in every field a completion carries. Two collapse rules were tried and measured wrong before the
  count moved to `BuildReflectionMemberItems`, the only place holding the `MethodInfo`s.
- **NOT EVERY REPEATED NAME IS AN OVERLOAD.** An identifier position offers `async` as keyword AND modifier,
  and `Add` twice with an identical signature (the declared-type table lists top-level functions beside the
  functions table). A naive count claimed the user's own one-line `Add` was overloaded. Three playground
  contracts caught it; the estate blocks that had been written first did not.
- **A GATE STEP THAT NOTHING INVOKES IS NOT COVERAGE.** May's `duplicateLabels` check lives in
  `editors/vscode/src/test/suite/index.ts`, reachable only through `npm run test:headless`, which
  `test-all-core.sh` never calls — which is why 27 duplicated labels survived four months. Assertions were
  added to the suite the gate DOES run. Check the invocation path before trusting a check exists.
- **THE ASSERTION-MARKER CEILING IS THE BINDING ONE FOR A `.ts`/`.cs` TEST FILE, AND `.test(` IS A MARKER.**
  `completion.test.ts` sat at its immutable epoch of 7 markers, so new coverage had to go inside an existing
  `test(`; and a regex `.test(...)` call counts as an eighth. `.exec(` does not. Census before writing.
- **TWO DISTINCT FACTS UNDER ONE CODE AT ONE SPAN READ AS A DUPLICATE.** O3's arms were genuinely
  independent checks and the file's header called the doubling deliberate — but independence is a fact about
  the CHECKS and had become the shape of the REPORT. Fold the finding, keep the checks, and say in the
  comment why the old comment was wrong rather than deleting it.
- **A HARNESS STEP THAT CANNOT FAIL VISIBLY WILL EVENTUALLY FAIL SILENTLY.** `killall … || … || true` reported
  success through three reloads that never restarted the editor, and the verification measured a build three
  commits old. Wait on the postcondition, bound the wait, and print the identity of what you just started —
  the proof line reads `LanguageServer.dll pid 2489 started Wed Sep 2 20:50:39 2026`, the exact fact the
  re-verification record had to establish by hand.


**Durable findings (the two chips — chip 1 CLOSED, chip 2 partial).**

- **THE EMITTER DOOR IS THE BLOCKER FOR CONSTRAINT CHECKING AT DECLARATION POSITIONS.** Four of the six refused shapes
  are in §1's door-coverage decode; **TWO MORE, and they are the pair that closes every route**: (5) a cross-class
  static returning a USER CLASS declines as a local initializer, so a small record type cannot be constructed and
  handed across; (6) a `TypeInfo` read from a `List<TypeInfo>` INDEX cannot then be passed as a `TypeInfo` argument —
  inferred, declared, or through a typed accessor — while one obtained from an `out` parameter can. With the dictionary
  refused (§1) and the by-index read refused, the two only ways to reach a bound type are both shut, which is why the
  specials-only fallback failed as well. Twenty-odd cycles went into routing around them; **the routing failed and the
  measurement is the deliverable.**
- **A DECLINE IS ONLY REPORTED FOR THE FIRST FAILING STATEMENT**, so a function with several unsupported shapes reveals
  them one build at a time. That is why the door decode cost twenty cycles rather than five: each fix exposed the next
  refusal rather than the set. A future door slice should expect to pay per-shape, not per-function.
- **`tests/native/generic-scope-invalid` IS NOT A HOME FOR AN ANALYZER DIAGNOSTIC** — its harness drives
  `MultiFileCompiler.CompileToIlAssembly`, an emit-only path that never runs the analyzer, so NL208 rows written there
  fail with an EMPTY message. End-to-end diagnostic rows belong in `cli-command-contracts`, which spawns the real CLI.
- **THE FORMATTER'S TYPE-CONSTRAINT ARM WAS FREE AND ITS AST KEY WAS NOT.** `FormatterWalk.AppendGenericConstraints`
  already existed for functions and the five type headers only had to call it; but `AstEq.FieldNames` is a REGISTERED
  table, not reflection, so without five new rows all 291 AST contracts would have compared the declaration and
  silently ignored its constraints. `nlc query ast` needed nothing — its writer is reflection-driven.
- **A CONSTRUCTOR'S DEFAULTED TRAILING PARAMETER IS SAFE TO OMIT**, unlike a free function's. Measured on the exact
  shape (subclass with `: base(...)`, `List<T>?` trailing default) in local-initializer, argument and explicit-supply
  positions — which is what kept the ~99 `new *Declaration(...)` sites untouched when the AST gained `Constraints`.
...
**Durable findings (FORMAT-FIDELITY).**

- **A DECLINE IS THE SAFETY NET; THE DEFECT IS THE FILE THAT FORMATS.** Re-emitting a raw literal's bare content produces valid, idempotent, WRONG code wherever the content lexes as one expression — `v := """abc"""` → `v := abc`, `P("""abc""")` → `P(abc)` — so both of `FormatSafe`'s gates pass and the file is rewritten on disk, and through `DocumentFormattingHandler` on format-on-save. The eight decliners were the files where the bare content happened NOT to re-read. Any census that counts what `FormatSafe` refused understates this class by construction; the instrument is **format twice and diff**, and the acceptance measurement is `--check` exit 0 over the whole tree.
- **FIXING ONE DEFECT IS WHAT EXPOSES THE NEXT, so re-run the census after every commit.** Five of the six were unreachable because the file carrying them declined for an unrelated reason. `ScalarCodePlan.tests.nl` holds the estate's ONLY bare separated numerals and had always declined on its raw string; `38-unmanaged-sort-comparer/Program.nl` holds the estate's ONLY real `where` clause and had always declined on `AllocExpression`. The 286-file reformat at `17aa99e58` dropped zero constraints and zero separators — that was luck, not safety, and the luck ran out on the two attributes it did reach.
- **TWO CONTRACTS ASSERTED THE DEFECT AS A FACT, EACH WITH A RATIONALE WRITTEN AGAINST IT.** `FormatterWalk.tests.nl` pinned `[A(n = 1)]` under "a named argument uses ` = ` here and `: ` in a call — that is the grammar, not an oversight" (`ParseAttributes` uses the same `ParseArgumentList` a call does, so `=` was output no parser could produce). `ParserTokenFacts.tests.nl` pinned `!CanStartExpression(TripleQuoteStringLiteral)` under "the two expression-start tables differ by exactly two tokens… folding them would break raw string literals". **Both would have been caught by a contract that named the FAMILY** — the three string forms, or the two argument spellings — instead of pinning the odd one out. Prefer a sweep over a family to an assertion about a member.
- **AN ANNOTATION IS NOT CODE, AND THE ONLY SAFE RULE IS THE SPAN.** An attribute's arguments are stored as EXPRESSIONS and its line structure is stored nowhere, so any re-render loses something: `mono-wasm` becomes a subtraction, five lines become one. There is no "re-render, but carefully" for a grammar the formatter does not own. `AttributeNode.SourceText` is the `[`-to-`]` slice, and the price — attribute argument SPACING is no longer normalised — is paid deliberately and recorded in the contract that used to assert the normalisation.
- **THE SOURCE-FIDELITY FIELDS ARE ALL THE SAME PATTERN, AND ALL DEFAULT TO "NO SPELLING".** `StringLiteralExpression.IsRaw`, `VariableDeclarationStatement.HasLetKeyword`, `AttributeNode.SourceText`, `Token.Spelling` / `Int`/`FloatLiteralExpression.Spelling`: each carries what the VALUE cannot, each defaults to the value-is-the-spelling case so every hand-built tree is unaffected, and each is read by the formatter alone — `Parse` keeps reading the stripped numeral, policy readers keep reading `Name` + `Arguments`. `Token.Spelling` is populated ONLY on the separator branch, so no ordinary numeral, identifier or keyword allocates a second string.
- **`Value` DOES NOT MEAN THE SAME THING FOR ALL THREE STRING TOKENS, AND ONLY THE FORMATTER NOTICED.** `ReadString` and `ReadInterpolatedRawString` append their delimiters; `ReadTripleQuoteString` appends neither. `StringLiteralDecoder.IsTripleQuoteStringLiteral` therefore tests for delimiters the token can never have, so a raw literal reaching `Decode` falls through to `DecodeBody` and its `\n` is decoded as an escape — still latent, and out of this batch's scope. The columnar EMIT path is immune because it reads a SOURCE SLICE (`TryGetNodeText`), which is why the defect could live in the AST for as long as it did.
- **`let` IS NOT RECOVERABLE FROM `VariableKind`, AND THE SOUNDNESS TEST IS ABOUT TEXT.** `let x: T = v` and the bare `x: T = v` are both `VariableKind.Let`. `ParseExpressionStatement`'s bare arm needs `LookAhead(2).Type == Identifier` and then an `=`, so a tuple type (opens on `(`), a by-ref (`&`), and a declaration with NO initializer are the three shapes that cannot drop the keyword. `TypedDeclarationNeedsLet` asks the emitted type TEXT rather than enumerating type kinds, so a type form added to `FormatTypeReference` later cannot silently reopen the hole.
- **A RAW LITERAL IS ONE TOKEN THAT SPANS LINES, AND THREE `EndLine` STAMPS ASSUMED OTHERWISE.** `Previous().Line` is the identity for every other token, which is why it was right for years. The formatter measures blank-line gaps from construct ENDS, so the under-report inserted a blank line above whatever followed a multi-line literal — for `$"""` as much as for `"""`, i.e. the bug predates this batch. `TokenEndLine` counts the token's own line breaks.
- **THE `--check` REPORT AND THE EXIT CODE ARE DIFFERENT QUESTIONS.** `GetCompletionKind`'s kinds 1 and 2 both exit 1, so testing `failed` first could never be caught by an exit code — it decided whether the command PRINTS which files need formatting. One unformattable file anywhere under the root turned `nlc format --check --project .` into a bare exit 1 with empty stdout. Ask the verify arm first; a decline with nothing to report still falls through, so an empty list is never printed as news.
- **THE PHASE-1 IDEMPOTENCE MYSTERY WAS THE PARSER DEFECT.** `return """abc"""` failed the idempotence gate while a hand-written `return abc` was a fixed point, which made no sense until `CanStartExpression` was read: pass 1 wrote `return` and its value on TWO lines (the value was never part of the return), and pass 2 re-read them as one statement and wrote one line. Recorded because the instinct — "the formatter is non-idempotent" — pointed at the wrong owner for the whole of PHASE 1.

### 4.11 Task 022 — one external type universe, and a NativeAOT `nlc`

| slice | commit | what moved | durable finding | numbers |
|---|---|---|---|---|
| 022/2f-b — scoped `typeof` resolution and external type admission | this commit (branch `stream/022-s3b2-load-surface`) | `ColumnarTypeOfPlanner` resolves production `typeof` names through the existing exact source/import/alias catalog and replaces external admission lists with an assembly-qualified identity round trip. Scalar, array, nullable, byref-like, source-builder and builder-generic lowering facts remain. Callerless JSON admission and `RequiredTextWriterType` are deleted. Five estate contracts plus the original 38-shape production-source table in `cli-command-contracts`; language-tour docs updated | The handoff counted **16 passes as 13**. The product catalog returns **Missing** for `System.Uri`; its separate probe's 22/22 resolution claim did not measure this catalog. `List<Source>` cannot round-trip through `Assembly.GetType` before the source builder closes: the ownership audit caught that structural regression despite a green estate, and its existing rebinding facts are retained and contracted. HashSet type admission and key eligibility are separate facts | Matrix **38: 16 PASS / 22 DECLINE → 37 PASS / 1 DECLINE**; 21 admissions, prior passes preserved. Estate **7,635/7,635**, Columnar dev **12/12**, native emit **59/59**, matrix **1/1**, ownership **18/18**. Zero C#, no new project, no SDK republish, both ratchet keys unchanged; integration gate owned by coordinator |
| 022/4c — `TypeResolver.cs` routed at the catalog, the seed universe retired | this commit (branch `stream/022-s3b2-load-surface`) | `TypeResolver.cs` 373 → **61**: the four `Type.GetType` seed names, the lazy double-checked load and lock, the type/exported-type/namespace caches and all three reflection walks are gone; what is left is three forwards and the shape change to the LSP's own record. `EditorTypeCatalogFacts.EditorUniverseSeedTypeNames()` DELETED. `Analyzer` gains a one-line `CreateEditorTypeCatalog()` and `DocumentManager`'s private analyzer field becomes a property — both NET-ZERO on their ratchet rows. Three of the facts contract's 34 blocks retire (the seed universe no longer exists) and the roster-reachability block is RE-AIMED onto the analyzer's universe | **A NECESSARY ONE-LINE ACCESSOR STILL HAS TO PAY FOR ITSELF.** The ratchet refuses a raised ceiling, and it does not net across files — `TypeResolver.cs` shedding 312 lines buys nothing for `Analyzer.cs`. The factory was made net-zero by dropping its doc comment (the explanation belongs on the N# owner, which a mechanical host should not be repeating) and tightening 3b-4a's own three-line comment to two; `DocumentManager` paid for its accessor by turning a field declaration into a property declaration rather than adding one. A slice that needs C# to grow should re-read what it is spending, not ask for the ceiling | `TypeResolver.cs` 373 → 61 (non-blank 326 → 53); `Analyzer.cs` 2,357 unchanged, `DocumentManager.cs` 1,449 unchanged, `Program.cs` untouched (DI resolves the new ctor); estate 7,603 → 7,600 (−3 retired, the re-aimed block kept); unit suite 593 Failed 0, LanguageServer classes 272 Failed 0, `tests/native/lsp-lifetime` 3/3; differential IDENTICAL byte-for-byte on 103 `project.yml` projects; corpus sweep 74/53/114, both comparisons 114 SAME, transcript diffs 0; decline census 0; audit 17/18 → 18/18, five rows repinned (four fingerprint-only, `TypeResolver.cs` shrunk) plus both head keys; format clean. **4d — the VS Code-enabled gate and the computer-use visual round — is the coordinator's and is NOT discharged here** |
| 022/4b — the editor's type universe becomes the analyzer's | this commit (branch `stream/022-s3b2-load-surface` off `8e6e28d9b`) | NEW `EditorTypeCatalog.nl` (399 lines) + `EditorImportableType`, holding the analyzer's `_mlcAssemblies` BY REFERENCE and doing the reflection the language server's `TypeResolver` does today: full-name resolution, the simple-name scan, the importable set and the namespace set, with the alias/array/generic pipeline routed through `EditorTypeCatalogFacts` unchanged. Every cache is keyed on the assembly list's identity. ZERO C#, ZERO product wiring — 4c routes `TypeResolver.cs` at it. NEW contract, 8 blocks | **THE EDITOR'S UNIVERSE STOPS BEING FIXED, AND THAT MAKES ITS CACHES A CORRECTNESS PROBLEM RATHER THAN A SPEED ONE.** `DocumentManager` calls `LoadFromProjectConfig` once per project directory INSIDE the analysis of the first file opened from it — after the window is up and after completions may already have been served — so a namespace set filled before that point and never re-asked would insist the user's own dependency does not exist, which is the defect the slice exists to remove. Two mutants, each biting its own block: dropping the identity key fails the centre block; breaking the index-parallel exported tables fails the late-assembly block. **The second mutant was INERT on the first attempt** because the fixture put a zero-export facade at index 0 — a table of length zero is refilled every time, so mis-keying it changes nothing; the fixture now puts `System.Private.CoreLib` there. A mutation that does not bite can be the FIXTURE's fault rather than the contract's | estate 7,595 → 7,603, Failed 0; owner 399 lines / contract 224 lines, 8 blocks; differential IDENTICAL byte-for-byte on 103 `project.yml` projects; corpus sweep 74/53/114, ctlA-vs-ctlB and ctlA-vs-slice both 114 SAME, transcript diffs 0; decline census 0; audit 18/18 with NO repin — 4b adds no C# and `Analyzer.cs` is untouched; root format check clean |
| 022/3b-4b — the resolver, and `MetadataLoadContext` reaches ZERO code lines in `src/**/*.cs` | this commit (branch `stream/022-s3b2-load-surface`) | NEW `AnalyzerMetadataAssemblyResolver.nl` (177 lines) subclasses the external abstract `MetadataAssemblyResolver` and carries D3, the four-stage probe order (already loaded by simple name; the search directories in order; the exact NuGet package directory; the cache prefix scan) plus record-and-continue on a bad file. `Analyzer.cs` loses the 115-line nested `NSharpMetadataResolver`, `GetNuGetPackagesRoot` and the `_metadataResolver` field; `LoadSystemAssemblies` becomes `_metadataLoadSurface.Open()` because the surface now builds the resolver, the search-directory ladder and the context in the one door the CLR's eager core binding requires. NEW contract, 6 blocks, driving the resolver through a real `MetadataLoadContext` | **A FIX FOR A SITELESS DECLINE IS INVISIBLE TO EVERY CONSUMER COMPILED BY THE OLDER TOOLSET.** This slice was republish-gated and the gate was invisible: `class X: MetadataAssemblyResolver` declined under the packaged SDK with NL103 and NO SITE — the exact defect 022/3b-1 had already fixed — because the package predated it. **And the probe ORDER is not observable in `Resolve`'s return value:** moving stage 1 after stage 2 returns the SAME assembly, since the CLR refuses the duplicate identity itself (`FileLoadException`), the failure is recorded and the relocated stage 1 answers anyway. The first version of the contract passed against that mutant. The observable that separates the two orders is the FAILURE TABLE — a correct probe order never attempts a load that has to fail | `Analyzer.cs` 2,495 → 2,357 (non-blank 2,340 → 2,223); **`MetadataLoadContext` 2 → 0 code lines in `src/**/*.cs`** (4 comment lines remain, one of them slice 4's) — slice 3b's terminal condition met; estate 7,530 → 7,536, Failed 0; TWO mutants, both bite (stage 1 moved after stage 2 → 1 FAIL; search directories reversed → 2 FAIL), each restored under `shasum -a 256`; differential IDENTICAL byte-for-byte on 102 `project.yml` projects; corpus sweep 74/53/114, ctlA-vs-ctlB 114 SAME and ctlA-vs-slice 114 SAME, transcript diffs 0 both ways; decline census 0; audit 17/18 → 18/18, head `head-v1:2dc98a36f5c57831` → `head-v1:4a939de2efa9f870` |
| 022/3b-4a — the three orchestration decisions become an ordered plan | this commit (branch `stream/022-s3b2-load-surface` off `09f4401ca`) | NEW `AnalyzerReferenceLoadOrchestration.nl` (380 lines): `PlanRequests` is PURE — a `ProjectConfig` and a directory in, an ordered `ReferenceLoadRequest` list out — and `Load` performs it. D1 (non-NuGet dependencies before NuGet ones) and D2 (a test dependency contributes its package NAME where a normal one contributes a PATH) are the plan's ORDER and KIND. `Analyzer.cs` loses `LoadFromProjectConfig`'s body, `LoadProjectReference`, `LoadNuGetPackage`, `TryGetRestoredPackageVersion`, `GetRestoredPackageVersions`, `LoadProjectReferenceFile`, the `_restoredPackageVersionsByProject` cache, the four callerless members and two dead `using`s, keeping one forward for the reflective AspNetCore blocks. The pinned-version table joins the search directories on the by-reference seam (`BeginResolverPinnedVersions`). NEW contract, 6 blocks | **A DECISION THAT CARRIES NO LITERAL IS UNPINNABLE WHILE THE METHOD THAT MAKES IT ALSO OPENS FILES.** D1 and D2 survived task 021's audit because neither is a string or a number, and nothing ever asserted them: the only observable was which assemblies happened to be loaded on the machine running the test, which depends on that box's NuGet cache and not on the rule. Splitting the walk into a pure plan and a performer is what made them statable — six blocks now run with no cache, no network, no restore output and no file of any kind. **CORRECTION to Phase 1's census:** `LoadReferencedAssemblyByName` was NOT callerless — `DriveImports` calls it from INSIDE `Analyzer.cs`, which a census that greps OTHER files cannot see. The compiler caught it; the census did not | `Analyzer.cs` 2,720 → 2,495 (−225 against the −209 predicted; non-blank 2,535 → 2,340); `MetadataLoadContext` code lines in `src/**/*.cs` unchanged at 2, both in the resolver; estate 7,524 → 7,530, Failed 0; differential IDENTICAL byte-for-byte on 102 `project.yml` projects; corpus sweep 74/53/114, ctlA-vs-ctlB 114 SAME and ctlA-vs-slice 114 SAME, transcript diffs 0 both ways; decline census 0; audit 17/18 → 18/18, head `head-v1:73f3912b20d84a27` → `head-v1:2dc98a36f5c57831` |
| 022/3b-3 — the context is opened and closed by its owner | this commit (branch `stream/022-s3b2-load-surface` off `cf320426f`) | `AnalyzerMetadataLoadSurface` gains `Open(resolver)` — which cores the context on `AnalyzerMetadataLoadPolicy.MetadataCoreAssemblyName()` — plus `Close()` and `CreateWellKnownTypes()`; `Attach`/`Detach` are deleted, the field becomes the readable `Context`. `Analyzer.cs` hands over a resolver and nothing else: the `_mlc` field, the `new MetadataLoadContext(...)`, the `_mlc.CoreAssembly ?? throw` and the three-line dispose arm all die. NEW contract block; the five existing ones rewired from `Attach` to `Open` | **A `MetadataLoadContext` BINDS ITS CORE EAGERLY, SO THE CORE IDENTITY CANNOT BE THE FIXTURE.** Coring on the production name (`System.Runtime`) loads the framework copy into the context before any block asks for anything, and the adopt stage then claims it by construction — 3b-2's migrated "a search directory must not win" turned RED against its own rule. The dedupe rules now run on `System.Console`, a NON-core twin that also ships in both directories under one identity (208 KB implementation vs 16 KB reference assembly). A fixture chosen before the owner constructed the context was measuring a different question | `Analyzer.cs` 2,726 → 2,720 (non-blank 2,541 → 2,535), the −6 predicted in Phase 1 exactly; `MetadataLoadContext` code lines in `src/**/*.cs` 4 → **2**, both now in the resolver; surface 228 → 260, contract 267 → 318 (6 → 7 blocks); estate 7,517 → 7,518, Failed 0; differential IDENTICAL byte-for-byte on 102 `project.yml` projects; corpus sweep 74/53/114 with ctlA-vs-ctlB 114 SAME and ctlA-vs-slice 114 SAME, transcript diffs 0 both ways; decline census 0; audit 17/18 → 18/18, head `head-v1:cd90565c118ebe9c` → `head-v1:a5b58b95b09ee8cd` |
| 022/3b-2 — the metadata LOAD SURFACE leaves `Analyzer.cs` | this commit (branch `stream/022-s3b2-load-surface` off `8a144587b`) | NEW `AnalyzerMetadataLoadSurface.nl` (228 lines) owns load-by-path, load-by-name, register, the three dedupe predicates, `AddSearchDirectory` and both `RecordFailure` doors, holding `_mlcAssemblies`, `_referenceLoadFailures` and the resolver's search-directory list BY REFERENCE and the `MetadataLoadContext` from `Attach` to `Detach`. `Analyzer.cs` keeps four one-line forwards; `RegisterMetadataAssembly` and the three `IsMetadataAssembly*` predicates are DELETED outright; `NSharpMetadataResolver` takes the shared list from `BeginResolverDirectories()` instead of `new()`-ing its own. `tests/AnalyzerMetadataLoadContextTests.cs` DELETED, its ratchet row `state:"removed"`. NEW `AnalyzerMetadataLoadSurface.tests.nl` (267 lines, 6 blocks) | **THE FIXTURE THE DELETED FILE SAID DID NOT EXIST WAS ALREADY ON THE MACHINE.** Its header refused migration because it needed "an estate kernel that can compile a throwaway managed library"; what the two `[Fact]`s actually need is ONE identity at TWO paths, and `System.Runtime, Version=10.0.0.0, PublicKeyToken=b03f5f7f11d50a3a` ships twice — the framework's 34 KB facade and the reference pack's 852 KB reference assembly. A refusal keyed on the fixture's IMPLEMENTATION outlived the fixture's requirement by two slices | `Analyzer.cs` 2,798 → 2,726 (non-blank 2,605 → 2,541); estate 7,508 → 7,514 (Failed 0); differential IDENTICAL byte-for-byte on 100 `project.yml` projects (18 examples, 43 native, the 597-file root check; stdout+stderr+exit code, 822 KB, 22 result-bearing); corpus sweep 74/53/114 with ctlA-vs-ctlB 114 SAME and ctlA-vs-slice 114 SAME, transcript diffs 0 both ways; decline census 0; audit 17/18 → 18/18, head `head-v1:c84392a028a8c371` → `head-v1:21d283e7657c020d` |
| 022/3b-1 — `override` finds its target on the base chain | this commit (branch `stream/022-s2-one-universe` off `a2d75f537`) | NEW `ColumnarOverrideTargetResolver.nl` walks the declared base and its ancestors for the first accessible virtual-or-abstract member matching name and signature, identity by ASSEMBLY-QUALIFIED NAME rather than `==`. `ColumnarIlEmitter.cs`'s `TryFindObjectOverrideTarget` -- a three-name list (`ToString`, `Equals(object)`, `GetHashCode`) looked up on `typeof(object)` -- is DELETED and its arm becomes one call plus a LOCATED `emit.declaration.override-target` decline. NEW `tests/native/external-abstract-override` (3 blocks) | **`override` OF ANYTHING A BASE CLASS DECLARED WAS IMPOSSIBLE, AND IT FAILED WITH NO SITE, NO FILE AND NO REASON.** The old arm `return false`d without calling `Decline(...)`, so the trace was empty and the error header said `code`. Isolated by five probes: external abstract base with no override BUILDS, CoreLib base + `override ToString()` BUILDS, an external parameter type on a non-override method BUILDS -- only the combination declined, and `StringComparer.Compare(string,string): int` declined IDENTICALLY, proving parameter types were never the discriminator. **The proof is dispatch, not compilation**: the contract hands an N# class to a real `MetadataLoadContext`, which calls `Resolve` through the abstract base slot and resolves `System.Private.CoreLib`, with a call counter showing the override was WIRED and not merely emitted; a CoreLib control holds a `StringComparer` subclass AS the base type across three abstract slots. **The negative is owned upstream**: an `override` naming no base member is `NL311` at the front door, so the walk never guesses. **Public-only, deliberately**: a protected slot is an equally legitimate target but `MethodBase::get_IsFamily` is unmodeled, and a row plus its first use cannot land in one commit under the packaged-SDK bootstrap | `ColumnarIlEmitter.cs` **20,784 -> 20,760** (-24, non-blank 19,766 -> 19,747); the C# arm shrank and nothing grew. Estate **7,502/7,502 Failed 0** = the measured baseline, 0 estate blocks added (the contracts are `tests/native`). `external-abstract-override` 3/3. Decline census **0**. ROOT `nlc format --check --project .` clean -- it caught an unformatted `.tests.nl` on the first pass. Corpus sweep at `9c3a41ca9`, control first: ctlA-vs-ctlB `PE_DIFFS=0`/`IL_DIFFS=0`; ctlA-vs-slice `PE_DIFFS=0` over 191 assemblies and **`IL_DIFFS=0` over 14,171 keyed bodies / 91,916 rows**. Ratchet 17/18 -> **18/18** after the two-key repin (row `text-v1:9affef29bfdec41a`, head -> `head-v1:44becd95c4855ff2`) |
| 022/3a-ii — the reflection-invoked constructors become `new` | this commit (branch `stream/022-s2-one-universe` off `b3ca5afe6`, after the toolset republish from `94ff758b5`) | `ExternalAssemblyScan.CreateMetadataLoadContext` is two lines -- `new PathAssemblyResolver(paths)` and `new MetadataLoadContext(resolver, "System.Runtime")` -- instead of two `GetConstructor` lookups, two `Type[]` parameter arrays, two `object[]` argument arrays and two `ConstructorInfo.Invoke` calls; `SetObject` is deleted with them. `NullabilityMetadataReflection.CreateNullabilityContext` becomes `new NullabilityInfoContext()`. `RecordFactsSetObject` (a different function that only shares a name fragment) is untouched. ZERO C# | **THE CENSUS IS RE-DERIVED FROM A FRESHLY BUILT DLL AND DIFFED, NOT GREPPED, AND THE COUNT GOING UP IS THE PROOF THE DIFF MATTERS.** `memberscan2` over the tests-EXCLUDED estate dll: `System.Reflection.ConstructorInfo::Invoke` is the only member REMOVED, and three are ADDED -- `PathAssemblyResolver::.ctor`, `MetadataLoadContext::.ctor`, `NullabilityInfoContext::.ctor` -- so 263 -> 265. A bare count would have read as a regression; the set diff shows exactly the intended trade. **An instrument trap worth keeping**: the first census read 271 with `ConstructorInfo::Invoke` still present, because `dotnet test` leaves a tests-INCLUDED dll in `bin` (17,437 MemberRefs vs 4,616) and the packaged comparison dll excludes them -- a census must state which build it read | Production `.Invoke(` in `.nl`: **0**. Distinct reflection members 263 -> 265, diffed member by member. Estate **7,441/7,441 Failed 0** = the measured baseline exactly, 0 new blocks. Decline census **0** -- the estate now compiles the direct construction, which it could not before the republish (the shape declined inside `ExternalAssemblyScan.nl` while building under the branch CLI; packaged SDK md5 `8fe8c779…` verified in the worktree). Corpus sweep at `b3ca5afe6`, control first: ctlA-vs-ctlB `PE_DIFFS=0`/`IL_DIFFS=0`; ctlA-vs-slice `PE_DIFFS=0` over 191 assemblies and **`IL_DIFFS=0` over 14,133 keyed bodies / 91,903 rows**. One transcript rc difference, chased not waved: `compile-time-bench` 50/51, which REPRODUCES with the BASE CLI in the same tree and is a TIMING gate (median 17,010 ms vs an 11,802 ms limit whose baseline is recorded 'idle machine'), so it is pre-existing and load-dependent, not this slice. Ratchet 18/18, 0 C# |
| 022/3a-i — the construction allow-list is retired for a general rule | this commit (branch `stream/022-s2-one-universe` off `fcba80dad`) | `ColumnarConstructionPlanner.TrySelectRuntimeConstructor`'s closed `targetType == typeof(X)` chain and `IsApprovedExceptionType`'s 17 entries are DELETED and replaced by a candidate scan over `GetConstructors()` scored with `ColumnarSourceDirectCallResolver.ArgumentsScoreWithFacts` -- the same rule the ordinary call resolver already applies to methods -- requiring a UNIQUE best, guarding builder-bound/abstract/interface/open-generic targets, and checking the owner by identity STRING rather than `==`. `TryAppendRuntimeConstruction` now collects argument types before selecting. `Types2/3/4` deleted as dead. 6 contracts; 2 pre-existing pinned declines CONVERTED. ZERO C# | **THE DEFECT WAS THE LIST, NOT THE MISSING ROWS, AND `nuget:` PROVENANCE WAS NEVER THE DISCRIMINATOR** -- `new DeserializerBuilder()` (nuget-sourced) built before this slice while `new NullabilityInfoContext()` (CoreLib) did not, because the only question the gate asked was membership in a hand list of what one slice happened to need. **Step (2) measured a NO-OP**: the two conversions the Phase-1 decode predicted would be needed -- `string[]` -> `IEnumerable<string>` and the upcast to an abstract parameter type -- were ALREADY general, admitted by `TryClassifyBuiltInReferenceFlow`'s `RuntimeAssignableFrom`, so no special case was added and 3a is one change rather than two. **Step (4) is blocked on the bootstrap boundary and was backed out rather than forced**: converting the three `ConstructorInfo.Invoke` sites makes BootstrapServices USE a newly admitted shape, and the estate is compiled by the packaged SDK, which still carries the allow-list -- `new PathAssemblyResolver(paths)` declines inside `ExternalAssemblyScan.nl` while building fine under the branch CLI. It lands as 3a-ii after a republish | Newly admitted, measured base-CLI vs slice-CLI rather than asserted: `new PathAssemblyResolver(string[])`, `new MetadataLoadContext(resolver, string)`, `new NullabilityInfoContext()`, `new Random()`, `new InvalidOperationException(string, Exception)` -- all `decline site=…` -> `Build successful`; `new ArgumentException(string,string)` was ALREADY admitted (the retired arm did describe it) and is recorded as such. Estate baseline re-measured at `fcba80dad` by stashing = **7,435** (not the quoted 7,433); with 6 net new blocks **7,441/7,441 Failed 0**. Decline census **0**. Corpus sweep at `fcba80dad`, control first: ctlA-vs-ctlB `PE_DIFFS=0`/`IL_DIFFS=0`; ctlA-vs-slice `PE_DIFFS=0` over 191 assemblies and **`IL_DIFFS=0` over 14,120 keyed bodies / 91,890 rows** -- so the widening changed **ZERO** corpus rows: no project contained a `new` the allow-list declined, said explicitly rather than left implied. Ratchet 18/18, 0 C# |
| 022/2e — the loader retirements that survive (measurement + record) | this commit (branch `stream/022-s2-one-universe` off `114c4cda3`) | ZERO product files. `ExternalAssemblyScan.nl`'s `Assembly.Load` was measured by MUTATION rather than retired by inspection, and it SURVIVES: it is not dead. The two `DocQueryTypeIndex.nl` `Assembly.Location` reads were then measured against the merged code-intelligence chips (`921a27f93`) and ONE of them re-sourced: `GetReferencePackDirectories` now offers `RuntimeEnvironment.GetRuntimeDirectory()` as the FIRST seed and the indexed assemblies after it | **`Assembly.Load` IS LOAD-BEARING, AND A CENSUS WOULD HAVE UNDER-REPORTED IT.** 2d re-sourced only `metadataPath`; the same `try` still uses the loaded assembly for `identityName` (which drives `FindSemanticIdentity`'s dedupe) and passes it as the entry's `RuntimeAssembly`, which is what `FoundResolution` needs for `HasRuntimeType` -- the flag every catalog-driven emit path gates on. The mutant (identity from `AssemblyName.GetAssemblyName(metadataPath)`, `null` runtime assembly) BUILDS CLEAN and then declines at run: `new List<Point>()` over a user type goes `decline site=emit.local.initializer … for 'points'`, while hello-world still builds -- so the blast radius is catalog-resolved types only, and a program touching just the hard-modeled BCL surface never sees it. That asymmetry is exactly why inspection or a call census would have called it dead. **`EmitIlAssembly.cs`'s Cecil corelib->contract rewrite is LOAD-BEARING until task 023 slice 3.** The emitted dll carries `System.Private.CoreLib` among 13 AsmRefs; the `refint` copy carries 17 with `System.Runtime` and none of CoreLib. The blocking shape is `TypeSpec` `GENERICINST List``1<VALUETYPE TypeDef#2>` with three `MemberRef`s parented on it (`.ctor`, `Add`, `get_Count`) -- i.e. `MakeGenericType(runtime List<>, PointTypeBuilder)`, which a metadata-cored builder cannot produce. And a reference assembly cannot be loaded for execution: `BadImageFormatException: Cannot load a reference assembly for execution.` | Mutation restored byte-identically under `shasum -a 256` (`8b1af76e728499a0…`), tree clean, control re-verified green after the restore (`new List<Point>()` builds and the program prints `1`). Production `Assembly.Load` sites unchanged. **2e(ii), after the code-intelligence merge**: chip B re-sourced only the HOVER seed (`CodeIntelligenceMemberDocs.DeclaringAssemblyLocation`), so `DocQueryTypeIndex:227` still derived the dotnet root from `Assembly.Location` on the `nlc query doc` path, which nothing seeds. It now offers the runtime directory first -- `FindDotNetRootCandidate` climbs the FIRST non-empty path, so under a single-file host discovery used to fall back to `DOTNET_ROOT` alone and produce NOTHING wherever that is unset. The second read (`GetXmlDocPath`, now `:285`) is KEPT: it is a beside-the-assembly XML lookup whose empty case is already the designed fallback (the owner says so), and retiring it would lose the XML for package references outside the shared framework. 2 contracts pin that the runtime directory is a viable seed and that an all-empty seed discovers nothing without `DOTNET_ROOT` -- the shape that made the old reading degrade quietly. Estate baseline re-measured at `921a27f93` = **7,418** (not the quoted 7,413, which predates the merge); with the 2 contracts **7,420/7,420 Failed 0**. Decline census 0; `doc-query` 11/11, `completion-engine` 13/13, `query-integration` 75/75; ratchet 18/18; 0 C#. TypeSpec signature bytes `15-12-15-01-11-08` decoded against ECMA-335 II.23.2.12 rather than read off a tool |
| 022/2h — is universe A reachable? (measurement only) | this commit (branch `stream/022-s2-one-universe` off `e7e68a3f5`) | ZERO product files. Two probes outside the repo (`…/probes/mixeduniverse`, `…/probes/universeA`) build and RUN all four possible arrangements of builder core x operand universe; full table, verbatim exceptions and the proposed terminal condition in `systems-language-closeout/decodes/2026-09-02-universe-a-viability-decode.md` | **UNIVERSE A IS UNREACHABLE THROUGH `PersistedAssemblyBuilder`, AND THE WALL IS STRUCTURAL.** No arrangement can emit a MEMBER CALL on an external generic closed over a builder-defined type (`List<Foo>().Add(x)`). `MakeGenericType` refuses a `TypeBuilder` argument whatever the builder's core (`This type 'Type: UserType' was not loaded by the MetadataLoadContext that loaded the generic type or method.`); `MakeGenericSignatureType` covers every DECLARATIVE position (field/param/return/local, saved and RUN) and reaches NO member (`TypeBuilder.GetConstructor/GetMethod`: `'type' must be or must contain a TypeBuilder as a generic argument.`); and the hybrid that takes definitions from `typeof(...)` writes a DUPLICATE corelib AssemblyRef and dies at runtime with `MissingMethodException: List``1.Add(!0)`. **Two corelib objects in one module is the single predictor of a non-binding image across every red row.** The only measured route left is the `MetadataBuilder` second executor that task 022 put OUT OF SCOPE -- that ruling rested on `PersistedAssemblyBuilder` sufficing, and for this one operation it does not | **The separable win, measured and RUN**: coring the context on the REFERENCE PACK's `System.Runtime` (verified by TypeDef scan as the ref-pack definer of `System.Object`) points every emitted reference at the CONTRACT assemblies -- `System.Object@System.Runtime`, `List``1@System.Collections` -- and the exe still runs `exit=0`, which is exactly the condition that makes `EmitIlAssembly.cs`'s Cecil corelib->contract rewrite a DELETION. Today's `examples/01-hello-world` writes `System.Object@System.Private.CoreLib`. **The facade is bigger than planned**: `ColumnarIlEmitter.cs` carries **1,800** `typeof` sites / 170 distinct arguments, of which 598 are emit operands/signatures, **654 are COMPARISONS that silently flip** (measured: `mlcString == typeof(string)` is False with identical `AssemblyQualifiedName`), 539 feed those, and only **9** are compiler-own types -- so ~1,791 of 1,800 route through the facade, not the signature subset. **The 2a-2d terminal condition cannot be carried into 2e-2g** and a replacement is proposed: scope-normalised body identity (contract-equivalence on the resolution scope), every `.tests.nl` corpus project green, identical stdout/exit for executable corpus programs, `ilverify` baseline unchanged, decline census 0 |
| 022/2d — the metadata path stops coming from `Assembly.Location` | this commit (branch `stream/022-s2-one-universe` off `e7e68a3f5`) | `ExternalAssemblyScan.OpenWithReferences` resolves each common assembly's `metadataPath` from DIRECTORIES (`CommonAssemblySearchDirectories` + `CommonAssemblyMetadataPath`) instead of from `runtimeAssembly.get_Location()`. The runtime directory (`RuntimeEnvironment.GetRuntimeDirectory()`) is searched FIRST and the project's resolved reference directories after. 4 contracts. ZERO C# | **THE RE-SOURCING IS PROVEN TO BE A CHANGE OF ROUTE, NOT OF CONTENT, BY A CONTRACT THAT COMPARES THE TWO READINGS**: for `System.Runtime` the resolved path is asserted EQUAL to `Assembly.Load("System.Runtime").get_Location()`, and all 27 common names resolve to files that exist. Ordering is load-bearing and pinned: a reference-pack facade and a runtime implementation of the same name do not carry the same types, so a reference directory may only answer a name the runtime directory LACKS -- the contract feeds a fake reference directory and asserts the runtime directory still comes first. `RuntimeEnvironment.GetRuntimeDirectory()` needed no binding-plan row: it already emits, and returns exactly the directory `get_Location()` was answering out of | 0 C#; `+64/-2` over 2 `.nl`. **Production `Assembly.Location` reads in `ExternalAssemblyScan.nl`: 1 -> 0** (the estate-wide production count is 3 -> 2, both survivors in `DocQueryTypeIndex`, which is `nlc query docs`, not emit). Estate **7,378/7,378 Failed 0** = 7,359 + 10 + 2 + 3 + 4. Decline census **0**. Ratchet 18/18, no repin owed. Corpus sweep at `e7e68a3f5`: `PE_DIFFS=0` over 191 assemblies, **`IL_DIFFS=0` over 14,107 keyed bodies**, and **ZERO transcript rc differences** -- including `compile-time-bench`, which had flipped in the ctlA-vs-ctlB control and matched here, confirming that family is load-dependent and not slice-dependent |
| 022/2c — the attribute tests stop using type identity | this commit (branch `stream/022-s2-one-universe` off `e7e68a3f5`) | All four `IsDefined(attributeType, ...)` sites become full-name `GetCustomAttributesData()` scans: three in `ColumnarExtensionMethodResolver` (host type, method, parameter) and one in `ColumnarOrdinaryRuntimeDirectCallResolver`. The two `Type.GetType` attribute lookups and `RequiredParamArrayAttributeType()` go with them, and the `paramArrayAttributeType` parameter is dropped from four signatures. `ColumnarTypeEquivalenceFacts.IsSameEnumType`'s `catch` widens from `NotSupportedException` to a bare `catch`. 3 contracts. ZERO C# | **ALL FOUR SITES DEGRADED SILENTLY, NOT LOUDLY, AND THAT IS WHY THEY HAD TO MOVE.** `IsDefined` throws over a `MetadataLoadContext` (`... Use CustomAttributeData instead.`), and every one of the four sat inside a `catch` returning FALSE -- so a metadata-sourced catalog would have produced an EMPTY extension index and a params-blind overload resolver in total silence. The `TypeHandle` clause is the mirror image: it caught only `NotSupportedException` while the metadata universe throws `InvalidOperationException`, so that one would have thrown UNCAUGHT. **The analyzer already knew the rule and the columnar side did not** -- `AnalyzerOverloadScoring`'s `HasExtensionAttribute` comment states it verbatim -- but the shared owner is unreachable: a static call into `AnalyzerOverloadScoring` from a `NSharpLang.Compiler.Columnar` owner declines at `emit.return.expression` for a `Type` argument, a `MethodInfo` argument and a uniquely-named target alike, while the identical body spelled inside the columnar namespace emits. `ExternalAssemblyScan` (same namespace) IS reachable from columnar owners, so this is narrower than 'cross-namespace' and the cause is not isolated | 0 C#; `+56/-45` over 5 `.nl`. `IsDefined` sites 4 -> 0; `Type.GetType` attribute lookups 3 -> 0. Estate **7,374/7,374 Failed 0** = 7,359 + 10 + 2 + 3. Decline census **0**. Ratchet 18/18, no repin owed. Corpus sweep at `e7e68a3f5`: ctlA-vs-ctlB `PE_DIFFS=0`/`IL_DIFFS=0`; ctlA-vs-2c `PE_DIFFS=0` over 191 assemblies and **`IL_DIFFS=0` over 14,107 keyed bodies**. **A transcript red was chased down rather than waved off**: the first 2c sweep showed `systems-analysis-census` 54/66, which the controls did not; it did not reproduce (66/66 twice in the same tree with the same CLI) and a clean re-run of the whole sweep gave 66/66 -- a load flake of the known systems family, caused by launching the ratchet audit alongside the sweep. The IL comparison never moved |
| 022/2b — the literal field is read from metadata | this commit (branch `stream/022-s2-one-universe` off `e7e68a3f5`) | `ColumnarExternalStaticMemberPlanner.TryAppendLiteralField` reads `GetRawConstantValue()` instead of `GetValue(null)`; `ColumnarExternalBindingPlans` gains the `FieldInfo::GetRawConstantValue` row. 2 contracts. ZERO C# | **THE SUBSTITUTE IS PINNED AGAINST THE THING IT REPLACES, IN THE ESTATE, OVER A REAL METADATA UNIVERSE.** A contract opens `ExternalAssemblyScan.OpenWithReferences(null)`, takes `System.Int32::MaxValue` off an `entry.MetadataAssembly`, and shows `GetRawConstantValue()` answering 2147483647 where the live read is refused -- so the change is not a preference, it is the only reading that survives the universe this task is moving to. The two agree on every shape the planner reaches, including a BYTE-backed enum (`JsonValueKind.Object`) where the live read boxes the enum and the metadata read boxes the underlying byte and `Convert.ToInt32` is what the planner puts between them. **New §2.1 wall**: the columnar door does not claim a `try` statement as a block child of another `try`'s block -- it declines the whole body at `emit.statement.block-child` (node kind 49) -- so a 'does this throw' probe inside a `try`/`finally` must be its own function | 0 C#; +11/-1 over 3 `.nl`. Estate **7,371/7,371 Failed 0** = the measured 7,359 baseline + 10 (2a) + 2, exactly. Decline census **0**. Corpus sweep at `e7e68a3f5`, control first: ctlA-vs-ctlB `PE_DIFFS=0`/`IL_DIFFS=0`; ctlA-vs-slice `PE_DIFFS=0` over 191 emitted assemblies and **`IL_DIFFS=0` over 14,107 keyed bodies / 91,881 rows** (2a and 2b together, so 2b adds nothing to 2a's already-clean sweep). Ratchet 18/18, no repin owed (zero C# rows moved) |
| 022/2a — the `CustomAttributeBuilder` conversion | this commit (branch `stream/022-s2-one-universe` off `7af97ac86`) | NEW N# owner `ColumnarAttributeBlobs.nl` (148 lines) writes the ECMA-335 II.23.3 CustomAttrib blob directly -- prolog, `SerString` fixed args, `PackedLen` in all three branches, hand-rolled UTF-8 including surrogate pairs -- and the four `ColumnarIlEmitter.cs` sites (`:4117` `IsByRefLike`, `:4960` `IsReadOnly`, `:5882` xunit `Trait`, `:5883` `Fact`) become `SetCustomAttribute(ConstructorInfo, byte[])`. `CustomAttributeBuilder` reaches **0 occurrences** in `src/**/*.cs`. 10 contracts | **THE CONVERSION MOVES NO EMITTED BYTE, AND THAT IS MEASURED TWICE.** Directly: an assembly built both ways in one process has `BLOBS_IDENTICAL=True` over all four blobs (`01-00-00-00` for the three no-arg sites; prolog + `11` + `NSharpDescription` + `PackedLen` + UTF-8 + `00-00` for `Trait`). End-to-end: xunit still discovers and names all 7,345 estate contracts, which it can only do by reading the hand-written `Trait`/`Fact` blobs. **Two language walls found while writing it**: an int LITERAL into a `byte[]` element declines (`b[0] = 1`; the route is `b[0] = Convert.ToByte(x)`, the `long[]` precedent generalises), and **N# has no `\uXXXX` escape** -- `"\ud83d"` is six ASCII characters, so a lone surrogate must be taken from a real pair with `Substring(0, 1)` | `ColumnarIlEmitter.cs` 20,784 lines UNCHANGED (4 lines replaced 1-for-1), 0 C# added; +148/+112 `.nl`. Estate **7,345/7,345 Failed 0** against the measured 7,335 baseline -- exactly +10, the new blocks. Decline census **0** for the whole BootstrapServices emit (`ExternalAssemblyScan.nl` included); instrument proven non-vacuous on a defaulted-param probe. Corpus sweep 98 targets / 262 assemblies, **control first**: ctlA-vs-ctlB `PE_DIFFS=0` and `IL_DIFFS=0`; ctlA-vs-slice `PE_DIFFS=0` over 191 emitted assemblies and **`IL_DIFFS=0` over 14,107 keyed method bodies / 91,881 rows**. Both granularities proven non-vacuous by a one-byte `ldc.i4.1`->`ldc.i4.2` flip (`PE_DIFFS=1`, `IL_DIFFS=1`, key named). Ratchet 17/18 -> **18/18** after the fingerprint-only two-key repin `head-v1:9a9904c46e3c0d73` -> `head-v1:95643491c76f5f24` (rebased onto `e7e68a3f5`, whose D4 batch had repinned it); manifest still 391 lines |
...
**Durable findings (022/3b-2).**

- **AN N# OWNER CAN HOLD A `MetadataLoadContext` IT DID NOT CONSTRUCT AS MUTABLE INSTANCE STATE, AND `try`/`catch` AROUND A
  CONTEXT CALL INSIDE A CLASS METHOD EMITS.** Both were probed before a line of production source was written, in a project built
  by the PACKAGED SDK — the only build that answers the question this file has to live under. `MetadataLoadContext::GetAssemblies`,
  `AssemblyName::GetAssemblyName`, `ReferenceMatchesDefinition`, `LoadFromAssemblyPath` and `Assembly::get_Location` are all
  already on the surface, so the whole move needed ZERO newly modeled members and was not republish-gated.
- **THE FOUR-STAGE PROBE'S LAST STAGE IS NOT AN OPTIMISATION, AND EXECUTION SAYS SO.** Asked for a second file carrying an
  already-loaded identity, `MetadataLoadContext.LoadFromAssemblyPath` throws `FileLoadException` ("has already loaded been loaded
  into this MetadataLoadContext"). Without the context probe the stale-beside-restored NuGet extraction would be recorded as a
  load FAILURE instead of resolving — i.e. "type not found" on types that plainly exist. The contract asserts the throw directly,
  so deleting the guard is not an equivalent mutant.
- **A CONTRACT CAN BE STRONGER THAN THE C# IT REPLACES WITHOUT REFLECTING OVER PRIVATE FIELDS.** The deleted `[Fact]`s reached
  `_mlc`, `_mlcAssemblies` and `_metadataResolver` by `BindingFlags.NonPublic`; the estate blocks drive the surface's own public
  doors over real on-disk assemblies and add three the C# never had (same path twice registers once, a failed load is recorded
  first-failure-per-identity, an unattached surface is a no-op — which is the state `Dispose` restores).
- **THE SEARCH-DIRECTORY LIST IS THE SURFACE'S STATE, NOT THE RESOLVER'S.** The resolver is rebuilt with every load context and the
  surface is not, so the list resets at `BeginResolverDirectories()` — the resolver-construction seam — and is then read in place.
  Spelling it as "the resolver clears and borrows" keeps a second `LoadSystemAssemblies` behaviourally identical to the C# that
  `new()`-ed a fresh list.
- **A CORPUS IL DIFF CAN BE THE HARNESS'S OWN SUPPORT ASSEMBLY.** ctlA-vs-slice first reported 53 DIFFERENT, every one of them
  `NSharpLang.Runtime.dll` — the C# support library the CLI copies verbatim into each project's output, byte-different only because
  the two CLIs were built in two different worktrees (the PE normaliser does not neutralise a CodeView path of different LENGTH).
  Zero EMITTED assemblies differed. Pinning that one file to the base CLI's copy and re-sweeping gave 114/114 SAME. Check whether a
  diff set is entirely support assemblies before reading it as a codegen change.
- **PROOF ORDER IS PART OF THE DELETION.** The two `[Fact]`s were run UNCHANGED against the moved surface — 2/2 green, twice, at
  two different tips — BEFORE the file was deleted, so the before/after control exists independently of the estate blocks that
  replace it. The ratchet row is `state:"removed"` with `text-v1:removed`, which OWN007 makes unreappearable.
- **`AnalyzerMetadataLoadPolicy.tests.nl`'s header carried the stale refusal and now carries the correction**, because the sentence
  that says why something could not move is exactly the sentence a later slice reads as a verdict.
...
**Durable findings (FORMAT-FOLLOWUPS).**

- **THE OWNER A DEFECT IS NAMED IN IS NOT ALWAYS THE OWNER THAT IS WRONG.** I filed `StringLiteralDecoder` as latent; all thirteen of its call sites hand it a SOURCE SLICE, so its classifiers succeed and it was never wrong in production. The live defect was a SECOND IMPLEMENTATION of the same rule — `PlaygroundRunFacts.DecodeStringLiteralText` — reading the AST `Value` instead. Before fixing a "latent" defect, census the inputs its call sites actually pass; two owners implementing one rule against two different inputs is the finding.
- **`StringLiteralExpression.Value` MEANS TWO DIFFERENT THINGS BY FORM, SO EVERY CONSUMER MUST BE TOLD.** The ordinary form carries its quotes (`Lexer.ReadString` appends them); the raw form does not (`ReadTripleQuoteString` appends neither). **Sniffing cannot work in principle** — a raw body is arbitrary text, so no prefix test separates one from an ordinary literal's contents. `IsRaw` is the signal, and any future consumer that reaches for a text test instead is reproducing this bug.
- **A CONTRACT THAT CONSTRUCTS ITS OWN INPUT IS ONLY AS GOOD AS ITS GUESS ABOUT THE CALLER.** `PlaygroundRunFacts.tests.nl:134` passed `"""a\nb"""` WITH delimiters and asserted the body came back verbatim — the one input the two production call sites can never supply, so it exercised a dead branch while the live branch corrupted every raw string. That is the third instance in two batches (`[A(n = 1)]`, `!CanStartExpression(TripleQuoteStringLiteral)`). **State the caller's shape, or reach the owner through the caller.**
- **A GUARD CAN BE LOAD-BEARING BY ACCIDENT, AND THAT IS WORTH A CONTRACT RATHER THAN A CLEANUP.** `SystemsAttributePolicy.Unquote` receives a raw literal's bare body and survives only because it refuses to half-strip a value that is not quoted on BOTH ends — `[trusted(reason: """abc""")]` arrives as `abc`, and a length-based "drop first and last" would answer `b`. Its comment claimed the parser keeps a literal's quotes; that is the ordinary form only.
- **AN UNREACHABLE ASYMMETRY IS CONTRACTED, NOT "FIXED".** `DecodeInterpolatedText` keys its verbatim arm on `$"""` specifically, so a plain raw literal's text would be escape-decoded — and no plain raw literal arrives, because both entry points gate on `text[0] == '$'`. Changing it would be a behaviour change with no caller to justify it; the contract records the reasoning so the next reader finds it instead of the surprise.
- **A NAME LIST CANNOT KEEP A WALK OUT OF OTHER TOOLS' DIRECTORIES.** The format exclusions already contained `.worktrees`, added for exactly this purpose, and the layout that exists is `.claude/worktrees` — `.claude` unlisted, `worktrees` undotted — so the root walk entered three other sessions' checkouts and reported 840 foreign files. A list predicts names; a rule about the first character does not have to.
- **ADOPT GOFMT'S DOT HALF AND REFUSE ITS UNDERSCORE HALF, ON EVIDENCE.** `editors/vscode/test/fixtures/simple/_rename_var.nl` and two siblings are legitimate `_`-prefixed N# sources; gofmt skips `_` because the GO TOOLCHAIN ignores those directories and `testdata` because it is a Go convention. Neither transfers. `bin`, `obj` and `node_modules` stay named because they are not hidden. Six dot-named entries were deleted, so there is one place to update rather than two.
...
| slice | commit | what moved | durable finding | numbers |
|---|---|---|---|---|
| Compiler generic-call binding and returns | `3949eb515`; assertions `14f6c4f3b` | Seven whole methods and fifteen direct routes; six planner controls; three complete C# end-to-end contracts migrate to N# | Preserve equality/identity distinction, partial binding writes, null failures and narrow return rules | C# emitter −174; C# tests −152/−12 markers; focused6, native7 baseline/candidate, declarations118, audit18; fresh448s gate590/7,849,52native,68IL. Proof (`2026-09-06-generic-binding-ownership.md`) |
| Compiler sibling constraint validation | `244c0998a`; seed `7f929aeb2` | Seven complete validation/substitution/interface methods move to N# with eight canonical controls; direct production routes | Explicit protected enumerators preserve disposal; SymbolType SZ-before-byref ordering is pinned | C# −155/−145 lines; focused 65, native 118 unchanged, audit 18; fresh 449s gate 593/7,843, 52 native projects, 68 IL. Proof (`2026-09-06-sibling-constraint-ownership.md`) |
| Compiler generic-constraint application | `be3defa12` | Complete application, declared lifting, consumed map and safe-array dependency become N#-owned; four C# methods deleted, fourteen direct routes | Preserve source-owner registration, mutation/output/catch phases and shared map identity | C# −107/−103/−5,028 bytes; focused 56, native 115, audit 18; fresh 468s gate 593/7,833, 52 native projects, 68 IL. Proof (`2026-09-06-generic-constraint-ownership.md`) |
| Compiler constraint seed | `eb750c4fa` | Exact N# interface-constraint setter admission and canonical/direct native controls; no C# change | Full proposed source drove one required row; source-owner registration belongs in generic fixtures | Fresh backend gate: 593/7,822, 52 native projects, 68 IL assemblies; packaged SDK 9/9, 12 payloads match. Proof (`2026-09-06-generic-constraint-ownership.md`) |
| 023/S2.2(l) — entry-point realization | product `070ad0a49`; tested `efa3a8cab` | Complete selection/wrapper owner becomes N#; reached awaiter type keyed | Preserve live indexed reads, concrete first-hit fallback, partial output and wrapper phases | C# −40/−37/−2,422 bytes;22 keyed/14 handle-only; gate 461s:593/7,821/115, audit 18,68 IL; fixed 94/2,184 each and strict 258/435 equal; l1 SDK unchanged. Proof (`2026-09-06-s22l-parity-proof.md`) |
| 023/S2.2(l1) — concrete dictionary value enumerator prerequisite | product `9e1093c67`; controls/tested `6687505e4` | Exact N# builder-bound enumerator admission; no C# move | Observe actual source-builder handle; retain closed/key/value identity boundaries and concrete addressed iteration/finally | +3 canonical, +3 native; gate 448s: 593/7,816/114, audit 18, 68 IL; fixed 94/2,184 each and strict 258/434 unchanged; SDK same-source 3/3, all 12 payloads verified; emitter/ratchet unchanged. Proof (`2026-09-06-s22l1-parity-proof.md`) |
| 023/S2.2(l0) — catalog reference array prerequisite | product `05187654e`; controls/tested `71d489a5c` | N# admits catalog reference elements through the existing exact identity gate; no C# move | Preserve real array signature and operations; honest/forged identity twin; direct Array.Empty generic binding remains separate | +4 canonical, +2 native; gate 450s: 593/7,813/111, audit 18, 68 IL; fixed 94/2,184 each and strict 258/434 unchanged; SDK same-source 2/2, all 12 payloads verified; emitter/ratchet unchanged. Proof (`2026-09-06-s22l0-parity-proof.md`) |
| 023/S2.2(k) — member iterator discovery/admission | initial `9dc7edc85`; ratchet `5614eb20c`; controls `159dfab2f`/`7d579e98e`; corrected/tested `137462ab1` | Complete member driver becomes N#-owned; necessary known-generic inheritance rule corrected in N# | Live first-hit/disposal/repeated-name/ordinal phases; concrete Count null trap; source-element IDE probe distinguishes the corrected path | Emitter 18,923/17,984 (−51/−51/−3,115 bytes);21 keyed/15 handle-only; 94 images/2,184 tests each equal; strict 260→258 with 258 baseline retained; native 109/109. Fresh 522s gate 593/7,809/36 VS Code/109, audit 18; visual PASS; j1 seed unchanged. Proof (`2026-09-06-s22k-parity-proof.md`) |
| 023/S2.2(j) — connected iterator realization | `20caf6997`; native `eaa10aaf5`; ratchet `269ead8d`; persisted/tested `4b5399140` | Complete sync/async drivers and canonical helper closure become N#-owned;15 consumed type rows keyed | Keep registration/decline/member/factory phases; actual persisted field retains machine VAR under factory MVAR owner; assert rows before bake and use actual generated identity | Emitter 18,974/18,035 (−290/−265/−18,756 bytes);21 keyed/15 handle-only; fixed 94 images/2,184 tests each equal; strict 258/434; native 109/109 image equal; persisted 2/2. Fresh 458s gate 593/7,803/109, audit 18; j1 seed unchanged. Proof (`2026-09-06-s22j-parity-proof.md`) |
| 023/S2.2(j1) — generic parameter declaration prerequisite | `4ab360c71`; native/tested `a8a5fb436` | One exact BCL generic-parameter call and concrete returned element admitted by existing N# owner; no C# move | Retain genuine GPBuilder[]; copy elements to Type[]; use Type receiver for MakeGenericType. Public return signature differs from covariant runtime implementation array | +2 canonical 2/2; native 7/7; declaration replay 1/1; fixed 94 images/2,184 tests each equal; strict258/433 unchanged. Fresh 452s gate 593/7,798/107, audit 18, SDK 1/1; ratchet unchanged. Proof (`2026-09-06-s22j1-parity-proof.md`) |
| 023/S2.2(j0) — continuation binding prerequisite | `74e0c908a`; wording `9be5346d4`; native/tested `b57676617` | One BCL Ldftn field admitted by existing N# binding owner; no C# move | Explicit MethodInfo/FieldInfo operands isolate binding; actual baked ctor token and delegate execution prove admission. Preserve existing source T[] refusal | +2 canonical, focused 3/3; native 6/6; fixed 94 images/2,184 tests each equal; strict 258 unchanged. Fresh 455s gate 593/7,796/107, audit18, SDK 1/1; ratchet unchanged. Proof (`2026-09-06-s22j0-parity-proof.md`) |
| 023/S2.2(i) — record synthesis | `1b067f203`; native `0d3c14c35`; direct/tested `2d156b43b`; ratchet `5dde45f26` | Complete PASS 0e driver and five structural type consumers become N#-owned; two C# helpers deleted | Later-row partial progress must execute the retained body, not merely observe its builder; old helper witness uses matching immutable dependencies | Emitter19,264/18,300 (−70/−66/−3,479bytes);6 keyed/30 handle-only; fixed94 images/2,184tests each equal; strict258 unchanged; native15/15 both; old witness1/1, mutation2pass/1fail. Fresh452s gate593/7,794/107, audit18; ratchet `head-v1:aa711599f6919ff9`. Proof (`2026-09-05-s22i-parity-proof.md`) |
| 023/S2.2(h) — generic-constraint lookup | `47d0a062d`; controls `68c0a8387`, `16a0f27f`, `5a5b3f001`, `18c079268`; tested `e170415204` | Exact-map/weak name+ordinal/reflection lookup becomes N#-owned; two C# helpers deleted and one direct forward | Same-map generic enumeration preserves array identity, guard/catch phases and disposal-before-reflection; real shared event trace replaces insufficient counters | Emitter19,334/18,366 (−43/−41/−1,128bytes); fixed94 images/2,184tests each equal; strict259→258 removes old unused-System NL010; old witness3/3, predicted mutation2pass/1fail. Fresh453s gate593/7,787/107, audit18; ratchet `head-v1:3535178e0c882670`. Proof (`2026-09-05-s22h-parity-proof.md`) |
| 023/S2.2(g) — live source-definition discovery | `770482e8e`; controls `de83c4197`; catch correction `096968ae7`; this integration commit | First-hit Builder identity, direct/struct/interface/closed discovery move to N#; three C# helpers deleted, eight direct routes | Exact generic Current, disposal/out timing and delayed Values are consumed IL; generator witness had to observe the cloned enumerator; empty generic catches now explicitly return false | Emitter19,377/18,407 (−107/−98/−2,646bytes); fixed94images/2,184tests each equal; strict 259 unchanged after measured NL011 correction; old private-helper witness 3/3 and predicted2-red mutation. Ratchet `head-v1:5605ce87ef5e4440`. Proof (`2026-09-05-s22g-parity-proof.md`) |
| 023/S2.2(g1) — BCL empty type-array field prerequisite | `f243d421` (worker `765776ce`); this integration commit | Four N# lines bind Type.EmptyTypes through the existing semantic static-field owner | Preserve the BCL empty-array identity and delayed registry.Values reads in the pending discovery cut; exact ldsfld and typed out assignment verified | No C# or ratchet change; estate 7,759; native declarations105; fixed corpus94 images/2,184 tests each equal; actual f543 same-source declines/f243 passes1; strict259 unchanged. Gate/live seed remain receipt-gated. Proof (`2026-09-05-s22g1-parity-proof.md`) |
| 023/S2.2(g0) — exact enumerator storage prerequisite | `504946215`; native controls `958bffd63`; this integration commit | N# admits genuine IEnumerator<T> over an already-admitted source element, preserving exact Current and disposal spelling | Storable protocol state stays separate from collections; exact AQN rejects foreign namesakes; discovery remains C#-owned until gated seed repin | No C# or ratchet change; estate 7,757; fixed corpus 94 images/2,179 tests each equal; compatible controls 103/103 and 48 MethodImpl rows equal; old source-element local declines/new passes1; strict259 unchanged; audit 18/18. Proof (`2026-09-05-s22g0-parity-proof.md`) |
| 023/S2.2(f) — base bindings and method realization | `86c88c496`; controls `453e467b8` / `f6191720f`; this integration commit | Derive consumed base bindings from the existing N# walk and remove ordinary DefineMethod replay from C# | Actual ancestor and open/effective signature remain distinct; captured host name/return/parameters are copied; validation still precedes all attachments after default metadata | Emitter 19,484/18,505 (−1/−1/−38 bytes); combined estate 7,756; declarations 99. Three-arm parity 94 images/2,177 tests each; exact controls 99+3 and 45+5 MethodImpl rows; strict 259 unchanged; audit 18/18, `head-v1:8484731b27e8462d`. Proof (`2026-09-05-s22f-parity-proof.md`) |
| 023/S2.2(e) — iterator declaration members | `588195300`; native controls `d9d4ccf60` / `78082a678`; this integration commit | Eleven C# declaration lookups move to consumed immutable N# bindings; shared owner-qualified signature relation | Open external VAR, machine VAR and factory MVAR remain distinct. Lookup/rebind/retain/attach/body phases and missing-member failures are preserved; incompatible existing-member mutation has an explicit new structural rejection boundary | Emitter 19,485/18,506 (−7/−7); estate 7,742; declarations 97. Three-arm parity 94 images/2,173 tests each, zero differences; exact controls 97+25 and 42+117 MethodImpl rows each; strict 259 unchanged; audit 18/18, `head-v1:69340a92fd35d5ea`. Proof (`2026-09-05-s22e-parity-proof.md`) |
| 023/S2.2(d) — external-interface members | `405483ca4`; controls `4343633df` / `e330dfacc` / `7009a190d`; this integration commit | External enumeration/matching/completeness move to N#; C# matcher and loops deleted; immutable external bindings consumed | Preserve unfiltered GetMethods/first-Methods policy and read order. External VAR/MVAR identity derives actual metadata owners; all bindings validate before attachment. Required/optional modifier lists retain separate order | Emitter 19,492/18,513 (−25/−24); estate 7,736; declarations 93. Three-arm parity 94 images/2,169 tests each, zero differences; exact controls 24/24 and 31 MethodImpl rows each; strict 259 unchanged; audit 18/18, `head-v1:5797117d0e0f694e`. Proof (`2026-09-05-s22d-parity-proof.md`) |
| 023/S2.2(c) — closed source-interface members | `0813c1ac`; controls `417f66e71` / `93ecaf6d`; this integration commit | Closed matching/completeness/substitution and shared generic method rebinding move to N#; four C# helpers deleted, immutable open/context/effective bindings consumed | Preserve TypesEquivalent, first Methods row, original ordinal context and reflection branch. Derive signature once; snapshot source facts and validate every binding before attachment | Emitter 19,517/18,537 (−96/−85); estate 7,726; declarations 89. Three-arm parity 94 images/2,165 tests each, zero differences; exact controls 20/20 and 24 MethodImpl rows each; strict 259 unchanged; audit 18/18, `head-v1:3e5ff5c7c0070ed0`. Proof (`2026-09-05-s22c-parity-proof.md`) |
| 023/S2.2(b) — ordinary source-interface members | `4b592311`; controls `ad303387`; this integration commit | Ordinary source-interface lookup moves to N# and produces an actual declaring/signature binding consumed before override attachment; C# recursive helper deleted | Preserve CLR Type equality, first Methods row and depth-first order. Validate against the consuming table with independent runtime companions before all attachments. Readonly storage correction closes the prior backing-field/array mutation gap | Emitter 19,613/18,622 (−16/−15); estate 7,714; declarations 85; iterators 25. Three-arm parity 94 images/2,159 tests each, zero differences; exact controls 16/16 and 19 MethodImpl rows each; strict 259 unchanged; audit 18/18, `head-v1:8ff51ba796192fbc`. Proof (`2026-09-05-s22b-parity-proof.md`) |
| 023/S2.2(a) — canonical structural resolution | `77686382`; this integration commit | Ordinary/generic-aware canonical resolution and dependency closure move into N#; thirteen C# helpers are deleted. The typeof pool/executor consumes the selected key/runtime pair | Fresh emission tokens, distinct source/synthesized and VAR/MVAR owners, erased-enum provenance and pre-IL validation. Actual pool census 36/12 corrects the earlier name-only count; 35 handle-only consumers remain | Emitter 19,629/18,637 (−1,059/−1,040); estate 7,703; declarations 79; iterators 25. Three-arm parity 94 images/2,155 tests each, zero differences; exact new controls 14/14 each; audit 18/18, `head-v1:0693c72e2dad21ec`. Proof (`2026-09-04-s22a-parity-proof.md`) |
| 023/S2.1(i) — method overrides | `75fa5a77`; this integration commit | All fourteen C# attachments move to consumed ordinary/iterator N# records; two MethodInfo equality domains, final flags, deferred decline and target order are N#-owned | Runtime handles are retained without eager MethodBuilder reflection. Physical MethodImpl table order differs from attachment call order; both are preserved. Fixture typeof/default-argument failures were measured and fixed without the discarded reflection wrapper | Emitter 20,688/19,677 (−26 each); estate 7,676; native declarations 76; both-CLI new controls 10/10. Three-arm parity 94 images/2,145 native tests each, zero differences; audit 18/18, `head-v1:dc4ce2dde3db9257`. Proof (`2026-09-04-s21i-parity-proof.md`) |
| 023/S2.1(h) — P/Invoke declarations | `0cde66a0`, checkpoint `69ad857b` | N# sparse source-ordinal rows own native selection/validity/declines/names/flags/conventions and PreserveSig OR; C# consumes after signature resolution. Shared flag helpers move to existing ColumnarFunctionInput, with one shorter InputBuilder expression | Stage-0 green did not imply strict-source green: an added NL412 exposed the global helper boundary and was removed without duplicating its flag. Parser-only negative probes were rejected; retained N# tests mutate parsed metadata to reach actual return → parameter → native → default failure order | Emitter 20,714/19,703; builder 1,044/975, −9 bytes. Estate 7,666; native declarations 66; both-CLI new controls 7/7. Three-arm parity 94 images/2,138 native tests each, zero differences; audit 18/18, `head-v1:1a04757c9e9e81ad`. Proof (`2026-09-04-s21h-parity-proof.md`) |
| 023/S2.1(g) — custom attributes | `f6ebcf42`, merge `4f2a8e63`, checkpoint `54faf94a` | Four attribute families' selection, blobs and ordered attachment live in N#; four C# SetCustomAttribute calls removed | Corpus commands must use their archived corpus root; all three arms were rerun after rejecting the live-cwd harness. Constructor resolution remains S2.2 | Estate 7,658; 94 images/2,138 native tests per arm, zero differences; audit 18/18. Fresh backend gate 474 s on exact pushed `54faf94a`. Proof (`2026-09-04-s21g-parity-proof.md`) |
| 023/S2.1(f) — shared empty constraint rows | `08c67e468` | The existing generic-constraint planner keeps one static readonly empty array; missing-row reads reuse it, and present rows retain their original identity. One reference-identity contract; zero C# changes | `dc3131202` allocated per missing row where the host used `Array.Empty<string>()`. That generic BCL call declines; a field initialized by a factory emits one `.cctor` allocation plus `ldsfld` reads. The lowercase cache/factory names still emit CLR-public metadata under the existing declaration policy | Focused estate **3/3**; identity probe **0/1 before, 1/1 after**; full estate **7,633/7,633**. Four-arm whole-PE proof **94 assemblies, IL_DIFFS=0**, identical path sets and outcomes; each arm **73/75 projects, 2,117 native tests passed**, only two identical systems-template declines. Root format clean; proof (`2026-09-04-s21f-parity-proof.md`) |
| 023/S2.1(f) — generic constraint guards, proof completed | `dc3131202` (implementation); this commit (evidence) | The two where-site bounds guards moved to the existing N# planner; their historical whole-PE proof is now complete against `51fa6592b` | Both compilers and corpus sources come from committed trees. Repo-shaped CLI snapshots preserve subprocess root discovery; committed Playground/LanguageServer outputs and both refint/runtime halves eliminate missing-dependency exclusions. Copied dependencies are excluded from the emitted-output count | Pristine control and pre/post: **94 assemblies, IL_DIFFS=0, no missing paths or outcome differences**. Historical estate **7,632/7,632**; native tests **2,117/2,117**. One-byte IL mutation changes exactly one assembly and the predicted stdout. Integration gate remains coordinator-owned; proof and reproducible harness (`2026-09-04-s21f-parity-proof.md`) |
| 023/2 S2.0 — what a second executor can execute (measurement + decode; the F2 fork DECIDED) | this commit (branch `stream/023-s2-writer` off `8a144587b`) | ZERO product files across four commits: one decode, `decodes/2026-09-03-writer-second-executor-decode.md`. The method-body key harness REBUILT OUTSIDE the repo (`nl23_bodykey.py`, front-ended by `ilspycmd -il`, **zero C# written**) because the B-arc's comparator survives in NEITHER `nl98keep` NOR `nl97keep` NOR any live worktree, and `nl89-ilcompare.py` is whole-PE and cannot serve an A/B between two writers. Corpus swept three times (baseline, a two-marker and a three-marker scratch mutation, each reverted) | **THERE IS NO DECLARATION PLAN-ROW IR AT ALL, AND THE FORK IS DECIDED BY A STATIC CENSUS RATHER THAN BY THE CORPUS RATE.** `TryEmitColumnarAssembly` walks `ColumnarProgramInput` — a SOURCE-shaped table whose type references are canonical STRINGS — and calls Reflection.Emit imperatively; the 26 N# owners it consults answer QUESTIONS, not rows. **FORK VERDICT (B), the plan-recording sink, coordinator-accepted**, with four binding constraints recorded in §4.4 of the decode: the sink is N#-OWNED beside `ColumnarCodePlan` and emits the SAME schema; the C# change is one field's type plus six locals with its before/after size DECLARED for the ratchet; every recorded body passes `ValidateMethodBodyStack` so maxstack and the region semantics come from ONE place for both producers; and a body the validator refuses is a LOCATED finding, never a silent skip. **THE READING THAT LOOKED LIKE A RESULT AND WAS NOT ONE:** `ALL − DOOR` is not "synthesized bodies" — `Execute` has FOURTEEN FRAGMENT-GRANULAR callers in N# that emit an expression's rows onto the host's own `_il` mid-body, so **the SCHEMA is the discriminator and the call site is not**; the two-marker design answered the wrong question and was rebuilt as three. **F6 RE-DERIVED:** `MethodBodyStreamEncoder`'s two non-`InstructionEncoder` `AddMethodBody` overloads and all six `ExceptionRegionEncoder` members carry no byref and no `Action<T>`, so slice 1's blanket "the body layer is REFUSED" does not describe them — they decline as CATALOG rows — and the writer needs neither: `AddMethodDefinition` takes a plain int `bodyOffset` and the whole `BlobBuilder` byte surface passes and RUNS, so byte-level bodies cost **no catalog row and no republish**. **THE INSTRUMENT CAUGHT ITS OWN MUTATION**: the marker arm's only build difference was `tests/native/ownership-audit` failing, because the growth ratchet refused the one C# line the marker added | Corpus at `8a144587b`: **73 targets, BUILT 66, 67 N#-emitted assemblies, 4,331 keys, 4,320 bodies**. **WHOLLY row-described (schema v4) 774 = 17.9%** (DOOR-claimed 566 = **13.1%**, synthesized 208 = 4.8%); partially row-described 3,243 = 75.1%; **ANY plan row 4,017 = 93.0%**; NONE 303 = 7.0%. The door figure CORROBORATES the inherited 461/3,590 = 12.8%. `DOOR ⊂ WHOLE` True; `DOOR ∩ FRAG` predicted 0, measured **2**, both CONSTRUCTORS whose field initialisers are fragment-planned and whose body is door-claimed — a real property, not an instrument fault. Fork census over `ColumnarIlEmitter.cs`: **1,104 `.Emit` sites** (235 no-operand / 869 operand), **88 distinct opcodes, only 37 operand-carrying**, **8 sink members**, **6 new opcode constants** (`Bge`, `Constrained`, `Ldftn`, `Ldobj`, `Stobj`, `Stind_Ref`), **0 new operand kinds**; plan publishes 109, union **123** (task text says 124 — OPEN, unattributed). **Three harness defects found by measuring, all fixed**: the deny list tested the FLATTENED collected filename so ~11,700 third-party bodies inflated the denominator (14,255 vs 2,531 — the first run's 16.0%/3.5% are VOID); repo-relative `dll:` deps broke in the archive copy, costing 23 of 72 projects (now linked: **49/72 → 66/73 built**, +1,789 bodies incl. the 1,059-body `AnalyzerCleanSource`); and the two-marker design itself. **NEW LANGUAGE DEFECT FILED**: the lone `U` (uint) literal suffix does not emit at any magnitude while `UL`/`L` do — owner `ColumnarScalarLiteralPlanner.TryParseIntegerLiteral`, and the rejection was CONTRACT-PINNED as a fact at `invalidIntegers[4] = "1U"` |
