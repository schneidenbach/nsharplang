...
| EMIT-REGRESSION (015 item 1) — PHASE 1 decode, read-only | no code commit yet; branch `stream/emit-path-regression` off `27f4a1b6b` | Nothing in the product moved. The 132.6 s emit is ROOT-CAUSED BY READING, with the existing 2026-09-01 artifacts as corroboration, to `ColumnarExternalTypeCatalog.TryGet` (`src/NSharpLang.Compiler.BootstrapServices/ColumnarBindingScopeFacts.nl:148`): every cache MISS calls `ExternalAssemblyScan.OpenWithReferences`, which does 27 `Assembly.Load`s, an `AssemblyName.GetAssemblyName` file-open per reference path, a `TryLoadExactRuntimeAssembly` LINEAR SCAN of every loaded assembly per reference path, a FRESH `MetadataLoadContext` over ~190 paths, a `LoadFromAssemblyPath` per entry — and then **disposes all of it**. The cache key is `sourceFileId + ":" + ownerName`, so the SAME external type name in each of 403 files is a separate full scan | **THE JULY NOTE'S NAMED FIRST SUSPECT IS NOT WHERE THE TIME IS, AND THE REGRESSION WINDOW IS TWO DAYS WIDE.** `ColumnarExternalTypeCatalog` was introduced at `6110bbbcf` (2026-07-10, "Own external static members in N#") — **two days after** the 3.4 s measurement at `2e1286d99` (2026-07-08), and `TryGet` has had the scan-per-miss shape since its first line. The per-member scratch arrays in `ColumnarProgramInputBuilder` (`cap = n + 1` at 9 sites, `(n+1)*4` at one) are real but bounded above by the whole front end: `nlc build` parses+lints the same 172,653 lines in 7.9 s, so the input builder can hold at most ~6 % of 132.6 s. **The SDK path is the only path that shows this**, because `EmitIlAssembly` copies MSBuild's `@(ReferencePath)` into `config.Dependencies` — 167 net10.0 ref-pack assemblies plus project/package refs — while `nlc build` on a corpus project passes only the handful of `project.yml` DLLs. That is why §2.2's corpus is silent and §2.1's large project is 100× worse | Corroboration from `artifacts/compile-time/2026-09-01/sdk-emit-only/rebuild-3.log`, never read before: **120.43 s user / 15.93 s sys / 133.26 s real** (single-threaded, CPU-bound) with **2,114,264 page reclaims** against a peak RSS of only 611 MB — i.e. ~34 GB of 16 KB pages touched and thrown away, the exact signature of hundreds of `MetadataLoadContext` create/map/dispose cycles, not of one big heap. 2.18e12 instructions retired. 27 `CommonAssemblyNames`, 167 ref-pack DLLs at `Microsoft.NETCore.App.Ref/10.0.5`, 6 `TryGet` call sites, 3 `OpenWithReferences` call sites (1 per-miss, 1 per-Prepare, 1 per-extension-index) |

**Durable findings (EMIT-REGRESSION phase 1) — hypothesis-grade until the instrument runs.**

- **The suspect is named and it is one function.** `ColumnarExternalTypeCatalog.TryGet` opens and disposes a whole `ExternalAssemblyScanResult` per cache miss. `Prepare` already opens one ("so every exact runtime implementation is admitted up front") and **immediately disposes it**, so the retained-scan fix is a two-line change of lifetime, not new machinery.
- **The cache key is per-file and the resolution is not.** `Key(sourceFileId, ownerName)` — but `ResolveOwner` reads only `facts.UnaliasedNamespaceImports`, so files sharing an import set share an answer. Keying by the import set instead of the file id collapses ~403 identical questions into one, and is the secondary fix.
- **`OpenWithReferences` is quadratic inside itself, twice.** `TryLoadExactRuntimeAssembly` is called once per reference path and linearly scans every loaded assembly, calling `Assembly.GetName().FullName` on each — ~175 × ~100+ = ~18,000 AssemblyName constructions per scan; and its `Assembly.LoadFrom(path)` fallback **grows the loaded-assembly list**, so every later scan's inner loop is longer. A `Dictionary<string, Assembly>` built once per scan removes it; that fix is worth taking even if scan reuse lands, as defence.
- **The regression window is `2e1286d99` (2026-07-08, 3.4 s) → `6110bbbcf` (2026-07-10).** 520 commits separate July 8 from tip, but only one introduces a per-site `MetadataLoadContext`. Independent soft corroboration at `2d2ddb39d` (2026-08-05): the Build.Tasks step including the self-emit was already "~6m20s serially on a quiet machine" — i.e. the regression predates the whole 015-B door arc (2026-08-27→) and is not the door's per-body claim/rollback.
- **The corpus could never have caught this and still cannot.** The cost is per (file × distinct external name × reference-set size); the 41 corpus projects are startup-bound at ~250 ms with a handful of references. The compile-time gate spawns `nlc build`, which stops at strict lint on BootstrapServices, so **nothing in the gate observes the emit path at all** — item 1 has no regression fence, which is a finding independent of the fix.
- **RSS is a decoy.** Peak RSS is 611 MB and healthy; the July fix that took 10.3 GB → 533 MB is intact. This regression is invisible in RSS and visible only in `sys` time and page-reclaim count.
- **Ruled out by reading:** `MetadataLoadContext` is analyzer-only outside this one path (`Analyzer.cs`); `ColumnarDeclineTrace` records at only 50 hard-decline sites and BootstrapServices emits exit 0; `MergeSourceFilesAtProjectRoot` is linear; the plan pools grow by `+1`, not worst-case; the packaged compiler is **Release** (Debuggable blob `01 00 02 00 00 00 00 00` in Compiler.dll / Runtime.dll / Build.Tasks.dll) and the N#-emitted BootstrapServices.dll carries no `DebuggableAttribute` at all, so "minopts on the emit path" is dead.
- **The instrument needs no product change and no C# growth.** A throwaway console app OUTSIDE the repo references the built `Compiler.dll` + `NSharpLang.Compiler.BootstrapServices.dll`, replicates `EmitIlAssembly.Execute` (`ProjectFileParser.Parse` + 167 ref-pack paths as `Reference { Dll = … }` + `MultiFileCompiler.CompileToIlAssembly(validateWithLegacyAnalysis: false)`), and reads the two numbers that close the case by reflection on internals: **`resolvedOwners.Count` after the emit = the number of scans**, and one direct timed call to `ExternalAssemblyScan.OpenWithReferences` = the cost per scan. Their product against 132.6 s is the whole attribution. `dotnet-stack report` in a loop over the `nsharp-columnar-emit` thread is the independent second opinion (`dotnet-trace` is not installed; `dotnet-stack` is).
- **Rules cost of the fix is zero.** Both owners are `.nl` (`ColumnarBindingScopeFacts.nl`, `ExternalAssemblyScan.nl`); the growth ratchet carries **no `.nl` rows**, so no repin is owed and no C# line moves. `ColumnarIlEmitter.cs` need not be touched.
- **Acceptance is a number AND byte identity.** Speed work must not change output: BootstrapServices assembly md5 before/after plus the corpus `IL_DIFFS=0` harness. The one real behavioural risk is that a retained scan sees a different `AppDomain` assembly list than a later fresh one would — mitigated because `Prepare`'s scan already performs every `Assembly.LoadFrom` side effect before the first `TryGet`, so a retained scan and a later fresh scan should produce identical entries. That is an argument, not a proof, and the md5 is the proof.
