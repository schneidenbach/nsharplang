...
- Internal-only: COTM is a large validation corpus, not public proof and not demo material.
...
- COTM as an internal validation target.

REJECT:

- Raw COTM screenshots/files/configs/logs in public material.
- “COTM successfully migrated” or “real app migrated” as a public proof point based on inventory alone.

DEFER:

- Public COTM story until there is a redacted subset, security approval, and fresh `nlc check`/build/test/idiom evidence.
...
Codex initial position: full `test-all` timed out, full VS Code integration is not green, screenshots are absent, COTM is inventory-only, package publishing is unverified.
...
### [CRITICAL] Raw COTM is not public-demo safe

Aspect: security

Codex initial position: COTM has sensitive-name/content signals and explicit “do not show raw COTM” guidance.

Debate result: DEFENDED as critical.

Decision: keep COTM internal until redacted, approved, and validated.

### [WARNING] COTM inventory is not migration proof

Aspect: correctness

Decision: defer public COTM migration success claims until check/build/test/idiom evidence exists.
...
7. Say explicitly: “The full end-to-end suite and COTM public demo are pre-talk gates, not proof points today.”
...
6. Produce redacted COTM subset plus fresh validation logs.
