...
`OutputFormatterDiagnosticKernels`, and `nlc query symbols --text`, `outline --text`, `definition --text`, and
`references --text` line shaping routes through `OutputFormatterTextKernels`. The active path is Stage 6 surface shrink plus SoA/emitter-port proof work before
