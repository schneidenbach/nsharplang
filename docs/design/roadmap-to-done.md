...
- [~] **Stage 6 — delete C#.** Remove the C# binder/analyzer/codegen paths the columnar pipeline replaces;
      ...
      beside `OutputFormatter` in `OutputFormatterDiagnosticKernels`; output-format inspect-summary
      and diagnostic-cluster reference-file summaries now live beside `OutputFormatter` in
      `OutputFormatterReferenceFileKernels`; DocQuery type/reference-pack de-duplication, best-type
      selection, and member ordering now live beside `DocQuery` in `DocQueryKernels`. Product parser
