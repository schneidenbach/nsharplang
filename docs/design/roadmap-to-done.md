...
- [x] **Stage 5 — end-to-end route.** Compile the dogfood corpus through the standalone columnar pipeline with no
      ...
      **PRODUCTION ROUTING LANDED (2026-06-08; default-on 2026-06-13):** the columnar backend is wired into `MultiFileCompiler.
      ...
      at least 454 methods. Single-file eligibility is no longer the product-routing metric: parser and semantic
...
- [~] **Stage 6 — delete C#.** Remove the C# binder/analyzer/codegen paths the columnar pipeline replaces;
      ...
      **Current directive (2026-06-18):** Stage 6 is about compiler/tooling ownership, not broadening the C#
      ...
      `CompilerErrorSeverityFilter`, lint diagnostic severity text rendering now lives beside `LintCommand` in
      `LintCommandKernels`, and diagnostic title/detail/summary/header/source-snippet text rendering now lives beside `OutputFormatter` in
      `OutputFormatterDiagnosticKernels`; batch query duplicate-id validation and packed
      ...
      construction now lives beside `UnifiedDiff` in `UnifiedDiffHunkRangeBuilder`, and unified-diff
      header/hunk/prefix text shaping now lives beside `UnifiedDiff` in `UnifiedDiffTextKernels`; fix safety/skipped/applied
      grouping routes now live beside `FixCommand` in `FixCommandKernels`; clean option summary, help/status/error text, and artifact-line rendering now live beside
      ...
      generation and generated location/signature/parameter text now live beside `DocCommand` in `DocCommandKernels`; tree option
      ...
      and effective output-mode selection now live beside `FixCommand` in `FixCommandArgumentKernels`, and
      help/status/error/applied/skipped text shaping now lives beside `FixCommand` in `FixCommandKernels`; add argument summary, message shaping, dependency-add planning, and duplicate checks now live beside
      ...
      `UpdateCommand` in `UpdateCommandKernels`; new argument summary, message shaping, template normalization, effective-template selection, and
      template source manifests now live beside `Program.NewCommand` in
      `NewCommandKernels`; tidy
      option summary, output-mode selection, import extraction, classification, status summary, fix filtering,
      text table rendering, and help/status/error message shaping now live beside `TidyCommand`
      ...
      project/single-file start text, success/output text, failed-elapsed text, and timing-breakdown text now live beside `Program.BuildCommand`
      in `BuildCommandKernels`; build/run define extraction now lives beside `Program` in
      `DefineArgumentKernels`; top-level command routing, help/version/error text shaping, and shared Program/export/tidy/query/pack stderr line wrapping now live beside `Program` in
...
content now routes through `RestoreCommandKernels`; `nlc export csharp` generated `.csproj` XML content now
routes through `ExportCommandKernels`; `nlc pack` generated `.nuspec` XML content now routes through
`PackCommandKernels`. The active path is Stage 6 surface shrink plus SoA/emitter-port proof work before
