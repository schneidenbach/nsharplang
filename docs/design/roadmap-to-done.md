...
- [~] **Stage 6 — delete C#.** Remove the C# binder/analyzer/codegen paths the columnar pipeline replaces;
      ...
      **Current directive (2026-06-18):** Stage 6 is about compiler/tooling ownership, not broadening the C#
      ...
      `CleanCommand` in `CleanArtifactDirectoryOrderer`; env option summary and output-mode selection now live
      beside `EnvCommand` in `EnvCommandKernels`; doctor option summary and output-mode selection now live beside
      `DoctorCommand` in `DoctorCommandKernels`; audit option summary and output-mode selection now live beside
      `AuditCommand` in `AuditCommandKernels`; init option summary now lives beside `InitCommand` in
      ...
      and effective output-mode selection now live beside `FixCommand` in `FixCommandArgumentKernels`; add argument summary, dependency-add planning, and duplicate checks now live beside
      ...
      option summary, import extraction, classification, status summary, and fix filtering now live beside `TidyCommand`
      ...
      option summary, directory traversal pruning, test-file exclusion, and discovered-path filtering now live beside
      ...
      call sites in `CompilationBackendSelectionKernels`; pack option summary and package-version source selection
      now live beside `PackCommand` in `PackCommandKernels`; build option/help and operand summaries now live beside `Program.BuildCommand`
      ...
      `ProgramCommandKernels`; test option/help, timeout parsing, outcome summaries, and
      ...
      forwarded-argument selection, and changed-file trigger filtering now live beside `WatchCommand` in `WatchCommandKernels`; daemon option/subcommand summary now lives beside
