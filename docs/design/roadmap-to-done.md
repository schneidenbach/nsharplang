...
- [~] **Stage 6 — delete C#.** Remove the C# binder/analyzer/codegen paths the columnar pipeline replaces;
      ...
      **Current directive (2026-06-18):** Stage 6 is about compiler/tooling ownership, not broadening the C#
      ...
      option summary, validation/unsupported/failure message shaping, and help routing now live beside `Program.PublishCommand` in
      `PublishCommandKernels`; shared build/run/check/test/publish backend selection now lives beside the CLI backend
      call sites in `CompilationBackendSelectionKernels`; pack option summary, output-mode selection, and package-version source selection
