...
- [~] **Stage 6 — delete C#.** Remove the C# binder/analyzer/codegen paths the columnar pipeline replaces;
      ...
      wrappers, property accessor wrappers, enum/interface/struct/union declaration-only shims, and
      the flattened interface-signature shim now extracted to the parity corpus when no product
      caller remains. The
