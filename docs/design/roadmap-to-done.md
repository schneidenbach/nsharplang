...
- [~] **Stage 6 — delete C#.** Remove the C# binder/analyzer/codegen paths the columnar pipeline replaces;
      ...
      **Current directive (2026-06-18):** Stage 6 is about compiler/tooling ownership, not broadening the C#
      ...
      adapter, and product `QueryCommand` top-level/global, command-specific argument, and `line:column`
      position parsing now lives beside `QueryCommand` in
      `QueryCommandKernels`;
      ...
      `ProgramCommandKernels`; test option/help, timeout parsing, and outcome summaries now live beside `Program.TestCommand` in
      `TestCommandKernels`; watch target routing, option summary, numeric option parsing, and
