...
- [~] **Stage 5 — end-to-end route.** Compile the dogfood corpus through the standalone columnar pipeline with no
      ...
      **PRODUCTION ROUTING LANDED (2026-06-08):** the columnar backend is wired into `MultiFileCompiler.
      ...
      proven to differ from the C# IL yet run identically (`Stage5_ColumnarBackend_*`). Corpus coverage **22/32
      (~69%) SINGLE-FILE, 25/32 (~78%) via MULTI-FILE merge** (2026-06-08: SourceTextLines via Array.Fill +
      ...
      unsigned scalar + `BitOperations.PopCount`; floor corrected 16→20 then →22).
      **MULTI-FILE merge LANDED** (`TryEmitColumnarProgramMultiFile`): the dogfood program is multi-file
      ...
      production path (currently single-file) + more feature coverage (StringBuilder + StringComparison enum gate
      CliArguments, CompletionReceivers, DiagnosticClusters; LexerTokenKindScanner needs lowercase
      `char.IsLetter`/`IsDigit`; the "0-feature" emit-decliners DiagnosticDeduplication + IdentifierSpans need
