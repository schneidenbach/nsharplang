...
`CliCompletionOptionSummaryInto` preserves the current `nlc completion` command-parser behavior for
help and shell classification. Stage 6 routes `CompletionCommand` through
`CompletionCommandKernels`; shell script materialization stays in C# as the output boundary, and the
previous parser shape remains as fallback/oracle logic.
...
`CliAddArgumentSummaryInto` preserves the current `nlc add` command-parser behavior for help,
`--version`, `--path`, package operand discovery, `--framework`, and `--prerelease`. Stage 6 routes
`AddCommand` through `AddCommandKernels` as a C# surface-shrink product path beside the package
operand compatibility route. The kernel writes option value indices and switch bits into a
caller-owned buffer, while the previous C# parser shape remains as fallback/oracle logic.

`CliRemoveArgumentSummaryInto` preserves the current `nlc remove` command-parser behavior for help
and package operand discovery. Stage 6 routes `RemoveCommand` through `RemoveCommandKernels` as a
C# surface-shrink product path beside the package operand compatibility route. The kernel writes the
package index and help bit into a caller-owned buffer, while the previous C# parser shape remains as
fallback/oracle logic.

`CliUpdateArgumentSummaryInto` preserves the current `nlc update` command-parser behavior for help,
`--dry-run`, and target package discovery. Stage 6 routes `UpdateCommand` through
`UpdateCommandKernels` as a C# surface-shrink product path beside the target-package compatibility
route and the existing dependency-filter kernels. The kernel writes the target package index and
...
`CliCleanOptionSummaryInto` preserves the current `nlc clean` command-parser behavior for help,
`--project`, and `--all`. Stage 6 routes `CleanCommand` through `CleanCommandKernels` as a C#
...
`CliDoctorOptionSummaryInto` preserves the current `nlc doctor` command-parser behavior for help,
`--json`, `--require-vscode`, and `--skip-vscode`. Stage 6 routes `DoctorCommand` through
`DoctorCommandKernels` as a C# surface-shrink product path beside the command implementation. The
kernel writes switch bits into a caller-owned buffer, while the previous C# parser shape remains as
fallback/oracle logic.
`CliAuditOptionSummaryInto` preserves the current `nlc audit` command-parser behavior for help,
`--project`, and `--json`. Stage 6 routes `AuditCommand` through `AuditCommandKernels` as a C#
surface-shrink product path beside the command implementation. The kernel writes the project value
index and switch bits into a caller-owned buffer, while the previous C# parser shape remains as
fallback/oracle logic.
`CliInitOptionSummaryInto` preserves the current `nlc init` command-parser behavior for help,
`--name`, `--type`, and `--force`. Stage 6 routes `InitCommand` through `InitCommandKernels` as a
C# surface-shrink product path beside the command implementation. The kernel writes option value
indices and switch bits into a caller-owned buffer, while the previous C# parser shape remains as
fallback/oracle logic.
`CliRestoreOptionSummaryInto` preserves the current `nlc restore` command-parser behavior for
`--help` and `-h`, including bare `help` remaining non-help. Stage 6 routes `RestoreCommand`
through `RestoreCommandKernels` as a C# surface-shrink product path beside the existing restore
reference kernels. The kernel writes the help bit into a caller-owned buffer, while the previous C#
parser shape remains as fallback/oracle logic.
...
selection, applied-fix file grouping, `nlc clean` option parsing and artifact directory ordering, `nlc env`, `nlc doctor`, `nlc audit`, `nlc init`, and `nlc restore` option parsing, `nlc export csharp` option parsing and reference de-duplication,
CLI reference-type filtering,
