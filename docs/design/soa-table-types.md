...
bool read expressions and comparisons, string concatenation, bool logical operators,
statement-context and expression-valued simple stores, compound assignment, prefix/postfix update
operands, expression-valued default stores without old-element reads, plus nullable-column `??` and
`??=` element operations.
