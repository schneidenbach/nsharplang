...
6. In progress: port symbol/type/diagnostic tables. `SemanticScopes.nl` now wraps source-position,
   ...
   signature, constructor signature/chain, property accessor, statement parsing, local-function discovery,
   type-reference canonicalization, enum/interface/struct/union declaration span/text wrappers, and
   the flattened interface-signature wrapper are no longer shipped product dogfood files; they live in
   the parity corpus because no product route calls them after the wrapper-aware cores took ownership.
