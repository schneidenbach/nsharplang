...
return, expression-bodied function/local function, call argument, field, expression-bodied property,
object initializer member, `with` expression member, assignment target, array literal element, array
initializer element, collection literal element, tuple literal element, or typed ternary/match result
arm.
...
- row type annotations: "SoA row type 'NodeTable.Row' is not part of this lowering" for parameter,
  return, local, or other declared-type positions;
...
or copying elements; `add`, `ensureCapacity`, and `clear` keep the same column-array shape without row
