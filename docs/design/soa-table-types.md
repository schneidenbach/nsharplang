...
materialization, array slicing, boxing, or dispatch. That wrapper rule includes scalar read
expressions and comparisons, statement-context and expression-valued simple stores, compound
assignment, prefix/postfix update operands, expression-valued default stores without old-element
reads, plus nullable-column `??` and `??=` element operations.
