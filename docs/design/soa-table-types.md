...
Char columns support equality/inequality and relational comparisons. Int-backed enum columns support
the enum language's comparison expressions, prefix/postfix increment and decrement forms, same-enum
bitwise expressions, and unary bitwise-not.
...
same-enum comparison expressions, bitwise and unary bitwise-not expression stores, plus prefix/postfix
update forms, integral `uint`/`long`/`char` update forms, and null-coalescing reads/assignments.
