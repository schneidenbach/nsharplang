...
`table.column.GetLowerBound(...)`, and
`table.column.GetUpperBound(...)`. Use direct element indexing, metadata
...
simple stores, default stores without old-element reads, numeric compound/update operands, and
...
with a call-site IL-shape pin proving no caller-side row allocation, boxing, delegate construction,
array allocation, or virtual dispatch. Alias-generated `new`, `wrap`, `add`, `ensureCapacity`, and
`copyRow` dynamic guard and bounds failures preserve the underlying table name in runtime messages.
The analyzer rejects materializing those generated operations, including `wrap` and
...
6. In progress: port symbol/type/diagnostic tables. `SemanticScopes.nl` now wraps source-position,
   ...
   declaration accessor core directly, and its getter/setter body-node cores now compose the
   ...
   constructor signature and declaration chain cores directly, and its body-node core now composes the
   statement parser core directly.
   ...
   fieldless value structs, generic base lists, generic static fields, and generic member/type-parameter
   name collisions. The lower flattened function
