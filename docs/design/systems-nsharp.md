...
| 35 | Build an async file reader feeding a hot parser. | `[boundary]` async IO, `ValueTask<T>` where API requires it, hot span parser. | Executable proof covers async boundary IO feeding a hot parser with a primitive `Task<int>` handoff; `ValueTask<Result<T,E>>` remains a compiler/analyzer gap and `[hot] async` is deferred. | [Project](systems-samples/proofs/35-async-file-hot-parser/) |
...
| 38 | Sort unmanaged records with a comparer. | generic parametric summaries, constrained value-type dispatch, no boxing. | Executable proof covers current `struct` constrained sortable records, no boxing, and constrained dispatch; exact `unmanaged` keyword plus separate comparer inference remains polish. | [Project](systems-samples/proofs/38-unmanaged-sort-comparer/) |
| 39 | Run a hot-compatible LINQ-style pipeline. | hot-LINQ contract, library HotSummary, closure/delegate facts, hot-readiness. | Executable proof covers a current N# extension-method pipeline over `ReadOnlySpan<int>` with no delegate, closure, boxing, dispatch, or hot-readiness sites; package-level ZLinq qualification remains contract work, not a language claim. | [Project](systems-samples/proofs/39-hot-linq-pipeline/) |
...
| 42 | Keep public APIs AOT/trimming friendly. | `aotSafe(target)`, `trimSafe`, source-generator guidance, boundary reports. | Executable analysis proof; native image later. | [Project](systems-samples/proofs/42-aot-friendly-public-api/) |
