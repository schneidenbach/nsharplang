...
Backend caveat (2026-06-10) — RESOLVED (2026-06-19, decline option): the value-struct lowering exists only in the C# ILCompiler (`IsValueStructEmittable` → `DeclareValueStructUnion`). Previously the columnar pipeline did NOT decline these unions — it accepted payload-free unions (D-10b zero-field cases) and emitted every union as a class hierarchy, so a columnar-routed build silently swapped the allocation-free public tag struct for heap case objects, and the behavioral parity oracle could not catch the layout difference. The columnar input builder (`ColumnarProgramInputBuilder.TryGetColumnarUnionInputs`) now DECLINES any value-struct-emittable union (small, closed, payload-free, non-generic) to the C# oracle, which emits the correct public value struct (pinned by `ILCompiler_PayloadFreeUnion_IsEmittedAsValueStruct`; columnar's decline is pinned by `ColumnarCodegen_Declines_ValueStructEmittableUnion`). The eligibility decision is owned by N# (`ColumnarUnionIsValueStructEmittable` in `ParserColumnarUnions.nl`, mirroring `UnionValueLayout.IsValueStructEmittable`). FUTURE: port the value-struct layout itself into `ColumnarIlEmitter` (tag-struct declaration + tag construction + tag-compare match + value adoption) so columnar OWNS the form and the decline is removed.
