...
Backend caveat (2026-06-10): the value-struct lowering exists only in the C# ILCompiler (`IsValueStructEmittable` → `DeclareValueStructUnion`). The columnar pipeline does NOT decline these unions — it accepts payload-free unions (D-10b zero-field cases) and emits every union as a class hierarchy, so a columnar-routed build silently swaps the allocation-free public tag struct for heap case objects today, and the behavioral parity oracle cannot catch the layout difference. Before Stage 6, either port the `UnionValueLayout` gating to `ColumnarIlEmitter` or make columnar decline value-struct-emittable unions.
