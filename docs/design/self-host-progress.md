...
`--diff`, and `--stdin` from `FormatCommandKernels`, which binds the shipped
`CliFormatOptionSummaryInto` dogfood kernel beside the existing format discovered-path route. The
previous direct scans remain only as fallback/oracle logic, preserving permissive `--project` value
handling and the shared positional-file collection route. This is a Stage 6 `C#-surface-shrink`
product-route slice.
...
`UpdateCommandKernels`, which binds the shipped `CliUpdateArgumentSummaryInto` dogfood kernel beside
the target-package compatibility route. The previous direct C# scans remain only as fallback/oracle
logic, preserving the command's first non-option package behavior. This is a Stage 6
`C#-surface-shrink` product-route slice.

Focused evidence:
`./scripts/dev.sh UpdateCommandKernels`;
...
too, so `src/NSharpLang.Cli` no longer carries local dogfood program-type loading or binding
exception fallback. Doc/export/fix/restore/tidy/batch/query/tree/update wrappers still own their
typed delegates, scratch buffers, result validation, and C# fallback semantics; only the host
boundary scaffolding is centralized in the compiler loader. Focused evidence:
`dotnet build src/NSharpLang.Cli/Cli.csproj --no-restore`; `dotnet test tests/Tests.csproj --no-restore --filter "FullyQualifiedName~DocCommandKernels_OrdersSymbolsForGeneration|FullyQualifiedName~DocCommandKernels_OrdersMembersForGeneration|FullyQualifiedName~DocCommandKernels_CreatesSlugs|FullyQualifiedName~ExportCommandKernels_SelectsInputOperandAfterOrderedOptionStripping|FullyQualifiedName~ExportCommandKernels_FiltersReferenceValuesByType|FullyQualifiedName~ExportCommandKernels_DeduplicatesReferenceValues|FullyQualifiedName~FixCommandKernels_FiltersFixesBySafety|FullyQualifiedName~FixCommandKernels_SelectsSkippedFixEntries|FullyQualifiedName~FixCommandKernels_GroupsAppliedFixEntriesByFile|FullyQualifiedName~QuerySymbolNameFilter_FiltersSymbolsByNamePattern|FullyQualifiedName~RestoreCommandKernels_DeduplicatesProjectReferences|FullyQualifiedName~RestoreCommandKernels_FiltersProjectReferences|FullyQualifiedName~TidyCommandKernels_SelectsAndClassifiesDependencies|FullyQualifiedName~TidyCommandKernels_FiltersRemovalLines|FullyQualifiedName~TreeDependencyDeduplicator_DeduplicatesAndOrdersDependencies|FullyQualifiedName~TreeDependencyDeduplicator_DeduplicatesTargetFrameworks|FullyQualifiedName~UpdateDependencyFilter_FiltersTargetNuGetDependencies|FullyQualifiedName~BatchQueryKernels_"`.
...
multi-delegate CLI `LoadBindings` sites remained owner-local until the follow-up slice above. Focused
...
receiver is hard-cast through table and alias types. `((NodeTable)nodes)[row].kind` and nested alias
casts load and store the generated backing arrays directly, with no row allocation, boxing, delegate
construction, heap array allocation, calls, or virtual dispatch. Focused evidence:
`dotnet test tests/Tests.csproj --no-restore --filter "FullyQualifiedName~HardCastedRowProjection_UsesColumnArrayLoadStoreWithoutRowAllocation"`.
...
hard-cast table/alias receivers. `((NodeTable)nodes).add`, `((Nodes)((NodeTable)nodes)).clear`,
`((NodeTable)nodes).ensureCapacity`, `((Nodes)((NodeTable)nodes)).copyRow`, and call arguments such
as `pull(((NodeTable)nodes).add)` now keep the SoA-specific diagnostic and render actionable call
suggestions like `nodes.add()` instead of placeholder receiver text. Focused evidence:
...
original table storage instead of spilling the cast wrapper into a temporary. `ensureCapacity`, `add`,
`copyRow`, and `clear` therefore mutate the caller-visible `length`, `capacity`, and backing columns
when invoked through `((NodeTable)nodes)` or nested alias casts. Focused evidence:
`dotnet test tests/Tests.csproj --no-restore --filter "FullyQualifiedName~HardCastedAliasedSoaGeneratedOperations_MutateOriginalTableShape|FullyQualifiedName~HardCastedAliasedSoaTableReceivers_UseGeneratedTableShape|FullyQualifiedName~AliasedSoaTableGeneratedOperations_UseUnderlyingTableShape"`.
...
named `sourceArray`, not only the destination. Both `Array.Copy(length: n, sourceArray: table.column,
destinationArray: target)` and the explicit `System.Array.Copy(..., sourceArray: table.column)` offset
overload load generated backing column fields directly, call the BCL copy helper, and allocate no rows,
arrays, delegates, boxes, or virtual dispatch in the hot function.
Focused evidence: `dotnet test tests/Tests.csproj --no-restore --filter
"FullyQualifiedName~DirectColumnBulkArrayOperationSourceColumnNamedArguments_UseBackingArraysWithoutRowAllocation"`.
...
pinned direct-column IL surface. The guard covers element access methods such as
`table.column.GetValue(...)`/`SetValue(...)`, allocation/dispatch surfaces such as
`table.column.Clone`, `table.column.CopyTo`, `table.column.GetEnumerator()`/`ToString()`, and dimension
method calls such as `table.column.GetLength(...)`. The accepted whole-column surface remains the static
`Array.Fill`/`Array.Copy`/`Array.Clear` kernels, while accepted metadata uses properties:
`table.column.Length`/`LongLength` lower through `ldlen` and `table.column.Rank` lowers to the known
SZ-array rank constant.
Focused evidence: `dotnet test tests/Tests.csproj --no-restore --filter "FullyQualifiedName~SoaRecordTests.Analyzer_SoaDirectColumnsCannotUseArrayInstanceMethods|FullyQualifiedName~SoaRecordTests.Analyzer_SoaDirectColumnArrayInstanceMethodsCannotBeUsedAsValues|FullyQualifiedName~SoaRecordILShapeTests.DirectColumnMetadataProperties_UseBackingArrayLengthWithoutDispatch"`;
`./scripts/dev.sh SoaDirectColumn`.
...
construct delegates, allocate arrays, or dispatch virtually in the hot function. The pin now covers
whole-array fill/copy/clear plus ranged fill, offset copy, and one-argument clear overloads. This
distinguishes ordinary value-preserving column mutation from the row-identity-breaking
`Array.Sort`/`Array.Reverse` cases that the analyzer rejects before emission.
Focused evidence: `dotnet test tests/Tests.csproj --no-restore --filter "FullyQualifiedName~SoaRecordILShapeTests.DirectColumnBulkArrayOperationOverloads_UseBackingArraysWithoutRowAllocation"`;
`./scripts/dev.sh DirectColumnBulkArrayOperation`.
...
names, and duplicate parameter names within an interface method before returning rows to the C# transition
boundary. The interface emitter no longer carries those duplicate-base/member/parameter fallback guards;
cross-type interface name collisions and base-interface resolution/self validation remain semantic
registry checks.
...
span scratch columns, member/index output columns, text output columns, and result slots behind named
N# table structs in `ParseColumnarStructInfoCore`. The public `ParseColumnarStructInfoInto` ABI stays
flattened for the current adapter and parity tests, while the product struct/class/record parser
wrapper no longer allocates and passes anonymous scratch arrays internally. Focused evidence:
`./scripts/dev.sh Parser_TopLevelDeclarationKinds_MatchProductionParser`,
`./scripts/dev.sh ColumnarCodegen_Parity_StructFieldsAndObjectInit`,
`./scripts/dev.sh ColumnarCodegen_Parity_ClassObjectInitAndMethods`,
`./scripts/dev.sh ColumnarCodegen_Parity_RecordFieldsAndObjectInit`,
`./scripts/dev.sh ColumnarCodegen_Parity_GenericClass_CtorFieldMethodProperty`,
`./scripts/dev.sh ColumnarCodegen_CompilesRealDogfoodCorpus_Coverage`, and
`./scripts/dev.sh ColumnarCodegen_MultiFile_EligibleClusterCompiles`.
...
struct/class/record parser wrapper. It keeps field/type/init/type-parameter/base span columns as N#
scratch data while exposing only declaration text, field text, static/init flags, member indices,
type-parameter text, and base-name text to the C# transition materializer.

`NSharpCompilerDogfoodAdapter.TryGetColumnarStructInputs` no longer binds
`ParseStructDeclarationInfoInto` or allocates the declaration span columns in C#. The span and
text-info ABIs remain emitted for direct parser parity tests and N# composition, while production
consumes the reduced rowset. Focused evidence:
`./scripts/dev.sh Parser_TopLevelDeclarationKinds_MatchProductionParser`,
`./scripts/dev.sh ColumnarCodegen_Parity_Struct`,
...
default-method block spans through `ParserDeclarationTokenTable` instead of unpacking raw token
arrays. Enum, struct, constructor-chain, and union declaration bodies remain as separate parser
cleanup slices. Focused evidence: `./scripts/dev.sh ColumnarCodegen_MultiFile_EligibleClusterCompiles`.
...
`ParserTokenTable` instead of unpacking raw token arrays. The ternary, assignment, and lambda
expression cores remain as separate parser cleanup slices. Focused evidence:
`./scripts/dev.sh Parser_Expression_MatchesProductionParser` and
`./scripts/dev.sh ColumnarCodegen_MultiFile_RealParserCluster`.

## 2026-06-15 — Unary expression parser keeps token tables shaped

`ParseUnaryExpressionNode` now reads `must`, `await`, and prefix-operator token columns through
`ParserTokenTable` instead of unpacking raw token arrays. The binary, ternary, assignment, and lambda
expression cores remain as separate parser cleanup slices. Focused evidence:
`./scripts/dev.sh Parser_Expression_MatchesProductionParser` and
`./scripts/dev.sh ColumnarCodegen_MultiFile_RealParserCluster`.
...
token columns through `ParserTokenTable` and keeps generic type arguments, call arguments, and
`with` initializer child ids behind `ParserArgumentStack`. The call-argument, unary, binary,
ternary, assignment, and lambda expression cores remain as separate parser cleanup slices. Focused
...
diagnostic instead of reaching a missing indexer setter in the value-type initializer emitter; the
same guard follows table aliases and target-typed `new(capacity)` construction. AST-level
collection/indexer shapes are guarded for both object initializers and `with` expressions before
they can fall into emitter-only failures, including when the table target is alias-typed.
...
members now have IL-shape evidence for row, literal from-end, and variable-held `System.Index`
forms. `(nodes.kind)[row] = -(nodes.kind)[row]`, `(nodes.flags)[^1] = ~(nodes.flags)[^1]`, and
`idx := ^1; (nodes.mask)[idx] = ~(nodes.mask)[idx]` stay on backing column arrays, preserve
expression-valued assignment results, and lower through direct `neg`/`not` opcodes with no row
...
`((nodes.text)[idx]) ?? fallback` / `((nodes.text)[idx]) ??= value` preserve assignment,
null-coalescing read/assignment, and prefix/postfix result semantics through offset-based
backing-array load/store lowering, with no slice allocation, boxing, delegate construction, or
virtual dispatch. Focused evidence: `dotnet test
tests/Tests.csproj --no-restore --filter
"FullyQualifiedName~ILCompiler_SoaRecordParenthesizedVariableFromEndDirectColumn"` (3 tests).
...
across row projection, direct column indexing, and direct from-end column indexing. The proof pins
direct column-array loads/stores plus `shl`, signed `shr`, and unsigned `shr.un`, with no row
construction, slice allocation, boxing, delegate construction, or virtual dispatch.
...
direct column indexing, and direct from-end column indexing. Comparisons read backing column arrays
directly and materialize boolean results with comparison opcodes, with no row construction, slice
allocation, boxing, delegate construction, or virtual dispatch.
...
projection, direct column indexing, and direct from-end column indexing. The proof covers null
backing slots concatenating through `string.Concat(string, string)` and storing back to the nullable
column, with no row construction, slice allocation, boxing, delegate construction, or virtual
dispatch.
...
across row projection, direct column indexing, and direct from-end column indexing. Comparisons read
the backing column arrays directly and materialize boolean results with comparison opcodes, with no
row construction, slice allocation, boxing, delegate construction, or virtual dispatch.
...
expression-valued stores across row projection, direct column indexing, and direct from-end column
indexing. `|`, `^`, and `&` stay on backing-column array traffic with no row construction, slice
allocation, boxing, delegate construction, or virtual dispatch.
...
returns, expression-bodied functions/local functions, call arguments, default parameter values, fields,
expression-bodied properties, object initializer members, `with` expression members, assignments, array
...
typed locals, returns, expression-bodied functions/local functions, call arguments, default parameter
values, fields, expression-bodied properties, object initializer members, `with` expression members,
assignments, array literals, array initializers, collection literal elements, tuple literal elements,
and typed ternary/match result arms now report a direct SoA construction diagnostic before IL lowering
...
the unavailable table body mentions migration-only or missing column types. Nested SoA declarations
under the experimental flag now do the same for the unsupported-nesting diagnostic, while enabled
top-level experimental tables still validate column names and supported element types before lowering.
