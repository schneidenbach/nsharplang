...
initializers, tuple literal elements, and typed ternary/match result arms. Even with an expected
`NodeTable` type, construction must spell a non-negative capacity with `new NodeTable(capacity)` or
use `NodeTable.wrap(...)`; zero-argument target-typed construction does not reach IL lowering.
...
literals, array initializers, tuple literal elements, and typed ternary/match result arms now report
a direct SoA construction diagnostic before IL lowering can emit an invalid wrapper value.
