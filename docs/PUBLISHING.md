...
`scripts/pack-nuget.sh` packs SDK/template/compiler NuGet packages and publishes the `nsharp-toolset.tar.gz` app artifact. `scripts/publish-nuget.sh` reads project-file versions when validating package names, so there is no second hard-coded version table to drift.
...
```bash
./scripts/install.sh --source ./artifacts/nuget --vsix-url "file://$(pwd)/artifacts/vscode/nsharp.vsix"
...
# or
NSHARP_VSIX_URL=https://.../nsharp.vsix ./scripts/install.sh
```
