...
- Never show raw COTM source/config/log files. Use only redacted docs under `docs/nsharp-conversion/` and task handoff summaries.
...
- COTM migration: source-grounded internal validation story only. Evidence: `docs/nsharp-conversion/ai-migration-recipe-library.md` and parent handoffs; the task-requested `baseline-report.md` path is not present in this checkout, so do not cite it as a local file until restored.
...
- “COTM compiles/runs end-to-end in N#” or “COTM is safe to show publicly.”
...
```bash
sed -n '1,80p' docs/nsharp-conversion/ai-migration-recipe-library.md
```
...
Evidence:
- `docs/nsharp-conversion/ai-migration-recipe-library.md` states the loop and recipe families.
...
- Do not say COTM is safe for public demo.
