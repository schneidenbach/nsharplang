...
| CLI top-level surface | `dotnet run --project src/NSharpLang.Cli/Cli.csproj -- --help` | Shows build/run/restore/publish/pack/check/fix/query/daemon/format/lint/test/bench/dependency/project/doc/env/completion commands. | Safe to say broad CLI surface exists. Safe to demo help output. |
...
| SampleMigration corpus inventory | Python inventory of `/Users/spencer/code/sample-migration` | Exists: 652 files excluding common generated dirs; 307 `.nl`, 7 `.csproj`, 1 `.sln`, 5 `project.yml`, 20 `.json`, 100 `.md`; top dirs include `sample-api` 327, `docs` 139, `sample-migration-frontend` 98, `sample-migration-access-analysis` 58. | Safe to say there is a large converted SampleMigration corpus. Not safe to demo raw. |
| SampleMigration redaction scan | Filename/content signal scan only; no secret values printed | Broad scan found 5 sensitive-name hits and 123 sensitive-content file hits for auth/password/token/key/connection-like signals. | Do not show raw SampleMigration files, screenshots, configs, docs, or logs publicly. Redacted subset required. |
...
| SampleMigration has a large N# migration corpus. | Safe to say | Inventory of `/Users/spencer/code/sample-migration`. | 307 `.nl` files and 652 total files excluding generated dirs. | "We have a large migrated SampleMigration corpus for internal validation." | Keep it internal unless redacted and approved. |
| SampleMigration is safe to show publicly. | Do not say | Redacted subset, security review, screenshot approval. | Sensitive-name/content signals found. | None. | Required: redaction pass, approved subset, no raw appsettings/auth/password/token/connection material, fresh compile/check evidence. |
| SampleMigration compiles/builds/runs in N#. | Experimental | SampleMigration-specific `nlc check`/build/test/behavioral commands. | Inventory only in this task; no compile/build acceptance evidence captured. | "SampleMigration migration is a validation target, not a public proof point yet." | Run SampleMigration-specific validation and archive logs after redaction. |
...
6. Slide caveat: full suite and SampleMigration are not yet public launch proof unless their gates below close.
...
| SampleMigration public gate | Redacted subset exists; security review approves it; SampleMigration-specific check/build evidence is captured. | Red: broad sensitive signals; inventory only. | Keep SampleMigration as internal validation story, not public demo content. |
