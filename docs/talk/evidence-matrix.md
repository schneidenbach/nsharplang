...
| COTM corpus inventory | Python inventory of `/Users/spencer/code/cotm2-nsharp` | Exists: 652 files excluding common generated dirs; 307 `.nl`, 7 `.csproj`, 1 `.sln`, 5 `project.yml`, 20 `.json`, 100 `.md`; top dirs include `cotm-backend-api` 327, `docs` 139, `cotm-frontend` 98, `cotm-access-analysis` 58. | Safe to say there is a large converted COTM corpus. Not safe to demo raw. |
| COTM redaction scan | Filename/content signal scan only; no secret values printed | Broad scan found 5 sensitive-name hits and 123 sensitive-content file hits for auth/password/token/key/connection-like signals. | Do not show raw COTM files, screenshots, configs, docs, or logs publicly. Redacted subset required. |
...
| COTM has a large N# migration corpus. | Safe to say | Inventory of `/Users/spencer/code/cotm2-nsharp`. | 307 `.nl` files and 652 total files excluding generated dirs. | "We have a large migrated COTM corpus for internal validation." | Keep it internal unless redacted and approved. |
| COTM is safe to show publicly. | Do not say | Redacted subset, security review, screenshot approval. | Sensitive-name/content signals found. | None. | Required: redaction pass, approved subset, no raw appsettings/auth/password/token/connection material, fresh compile/check evidence. |
| COTM compiles/builds/runs in N#. | Experimental | COTM-specific `nlc check`/build/test/behavioral commands. | Inventory only in this task; no compile/build acceptance evidence captured. | "COTM migration is a validation target, not a public proof point yet." | Run COTM-specific validation and archive logs after redaction. |
...
6. Slide caveat: full suite and COTM are not yet public launch proof unless their gates below close.
...
| COTM public gate | Redacted subset exists; security review approves it; COTM-specific check/build evidence is captured. | Red: broad sensitive signals; inventory only. | Keep COTM as internal validation story, not public demo content. |
