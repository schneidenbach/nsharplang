...
Then verify everything the installer can see:
...
`nlc doctor` checks the CLI, SDK/template restore path, language server, and the VS Code extension when the `code` CLI is available. The installer also writes `~/.nsharp/env` and updates common shell profiles so new terminals can find `nlc` and the right .NET root. On headless machines, run `nlc doctor --skip-vscode`.
