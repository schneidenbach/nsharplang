...
`nlc doctor` checks the CLI, SDK/template restore path, language server, and the VS Code extension when the `code` CLI is available. On headless machines, run `nlc doctor --skip-vscode`.
