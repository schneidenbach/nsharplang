...
- The COTM migration corpus is an internal validation target, not a public converter demo. Review COTM through the AI diagnostic migration loop: AI-authored `.nl`, `nlc query diagnostics --clusters`, `nlc check --json`, `nlc idiom`, reviewed `nlc fix --dry-run --json`, format, and tests.
