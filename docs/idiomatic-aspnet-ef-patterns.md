...
- Treat private migration corpora as internal validation targets, not public converter demos. Review migrations through the diagnostic loop: AI-authored `.nl`, `nlc query diagnostics --clusters`, `nlc check --json`, reviewed `nlc fix --dry-run --json`, format, and tests.
