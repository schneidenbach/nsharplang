...
- **No public `nlc convert` contract.** C#→N# migration is AI-assisted and diagnostic-driven: produce idiomatic `.nl`, then iterate through `nlc check`, `nlc query diagnostics --clusters`, `nlc idiom`, reviewed `nlc fix --dry-run`, formatting, and tests. Prototype converter code, if present in a checkout, is scratch/internal and should not appear as a public migration promise.
...
- Keep COTM/internal migration artifacts private unless explicitly redacted and approved.
