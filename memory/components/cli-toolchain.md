...
| `tests/native/cli-command-contracts` | The shipped CLI's own contracts, SPAWNED as real processes: `--help` for `tree`/`clean`/`env`/`audit`/`doctor`/`daemon`, the `nlc tree` and `nlc env` JSON envelopes, the stderr routes that prove the silence claims non-vacuous, and top-level dispatch (`nlc help`, `--version`, an UPPERCASE command, an unknown command, `query help` / `query wat`) |
| `src/NSharpLang.Compiler.BootstrapServices/*CommandKernels.tests.nl` | The per-command option summaries, output modes and user-facing sentences, called directly in the estate: `TreeCommandKernels`, `CleanCommandKernels`, `EnvCommandKernels`, `AuditCommandKernels`, `DoctorCommandKernels`, `DaemonCommandKernels`, `RunCommandKernels`, `InitCommandKernels`, `ProgramCommandKernels`, `QueryCommandKernels` (parsing and messages), `BatchQueryKernels`, `DefineArgumentKernels`, `PositionalArgumentKernels`, `CleanArtifactDirectoryOrderer` |
