...
blocking-await lowering, including nested expression operands, and never consume a resume state from
...
methods remain declined at `emit.iterator.async-unsupported`.
...
source declaration lookup removes type arguments while preserving namespace and enclosing-type identity.
Semantic value lookup receives the original receiver first, so a qualified type is not confused with
an unrelated local value that shares its short name.
