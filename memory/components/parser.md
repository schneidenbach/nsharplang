...
- Emits concrete diagnostics for common editing mistakes such as incomplete member access, missing braces, missing line-ending `)` / `]` delimiters, malformed string/character literals, dangling binary operators, and C#-style `=` inside N# object initializers.
