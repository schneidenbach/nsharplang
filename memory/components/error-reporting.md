...
- `NL311`: InvalidModifier — the inheritance-modifier rule, in three parts. (1) A member declared `override` must have a base member of that name, and that base member must be `virtual`, `abstract`, or a non-`sealed` `override`; the two faults get two sentences ("has no base member of that name" / "is not marked 'virtual', 'abstract' or 'override'") and the span underlines the member NAME. The walk is CONSERVATIVE and silence is its default: it answers only over a source shape it can read, a CLR type it can reflect over, or the implicit `object` at the end of a source chain, and a base that resolves to nothing — or a `:` clause it could not resolve — reports nothing at all. `override func ToString()` on a class with no `:` clause is therefore silent (it takes `object.ToString`'s slot), while `override func GetType()` is an error because `object.GetType` is not virtual. (2) The same two sentences cover an `override` PROPERTY, through a separate pair of classifiers: the source one matches property members only (a base method of that name is not a property slot, and vice versa), and the metadata one walks `GetProperties` and tests the ACCESSORS for `IsVirtual && !IsFinal` — the method classifier structurally cannot answer, because it skips every `IsSpecialName` and a property accessor is one. So `override Message: string => …` on a `class : Exception` is silent while `override HResult: int => …` is an error. (3) A FIELD declared `virtual`, `abstract` or `override` is an error in its own sentence — "a field cannot be virtual, abstract or overridden" — because N# decides field-vs-property from what FOLLOWS the type and never from the modifiers: `virtual Label: string` is a field, and the emitted metadata makes it a CLR field, which can be none of those three. The suggestion names both ways out: give the member an accessor so it becomes a property, or drop the word. `static`, `readonly` and `required` fields are untouched. (4) IFACE added the `abstract`-owner rule (`ReportAbstractMemberFault`), which returns whether it reported so the override walk beside it does not add a second sentence about one declaration. Two faults, measured at 836aac8af: `class Widget { abstract func Touch() }` reached `TypeBuilder.CreateType` and the whole check printed `Type must be declared abstract if any of its methods are abstract.` with no file, line or column; and `abstract func Touch() { … }` inside an ABSTRACT class was accepted in SILENCE and emitted as a slot, so the body never ran. The verdicts, in order: an INTERFACE member is a slot already; `static abstract` has no slot to dispatch through; an abstract member SUPPLIES NO BODY (checked before the owner, because the body is the more local wrong thing and the dropped-body case is the worse bug); a SEALED class has no derived type to fill the slot; and any other non-`abstract class` owner names itself, with a form-aware suggestion (a plain class becomes `abstract class`; a struct or record struct is sealed by the CLR, so the member must move). A PROPERTY always has a body in N#, so its body sentence names the two spellings that DO give a slot — an `abstract func`, or an interface's value member — rather than telling the reader to remove a body they cannot remove. (5) IFACE also added the NARROWING-OVERRIDE rule (`ValidateOverrideAccessibility`): `public virtual func Speak()` overridden by `protected override func Speak()` compiled, verified and threw `System.TypeLoadException: … cannot reduce access.` at the first call with no diagnostic. The slot's accessibility is found by the walk `ClassifyOverrideTarget` makes (source members from `DeclaredModifiers`, CLR members from `MethodInfo`, a property's from its getter), reduced to ECMA-335's `MemberAccess` ladder — 1 private, 2 private protected, 3 internal, 4 protected, 5 protected internal, 6 public — and the declared side reads the CASING convention when no visibility word is written, or every unannotated member would look private. Only NARROWING is reported: widening loads and runs, so the CLR is the arbiter here rather than C#'s stricter `CS0507`.
- `NL342`: InterfaceValueMemberWrite — a write through an interface-typed receiver to a member the interface declares as a VALUE. An interface's value member is a GET-ONLY slot (N# has no body-less accessor with which to spell a settable one), so the store has nothing to go into; it used to reach the emitter and decline as an unmodelled statement (`emit.statement.block-child`, node kind 23), which said nothing about the member. Reported from `AnalyzerWriteTargets.ReportReadOnlyPropertyWriteTargetIfNeeded` — ahead of rule 4, and sharing its operator fork, so `=`, `++`, `--` and the compound forms each name what the reader wrote. Only a MEMBER ACCESS answers, and only over a SOURCE interface: an external interface's property carries its own `set` accessor in metadata, which rule 4 already reads.
...
- `NL318`: InvalidEventSubscription — `off` applied to something that is not a subscription returned by `on`, `on` pointed at something that is not a .NET event, or an `on` handler whose delegate type is not the event's.
