...
- `NL323`: FeatureNotImplemented — a language feature is syntactically recognized and represented in the AST, but is intentionally blocked from production builds until its implementation and verification gates land. Current use: non-generic `soa record` declarations for the compiler table-model port.
