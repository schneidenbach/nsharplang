...
- Shorthand declarations such as `message := "hi"` store the identifier column, so hygiene diagnostics like `NL001` (unused-variable) underline `message`, not the `:=` operator or the tail of the identifier.
