...
/// <summary>
/// Dogfood benchmark for stable first-source reference de-duplication in <c>nlc export csharp</c>
/// after project, framework, DLL, and package references have been resolved to host values. The C#
/// baseline mirrors the command fallback shape for string references: ordinal-ignore-case
/// <c>Distinct</c> and materialization. The N# candidate runs after the host has assigned compact
/// equality ranks and writes the first source index for each distinct rank into caller-owned buffers.
/// </summary>
[MemoryDiagnoser]
[Orderer(SummaryOrderPolicy.FastestToSlowest)]
public class CliExportReferenceDeduplicationBenchmarks
{
    ...
    public void Setup()
    {
        ...
        var expectedChecksum = CSharpExportReferences_DeduplicateOrdinalIgnoreCase();
        var actualChecksum = NSharpExportReferences_DeduplicateOrdinalIgnoreCase();
        if (expectedChecksum != actualChecksum)
        {
            throw new InvalidOperationException(
                $"N# export reference checksum mismatch for {Corpus}: " +
        ...
        for (var i = 0; i < expectedCount; i++)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"N# export reference mismatch for {Corpus} at result {i}: " +
    ...
    public int CSharpExportReferences_DeduplicateOrdinalIgnoreCase()
    {
    ...
    [Benchmark]
    public int NSharpExportReferences_DeduplicateOrdinalIgnoreCase() =>
        _nsharpStableDistinctRankChecksumInto(
            _referenceRanks,
            _uniqueRankCount,
            _nsharpSeenRanks,
    ...
    private void BuildReferences()
    {
        ...
        for (var i = 0; i < _referenceCount; i++)
        {
            ...
            var reference = (key % 6) switch
            {
                0 => $"../Shared/Project{key % 113}/Shared{key % 113}.csproj",
                1 => $"_nsharp_refs/Model{key % 107}/Model{key % 107}.csproj",
                2 => $"Microsoft.AspNetCore.App/{key % 103}",
                3 => $"System.Text.Json/{key % 97}",
                4 => $"libs/Utilities{key % 89}.dll",
                _ => $"packages/Newtonsoft.Json/{key % 83}"
            };
