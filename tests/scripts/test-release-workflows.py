...
class PackageClosureTests(unittest.TestCase):
    def validate(self, omit='', core_version='1.0.0', duplicate=False, missing_loader=False):
        import zipfile
        with tempfile.TemporaryDirectory() as d:
            names=['Sdk', 'Runtime', 'Templates', 'Compiler', 'Compiler.Driver', 'Compiler.Tooling', 'Compiler.CodeIntel', 'Compiler.Core', 'Compiler.Syntax', 'Compiler.Model']
            for name in names:
                if name == omit:
                    continue
                version=core_version if name == 'Compiler.Core' else '1.0.0'
                dependency=f'<dependencies><dependency id="NSharpLang.Compiler.{ {"Compiler": "Driver", "Compiler.Driver": "Tooling", "Compiler.Tooling": "CodeIntel", "Compiler.CodeIntel": "Core", "Compiler.Core": "Syntax"}.get(name, "Model") }" version="1.0.0" /></dependencies>' if name in ('Compiler', 'Compiler.Driver', 'Compiler.Tooling', 'Compiler.CodeIntel', 'Compiler.Core', 'Compiler.Syntax') else ''
    ...
    def test_missing_compiler_package_is_rejected(self):
        self.assertTrue(all(self.validate(omit=name).returncode != 0 for name in ('Compiler.Driver', 'Compiler.Tooling', 'Compiler.CodeIntel', 'Compiler.Core', 'Compiler.Syntax', 'Compiler.Model')))
