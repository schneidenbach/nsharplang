...
class PackageClosureTests(unittest.TestCase):
    def validate(self, omit='', core_version='1.0.0', duplicate=False, missing_loader=False):
        import zipfile
        with tempfile.TemporaryDirectory() as d:
            names=['Sdk', 'Runtime', 'Templates', 'Compiler', 'Compiler.Core', 'Compiler.Model']
            for name in names:
                if name == omit:
                    continue
                version=core_version if name == 'Compiler.Core' else '1.0.0'
                dependency=f'<dependencies><dependency id="NSharpLang.Compiler.{"Core" if name == "Compiler" else "Model"}" version="1.0.0" /></dependencies>' if name in ('Compiler', 'Compiler.Core') else ''
    ...
    def test_missing_compiler_package_is_rejected(self):
        self.assertTrue(all(self.validate(omit=name).returncode != 0 for name in ('Compiler.Core', 'Compiler.Model')))
