...
""");
    ...
    [Fact]
    public void MultiFileCompiler_EmitsAutoDiscoveredProjectTypes()
    {
        var tempDir = CreateTempDir();
...
""");
            ...
            var compiler = new MultiFileCompiler(tempDir, config);
            var outputPath = Path.Combine(outputDir, "AutoDiscoveryIlProject.dll");
            var result = compiler.CompileToIlAssembly("AutoDiscoveryIlProject", outputPath);
            ...
            var runResult = DotnetRunner.Run($"\"{outputPath}\"", workingDirectory: tempDir);
            Assert.Equal(0, runResult.ExitCode);
            Assert.Contains("Added: Laptop ($999)", runResult.Stdout);
            Assert.Contains("Total products: 2", runResult.Stdout);
            Assert.Contains("Book ($29)", runResult.Stdout);
            Assert.Contains("Category: books", runResult.Stdout);
        }
        finally
        {
            Directory.Delete(tempDir, true);
