...
public class SoaRecordILShapeTests
{
    ...
    [Fact]
    public void HardCastedDirectColumnElementAccessCheckedUncheckedWrappers_UseColumnArrayLoadStoreWithoutRowAllocation()
    {
        using var _ = SetEnvironmentVariable(ExperimentalSoaEnvironmentVariable, "1");
        ...
        const string source = """
            ...
            func update(nodes: Nodes, row: int): int {
                (checked(((NodeTable)nodes).kind))[row] = 3
                (unchecked(((Nodes)((NodeTable)nodes)).kind))[row] += 4
                old := (checked(((NodeTable)((Nodes)nodes)).kind))[row]++
                (unchecked(((Nodes)((NodeTable)nodes)).start))[^1] = old
                idx := ^1
                total := old * 1000
                total += (checked(((Nodes)((NodeTable)nodes)).kind))[row] * 100
                total += (unchecked(((NodeTable)nodes).start))[idx] * 10
                total += checked(((NodeTable)((Nodes)nodes)).start).Length
                return total
            }
        ...
        ILShapeInspector.Compile(source, assembly =>
        {
            ...
            Assert.True(
                ILShapeInspector.CountOpcode(update, OpCodes.Ldfld) >= 7,
                "Hard-casted checked/unchecked direct SoA column element operations should load backing column fields directly.");
            Assert.Equal(3, ILShapeInspector.CountOpcode(update, OpCodes.Ldlen));
            Assert.Equal(4, CountArrayElementLoads(update));
            Assert.Equal(4, CountArrayElementStores(update));
    ...
    [Fact]
    public void HardCastedDirectColumnNullCoalescingCheckedUncheckedWrappers_UseColumnArrayLoadStoreWithoutRowAllocation()
    {
        using var _ = SetEnvironmentVariable(ExperimentalSoaEnvironmentVariable, "1");
        ...
        const string source = """
            ...
            func adopt(nodes: Nodes, row: int): string {
                missing := (checked(((NodeTable)nodes).text))[row] ?? "missing"
                (unchecked(((Nodes)((NodeTable)nodes)).text))[row] ??= "assigned"
                current := (checked(((NodeTable)((Nodes)nodes)).text))[row] ??= "ignored"
                (unchecked(((NodeTable)nodes).text))[^1] ??= "tail"
                idx := ^1
                tail := (checked(((Nodes)((NodeTable)nodes)).text))[idx] ?? "fallback"
                return missing + ":" + current + ":" + tail
            }
        ...
        ILShapeInspector.Compile(source, assembly =>
        {
            ...
            Assert.True(
                ILShapeInspector.CountOpcode(adopt, OpCodes.Ldfld) >= 5,
                "Hard-casted checked/unchecked direct SoA column null coalescing should load backing column fields directly.");
            Assert.True(
                CountArrayElementLoads(adopt) >= 5,
                "Hard-casted checked/unchecked direct SoA column null coalescing should read current values from backing arrays.");
            Assert.Equal(3, CountArrayElementStores(adopt));
    ...
    [Fact]
    public void HardCastedDirectColumnDefaultStoresCheckedUncheckedWrappers_DoNotReadOldElement()
    {
        using var _ = SetEnvironmentVariable(ExperimentalSoaEnvironmentVariable, "1");
        ...
        const string source = """
            ...
            func clear(nodes: Nodes, row: int) {
                (checked(((NodeTable)nodes).kind))[row] = default
                (unchecked(((Nodes)((NodeTable)nodes)).text))[row] = default
                (checked(((NodeTable)((Nodes)nodes)).kind))[^1] = default
                (unchecked(((NodeTable)nodes).text))[^1] = default
                idx := ^1
                (checked(((Nodes)((NodeTable)nodes)).kind))[idx] = default
                (unchecked(((NodeTable)((Nodes)nodes)).text))[idx] = default
            }

            func clearAsExpression(nodes: Nodes, row: int): int {
                idx := ^1
                directKind := (checked(((NodeTable)nodes).kind))[row] = default
                directText := (unchecked(((Nodes)((NodeTable)nodes)).text))[row] = default
                literalKind := (checked(((NodeTable)((Nodes)nodes)).kind))[^1] = default
                literalText := (unchecked(((NodeTable)nodes).text))[^1] = default
                variableKind := (checked(((Nodes)((NodeTable)nodes)).kind))[idx] = default
                variableText := (unchecked(((NodeTable)((Nodes)nodes)).text))[idx] = default
                total := directKind + literalKind + variableKind
                total += (directText == null ? 10 : 0)
                total += (literalText == null ? 100 : 0)
                total += (variableText == null ? 1000 : 0)
        ...
        ILShapeInspector.Compile(source, assembly =>
        {
            ...
            Assert.True(
                ILShapeInspector.CountOpcode(clear, OpCodes.Ldfld) >= 6,
                "Hard-casted checked/unchecked direct SoA column default stores should load backing column fields directly.");
            Assert.Equal(0, CountArrayElementLoads(clear));
            Assert.Equal(6, CountArrayElementStores(clear));
            ...
            Assert.True(
                ILShapeInspector.CountOpcode(clearAsExpression, OpCodes.Ldfld) >= 6,
                "Hard-casted checked/unchecked direct SoA column default store expressions should load backing column fields directly.");
            Assert.Equal(0, CountArrayElementLoads(clearAsExpression));
            Assert.Equal(6, CountArrayElementStores(clearAsExpression));
    ...
    [Fact]
    public void HardCastedDirectColumnMetadataProperties_UseBackingArrayLengthWithoutDispatch()
    {
        using var _ = SetEnvironmentVariable(ExperimentalSoaEnvironmentVariable, "1");
        ...
        const string source = """
            ...
            func metadata(nodes: Nodes): int {
                return ((NodeTable)nodes).kind.Length
                    + (int)((Nodes)((NodeTable)nodes)).start.LongLength
                    + checked(((NodeTable)((Nodes)nodes)).kind).Rank
                    + unchecked(((Nodes)((NodeTable)nodes)).start).Length
            }
        ...
        ILShapeInspector.Compile(source, assembly =>
        {
            ...
            Assert.Equal(10, Assert.IsType<int>(main.Invoke(null, null)));

            AssertNoAllocationOrDispatch(metadata);
            Assert.Equal(3, ILShapeInspector.CountOpcode(metadata, OpCodes.Ldlen));
            Assert.True(
                ILShapeInspector.CountOpcode(metadata, OpCodes.Ldfld) >= 4,
                "Hard-casted direct SoA column metadata should load backing column arrays directly.");
            Assert.Equal(0, CountArrayElementLoads(metadata));
            Assert.Equal(0, CountArrayElementStores(metadata));
    ...
    [Fact]
    public void HardCastedDirectColumnBulkArrayOperationCheckedUncheckedWrappers_UseBackingArraysWithoutRowAllocation()
    {
        using var _ = SetEnvironmentVariable(ExperimentalSoaEnvironmentVariable, "1");
        ...
        const string source = """
            ...
            func bulk(nodes: Nodes, source: int[], target: int[]): int {
                Array.Fill(array: checked(((NodeTable)nodes).kind), value: 6)
                Array.Copy(length: 2, destinationArray: target, sourceArray: unchecked(((Nodes)((NodeTable)nodes)).kind))
                System.Array.Copy(length: 1, destinationIndex: 1, destinationArray: checked(((NodeTable)((Nodes)nodes)).start), sourceIndex: 1, sourceArray: source)
                Array.Clear(length: 1, index: 0, array: unchecked(((Nodes)((NodeTable)nodes)).start))
                return target[0] * 1000 + target[1] * 100 + nodes.start[0] * 10 + nodes.start[1]
            }
        ...
        ILShapeInspector.Compile(source, assembly =>
        {
            ...
            Assert.True(
                ILShapeInspector.CountOpcode(bulk, OpCodes.Ldfld) >= 4,
                "Hard-casted checked/unchecked direct SoA column bulk operations should load backing column arrays directly.");
            Assert.Equal(4, CountArrayElementLoads(bulk));
            Assert.Equal(0, CountArrayElementStores(bulk));
