...
// it into (`src/NSharpLang.Compiler.<Slice>`; Model and Syntax are carved), so a file may read a
// top-level name declared in its own slice or in a LOWER one, and never in a higher one - between
// assemblies a reach upward is a reference cycle and the build cannot exist. Inside Core nothing but
// this walk sees such a reach, because one project compiles every direction alike, so the walk reads
// Core and every carved project together, each file ranked by the slice it belongs to.
//
