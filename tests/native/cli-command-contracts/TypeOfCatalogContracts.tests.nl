...
test "canonical collection resolution preserves source head shadowing" {
    ...
    try {
        ...
        File.WriteAllText(
            ...
            // NO IMPORT ON THE SHADOWED HALF, AND THAT IS THE SHAPE. A source `List<T>` takes the
            // name outright, so `import System.Collections.Generic` supplies nothing and NL010 would
            // stop the build before emission — which is the answer this row is NOT about.
