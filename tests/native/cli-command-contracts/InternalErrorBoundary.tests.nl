...
func InternalBoundaryProbe(source: string): CliRun {
    ...
    try {
        ...
        // the dispatch pipeline. The compiler is several assemblies now: Driver on top, Tooling
        // beneath it, then CodeIntel, then Core, then Syntax, then Model.
        project := "name: InternalBoundaryProbe\nbackend: il\noutputType: exe\ntargetFramework: net10.0\ndependencies:\n  - dll: " + CliDll() + "\n  - dll: " + Path.Combine(cliDirectory, "NSharpLang.Compiler.Model.dll") + "\n  - dll: " + Path.Combine(cliDirectory, "NSharpLang.Compiler.Syntax.dll") + "\n  - dll: " + Path.Combine(cliDirectory, "NSharpLang.Compiler.Core.dll") + "\n  - dll: " + Path.Combine(cliDirectory, "NSharpLang.Compiler.CodeIntel.dll") + "\n  - dll: " + Path.Combine(cliDirectory, "NSharpLang.Compiler.Tooling.dll") + "\n  - dll: " + Path.Combine(cliDirectory, "NSharpLang.Compiler.Driver.dll") + "\n  - dll: " + Path.Combine(cliDirectory, "Compiler.dll") + "\n  - dll: " + Path.Combine(cliDirectory, "NSharpLang.TestHost.dll") + "\n"
