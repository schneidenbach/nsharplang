...
// translation reaches the declared `Equals(Union<T0, T1>)` rather than the object overload. Both
// spellings state it: the constructed interface bound to a LOCAL, and the comparer built from it.
test "the declared IEquatable implementation is what BCL dispatch reaches" {
    ...
    declared: IEquatable<Union<int, string>> = intArm
    assert declared.Equals(sameIntArm)
    assert !declared.Equals(otherIntArm)
