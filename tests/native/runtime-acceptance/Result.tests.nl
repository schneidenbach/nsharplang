...
// object overload. That is the whole observable meaning of the base list, and BOTH spellings state
// it: the constructed interface bound to a LOCAL, and the comparer the BCL builds from it.
test "the declared IEquatable implementation is what BCL dispatch reaches" {
    ...
    // A local typed by an external generic over a COMPLETE source type: the struct boxes into the
    // constructed interface and the call lands on the declared implementation, not the object one.
    declared: IEquatable<Result<int, string>> = ok
    assert declared.Equals(sameOk)
    assert !declared.Equals(otherOk)
    assert !declared.Equals(err)
