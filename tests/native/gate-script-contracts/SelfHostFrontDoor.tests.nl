...
// CLI or a check.
//
// Step 2d checks four projects with the CLI the gate just built and compares each count to a
// ceiling. Two of the four — `src/NSharpLang.Compiler` and `src/NSharpLang.Playground` — carry the
...
test "the self-host front door still checks Compiler.Core itself, first, and records the count the blocked projects are judged against" {
    coreScript := CoreScript()

    projects := SelfHostProjects(coreScript)
    assert projects.Count == 4, "The self-host front door must still check four projects; found " + projects.Count.ToString() + "."
    assert projects[0] == "src/NSharpLang.Compiler.Core", "Compiler.Core must be checked FIRST: the skip for the projects that reference it is keyed on the count this run measured for Core, and a later position would leave that count unread."
...
test "skipping the doomed build changes no ceiling and no ratchet comparison in the self-host front door" {
    ...
    assert ceilings[0] == "1281", "Compiler.Core's front-door ceiling must stay 1281; found '" + ceilings[0] + "'."
    assert ceilings[1] == "-1"
    assert ceilings[2] == "-1"
