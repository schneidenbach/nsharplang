...
test "the synchronous package loop traces runtime once and exact compiler PDB flags in dry-run" {
    ...
    try {
        ...
        // Syntax's package depends on Model's, and Core's on Syntax's. Compiler.CodeIntel,
        // Compiler.Tooling and Compiler.Driver are carved out ABOVE Core and packed after it, in that
        // order (Driver's package depends on Tooling's, Tooling's on CodeIntel's, CodeIntel's on Core's),
        // before the Compiler facade whose package depends on Driver's.
