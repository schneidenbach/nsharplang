...
func BenchRenderMarkdown(
    ...
    BenchAppendLine(builder, "  calls `MultiFileCompiler.CompileToIlAssembly(validateStrictLint: true)`. Its current validation")
    BenchAppendLine(builder, "  order is parse, semantic analysis, then strict lint; a strict-lint")
    BenchAppendLine(builder, "  error returns before emit. Read the `check results` census below to see what stopped a row.")
    BenchAppendLine(builder, "  The regression gate pins the human-readable stage, exit code, and a semantic phase canary whose")
    BenchAppendLine(builder, "  exact diagnostic multiset changes if analysis no longer runs before strict lint.")
    BenchAppendLine(builder, "- **A project with no non-test sources is classified, not failed for `build`.** `build` is not")
    BenchAppendLine(builder, "  spawned for it; `check` still reads its test files. See the section below.")
    BenchAppendLine(builder, "")
    ...
    BenchAppendLine(builder, "`nlc check` reaches the SAME function through")
    BenchAppendLine(builder, "`CodeIntelligenceService.LoadProjectIncludingTests` ->")
    BenchAppendLine(builder, "`MultiFileCompilerInputBuilder.BuildFromProject` -> `DiscoverSourceFiles`, which passes `true`.")
    BenchAppendLine(builder, "**Check therefore selects the program `nlc test` compiles and build selects the shipped")
    BenchAppendLine(builder, "assembly's sources, so a project with test files has two file counts, not one.** The rule is:")
    BenchAppendLine(builder, "")
    ...
    BenchAppendLine(builder, "3. For `build` only, drop every path ending in `.tests.nl`, case-insensitively.")
    BenchAppendLine(builder, "")
